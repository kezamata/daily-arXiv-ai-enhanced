...
TL;DR: 本文提出了一种名为PuzzlePole的新型圆柱形基准标记，基于PuzzleBoard校准模式，能够从360度视角进行可靠识别和姿态估计，具有高精度和抗遮挡能力。


<details>
  <summary>Details</summary>
Motivation: 自主系统需要可靠的环境感知能力，而校准和定位任务通常依赖于鲁棒的视觉标记。现有标记在全方位视角识别和抗遮挡方面存在局限。

Method: 基于PuzzleBoard模式的独特组合结构，开发圆柱形PuzzlePole标记，利用其几何特性实现360度视角的可靠识别和姿态估计。

Result: PuzzlePole标记能够在全方位视角下提供高精度的定位和方向估计，同时对遮挡具有鲁棒性。

Conclusion: PuzzlePole为自主系统提供了灵活可靠的视觉标记解决方案，适用于机器人导航、SLAM和可触摸界面等多种应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型评估方法（通用奖励函数或基于相似性的指标）无法捕捉个人视觉偏好的多样性和复杂性，需要开发能够评估与个体用户偏好对齐程度的新方法。

Method: 采用自引导策略，通过有限参考数据进行推理构建丰富的用户上下文，动态生成用户条件评估维度，并通过CoT推理评估图像，实现无需用户特定训练的个性化评估。

Result: PIGReward在准确性和可解释性方面均优于现有方法，为个性化文本到图像评估和优化建立了可扩展的基于推理的基础。

Conclusion: PIGReward是朝着个体对齐文本到图像生成的稳健步骤，能够提供个性化反馈并驱动用户特定的提示优化，改善生成图像与个体意图的对齐。
...
TL;DR: 本文提出了SG-OIF框架，通过算法稳定性作为实时控制器，解决深度学习视觉模型中训练点对测试预测影响的在线估计问题，在噪声标签和分布外检测任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习视觉模型中估计训练点对测试预测的影响对于定位噪声数据至关重要，但现有方法面临计算成本高、训练非平稳性使静态近似失效、离线计算滞后于训练动态等挑战。

Method: SG-OIF框架将算法稳定性作为实时控制器，使用随机Richardson和预条件Neumann方法维护轻量级锚点IHVP，通过模块化曲率后端使用稳定性引导的残差阈值、异常门控和置信度来调节每个样本的影响分数。

Result: 在多个数据集和各种损坏情况下，SG-OIF在噪声标签和分布外检测任务上达到SOTA性能，在CIFAR-10（20%不对称）数据集中前1%预测样本准确率达到91.1%，在MNIST上AUPR得分达到99.8%。

Conclusion: SG-OIF框架是用于在线影响估计的实用控制器，有效解决了深度学习视觉模型中训练点影响估计的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测基准缺乏场景多样性、平衡的异常覆盖和时间复杂性，无法可靠评估真实世界性能。同时视频异常理解需要更深层的语义和因果推理，但人工标注成本高昂难以基准化。

Method: 利用视频生成模型的进展，通过场景条件异常分配、多步骤故事线生成和时间一致的长格式合成策略，构建可控的生成式流水线来创建41秒的连贯视频。

Result: Pistachio基准展示了规模、多样性和复杂性，为现有方法带来新挑战，并推动动态和多事件异常理解的未来研究。

Conclusion: 生成式流水线能够有效消除互联网收集数据集的偏见和限制，为视频异常检测与理解提供更可靠的评估基准。
...
TL;DR: 提出SATA框架，统一多种跟踪和分割子任务，通过解耦混合专家机制和任务感知多目标跟踪管道解决跨模态分布差异和跨任务特征表示差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用专门架构或模态特定参数处理跟踪和分割任务，限制了泛化性和可扩展性，且忽视了跨模态分布差异和跨任务特征表示差距这两个关键挑战。

Method: 提出SATA框架，包含解耦混合专家机制分离跨模态共享知识和特定信息建模，以及任务感知多目标跟踪管道统一所有任务输出为带校准ID信息的实例集合。

Result: 在18个具有挑战性的跟踪和分割基准测试中表现出优越性能。

Conclusion: SATA为更通用的视频理解提供了新的视角，通过统一表示学习和任务输出标准化实现了跨任务和跨模态的有效知识共享。
...
TL;DR: 本文提出了一种基于行列式比值矩阵(DRaM)的新方法，用于解决3D-3D(EnP)和3D-2D正交投影(OnP)姿态估计问题，并将现有方法统一到DRaM框架下。


<details>
  <summary>Details</summary>
Motivation: 姿态估计是计算机视觉中的重要问题，现有方法如SVD和四元数特征系统虽然能精确解决噪声EnP问题，但对于噪声OnP问题缺乏类似的闭式解。本文旨在提供一种统一的解决方案框架。

Method: 使用行列式比值矩阵(DRaM)方法求解无误差EnP和OnP问题的最小二乘系统，噪声数据可通过简单的旋转校正方案处理。

Result: 提出了EnP和OnP旋转估计问题的DRaM家族解决方案，并与现有方法进行了比较分析。

Conclusion: 本文不仅为3D和2D正交投影姿态估计问题提供了新解决方案，还深入洞察了这类问题的本质，证明DRaM方法可推广到所有N维欧几里得姿态估计问题。
...
TL;DR: LatentDreamer是一个从单张图像生成3D对象的新框架，使用预训练的变分自编码器将3D几何映射到潜在特征，通过顺序生成粗略几何、精细几何和真实纹理，在70秒内完成高保真3D生成。
...
Method: 使用预训练的变分自编码器将3D几何映射到潜在特征，通过顺序生成粗略几何、精细几何和真实纹理的流水线。

Result: 生成的3D对象对输入图像具有高保真度，整个生成过程可在70秒内完成，在少量训练下与当代方法相比具有竞争力。

Conclusion: LatentDreamer通过潜在特征映射和顺序生成流程，实现了快速、高保真的3D对象生成，在效率和保真度方面表现出色。
...
Conclusion: 建立了模型容量与视觉语义token需求之间的基本关系，为理解视觉表示的效率和可扩展性提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型规模不断增长，部署和效率面临严峻挑战。现有压缩方法依赖启发式重要性指标或经验性剪枝规则，缺乏信息保存的理论保证。

Method: 基于信息瓶颈原理，引入基于熵的有效秩(eRank)和Kolmogorov-Smirnov距离来量化注意力头的贡献，提出两种互补方案：基于信息损失目标的训练式注意力头剪枝，以及通过自适应低秩近似的免训练FFN压缩。

Result: 在VQAv2、TextVQA和GQA数据集上的广泛实验表明，InfoPrune实现了高达3.2倍的FLOP减少和1.8倍的加速，性能下降可忽略不计。
...
TL;DR: 提出了一种名为Eyelid Angle (ELA)的新颖眼睑角度度量，用于可靠检测驾驶员疲劳。ELA基于3D面部特征点，相比传统方法对相机角度变化更鲁棒，并能生成合成数据集增强训练数据。


<details>
  <summary>Details</summary>
Motivation: 可靠检测驾驶员疲劳对道路安全和高级驾驶辅助系统至关重要。现有方法如Eye Aspect Ratio (EAR)对相机角度变化敏感，且缺乏自然疲劳数据收集。

Method: 基于3D面部特征点开发ELA度量，设计眨眼检测框架提取时间特征。利用ELA信号在Blender 3D中动画化虚拟角色，生成可控噪声、相机视角和眨眼动态的合成数据集。

Result: 在公共驾驶员监测数据集上的实验表明，ELA在视角变化下比EAR具有更低的方差，实现准确的眨眼检测。合成数据增强扩展了疲劳识别训练数据的多样性。
...
TL;DR: VideoChat-M1是一个用于视频理解的多智能体系统，采用协作策略规划范式，通过多智能体生成、执行和通信工具调用策略，并结合多智能体强化学习进行优化，在多个基准测试中达到最先进性能。
...
Method: 提出协作策略规划范式，包含三个关键过程：策略生成（每个智能体生成独特的工具调用策略）、策略执行（顺序调用相关工具探索视频内容）、策略通信（智能体在策略执行过程中交互更新策略）。同时采用多智能体强化学习方法进行联合优化。

Result: 在涵盖四个任务的八个基准测试中达到最先进性能。在LongVideoBench上，比最先进模型Gemini 2.5 pro高出3.6%，比GPT-4o高出15.6%。

Conclusion: VideoChat-M1通过协作策略规划范式和多智能体强化学习，有效提升了视频理解的性能，证明了动态协作策略规划在复杂视频理解任务中的有效性。
...
TL;DR: 提出了感知分类法（Perceptual Taxonomy）作为场景理解的结构化过程，并创建了相应的基准测试来评估物理基础的视觉推理能力。现有视觉语言模型在识别任务上表现良好，但在需要多步推理的属性驱动问题上性能下降10-20%。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基准缺乏对这种能力的全面评估，主要关注表面级识别或图像文本对齐，而人类认知需要更深层次的推理能力。

Method: 标注了3173个对象的84个细粒度属性，构建了包含5802张图像的多选题基准，包含28033个模板问题和50个专家设计的问题，涵盖对象描述、空间推理、属性匹配和分类推理四种类型。

Result: 领先的视觉语言模型在识别任务上表现良好，但在属性驱动问题上性能下降10-20%，特别是在需要多步推理的结构化属性问题上。提供模拟场景的上下文推理示例可以提高真实世界和专家设计问题的性能。
...
TL;DR: Vidi2模型在视频理解方面取得重大进展，具备细粒度时空定位能力，并扩展到视频问答，在VUE-STG和VUE-TR-V2基准测试中超越GPT-5等专有系统。


<details>
  <summary>Details</summary>
Motivation: 视频已成为互联网主要通信和创作媒介，需要可扩展、高质量的视频生产解决方案，推动下一代视频创作模型的发展。

Method: Vidi2模型通过端到端时空定位能力，能够根据文本查询识别对应时间戳和边界框，支持复杂编辑场景应用。

Result: Vidi2在VUE-TR-V2和VUE-STG基准测试中显著优于Gemini 3 Pro和GPT-5等专有系统，在视频问答基准上与同规模开源模型竞争激烈。

Conclusion: Vidi2通过先进的时空定位和视频理解能力，为视频创作和编辑提供了强大的多模态推理工具，在多个基准测试中表现优异。
...
Result: 跨区域评估显示，该模型在未见区域性能下降最小，优于传统计算机视觉和transformer基线方法。
...
TL;DR: 本文提出了大规模分析制图遗产的方法和数据集，利用77万+地图记录和近10万张数字化图像，从文化视角研究地图作为语义符号系统和政治文化反映的演变历程。


<details>
  <summary>Details</summary>
Motivation: 当前自动化地图分析技术很少与制图历史结合，忽视了地图作为语义符号系统和反映政治认知期望的文化对象。本文旨在从文化视角大规模研究制图遗产。

Method: 收集来自38个数字目录的771,561条地图记录和99,715张数字化图像，进行数据标准化；开发语义分割和目标检测模型用于土地类别和制图符号的通用识别；分析地理结构、出版时间线和制图符号的演变。

Result: 数据集涵盖1492-1948年六个世纪，包含236,925名贡献者；揭示了制图空间焦点与政治动态的关联，如大西洋海图与三角贸易、殖民扩张的联系；记录了6,300万个符号和2,500万个片段，发现制图符号形成局部一致系统；分析了合作与传播中合法性、大参与者和主要城市在符号规范传播中的作用。

Conclusion: 地图是经过设计的图像，其构图和框架通过中心化和语义对称强调特征；制图符号系统具有局部一致性，并随着时间发生演变；制图文化的传播受到合法性、大型参与者和主要城市的影响。
...
TL;DR: ThiFAN-VQA是一个用于灾害场景视觉问答的两阶段推理框架，通过思维链提示和上下文学习生成结构化推理轨迹，然后通过答案选择模块评估响应，在洪水灾害评估任务中实现了优越的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有AI灾害评估方法存在数据集有限、答案空间固定、生成模型易产生幻觉或通用响应等问题，需要开发灵活且准确的灾害视觉问答系统。

Method: 提出两阶段推理框架：第一阶段使用思维链提示和上下文学习生成结构化推理轨迹；第二阶段通过答案选择模块评估响应并选择最连贯准确的答案，结合定制信息检索系统和领域特定提示。

Result: 在FloodNet和RescueNet-VQA数据集上的实验表明，ThiFAN-VQA在准确性、可解释性和适应性方面优于现有方法，适用于真实世界的灾后损害评估任务。

Conclusion: ThiFAN-VQA通过结合零样本和监督方法的优势，在有限监督下实现了灵活且一致的灾害视觉问答，为灾后应急响应提供了有效的技术解决方案。
...
TL;DR: HunyuanOCR是一个商业级、开源、轻量级（10亿参数）的视觉语言模型，专门用于OCR任务。它在感知任务（文本定位、解析）和语义任务（信息提取、文本图像翻译）上表现出色，在ICDAR 2025 DIMT挑战赛小型模型赛道获得第一名，并在OCRBench上实现了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 解决传统OCR专家模型功能单一和通用视觉语言模型效率低下的问题，实现多功能与效率的统一，采用端到端架构避免传统流水线的错误传播，简化系统部署。

Method: 采用原生视觉Transformer和轻量级LLM通过MLP适配器连接的架构，结合高质量数据和首次在OCR任务中应用的强化学习策略。

Result: 在多个基准测试中超越商业API、传统流水线和更大模型，在ICDAR 2025 DIMT挑战赛小型模型赛道获得第一名，在OCRBench上实现SOTA结果。
...
TL;DR: 本文提出了一种基于VAE的多尺度向量量化变分自编码器(MSVQ-VAE)，用于生成无线胶囊内窥镜(WCE)医学图像，特别是能够无缝地将异常引入正常WCE图像中，支持条件生成多种类型的异常图像。


<details>
  <summary>Details</summary>
Motivation: 由于隐私限制和标注成本，医学图像数据稀缺，阻碍了临床决策支持系统的开发。生成式机器学习为解决这一限制提供了可行方案，但现有方法在训练稳定性和捕捉视觉多样性方面存在挑战。

Method: 提出多尺度向量量化变分自编码器(MSVQ-VAE)，能够无缝地将异常引入正常WCE图像中，支持条件生成多种类型的异常图像，包括息肉、血管和炎症等病变。

Result: 通过图像分类实验评估生成图像对CDS的实用性。比较实验表明，使用该方法生成的异常图像训练的CDS分类器，与仅使用真实数据训练的分类器性能相当。

Conclusion: 该方法具有良好的通用性，有望应用于各种医学多媒体相关领域。
...
TL;DR: SkillSight是一个用于从第一人称视角数据进行节能技能评估的系统，通过结合注视数据和自我中心视频来预测技能水平，最终仅使用注视数据进行推理，大幅降低功耗。
...
Method: 采用两阶段框架：首先学习联合建模注视和自我中心视频来预测技能水平，然后蒸馏出仅使用注视数据的学生模型。在推理时，学生模型仅需注视输入，消除了连续视频处理。

Result: 在烹饪、音乐和体育三个数据集上的实验首次确立了注视在多样化现实场景中技能理解中的重要作用。SkillSight教师模型达到最先进性能，而仅注视的学生变体在保持高精度的同时，功耗比竞争方法低73倍。
...
TL;DR: 本研究提出了一种语义分布引导的MRI重建框架，利用视觉-语言基础模型将重建图像和辅助信息编码为高层语义特征，通过对比目标确保重建表示与目标语义分布一致。


<details>
  <summary>Details</summary>
Motivation: 探索视觉-语言基础模型是否能够通过提供超出传统先验的高层上下文信息来增强欠采样MRI重建。

Method: 提出语义分布引导重建框架，使用预训练的视觉-语言基础模型编码图像和辅助信息为语义特征，通过对比目标对齐重建表示与目标语义分布，可与各种深度学习重建方法结合。

Result: 在膝盖和脑部数据集上的实验表明，基于图像的语义先验能保留精细解剖结构，获得更好的感知质量（更低的LPIPS值、更高的Tenengrad分数和改进的读者研究分数），图像-语言信息进一步扩展语义分布并实现重建属性的高层控制。
...
TL;DR: GIANT框架首次允许大型多模态模型像病理学家一样迭代导航全切片图像，在病理学图像解释任务中显著优于传统的补丁和缩略图方法，接近或超越专门训练模型的表现。
...
Result: GIANT系统显著优于传统基线方法，在病理学家编写的问题上，GPT-5与GIANT组合达到62.5%准确率，优于TITAN(43.8%)和SlideChat(37.5%)等专业病理学模型。
...
Method: 使用基于注意力的编码器-解码器架构分割四个ROI（肿块、钙化、腋窝发现、乳腺组织），预测十项临床特征，并开发两种晚期融合策略整合多模态数据。

Result: 实现了最先进的分割精度，稳健预测临床特征，通过多模态数据融合提高诊断精度，减少观察者间变异性。

Conclusion: OncoVision通过结合准确分割与临床直觉，为基于AI的乳腺X光检查设定了新标准，提供可扩展且公平的解决方案，实现早期乳腺癌检测和及时干预治疗。
...
TL;DR: IndEgo是一个多模态工业任务数据集，包含3,460个第一人称视角和1,092个第三人称视角的录制，专注于协作任务，提供丰富的注释和基准测试。


<details>
  <summary>Details</summary>
Motivation: 解决工业任务中协作工作的数据需求，提供包含多种模态信息的综合数据集，以支持任务理解和错误检测等研究。

Method: 收集并标注工业任务的多模态数据，包括第一人称和第三人称视角的录制，添加眼动追踪、叙述、声音、动作等上下文信息。

Result: 数据集包含大量录制和注释，基准测试显示现有多模态模型在处理该数据集时面临挑战。

Conclusion: IndEgo数据集为工业任务的多模态理解提供了有价值的资源，并展示了当前模型的局限性。
...
TL;DR: 该论文提出了一种基于分支的结构重参数化技术，通过训练阶段在Transformer块中添加并行分支，然后将其合并为精简的单路径模型，从而减少ViT的层数而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 现有ViT加速策略主要关注算法级优化（如token剪枝和注意力加速），而忽略了减少堆叠Transformer层数的可能性。本文旨在探索是否能在保持表示能力的同时减少层数。

Method: 采用基于分支的结构重参数化技术，在训练阶段为Transformer块添加并行分支，然后通过非线性组件入口点逐步合并分支，实现FFN和MHSA模块的精确数学重参数化。

Result: 在ViT-Tiny上成功将12层架构压缩到6、4甚至3层，同时保持ImageNet-1K分类精度，在移动CPU平台上推理速度提升高达37%。

Conclusion: 传统偏好极深Transformer堆叠的智慧可能过于保守，该方法为构建高效视觉Transformer开辟了新途径。
...
TL;DR: 提出了一种基于无人机高度的动态分块方法，通过自适应图像分割提升小型物体检测性能，在SAR任务中显著提高检测精度和推理速度。


<details>
  <summary>Details</summary>
Motivation: 无人机在搜救任务中监测广阔海域时，由于高空拍摄导致的小物体像素比例低，难以有效检测小型物体。

Method: 采用高度感知的动态分块方法，结合高度相关缩放和自适应分块因子，对图像进行缩放和自适应细分。

Result: 在SeaDronesSee数据集上测试，相比基线方法，小物体检测的mAP提升了38%，推理速度比静态分块快两倍以上。
...
### [27] [CropVLM: Learning to Zoom for Fine-Grained Vision-Language Perception](https://arxiv.org/abs/2511.19820)
...
Method: 使用强化学习训练CropVLM模型，无需人工标注的边界框作为监督信号，也不依赖昂贵的合成评估。该方法可作为外部模块与开源和专有视觉语言模型配合使用。

Result: 在需要高分辨率图像理解的任务上取得显著改进，特别是在目标视觉语言模型的域外基准测试中表现突出。
...
TL;DR: 本文研究了切片最优传输计划的迁移性，证明了在数据分布轻微扰动下优化的切片器仍能保持接近，并引入了小批量min-STP以提高可扩展性，在点云对齐和流生成建模中实现了强迁移性能。


<details>
  <summary>Details</summary>
Motivation: 切片传输方法虽然能降低OT计算成本，但现有方法未解决学习到的最优切片器能否在分布偏移下迁移到新分布对的问题，这在数据演化或重复OT计算场景中至关重要。

Method: 提出min-Sliced Transport Plan框架，研究优化切片器的迁移性，引入小批量min-STP公式化，并提供统计精度保证。

Result: 理论上证明优化切片器在数据分布轻微扰动下保持接近；实验表明可迁移min-STP在点云对齐和流生成建模中实现强一次性匹配性能和摊销训练。

Conclusion: 优化的切片器具有跨相关任务的迁移能力，min-STP框架为处理演化数据或重复OT计算提供了高效解决方案。
...
Method: MAPS框架通过系统分析发现模块松弛顺序，线性调度邻近度约束，让视觉编码器保持接近预训练先验，而动作导向的语言层更自由地适应。

Result: 在MiniVLA-VQ、MiniVLA-OFT、OpenVLA-OFT等多个模型和基准测试中，MAPS一致提升了分布内和分布外性能（最高+30%），并在真实机器人平台上验证有效。
...
<details>
  <summary>Details</summary>
Motivation: 解决计算病理学基础模型在特定临床任务中适应性差的挑战，包括全切片图像处理的复杂性、学习特征的不透明性以及广泛适应策略的选择困难。

Method: 开发PathFMTools工具包，用于与CONCH和MUSK等视觉语言基础模型交互，并在440个皮肤鳞状细胞癌H&E全切片图像上对多种适应策略进行基准测试。

Result: 展示了不同预测方法之间的权衡，验证了使用基础模型嵌入训练小型专家模型的可行性，证明了病理学基础模型在现实世界临床应用中的潜力。

Conclusion: 病理学基础模型在临床应用中具有巨大潜力，PathFMTools工具能够实现高效的分析和验证，为模型适应提供实用解决方案。
...
Method: 引入了CounterVQA视频基准，包含三个渐进难度级别来评估反事实推理的不同方面。开发了CFGPT后训练方法，通过从语言模态中提炼反事实推理能力来增强模型的视觉反事实推理能力。

Result: 对最先进的开源和闭源模型进行全面评估发现：模型在简单反事实问题上能达到合理准确率，但在复杂多跳因果链上的性能显著下降。CFGPT方法在所有CounterVQA难度级别上都带来了持续改进。

Conclusion: 视觉语言模型在反事实推理方面存在显著能力差距，特别是在复杂因果推理任务上。CFGPT方法能有效提升模型的视觉反事实推理能力，为更稳健的视频理解系统提供了解决方案。
...
Method: 两阶段方法：第一阶段训练VESSA作为参考引导的分割助手，使用包含金标准示例的模板库；第二阶段将VESSA集成到SSL框架中，实现与学生模型的动态交互。
...
TL;DR: 该研究提出了一种新的特征——相对角度，用于点云损伤重建，该特征在保持与法向量相似性能的同时，显著减少了存储需求和输入通道数。


<details>
  <summary>Details</summary>
Motivation: 点云损伤重建方法虽然能有效避免基于图像方法易受背景噪声影响的问题，但由于3D数据量大，其应用受到限制。需要一种既能保留损伤特征信息又能减少数据量的方法。

Method: 提出相对角度特征，计算为点的法向量与其父点云平均法向量之间的夹角。通过基于熵的特征评估来筛选冗余信息，并使用PointNet++模型进行训练和测试。
...
TL;DR: Prune-Then-Plan框架通过步骤级校准解决大型视觉语言模型在具身问答中的边界振荡问题，使用统计修剪和覆盖规划器实现稳定探索，显著提升导航效率和答案质量。
...
Method: 提出Prune-Then-Plan框架：首先使用基于Holm-Bonferroni的统计修剪程序去除不可行的边界选择，然后将最终决策委托给基于覆盖的规划器，将过度自信的预测转化为保守可解释的动作。

Result: 在3D-Mem EQA框架中集成该方法，在OpenEQA和EXPRESS-Bench数据集上，视觉基础SPL指标相对提升49%，LLM-Match指标相对提升33%，在相同探索预算下实现更好的场景覆盖。

Conclusion: 通过分离统计修剪和规划决策，Prune-Then-Plan框架有效校准了视觉语言模型的步骤级行为，解决了边界振荡问题，显著提升了具身问答的探索效率和性能。
...
TL;DR: 本文发现扩散变换器在混合分辨率去噪中使用线性插值旋转位置编码时会出现核心故障模式，导致注意力机制崩溃。作者提出了一种无需训练的解决方案CRPA，通过修改RoPE索引映射来消除相位混叠问题。


<details>
  <summary>Details</summary>
Motivation: 解决扩散变换器在混合分辨率生成中因线性坐标重映射导致的注意力机制崩溃问题，该问题会造成模糊、伪影或完全崩溃。

Method: 提出Cross-Resolution Phase-Aligned Attention (CRPA)方法，修改每个注意力调用中的RoPE索引映射，使所有Q/K位置在查询的步幅上表达，确保相等的物理距离始终产生相同的相位增量。

Result: CRPA能够稳定所有头部和层，实现高保真和高效的混合分辨率生成，在图像和视频生成任务上优于先前的最先进方法。

Conclusion: CRPA是一种无需训练的直接修复方案，能够从根本上消除混合分辨率去噪中的相位混叠问题，恢复扩散变换器依赖的精确相位模式。
...
TL;DR: 本文提出潜在表示探测(LRP)方法，通过训练轻量级探测器分析VLM的内部隐藏状态或注意力模式来检测不确定性，在STVQA任务中显著提升了弃权准确性。


<details>
  <summary>Details</summary>
Motivation: 由于VLM在安全关键应用中的部署需求，需要其能够在不确定时弃权回答。现有弃权方法要么依赖错误校准的输出概率，要么需要语义一致性但不适用于OCR任务，因此需要寻找新的不确定性信号来源。

Method: 提出LRP方法：1）跨层连接所有表示；2）聚合视觉标记的注意力；3）通过多数投票集成单层探测器。在图像和视频模态的四个基准测试上进行实验。

Result: LRP在弃权准确性上比最佳基线方法提升了7.6%。探测器能够泛化到各种不确定性来源和数据集，最优信号来自中间层而非最终层。

Conclusion: 建立了一个基于内部状态而非不可靠输出来检测置信信号的原则性框架，为构建部署就绪的AI系统提供了新思路。
...
TL;DR: ReDirector是一种新颖的相机控制视频重拍生成方法，通过改进RoPE的位置编码和引入Rotary Camera Encoding来提升动态捕获变长视频的重拍质量。


<details>
  <summary>Details</summary>
Motivation: 解决先前工作中RoPE的误用问题，改进输入视频和目标重拍视频之间的时空位置对齐，并处理多视角关系。

Method: 提出Rotary Camera Encoding (RoCE)，一种相机条件的RoPE相位偏移，用于捕获和整合输入与目标视频内部及跨视频的多视角关系。

Result: 实验表明该方法在相机可控性、几何一致性和视频质量方面有显著提升，能够泛化到分布外的相机轨迹和视频长度。

Conclusion: 通过将相机条件整合到RoPE中，ReDirector能够改进动态对象定位和静态背景保持，在各种轨迹和长度下都表现出优越性能。
...
TL;DR: 本文提出Rectified SpaAttn方法，通过隐式全注意力参考修正注意力分配，解决现有稀疏注意力方法中的系统偏差问题，在保持生成质量的同时显著提升计算速度。
...
Result: 使用Triton定制和集成Rectified SpaAttn内核，在HunyuanVideo和Wan 2.1上分别实现了高达3.33倍和2.08倍的加速，同时保持高质量生成。
...
TL;DR: STAvatar提出了一种从单目视频重建高保真可动画3D头部化身的方法，通过UV自适应软绑定框架和时间自适应密度控制策略，解决了现有方法在运动刚性和遮挡区域处理方面的限制。
...
Method: STAvatar包含两个关键组件：(1) UV自适应软绑定框架，利用图像和几何先验在UV空间中学习每个高斯的特征偏移；(2) 时间自适应密度控制策略，首先聚类结构相似的帧，然后引入融合感知误差作为克隆标准来联合捕捉几何和纹理差异。

Result: 在四个基准数据集上的广泛实验表明，STAvatar实现了最先进的重建性能，特别是在捕捉细粒度细节和重建频繁遮挡区域方面表现优异。

Conclusion: STAvatar通过创新的UV自适应软绑定和时间自适应密度控制策略，显著提升了3D头部化身的重建质量和动画能力，代码将公开可用。
...
Method: 提出"预热-对齐"范式：首先通过双自编码器和共享量化器在大规模数据集上进行自监督训练学习模态共享表示，然后冻结编码器和量化器，引入投影在表示层面对齐时间和视觉样本。
...
TL;DR: GigaWorld-0是一个统一的世界模型框架，通过视频生成和3D生成建模的协同优化，为视觉-语言-动作学习提供高质量、可扩展的合成数据，使VLA模型在无需真实世界交互的情况下实现强健的机器人任务性能。


<details>
  <summary>Details</summary>
Motivation: 世界模型正在成为可扩展、数据高效的具身AI基础范式，需要解决高质量合成数据的生成问题，以支持VLA模型的训练。

Method: GigaWorld-0包含两个组件：GigaWorld-0-Video利用大规模视频生成创建多样化的具身序列；GigaWorld-0-3D结合3D生成建模、高斯泼溅重建、物理系统识别和运动规划确保几何一致性和物理真实性。通过GigaTrain框架进行高效训练。

Result: GigaWorld-0能够生成高质量、多样化且可控的数据，在GigaWorld-0生成数据上训练的VLA模型（如GigaBrain-0）在物理机器人上表现出强大的真实世界性能，显著提高了泛化能力和任务成功率。

Conclusion: GigaWorld-0框架为VLA学习提供了有效的合成数据引擎，使模型在无需真实世界交互训练的情况下实现优异的机器人任务性能，为具身AI的发展提供了重要支持。
...
<details>
  <summary>Details</summary>
Motivation: 多时相遥感图像变化检测面临异质性和时空错位挑战，现有方法在时间序列化过程中破坏局部结构一致性，导致在错位情况下难以准确识别变化。

Method: 提出ChessMamba框架，集成SpatialMamba编码器和轻量级跨源交互模块，采用棋盘交错与蛇形扫描顺序将多时相特征序列化，并通过多膨胀卷积进行结构感知融合。

Result: 在二进制变化检测、语义变化检测和多模态建筑损伤评估三个任务上的综合评估表明，ChessMamba能有效融合异质特征，相比最先进方法获得显著精度提升。

Conclusion: ChessMamba通过结构感知的状态空间建模，在多时相遥感图像变化检测中实现了鲁棒的性能，解决了时空错位问题，代码将在GitHub上开源。
...
TL;DR: 提出频率解耦的跨模态知识蒸馏方法，通过频域特征分离和平衡知识传递，在低频率特征上强制对齐，在高频率特征上采用宽松对齐，显著提升跨模态知识蒸馏效果。
...
Method: 利用频域特征解耦知识传递：低频率特征具有高跨模态一致性，采用强对齐损失；高频率特征跨模态相似性低，采用宽松对齐损失。还提出尺度一致性损失和共享分类器来统一特征空间。
...
Method: 基于动脉期增强CT构建包含150个病例的多任务肝脏数据集，涵盖四种肝脏疾病和正常病例，每个病例都经过经验丰富的临床医生仔细标注和校准。

Result: 成功构建了公开的多任务肝脏数据集LiMT，提供了相关基线实验结果，并回顾了现有肝脏相关任务的数据集和方法。
...
TL;DR: 提出了一个生成-验证框架来创建高质量的SVQA数据集VeriSciQA，该数据集包含20,351个QA对，涵盖20个科学领域和12种图表类型，显著提升了开源模型在科学视觉问答任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 开源大视觉语言模型在科学视觉问答任务上表现不佳，主要瓶颈在于缺乏高质量、大规模的公开数据集。现有方法使用LVLMs合成数据存在系统性错误。

Method: 提出了生成-验证框架：首先生成带有图表相关文本上下文的QA对，然后通过跨模态一致性检查和辅助过滤器来消除错误对。

Result: 构建了VeriSciQA数据集，包含20,351个QA对。开源模型在该数据集上准确率为64%，而专有模型达到82%。在VeriSciQA上微调的模型在SVQA基准测试中表现一致提升，性能随数据规模扩大而提高。
...
TL;DR: Agent0-VL是一个自演化的视觉语言智能体，通过工具集成推理实现持续自我改进，无需外部奖励或人工标注，在几何问题解决和视觉科学分析任务上比基础模型提升12.5%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言智能体的学习受限于人工标注监督，而纯文本自评估难以验证复杂的视觉推理步骤且容易产生评估幻觉，需要克服这些限制。

Method: 提出Agent0-VL框架，在单个LVLM中统一两个协同角色：执行多轮工具集成推理的Solver，以及通过工具基础批判生成结构化反馈和细粒度自奖励的Verifier，通过自演化推理循环实现稳定自我改进。

Result: 在几何问题解决和视觉科学分析实验中，Agent0-VL相比基础模型实现了12.5%的性能提升。

Conclusion: Agent0-VL通过工具集成推理实现了零外部奖励的持续自我演化，无需人工标注或外部奖励模型即可对齐推理和验证行为。
...
TL;DR: 本文提出了一种用于连续手语识别的多模态方法，首先使用机器学习检测美国手语句子视频中手语的开始和结束帧，然后识别分割后的手语。该方法使用3D骨骼特征和手形分类器来提高边界检测的鲁棒性，并通过多模态融合模块整合这些信息。


<details>
  <summary>Details</summary>
Motivation: 提高连续手语识别的准确性，通过改进手语边界检测和整合手形信息来解决连续手语中手语边界模糊的问题。

Method: 1. 使用3D骨骼特征捕捉手语特性及其动态变化；2. 预训练87个语言学定义的标准手形分类器；3. 通过多模态融合模块整合视频分割框架和手形分类模型；4. 在包含孤立手语和连续手语的大型数据库上训练识别模型。

Result: 在ASLLRP语料库上的评估显示，该方法相比之前的工作有显著改进。

Conclusion: 提出的多模态方法通过结合3D骨骼特征和手形信息，有效提高了连续手语识别的性能，特别是在手语边界检测方面取得了显著进展。
...
<details>
  <summary>Details</summary>
Motivation: 先前工作使用几何、生成或仿真先验来指导运动迁移，但这些外部先验引入辅助约束，导致通用性和时间一致性之间的权衡。
...
Conclusion: 该方法通过内部空间-时间先验解决了运动迁移中的通用性和时间一致性问题，为高效视频制作提供了灵活的工具。
...
TL;DR: 提出Reasoning-VLA框架，通过可学习的动作查询和推理增强的视觉语言特征，实现高效并行的连续动作轨迹生成，在自动驾驶中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在推理效率和泛化到新车辆配置及驾驶场景方面存在困难，需要开发更通用快速的行动生成框架。

Method: 使用高斯采样初始化可学习动作查询，与推理增强的视觉语言特征交互生成连续动作轨迹；整合8个自动驾驶数据集为标准化Chain-of-Thought格式；结合监督学习和强化学习微调。
...
TL;DR: 本文提出了一个预测3D打印物体光化学转化的框架，通过C-PGA架构从3D视觉数据预测非视觉的体物理性质，解决了传统视觉模型在处理光物理与材料物理耦合作用时的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统视觉模型缺乏对光学物理（衍射、吸收）和材料物理（扩散、对流）非线性耦合相互作用的归纳偏置，无法准确预测3D打印物体的最终化学状态，需要开发新的方法来处理这种复杂的物理耦合问题。

Method: 提出了耦合物理门控自适应（C-PGA）多模态融合架构，使用稀疏几何和工艺参数作为查询，通过特征线性调制（FiLM）动态门控和调整密集视觉特征，空间调制双3D视觉流（原始投影堆栈及其扩散-衍射校正对应物）。

Result: 该方法在虚拟化学表征方面取得突破，消除了传统打印后测量的需求，实现了对化学转化状态的精确控制。

Conclusion: C-PGA架构通过显式建模物理耦合，能够从3D视觉数据准确预测非视觉的体物理性质，为复杂3D打印物体的光化学转化预测提供了有效解决方案。
...
TL;DR: 本文提出LoTTS框架，首次实现无需训练的区域化测试时缩放方法，通过定位和修复图像中的局部缺陷区域，在保持高质量区域的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法在全图像级别操作，忽略了图像质量的区域异质性，导致对已满意区域进行不必要的计算，对局部缺陷修复不足。

Method: LoTTS通过对比交叉注意力和自注意力信号来定位缺陷区域，然后仅对缺陷区域进行局部扰动和去噪，保持其他区域不变。

Result: 在SD2.1、SDXL和FLUX上的实验表明，LoTTS在提升局部质量和全局保真度的同时，将GPU成本比Best-of-N采样降低了2-4倍。

Conclusion: 区域化测试时缩放是扩散模型在推理时进行缩放的一个有前景的新方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统文档布局分析方法依赖于经验先验或固定数量的可学习查询，难以处理现代文档中元素数量多变和布局复杂的挑战。
...
Conclusion: HybriDLA通过结合扩散和自回归解码，有效解决了现代文档布局分析的挑战，为处理复杂文档布局提供了强大的解决方案。
...
TL;DR: DetVLM是一个新颖的智能图像搜索框架，融合目标检测和视觉大模型，实现组件状态检索和零样本搜索，在车辆组件数据集上达到94.82%的检索准确率。


<details>
  <summary>Details</summary>
Motivation: 解决细粒度图像检索中的关键问题：传统方法如手动特征缺乏鲁棒性，基于深度学习的检测器无法进行状态特定检索或零样本搜索，而视觉大模型存在空间定位差和计算成本高的问题。

Method: 提出两阶段流水线：首先使用YOLO检测器进行高效的组件级筛选，然后使用VLM作为召回增强单元，对检测器遗漏的组件进行二次验证。

Result: 在车辆组件数据集上实现94.82%的整体检索准确率，零样本搜索中达到94.95%的准确率，状态搜索任务平均准确率超过90%。

Conclusion: DetVLM成功融合检测器和VLM的优势，实现了高效的组件状态检索和零样本搜索能力，显著优于仅使用检测器的基线方法。
...
TL;DR: CPDATrack是一个基于Transformer的目标跟踪框架，通过概率驱动的token剪枝和选择性注意力机制，有效抑制背景和干扰物的干扰，同时保持目标上下文信息，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的一流Transformer跟踪器将所有token连接进行联合注意力，但过多的背景搜索token关注目标模板token会削弱跟踪器的判别能力。现有的token剪枝方法往往会删除目标附近的token，导致丢失关键上下文信息，而且搜索token中的干扰物进一步降低了准确识别目标的能力。

Method: 提出CPDATrack框架：1）在两个指定编码器层之间集成可学习模块，估计每个搜索token与目标关联的概率；2）基于概率估计剪除信息量较少的背景token，同时保留目标周围的上下文线索；3）在早期层采用判别性选择性注意力机制，完全阻断搜索到模板的注意力；4）在后续编码器层中，从局部区域选择性提取高概率目标token来关注模板token。

Result: CPDATrack在多个基准测试中实现了最先进的性能，特别是在GOT-10k上达到了75.1%的平均重叠率。

Conclusion: CPDATrack通过概率驱动的token剪枝和选择性注意力机制，有效解决了背景和干扰物干扰问题，同时提高了计算效率，在目标跟踪任务中表现出卓越性能。
...
TL;DR: 本文提出DRIFT框架，利用预训练图像扩散模型的自注意力机制实现零样本视频对象跟踪，通过语义标签传播和SAM引导的掩码优化，在标准视频对象分割基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 图像扩散模型虽然最初用于图像生成，但隐含地捕获了丰富的语义结构，可以用于识别和定位任务。本研究旨在探索如何利用扩散模型的自注意力图作为语义标签传播核，实现像素级对应关系。

Method: 将扩散模型的自注意力图重新解释为语义标签传播核，扩展到视频帧间形成时间传播核，实现零样本对象跟踪。采用测试时优化策略（DDIM反演、文本反演和自适应头权重）来适应扩散特征，并引入DRIFT框架结合SAM引导的掩码优化。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率内容创建是视觉和图形社区的核心挑战，现有方法仅限于低分辨率设置（最高1K分辨率），需要有效的可控高分辨率图像编辑机制。

Method: 采用测试时优化框架，包括对高分辨率源图像进行分块优化，然后使用细粒度细节传递模块和新颖的同步策略来保持分块间的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖密集监督和计算昂贵的微调，而无需训练的方法在生物医学分割中仍未被充分探索，因此需要开发完全无需训练和标注的核实例分割框架。

Method: SPROUT利用组织学先验构建特定切片的参考原型来缓解领域差距，通过部分最优传输方案逐步引导特征对齐，将前景和背景特征转换为正负点提示，使SAM模型无需参数更新即可进行分割。
...
TL;DR: 该论文提出HiCoGen框架，通过链式合成和强化学习解决复杂提示下图像生成的构图问题，显著提升概念覆盖率和组合准确性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在处理涉及多个对象和层次结构的复杂提示时，存在概念遗漏、混淆和构图能力差的问题，需要改进复杂提示下的图像生成质量。
...
Conclusion: HiCoGen框架通过链式合成和强化学习有效解决了复杂提示下的图像生成问题，证明了分层组合方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的4D方法通常依赖外部先验、繁重的后优化或需要在4D数据集上微调，而3D基础模型VGGT在处理移动对象主导的场景时性能显著下降。

Method: 通过Gram相似性挖掘和放大全局动态线索，在时间窗口内聚合；引入投影梯度驱动的细化策略来锐化掩码边界；将精确掩码集成到VGGT的早期推理阶段。

Result: 在六个数据集上，该方法在动态对象分割、相机姿态估计和密集重建方面实现了优越性能，并支持超过500帧序列的单次推理。

Conclusion: VGGT4D成功扩展了3D基础模型VGGT用于稳健的4D场景重建，无需额外训练即可有效缓解运动干扰。
...
Method: 提出生成-理解-反馈强化范式，包括：1）情感感知奖励反馈策略，利用LVLM评估生成图像的情感值并计算奖励；2）自我提升文本反馈框架，LVLM迭代分析图像情感内容并自适应生成下一轮提示的优化建议。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于引导的方法理论上可用于图像修复等逆问题，但实际效果有限，导致需要专门的修复模型。本文旨在开发无需训练的修复方法。

Method: 优化初始种子噪声以匹配未掩码区域，采用线性近似避免计算成本高的展开过程，并在频域进行优化以提高稳定性。

Result: 在各种图像修复任务中表现出色，超越了现有最先进方法。

Conclusion: 通过优化初始种子噪声并结合线性近似和频域优化，实现了高效且无需训练的图像修复方法，显著提升了修复质量。
...
TL;DR: 本文提出了一种结合时间注视模式、头部运动数据和视觉场景信息的多模态VR注视预测方法，通过门控融合机制和跨模态注意力自适应加权不同模态信息，在22个VR场景的530万注视样本数据集上验证了优于单模态方法的预测性能。
...
Method: 采用多模态方法，结合时间注视模式、头部运动数据和视觉场景信息，使用门控融合机制和跨模态注意力自适应加权不同模态的上下文相关性。

Result: 在22个VR场景的530万注视样本数据集上，多模态方法相比单模态方法显著提升了预测准确性，在1-3个未来帧上表现优异，跨场景泛化测试达到93.1%的验证准确率，预测的注视轨迹具有时间一致性。

Conclusion: 该方法通过融合过去注视轨迹、头部朝向和场景内容，有效提升了VR注视预测性能，为虚拟环境中的注意力机制理解提供了新见解，在渲染优化、交互设计和用户体验评估方面具有应用潜力。
...
TL;DR: 本文提出了一种细节感知的细化框架，通过两个连续阶段的参考驱动校正来增强像素级一致性，解决了现有扩散模型在参考图像引导下难以保留细粒度视觉细节的问题。
...
Method: 首先通过微调单图像扩散编辑器，使其能够同时处理草稿图像和参考图像，实现全局一致的细化同时保持结构保真度；然后应用强化学习进一步强化局部编辑能力，明确优化细节准确性和语义一致性。
...
TL;DR: 该论文提出了第一个类型特定的创造力奖励模型CREward，涵盖几何、材料和纹理三个创造力维度，通过人类基准评估和大型视觉语言模型验证，构建了可用于创造力评估、可解释创造力和创意样本获取的模型。
...
Method: 首先进行人类基准评估，捕捉人类对每种创造力类型的感知；然后分析人类判断与大型视觉语言模型预测的相关性；基于此收集LVLM生成的标签来训练CREward模型。

Result: 确认大型视觉语言模型与人类感知有很强的一致性；成功训练出CREward模型，可用于创造力评估、可解释创造力和创意样本获取。

Conclusion: CREward模型能够有效评估和生成创意图像，在创造力评估、解释和样本获取方面具有实际应用价值，为人类设计灵感提供支持，并通过低秩适应指导创意生成。
...
### [65] [On the Feasibility of Hijacking MLLMs' Decision Chain via One Perturbation](https://arxiv.org/abs/2511.20002)
*Changyue Li,Jiaying Li,Youliang Yuan,Jiaming He,Zhicong Huang,Pinjia He*

Main category: cs.CV

TL;DR: 该论文揭示了一种新型威胁：单个扰动可以劫持整个决策链，使模型在多个预定义目标上同时产生错误输出。作者提出了语义感知通用扰动（SAUPs）来暴露这种威胁，并在真实世界数据集上验证了多模态大语言模型的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现实世界模型通常在一系列决策中运行，孤立错误容易被纠正，但级联错误可能导致严重风险。传统对抗攻击只关注操纵单个决策，无法揭示这种级联威胁。

Method: 提出语义感知通用扰动（SAUPs），通过在归一化空间中使用语义分离策略搜索扰动，开发有效的算法来克服优化挑战。使用RIST数据集（具有细粒度语义标注的真实世界图像数据集）进行评估。

Result: 在三个多模态大语言模型上的广泛实验表明其脆弱性，仅使用一个对抗帧就能在控制五个不同目标时达到70%的攻击成功率。

Conclusion: 该研究揭示了单个扰动劫持整个决策链的可行性，证明了现实世界AI系统中存在严重的级联错误风险，需要开发更鲁棒的防御机制。
...
TL;DR: 提出了一种注意力引导的跨模态交互Transformer（ACIT）用于行人过街意图预测，通过六种视觉和运动模态的三组交互对，结合双路径注意力机制和Transformer时序特征聚合，在JAAD数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 有效提取和整合不同类型数据中的互补线索是行人过街意图预测的主要挑战之一，需要解决多模态数据融合的问题。

Method: 使用六种视觉和运动模态，分为三组交互对：全局语义图和全局光流、局部RGB图像和局部光流、自车速度和行人边界框。采用双路径注意力机制增强主模态的显著区域，并通过光流引导注意力与辅助模态深度交互。使用跨模态注意力建模跨模态动态，并引入Transformer时序特征聚合模块捕获序列依赖。
...
TL;DR: 论文提出了安全关键推理任务，通过多视角输入解决自动驾驶中避免一个风险可能引发另一个风险的复杂场景。作者构建了WaymoQA数据集，包含35,000个人工标注的问答对，实验表明微调能显著提升多模态大语言模型的安全推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在自动驾驶场景中表现出色，但在安全关键场景中的高级推理能力不足，特别是当避免一个交通风险可能引发另一个风险时。这种推理通常需要环境的全面视角，而不仅仅是单一前视图。

Method: 定义安全关键推理任务，将其分解为两个阶段：首先解决即时风险，然后缓解决策引发的下游风险。构建WaymoQA数据集，包含35,000个问答对，涵盖复杂高风险驾驶场景，支持多选和开放式问答格式。

Result: 实验表明现有MLLMs在安全关键场景中表现不佳，但使用WaymoQA进行微调后，其推理能力显著提升，证明了数据集在开发更安全、更具推理能力的驾驶智能体方面的有效性。

Conclusion: WaymoQA数据集和提出的安全关键推理框架为提升自动驾驶系统的安全性和推理能力提供了有效解决方案，多视角输入和两阶段推理方法能够更好地处理复杂安全风险场景。
...
TL;DR: VGA是一种无需训练的方法，通过利用视觉令牌的语义内容构建精确的视觉基础，引导模型关注相关视觉区域，从而减少MLLMs的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: MLLMs在视觉注意力机制中定位能力有限，导致产生幻觉，虽然能准确提取视觉语义，但在后续推理中未能充分利用这一优势。

Method: 提出Vision-Guided Attention (VGA)，首先构建精确的视觉基础，然后使用该基础引导模型关注相关视觉区域；在图像描述任务中，动态抑制已描述区域。
...
TL;DR: COVec是一个基于光照感知的矢量图像化框架，通过引入内在图像分解技术，将图像分离为反照率、阴影和光照层，在矢量域中实现统一表示，显著提升了复杂真实世界图像的矢量表示质量和可编辑性。


<details>
  <summary>Details</summary>
Motivation: 现有矢量图像化方法在处理复杂真实世界图像时存在困难，往往产生碎片化形状且缺乏语义简洁性，需要一种能够更好保持视觉保真度和提升可编辑性的新方法。

Method: 提出COVec框架，首次在矢量域中引入内在图像分解，将图像分离为反照率、阴影和光照层；采用语义引导的初始化和两阶段优化策略，结合可微分渲染技术进行精细化处理。

Result: 在多个数据集上的实验表明，COVec相比现有方法实现了更高的视觉保真度和显著提升的可编辑性。

Conclusion: COVec通过光照感知的矢量表示方法，成功解决了复杂真实世界图像矢量化的挑战，为高质量矢量图像生成提供了有效解决方案。
...
Method: 提出多尺度流匹配框架，采用粗到细的生成范式，引入结构化下采样和上采样策略来保持几何结构并确保跨分辨率的一致分布过渡。
...
TL;DR: HACBSR是一种无监督盲超分辨率框架，无需真实图像和外部核先验，通过对比核采样和历史增强对比学习来提升行星遥感图像质量。
...
Result: 在多个放大因子下，HACBSR与最先进的无监督方法相比具有竞争性性能，并引入了Ceres-50数据集用于行星应用评估。

Conclusion: HACBSR为无监督盲超分辨率提供了一种有效方法，在行星遥感图像处理中展现出良好性能。
...
<details>
  <summary>Details</summary>
Motivation: 自回归图像生成器能够生成逼真的图像，但目前缺乏专门检测这类生成图像的方法，需要可靠的检测技术来应对这一挑战。

Method: 通过检查自回归标记序列中模型条件概率与无条件概率的比率，利用这些比率在特定模型生成图像时表现出的独特特征进行阈值检测和归因。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过将视觉内容与任务和步骤级别的文本描述对齐来注入过程语义，但由于任务和步骤描述的高度抽象性，难以与视觉数据中的具体可观察细节形成稳健对齐。

Method: 提出任务-步骤-状态（TSS）框架，其中任务通过步骤实现，步骤驱动可观察状态之间的转换。采用渐进式预训练策略展开TSS层次结构，强制模型将表示基于状态，同时将其与步骤和高级任务关联。

Result: 在COIN和CrossTask数据集上的实验表明，该方法在任务识别、步骤识别和下一步预测等多个下游任务上优于基线模型。消融研究显示，引入状态监督是所有任务性能提升的关键驱动因素。

Conclusion: 渐进式预训练策略比标准联合训练更有效，能更好地强制执行预期的层次结构。状态作为视觉基础语义层对于连接抽象程序和可观察视觉细节至关重要。
...
TL;DR: 提出了一种新的盲自适应局部去噪方法（BALD），用于解决CEST MRI中由于硬件限制和复杂成像协议导致的异方差噪声问题，该方法无需先验噪声知识，通过自适应方差稳定变换和两阶段去噪来分离分子信号与噪声。


<details>
  <summary>Details</summary>
Motivation: CEST MRI在临床转化中面临空间变化噪声和异方差性的挑战，传统去噪方法无法处理这种复杂噪声且会改变关键生物医学信息，因此需要开发专门针对CEST数据的去噪方法。

Method: BALD方法利用CEST数据的自相似性，推导自适应方差稳定变换来均衡噪声分布，然后在线性变换后的数据上进行两阶段去噪，使用局部SVD分解作为线性变换以防止空间和光谱去噪伪影。

Result: 在多个体模和体内CEST扫描的广泛验证实验中，BALD在去噪指标和下游任务（如分子浓度图估计和癌症检测）方面始终优于最先进的CEST去噪器。

Conclusion: BALD方法有效解决了CEST MRI中的噪声问题，提高了定量对比映射的准确性，在生物医学分析中展现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉异常检测方法虽然能提供视觉解释，但缺乏语义层面的直接解释性，用户难以理解异常的具体含义。

Method: 开发概念数据集，改进CBM架构生成概念和视觉双重解释，引入人工异常合成管道以减少对罕见异常样本的依赖。

Result: CONVAD方法在性能上与经典VAD方法相当，同时提供更丰富的概念驱动解释，增强了系统的可解释性和可信度。

Conclusion: 概念瓶颈模型成功应用于视觉异常检测，实现了语义和定位解释性的结合，为VAD系统提供了更深入的解释能力。
...
Conclusion: WPT训练范式能够有效平衡规划性能和实时部署需求，在保持高性能的同时实现快速推理。
...
TL;DR: 本文提出了一个用于早期森林火灾检测的数据集，通过游戏模拟器合成烟雾和初始火灾图像，并比较了YOLOv7和检测变换器在火灾检测任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有森林火灾检测方法由于缺乏大规模数据集和专门优化的模型，存在漏检问题，需要开发针对早期火灾检测的视觉分析系统。

Method: 利用Red Dead Redemption 2等游戏模拟器合成烟雾和初始火灾图像创建数据集，结合已有图像构建更全面的数据集，使用YOLOv7和不同检测变换器模型进行图像分类和定位比较。

Result: 成功创建了一个包含烟雾羽流和初始火灾实例的数据集，为早期森林火灾检测提供了专门的视觉分析资源。

Conclusion: 提出的合成数据集方法有效解决了早期火灾检测数据不足的问题，YOLOv7和检测变换器模型在该任务上表现出良好的检测性能。
...
TL;DR: 本文提出了一种基于深度学习工具和注意力机制的集成方法，用于自动检测X射线图像中的心脏肥大。该方法采用CNN配置和Inception V3模型，结合多层注意力机制，能够自动学习特征并选择性关注输入区域，实现高精度的心脏肥大诊断。


<details>
  <summary>Details</summary>
Motivation: 医疗影像技术的革新对心血管疾病诊断产生重大影响，特别是心脏肥大等结构异常的检测。通过深度学习工具和注意力机制，可以实现对X射线图像中心脏肥大的自动检测，提高诊断效率和准确性。

Method: 系统采用数据收集、预处理、CNN配置（基于Inception V3模型）和多层注意力机制。关键特点是多头注意力机制，能够自动学习特征并选择性关注输入区域，通过注意力评分增强主要数据的表示。

Result: 模型在评估阶段表现出色：准确率95.6%，精确率95.2%，召回率96.2%，灵敏度95.7%，特异性96.1%，AUC值为96.0。各项指标均通过严格评估验证。

Conclusion: 该方法能够有效识别心脏肥大，证明了其临床意义。集成深度学习和注意力机制的方法在心脏肥大检测中取得了优异性能，为医疗诊断提供了可靠的技术支持。
...
<details>
  <summary>Details</summary>
Motivation: 随着更多国家引入基于低剂量CT的肺癌筛查项目，需要可扩展的方法来处理整个肺部体积，现有方法要么过度依赖像素级注释，要么将肺部分析为片段，限制了性能和可扩展性。

Method: 提出LungEvaty框架，基于transformer处理整个肺部输入，直接从大规模筛查数据中学习，捕获与恶性肿瘤风险相关的全面解剖和病理线索，无需区域监督，可选解剖感知注意力引导损失。

Result: 在超过90,000个CT扫描上训练，包括28,000个用于微调和6,000个用于评估，仅使用成像数据且无区域监督，匹配最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散变换器在训练长度外推时存在周期性内容重复和普遍质量下降的问题，现有方法仅通过位置编码解决重复问题，忽视了质量下降。

Method: 从注意力图角度分析问题，发现两种失败模式源于统一原因：注意力分散。提出UltraViCo方法，通过恒定衰减因子抑制训练窗口外token的注意力。

Result: 在4倍外推时，动态程度和图像质量分别比之前最佳方法提升233%和40.5%，外推限制从2倍提升到4倍。

Conclusion: UltraViCo方法能有效解决视频长度外推问题，无需训练即可应用，并可无缝推广到可控视频合成和编辑等下游任务。
...
TL;DR: 提出了PETRG-3D框架，用于自动化PET/CT报告生成，解决了功能成像的特殊挑战，并在淋巴瘤数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: PET/CT在肿瘤学中至关重要，但扫描仪的快速扩张超过了专业人员的可用性，需要自动化报告生成来减轻临床工作负担。功能PET成像面临代谢模式变化和全身3D上下文信息需求的独特挑战。

Method: 提出了PETRG-3D，一个端到端3D双分支框架，分别编码PET和CT体积，并包含风格自适应提示以减轻医院间报告实践的差异。构建了多中心淋巴瘤数据集PETRG-Lym和公开基准AutoPET-RG-Lym。
...
TL;DR: Restora-Flow是一种基于流匹配的无训练图像修复方法，通过退化掩码引导采样并引入轨迹校正机制，在保持高质量图像生成的同时显著减少处理时间。


<details>
  <summary>Details</summary>
Motivation: 当前基于流模型的图像修复方法存在处理时间长或产生过度平滑结果的问题，需要一种既能保持高质量生成又能快速处理的方法。

Method: 提出Restora-Flow方法，使用退化掩码引导流匹配采样过程，并加入轨迹校正机制来确保与退化输入的一致性，整个过程无需训练。

Result: 在自然和医学数据集上的多种图像修复任务（修复、超分辨率和去噪）中，相比扩散模型和流匹配基准方法，展现出更优的感知质量和更快的处理时间。

Conclusion: Restora-Flow通过结合退化掩码引导和轨迹校正，成功实现了高质量且快速的图像修复，为基于流匹配的生成先验提供了有效的应用方案。
...
TL;DR: 提出R-TNAG框架，通过黎曼流形映射、时间感知神经常微分方程和注意力机制，解决不规则采样纵向MRI数据中的阿尔茨海默病进展建模问题。


<details>
  <summary>Details</summary>
Motivation: 临床检查的不确定性导致纵向成像数据观测间隔不规则，传统欧几里得空间模型无法充分捕捉不规则采样纵向图像的内在连续性和非线性几何结构。

Method: 首先将sMRI特征投影到流形空间保留疾病进展的内在几何结构；使用时间感知神经常微分方程建模观测间潜在状态的连续演化；采用注意力机制黎曼门控循环单元自适应整合历史和当前信息处理不规则间隔。

Result: 实验表明该方法在疾病状态预测和认知评分回归方面均优于现有最优模型；消融研究验证各模块的互补作用；在不同序列长度和缺失率下表现稳定；跨数据集验证显示其鲁棒性和适用性。

Conclusion: R-TNAG框架通过联合设计提高了时间一致性，在不规则采样下实现稳健的AD轨迹预测，具有良好的时间泛化能力和临床适用性。
...
TL;DR: MAP-World是一个无先验的多模态规划框架，结合掩码动作规划和路径加权世界模型，用于自动驾驶运动规划。它通过将未来自车运动视为掩码序列补全，生成多样化的轨迹模式，无需依赖手工锚点或强化学习。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端系统和基于世界模型的规划器通常依赖手工锚点或强化学习来选择单一最佳模式进行训练和控制，这会丢弃替代未来信息并使优化复杂化。需要一种能够处理多个合理未来同时保持计算效率的方法。

Method: 提出MAP-World框架：1) 掩码动作规划模块将未来自车运动视为掩码序列补全，使用过去路径点作为可见标记，未来路径点作为掩码标记，驾驶意图路径提供粗略支架；2) 紧凑潜在规划状态通过注入噪声扩展为多个轨迹查询；3) 轻量级世界模型基于每个候选轨迹展开未来BEV语义。

Result: 在NAVSIM数据集上，该方法与基于锚点的方法相当，并在基于世界模型的方法中达到最先进性能，同时避免使用强化学习并保持实时推理延迟。

Conclusion: MAP-World提供了一种有效的多模态规划方法，能够从完整合理未来分布中学习，而不依赖手工锚点或强化学习，在保持实时性能的同时实现了先进的规划能力。
...
TL;DR: 本文提出了Harmonious Parameter Adaptation (HPA)框架，用于解决安全对齐多模态大语言模型在持续视觉指令调优中的安全性和任务性能平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有持续视觉指令调优研究主要关注未进行安全对齐的模型，忽略了真实世界多模态大语言模型需要安全机制来降低潜在风险。本文关注安全对齐模型在持续适应过程中不仅面临任务遗忘，还会出现安全性退化的问题。

Method: 提出HPA后训练框架，包括基于关注度的参数划分、和谐平衡的参数选择和正交参数调整。将参数按关注安全或任务性能划分为两类，从平衡角度选择需要保留的参数，并对参数更新施加正交性约束以缓解灾难性遗忘。

Result: 在CVIT基准测试和安全评估数据集上的广泛实验表明，HPA比现有基线方法更好地保持高安全性和缓解遗忘问题。

Conclusion: HPA框架能够有效平衡安全对齐多模态大语言模型在持续视觉指令调优中的安全性和任务性能，为解决这一关键挑战提供了有效方案。
...
TL;DR: 该研究评估了大型多模态模型在视频中定位物理交互事件的能力，发现虽然模型能准确描述对象和动作，但无法精确定位交互开始/结束的时刻和位置。


<details>
  <summary>Details</summary>
Motivation: 探索大型多模态模型是否真正将语义理解建立在视觉输入基础上，特别是在物理交互事件定位方面的能力。

Method: 创建了包含2万多个标注交互事件的大规模数据集，使用250名人工标注者标注接触和释放事件，然后让Qwen-2.5VL和GPT-4o两个模型在短视频中定位这些事件。

Result: 模型能够可靠地命名目标对象、识别动作并提供连贯推理，但持续无法识别交互开始/结束的帧，也无法在场景中定位事件位置。
...
Method: 提出ADNet基准，整合49个公开数据集，包含380个类别、196,294张标准化RGB图像，具有像素级标注和结构化文本描述。同时提出Dinomaly-m方法，基于上下文引导的专家混合扩展，在不增加推理成本的情况下提升解码器容量。

Result: 现有最优方法在一对一设置下达到90.6% I-AUROC，但在380类别多类别设置下下降至78.5%。Dinomaly-m方法达到83.2% I-AUROC和93.1% P-AUROC，优于现有方法。

Conclusion: ADNet作为标准化可扩展基准，支持社区扩展异常检测数据集，为未来异常检测基础模型提供可扩展基础。揭示了异常检测方法在扩展到大规模多类别时的可扩展性挑战。
...
TL;DR: Exo2EgoSyn是基于WAN 2.2的跨视角视频生成框架，能够从第三人称视角生成第一人称视角视频，通过三个关键模块实现视角转换。


<details>
  <summary>Details</summary>
Motivation: 现有的基础视频生成模型如WAN 2.2仅限于同视角生成，无法实现从外中心视角到自我中心视角的跨视角视频合成。

Method: 使用三个核心模块：EgoExo-Align实现视角对齐，MultiExoCon聚合多视角视频条件，PoseInj注入相机姿态信息指导几何感知合成。

Result: 在ExoEgo4D数据集上的实验验证了Exo2EgoSyn显著改善了Ego2Exo合成效果，实现了高质量的跨视角视频生成。

Conclusion: 该工作为基于基础模型的可扩展跨视角视频生成开辟了新途径，无需从头训练即可实现高保真的视角转换。
...
TL;DR: SFA是一个无需训练的视频文本视觉问答框架，通过自适应扫描视频帧、选择性关注关键区域并直接放大这些区域，引导Video-LLM关注关键线索，在多个公开数据集上达到最先进水平。
...
Method: 提出SFA训练免费框架，这是首个基于Video-LLM的视频文本视觉问答方法。通过自适应扫描视频帧、选择性关注关键区域并直接放大这些区域，有效引导Video-LLM的注意力朝向关键线索。

Result: SFA在多个公开视频文本视觉问答数据集上取得了新的最先进结果，显著超越了先前的方法，证明了其有效性和泛化能力。

Conclusion: SFA框架通过模拟人类答题过程，能够有效解决视频文本视觉问答中的挑战，在多个数据集上表现出色，为视频文本理解任务提供了有效的解决方案。
...
TL;DR: 本文提出了一种用于脑肿瘤MRI图像修复的深度学习框架，在ASNR-MICCAI BraTS-Inpainting 2025挑战赛中获得了第一名，性能超越了2023和2024年的获胜方案。
...
Method: 采用U-Net架构，通过训练来修复合成损坏区域，并结合随机掩码增强策略以提高泛化能力。
...
TL;DR: OmniAlpha是首个统一的多任务生成框架，用于序列到序列的RGBA图像生成和编辑，通过新颖的MSRoPE-BiL架构和AlphaLayers数据集，在21个多样化任务上超越专业基线模型。


<details>
  <summary>Details</summary>
Motivation: 生成模型在RGB合成方面表现出色，但实际应用需要RGBA操作。现有模型要么是专门处理alpha但缺乏多功能性，要么是统一多任务框架但局限于RGB领域，需要填补这一关键空白。

Method: 提出OmniAlpha框架，采用具有双向可扩展层轴的新型RoPE方法MSRoPE-BiL作为DiT骨干，能够并行处理多个输入和目标RGBA层。构建AlphaLayers数据集，包含1000个高质量多层三元组。

Result: 在21个多样化任务上的联合训练显示，统一方法始终优于强大的专业基线模型。在AIM-500上的无掩码抠图实现了84.8%的相对SAD减少，在层条件完成中赢得超过90%的人类偏好。

Conclusion: 统一的多任务模型可以学习到更优越的RGBA共享表示，为更强大的层感知生成系统铺平了道路。
...
TL;DR: CT-CIG是一种可控文本引导的伪装图像生成方法，通过大视觉语言模型标注高质量文本提示，结合轻量级控制器和频率交互细化模块，生成真实且逻辑合理的伪装图像。
...
Method: 设计伪装-揭示对话机制标注数据集，微调Stable Diffusion并加入轻量级控制器，使用频率交互细化模块捕捉高频纹理特征。

Result: 通过CLIPScore评估和伪装效果评估，证明生成的文本提示语义对齐，并能产生逼真的伪装图像。

Conclusion: CT-CIG方法能够生成语义对齐且视觉逼真的伪装图像，解决了现有方法中逻辑关系不足的问题。
...
TL;DR: 该研究针对胶质母细胞瘤的异质性挑战，开发了一种基于预训练Vision Transformer的深度学习方法，在BraTS-Path 2025挑战赛中获得了第二名。


<details>
  <summary>Details</summary>
Motivation: 胶质母细胞瘤具有显著的分子和病理异质性，这给诊断和患者分层带来了困难。传统组织病理学评估虽然仍是标准方法，但深度学习为全切片图像的客观自动化分析提供了有前景的途径。

Method: 开发了一种方法，在官方训练数据集上对预训练的Vision Transformer编码器进行微调，并配备了专用的分类头。

Result: 在线验证集上获得了0.7064的MCC和0.7676的F1分数；在最终测试集上获得了0.6509的MCC和0.5330的F1分数，在BraTS-Pathology 2025挑战赛中排名第二。
...
TL;DR: V-Attack是一种针对大型视觉语言模型的精确局部语义攻击方法，通过操作transformer注意力块中的值特征(V)而非传统的补丁特征，实现了对图像中特定概念语义的精确操控。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法在大型视觉语言模型上缺乏可控性，难以精确操纵图像中特定概念的语义，主要原因是补丁-令牌表示中的语义纠缠问题。

Method: 提出V-Attack方法，包含两个核心组件：自值增强模块来优化V的语义丰富度，以及文本引导的值操作模块利用文本提示定位源概念并优化到目标概念。

Result: 在LLaVA、InternVL、DeepseekVL和GPT-4o等多种LVLM上的广泛实验表明，V-Attack相比最先进方法平均提高了36%的攻击成功率。
...
TL;DR: 提出了HistoSpeckle-Net深度学习架构，用于从多模光纤散斑重建结构丰富的医学图像，采用分布感知学习策略和直方图互信息损失，在有限数据和光纤弯曲条件下实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有多模光纤成像方法主要关注简单数据集，难以处理复杂真实世界成像任务，且通常需要大量数据。本文旨在开发能处理复杂医学图像、减少数据依赖的实用多模光纤成像方法。

Method: 构建光学实验装置，使用空间光调制器将激光耦合到多模光纤中，捕获OrganAMNIST图像对应的输出散斑模式。提出直方图互信息损失和三尺度特征细化模块，结合多尺度结构相似性损失。

Result: 在复杂OrganAMNIST数据集上，HistoSpeckle-Net比U-Net和Pix2Pix等基线模型获得更高保真度，在有限训练样本和不同光纤弯曲条件下均表现优异。

Conclusion: 通过有效重建复杂解剖特征并减少数据需求，HistoSpeckle-Net使多模光纤成像更接近真实临床环境的实际部署。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在长提示词下存在保真度与多样性的权衡问题，长提示词往往导致输出重复且缺乏创意，这一问题尚未得到系统研究。

Method: 作者首先系统分析了长提示词下的保真度-多样性困境，然后提出了PromptMoG方法，该方法在嵌入空间中对提示词嵌入进行混合高斯采样，以增加采样熵并增强多样性，同时保持语义一致性。

Result: 在SD3.5-Large、Flux.1-Krea-Dev、CogView4和Qwen-Image四个先进模型上的实验表明，PromptMoG能够一致地提高长提示词生成的多样性，且不会导致语义漂移。

Conclusion: 本文揭示了文本到图像生成模型在长提示词下的多样性下降问题，提出的PromptMoG方法有效解决了这一困境，为长提示词生成提供了实用的解决方案。
...
TL;DR: Zoo3D是首个无需训练的3D目标检测框架，通过2D实例掩码的图聚类构建3D边界框，并使用开放词汇模块进行语义标注，在零样本和自监督两种模式下均实现最先进的性能。
...
Method: 通过2D实例掩码的图聚类构建3D边界框，使用具有最佳视图选择和视图一致性掩码生成的新型开放词汇模块分配语义标签。提供零样本Zoo3D₀（完全无需训练）和自监督Zoo3D₁（在伪标签上训练类无关检测器）两种模式。

Result: 在ScanNet200和ARKitScenes基准测试中，Zoo3D₀和Zoo3D₁在开放词汇3D目标检测方面均达到最先进结果。零样本Zoo3D₀甚至优于所有现有的自监督方法。

Conclusion: 训练即用的无训练方法在真实世界3D理解中展现出强大的能力和适应性，为3D目标检测提供了新的解决方案。
...
TL;DR: Adv-GRPO是一个对抗性奖励的强化学习框架，通过迭代更新奖励模型和生成器来解决传统标量奖励无法捕捉人类感知和易受奖励攻击的问题。该方法使用参考图像作为正样本监督奖励模型，并利用视觉基础模型提供丰富的视觉奖励信号。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习图像生成方法依赖预训练的偏好模型输出标量奖励来近似人类偏好，但这些奖励往往无法准确捕捉人类感知，且容易受到奖励攻击（reward hacking），即高分并不对应更好的图像质量。

Method: 提出Adv-GRPO框架：1）使用对抗性奖励，迭代更新奖励模型和生成器；2）用参考图像作为正样本监督奖励模型，避免被攻击；3）直接使用图像本身作为奖励，结合参考图像和视觉基础模型（如DINO）提供密集的视觉奖励信号；4）支持分布转移和灵活的风格定制。

Result: 在人类评估中，Adv-GRPO在图像质量和美学方面分别以70.0%和72.4%的胜率优于Flow-GRPO和SD3。该方法在图像质量、美学和任务特定指标上均取得一致提升。

Conclusion: Adv-GRPO通过对抗性奖励框架和密集视觉奖励信号，有效解决了传统标量奖励的局限性，显著提升了图像生成质量，并支持灵活的分布转移和风格定制。
...
TL;DR: MBCD是一个多模态领域泛化的统一协作蒸馏框架，通过自适应模态丢弃、梯度一致性约束和基于权重平均的跨模态蒸馏，解决了权重平均在多模态场景中的模态优化速度差异问题，实现了更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 权重平均(WA)技术能促进收敛到平坦损失景观，增强泛化能力，但在多模态领域泛化(MMDG)中直接应用存在问题：不同模态优化速度差异导致WA在早期阶段过拟合到收敛更快的模态，抑制了较慢但互补模态的贡献，阻碍有效的模态融合并使损失表面偏向更尖锐、泛化性更差的最小值。

Method: MBCD框架包含三个关键组件：1）学生模型中的自适应模态丢弃，抑制对主导模态的早期偏置；2）梯度一致性约束，对齐单模态分支和融合表示之间的学习信号；3）基于WA的教师模型进行跨模态蒸馏，将融合知识转移到每个单模态分支，增强跨模态交互并引导收敛到更平坦的解。

Result: 在MMDG基准测试上的广泛实验表明，MBCD始终优于现有方法，在不同未见领域上实现了卓越的准确性和鲁棒性。

Conclusion: MBCD成功保留了权重平均的平坦性诱导优势，同时克服了其在多模态上下文中的缺点，为多模态领域泛化提供了一个有效的解决方案。
...
Conclusion: DiDiCM框架为高不确定性条件下的图像分类提供了有效解决方案，在计算和内存权衡方面具有灵活性。
...
Method: 采用强化学习采样器自适应选择信息丰富的图像块，结合自编码器生成损失分布图突出异常区域，预测器在损失分布空间进行分割。

Result: 在MVTec AD数据集上实验表明，该方法在F1_max和AUC指标上分别平均提升0.15和0.06，最佳情况下F1_max提升达0.37。
...
TL;DR: VKnowU是一个评估多模态大语言模型视觉知识能力的基准，包含1,680个问题和1,249个视频，涵盖8种核心视觉知识类型。评估显示现有模型在人类中心型知识上表现较好，但在世界中心型知识上仍有显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然擅长物体识别，但缺乏对人类世界物理和社会原则的直观理解，这种视觉知识能力是连接感知与推理的关键桥梁，但在现有研究中尚未得到充分探索。

Method: 提出了VKnowU基准和VKnowQA数据集，并开发了VideoKnow+基线模型，该模型采用结构化的See-Think-Answer范式，并通过视觉知识奖励进行强化学习。

Result: 评估23个最先进的多模态大语言模型发现，领先模型仍落后于人类表现，特别是在世界中心型知识方面。VideoKnow+在VKnowU上实现了+3.7%的提升，并在多个基准测试中表现一致提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型主要处理单对象分类或短描述检索，但真实场景包含多个对象和动作的复杂组合结构。现有方法缺乏对关系和组合结构的显式建模。

Method: 提出ScenarioCLIP模型，接受输入文本、基础关系、输入图像以及突出关系的聚焦区域。在精心策划的场景数据上进行预训练，并在专门的下游任务上进行微调。构建了新的数据集，使用语言模型管道来基础动作、对象和关系。

Result: ScenarioCLIP在各种领域特定任务上表现出强大的零样本和微调性能，在多个场景基准任务中优于基线方法。

Conclusion: ScenarioCLIP通过显式建模对象间关系和组合结构，显著提升了CLIP模型在复杂场景分析中的性能，为细粒度视觉理解任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基于变换器架构的端到端跟踪取得了进展，但检测性能差以及检测与关联在联合架构中的冲突仍然是关键问题。现有方法通过使用高级去噪或标签分配策略，或通过蒸馏或锚点提案技术从外部目标检测器引入检测先验来缓解这些问题。

Method: 受集成检测先验成功的启发，以及发现MOTR类模型实际上是强大的检测模型的关键洞察，作者提出了SelfMOTR，这是一种依赖自生成检测先验的新型跟踪变换器。通过广泛分析和消融研究，揭示了MOTR类模型的隐藏检测能力，并提供了有效利用这些能力的实用工具集。

Result: 在DanceTrack数据集上，SelfMOTR实现了强大的性能，与最近最先进的端到端跟踪方法竞争。

Conclusion: SelfMOTR通过利用自生成的检测先验，成功解决了端到端跟踪中的关键问题，证明了MOTR类模型具有隐藏的检测能力，为相关研究提供了新的思路和实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在视觉质量上取得了显著进展，但在符合现实世界物理原理方面仍存在不足，需要开发能够提升物理一致性的方法。

Method: 采用训练免费、即插即用的迭代自优化框架，利用多模态思维链过程基于物理不一致性反馈来优化提示词，逐步提升生成质量。
...
TL;DR: CrossEarth-Gate是一个用于遥感图像语义分割的参数高效微调方法，通过建立包含空间、语义和频率模块的工具箱，并基于Fisher信息动态选择关键模块，有效处理遥感数据中的多维度域差距问题。
...
Method: 1) 建立全面的遥感模块工具箱，包含空间、语义和频率模块来处理多维度域差距；2) 开发基于Fisher信息的自适应选择机制，量化每个模块对任务特定梯度流的贡献，动态激活最关键模块。

Result: 在16个跨域遥感语义分割基准测试中，CrossEarth-Gate实现了最先进的性能，验证了方法的有效性和泛化能力。

Conclusion: CrossEarth-Gate通过模块化工具箱和自适应选择机制，成功解决了遥感数据中的多维度域差距问题，为参数高效微调在遥感领域的应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然整流流模型在图像和视频合成方面取得了进展，但在图像到图像翻译中仍依赖昂贵的多步去噪，阻碍实时应用。直接应用对抗训练到RF模型会导致严重收敛问题。

Method: TReFT直接使用预训练DiT或UNet预测的速度作为输出，采用内存高效的潜在循环一致性和身份损失，以及轻量级架构简化以实现更快推理。

Result: 使用TReFT微调的预训练RF模型在多个图像翻译数据集上达到与最先进方法相当的性能，同时实现实时推理。

Conclusion: TReFT成功解决了RF模型在对抗训练中的收敛问题，实现了高质量的一步图像翻译，为实时应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临低信噪比、复杂背景和缺乏明显目标特征的挑战，现有深度学习方法在不同场景下存在模式漂移问题，限制了鲁棒性。

Method: 提出IrisNet框架，通过图像到解码器变换器建立红外图像特征与整个解码器参数之间的动态映射，将参数化解码器表示为结构化2D张量，利用自注意力建模层间依赖关系，通过交叉注意力生成自适应解码模式，并集成高频组件增强感知能力。
...
TL;DR: AMB3R是一个多视图前馈模型，用于度量尺度的密集3D重建，能够处理多种3D视觉任务。它使用稀疏但紧凑的体素场景表示作为后端，通过几何推理实现空间紧凑性。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多种3D视觉任务的统一模型，避免为不同任务进行特定微调或测试时优化。

Method: 利用稀疏紧凑的体素场景表示作为后端，进行多视图重建训练，但可以无缝扩展到未标定的视觉里程计和大规模运动结构估计。

Result: 在相机姿态、深度和度量尺度估计、3D重建方面达到最先进性能，甚至超过基于优化的SLAM和SfM方法。

Conclusion: AMB3R展示了单一模型处理多种3D视觉任务的潜力，在多个基准测试中表现出色。
...
TL;DR: 提出了一种仅使用相机的3D重建流程，通过多视角图像进行3D高斯溅射重建，结合视觉模型提取语义材质掩码，为数字孪生提供物理准确的传感器模拟。
...
Result: 该方法在内部数据集上验证，利用LiDAR作为反射率真值，实现了与LiDAR-相机融合相当的传感器模拟保真度。

Conclusion: 相机专用方法结合了照片级真实重建与物理材质分配，消除了硬件复杂性和校准需求，为数字孪生提供了有效的解决方案。
...
TL;DR: 本文提出了人形视觉搜索任务，让智能体在360°全景图像中主动旋转头部来搜索物体或路径，并构建了H* Bench基准测试来研究视觉拥挤的真实场景中的视觉搜索能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索方法局限于静态图像，忽略了物理体现与3D世界的交互。本文旨在开发与人类一样高效的体现视觉搜索智能体，同时规避真实世界硬件的限制。

Method: 提出人形视觉搜索任务，构建H* Bench基准测试，包含交通枢纽、大型零售空间等具有挑战性的真实场景。使用后训练技术增强开源模型Qwen2.5-VL的性能。

Result: 实验显示顶级专有模型在物体和路径搜索中成功率仅约30%。通过后训练，Qwen2.5-VL的物体搜索成功率从14.83%提升至47.38%，路径搜索从6.44%提升至24.94%。路径搜索的难度更高，需要复杂的空间常识。

Conclusion: 研究展示了构建能够无缝融入人类日常生活的MLLM智能体的前进方向，同时也量化了剩余的巨大挑战，特别是在需要复杂空间常识推理的任务中。
...
TL;DR: GS-Checker是一种用于定位3D高斯泼溅模型中篡改区域的新方法，通过集成3D篡改属性和3D对比机制来检测篡改区域，无需昂贵的3D标签监督。
...
TL;DR: BoxPromptIML是一个弱监督图像篡改定位框架，通过粗粒度区域标注策略平衡标注成本与定位精度，使用知识蒸馏和双引导特征融合模块实现高效精细定位。
...
Conclusion: BoxPromptIML有效平衡了标注成本与定位性能，为图像篡改定位提供了可扩展且实用的解决方案。
...
Method: 结合VGGT 3D基础模型和Pixel3DMM注入拓扑信息，将像素对齐点图转换为带拓扑的点云，提出拓扑感知束调整策略，构建拉普拉斯能量优化目标。

Result: 在单张NVIDIA RTX 4090上16个视图仅需10秒完成高质量重建，在基准测试中达到最先进结果，对野外数据具有出色的泛化能力。

Conclusion: VGGTFace通过创新的3D基础模型应用和拓扑感知优化，实现了高效、高质量的拓扑一致面部几何重建，解决了现有方法的局限性。
...
TL;DR: FREE是一个针对扩散Transformer（DiTs）的加速框架，通过特征级自回归和并行验证实现无损加速，在ImageNet-512²上达到1.86倍加速；FREE (relax)通过不确定性引导松弛策略进一步提升至2.25倍加速。


<details>
  <summary>Details</summary>
Motivation: DiTs虽然生成质量达到最先进水平，但由于需要长序列去噪轨迹，导致推理延迟高。现有的推测推理方法在DiTs上加速有限，因为验证阶段的草稿准确性不足。

Method: 1. 分析DiTs特征动态，发现最终transformer层的特征具有强时间一致性和丰富语义抽象；2. 提出FREE框架，使用轻量级草稿器进行特征级自回归并行验证；3. 引入不确定性引导松弛策略（FREE (relax)），动态调整接受概率以应对不确定性增加。

Result: 在ImageNet-512²上的实验表明：FREE实现最高1.86倍加速，FREE (relax)进一步提升至2.25倍加速，同时保持生成质量的高感知和定量保真度。

Conclusion: FREE框架通过特征级自回归和并行验证有效解决了DiTs的推理延迟问题，不确定性引导松弛策略进一步提升了加速效果，为扩散模型的快速推理提供了有效解决方案。
...
TL;DR: MultiID是一个无需训练的多ID定制方法，通过ID解耦交叉注意力机制将多个ID嵌入到相应图像区域，解决了多ID定制中的复制粘贴问题和文本控制性问题。


<details>
  <summary>Details</summary>
Motivation: 多ID定制比单ID定制更困难，存在两个主要挑战：1) 由于模型训练时基于裁剪的人物区域重建图像，推理时容易出现复制粘贴问题导致质量下降；2) 文本控制性较差，生成结果只是简单组合多个人物，与输入文本对齐性不足。

Method: 提出MultiID方法，采用ID解耦交叉注意力机制将不同ID嵌入注入到相应图像区域；引入局部提示、深度引导空间控制和扩展自注意力三种策略来增强生成可控性；构建IDBench基准进行评估。

Result: 广泛的定性和定量结果表明，MultiID在解决上述两个挑战方面具有有效性，其性能与基于训练的多ID定制方法相当甚至更好。

Conclusion: MultiID通过训练自由的方式成功实现了多ID定制，解决了复制粘贴和文本控制性问题，性能优于现有方法。
...
TL;DR: TBCM是一种无需外部训练数据的轨迹反向一致性模型，通过直接从教师模型的生成轨迹中提取潜在表示，显著提高了扩散模型的时间步蒸馏效率，在减少40%训练时间和节省GPU内存的同时保持生成质量。
...
Conclusion: TBCM展示了在保持质量的同时显著提高蒸馏效率的潜力，并揭示了连续时间一致性蒸馏中的扩散-生成空间差异，为未来蒸馏研究提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前多目标跟踪方法在计算资源有限条件下无法有效运行的问题，特别是针对低频检测场景下的跟踪质量稳定化需求。

Method: 提出两阶段匹配策略改进跨帧关联，引入Bbox-Based距离替代传统马氏距离，将视觉跟踪集成到卡尔曼滤波和整体跟踪流程中。

Result: 在MOT17-val数据集1Hz检测频率下HOTA指标提升11.6%，同时在标准MOT17、MOT20和DanceTrack基准测试中保持与最佳方法相当的性能。
...
Method: LLM引导规划：单次LLM调用预测关系序列，通过广度优先搜索执行；嵌入引导神经搜索：融合文本和图嵌入，使用670万参数边缘评分器；知识蒸馏：将规划能力压缩到40亿参数模型中。

Result: LLM引导规划实现接近完美准确率（micro-F1 > 0.90），所有答案都基于知识图谱；嵌入引导神经搜索实现100倍加速，保持竞争力准确率；蒸馏模型在零API成本下匹配大模型性能。
...
TL;DR: 评估波斯语情感分类中LLM生成解释的忠实度，发现模型解释与人类标注存在显著差异，提示策略影响有限。


<details>
  <summary>Details</summary>
Motivation: LLM在低资源语言中生成自解释的忠实度存在担忧，特别是在波斯语情感分类任务中需要验证解释的可靠性。

Method: 通过比较LLM识别的重要词汇与人类标注，使用基于token级对数概率的置信度评估忠实度，测试两种提示策略（预测后解释和解释后预测）。
...
TL;DR: 该论文提出语言理解需要将信息从核心语言系统导出到其他大脑区域，以构建丰富的心理模型和情境表征。


<details>
  <summary>Details</summary>
Motivation: 探索语言理解的深层认知神经机制，超越表层意义提取，研究大脑如何构建情境心理模型。

Method: 回顾现有证据并提出假设，认为认知神经科学的进展为直接检验该假设提供了概念基础和方法。

Result: 论证了语言系统处理能力有限，深度理解需要将信息导出到处理感知、运动、心理模型和记忆的其他大脑区域。

Conclusion: 该研究策略为揭示语言理解的认知和神经基础开辟了新途径，有助于理解什么是真正的语言理解。
...
TL;DR: 该论文研究大型语言模型在情感心智理论中的性别偏见问题，评估了多种去偏策略，发现基于训练的方法比推理时提示工程更有效。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在日常生活中的广泛应用，评估和确保其公平性变得尤为重要，特别是在情感心智理论领域是否存在性别偏见。

Method: 通过向模型描述一个人及其环境并询问"这个人感觉如何？"来评估性别偏见，并提出了多种去偏策略进行比较。

Result: 研究发现，仅依靠推理时的提示工程方法无法显著减少偏见，而基于训练的方法才能实现有意义的去偏效果。

Conclusion: 在情感心智理论领域有效减少大型语言模型的性别偏见需要采用基于训练的方法，而非仅依赖推理时的提示调整。
...
TL;DR: 本文提出了NRC VAD词典v2版本，增加了10,000个英语多词表达及其组成词的效价、唤醒度和支配度人工评分，并将单字词覆盖扩展到25,000个词，总词条数大幅增加。


<details>
  <summary>Details</summary>
Motivation: 现有词典如2018年发布的NRC VAD词典仅包含单词的VAD关联评分，需要扩展到多词表达和更全面的词汇覆盖，特别是2018年后更常见的词汇。

Method: 通过人工评分方法收集了10,000个多词表达及其组成词的效价、唤醒度和支配度评分，并增加了单字词的覆盖范围。

Result: 新词典包含10,000个多词表达和25,000个单词，评分具有高度可靠性。研究发现多词表达具有强烈的情感特征，并分析了多词表达的情感组合性。

Conclusion: NRC VAD词典v2为NLP、心理学、公共卫生、数字人文和社会科学等领域的研究提供了重要资源，支持广泛的研究应用。
...
Method: 开发了一种语言无关的情感标注方法，利用情感表情符号和词汇信息来自动标注文本情感。

Result: 在SAfriSenti多语言情感语料库上的实验显示：英语推文标注准确率66%，Sepedi推文69%，Setswana推文63%，平均只需修正34%的自动标签。

Conclusion: 该方法能够有效减少人工标注工作量，为低资源语言的情感分析提供了实用的自动标注解决方案。
...
Method: 利用LLMs对人格特质的固有知识迭代增强角色扮演提示词，同时整合情境响应基准作为评分工具，指导优化过程。

Result: 定量评估显示PersonaPulse生成的提示词优于先前基于心理学研究设计的方法；探索了模型大小与人格建模的关系；发现某些人格特质的激发程度可以通过暂停优化过程来部分控制。

Conclusion: 提示词优化在塑造LLMs中人格表达方面具有重要性，为未来自适应AI交互研究提供了有价值的见解。
...
Method: 使用MEDEC数据集评估9种指令调优的大型语言模型，采用零样本提示、静态随机示例提示和检索增强动态提示三种策略，通过准确率、召回率、假阳性率等指标衡量性能。

Result: 检索增强动态提示将假阳性率降低约15%，在错误句子检测中召回率提高5-10%，并生成更符合上下文的修正结果。

Conclusion: 检索增强动态提示在多种大型语言模型中均优于零样本和静态随机示例提示，使用检索示例可提高检测准确性、减少假阳性并增强医疗错误修正的可靠性。
...
Method: 提出R2R框架，结合动态专家路由和两阶段训练策略EAG。EAG通过掩码最具预测性的表面线索，强制重排序器学习领域不变的相关性模式。使用轻量级潜在语义路由器从冻结的解码器中选择最佳LoRA专家。
...
TL;DR: 本文通过完全合成的、熵控制的基准测试，揭示了因果Transformer训练中固有的方向性摩擦，即使移除了语言先验、词频和语料库级时间不对称性，这种方向性差距依然存在。


<details>
  <summary>Details</summary>
Motivation: 解决一个未解问题：方向性失败是源于语言统计特性，还是架构本身？通过合成基准测试来隔离和量化Transformer中的方向性偏差。

Method: 使用具有可调分支因子K的随机字符串映射构建前向任务（零条件熵）和逆任务（具有分析确定的熵下限），在GPT-2模型上进行训练测试。

Result: 即使是从头训练的GPT-2模型也表现出强烈的、可复现的方向性优化差距（例如K=5时为1.16 nats），远大于在相同数据上训练的MLP。预训练初始化改变了优化行为但未消除这一差距。

Conclusion: 研究结果分离出了一个最小的、无语义的方向性摩擦特征，这种特征是因果Transformer训练固有的，为剖析现代序列模型中的方向性偏差提供了受控工具。
...
Method: 使用双曲表示实现在线概念学习，采用免训练范式在测试时生成概念嵌入，使个性化视觉语言模型更具可扩展性和高效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前个性化大语言模型为每个用户单独微调模块存在存储成本线性增长和稀疏数据性能不佳的问题，需要可扩展且高效的解决方案。

Method: MTA框架：1)构建共享元LoRA库；2)自适应LoRA融合动态合成用户特定模块；3)LoRA堆叠用于少样本个性化微调。

Result: 在LaMP基准测试中，该方法在多个任务上超越了现有最先进方法。

Conclusion: MTA框架有效解决了PLLMs的可扩展性和稀疏数据问题，实现了高效的个性化建模。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在多项选择题任务中存在关键限制：答案选项通常在没有上下文基础或解释的情况下呈现给LLMs，这导致无法充分探索所有可能答案，最终降低了模型的推理能力。

Method: BiasPrompting包含两个组件：1）推理生成阶段，模型被提示为每个答案选项生成支持性推理；2）推理引导的一致性阶段，生成的推理被综合以选择最合理的答案。

Result: 通过全面评估，BiasPrompting在五个广泛使用的多项选择题回答基准测试中显示出显著改进。
...
TL;DR: SSA（稀疏稀疏注意力）是一个统一的训练框架，通过同时考虑稀疏和全注意力，在每层强制执行双向对齐，解决了原生稀疏注意力方法中的梯度更新缺陷问题。


<details>
  <summary>Details</summary>
Motivation: 解决全注意力的二次复杂度问题，同时克服现有稀疏注意力方法中由于梯度更新不足导致的性能下降和稀疏度不足的悖论。

Method: 提出SSA框架，同时训练稀疏和全注意力，在每层进行双向对齐，保持所有token的梯度流，并显式鼓励稀疏注意力输出与全注意力对应部分对齐。

Result: 在多个常识基准测试中，SSA在稀疏和全注意力推理下都达到了最先进的性能，能够平滑适应不同的稀疏度预算，并展现出最强的长上下文外推能力。

Conclusion: SSA通过解决梯度更新缺陷，实现了更好的稀疏注意力训练，支持灵活的计算-性能权衡，并改善了长上下文外推能力。
...
TL;DR: EM2LDL是一个新颖的多语言语音语料库，通过标签分布学习推进混合情感识别，包含英语、普通话和粤语的情感表达，解决了单语言单标签语料库在语言多样性、混合情感建模和生态效度方面的限制。


<details>
  <summary>Details</summary>
Motivation: 解决现有情感语料库主要为单语言和单标签的局限性，这些限制阻碍了语言多样性、无法建模混合情感且缺乏生态效度，特别是在香港和澳门等多语言地区。

Method: 构建包含英语、普通话和粤语表达性话语的多语言语料库，捕捉语内语码转换现象，整合来自在线平台的自发情感表达，并使用32个类别的细粒度情感分布进行标注。

Result: 使用自监督学习模型的实验基线在说话人独立的性别、年龄和个性评估中表现出稳健性能，其中HuBERT-large-EN模型取得了最佳结果。

Conclusion: EM2LDL通过整合语言多样性和生态效度，为多语言环境中复杂情感动态的探索提供了可能，为开发适应性强的共情系统提供了多功能测试平台，数据集、标注和基线代码已公开。
...
<details>
  <summary>Details</summary>
Motivation: 传统发音错误检测方法需要训练评分模型或音素级模型，过程复杂且需要大量标注数据。本文旨在开发一种无需训练的方法，简化流程同时保持准确性。

Method: 使用预训练的自动语音识别模型结合检索技术，无需音素特定建模或额外任务特定训练，直接进行发音错误检测与诊断。

Result: 在L2-ARCTIC数据集上的实验表明，该方法达到了69.60%的F1分数，优于传统方法，同时避免了模型训练的复杂性。

Conclusion: 提出的训练免费框架在发音错误检测与诊断任务中表现出色，为语言学习和语音治疗提供了更简单有效的解决方案。
...
TL;DR: 本文研究了使用大型语言模型（如GPT-4.1、Gemini-2.5和LLaMA-4）的提示方法，通过零样本和少样本策略来改进低资源印度语言的语法错误纠正任务，在多个印度语言上取得了领先结果。


<details>
  <summary>Details</summary>
Motivation: 虽然基于Transformer的模型和大规模标注数据集显著提升了英语等高资源语言的语法错误纠正性能，但大多数印度语言由于资源有限、语言多样性和复杂形态学，GEC仍然具有挑战性。

Method: 采用基于提示的方法，结合最先进的大型语言模型（GPT-4.1、Gemini-2.5、LLaMA-4）和少样本策略，通过精心设计的提示和轻量级适应来适应低资源设置。
...
Conclusion: 研究表明，即使是基本的提示策略也能使LLMs显著优于经过微调的印度语言模型，突显了当代LLMs在GEC任务中的卓越多语言泛化能力，以及提示驱动NLP技术在弥合多语言GEC资源差距方面的潜力。
...
TL;DR: 本文提出了一种结合图像数据增强技术的网格标记方法，用于改进非连续命名实体识别，解决了传统方法在跨句子非连续实体分割和遗漏方面的问题。


<details>
  <summary>Details</summary>
Motivation: 命名实体识别中非连续实体识别具有挑战性，传统方法在文本分割时容易错误分割或完全遗漏跨句子的非连续实体，严重影响识别准确性。

Method: 基于网格标记方法，整合图像数据增强技术（如裁剪、缩放和填充）到网格模型中，增强模型识别非连续实体和处理分割挑战的能力。

Result: 实验结果显示传统分割方法在捕捉跨句子非连续实体方面表现不佳，而增强的网格模型在CADEC、ShARe13和ShARe14数据集上整体F1分数提升1-2.5%，非连续实体识别提升3.7-8.4%。

Conclusion: 结合图像数据增强的网格标记方法能有效提升非连续命名实体识别的性能，证明了该方法的有效性。
...
TL;DR: 开发了首个公开的吉尔吉斯语单语BERT模型KyrgyzBERT，包含35.9M参数和定制分词器，在情感分析任务上表现与5倍大的多语言BERT相当。
...
Method: 构建KyrgyzBERT模型，使用定制分词器适应吉尔吉斯语形态结构；通过翻译斯坦福情感树库创建kyrgyz-sst2基准数据集并手动标注测试集。

Result: KyrgyzBERT在情感分析任务上获得0.8280的F1分数，与参数量5倍大的多语言BERT模型性能相当。

Conclusion: KyrgyzBERT填补了吉尔吉斯语NLP工具的空白，所有模型、数据和代码均已发布以支持未来研究。
...
TL;DR: REFLEX是一种基于大语言模型的自动事实核查范式，通过角色扮演对话和对比激活对提取，在无需外部知识源的情况下提高核查准确性和解释质量。
...
Method: 将事实核查重构为角色扮演对话，联合训练判定预测和解释生成，通过提取骨干模型与其微调变体之间的对比激活对来构建引导向量，自然分离真相的风格和实质。

Result: 在真实数据集上的实验表明，REFLEX优于仅向单一真相方向引导的先前方法，仅用465个自精炼训练样本就达到最先进性能，解释信号可提升无解释模型7.57%的性能。

Conclusion: REFLEX证明了内部解释信号在解释和增强事实推理中的双重作用，为实时、可靠的事实核查提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的推测解码方法假设有大量可用计算资源，但实际推理系统中批处理等方法压缩了空闲计算资源，因此需要在低验证资源和低调度成本下进行推测解码。

Method: 提出SpecFormer架构，整合单向和双向注意力机制，结合自回归模型的全局信息提取能力和非自回归模型的并行生成优势，消除对大前缀树的依赖。

Result: 通过在不同规模模型上的无损推测解码实验，证明SpecFormer为LLM推理扩展设立了新标准，具有更低的训练需求和计算成本。

Conclusion: SpecFormer能够在低验证资源和低调度成本下实现一致的加速效果，特别适合大规模批处理场景，为LLM推理扩展提供了更高效的解决方案。
...
TL;DR: 本文探讨了大语言模型在类比推理中的能力，发现LLMs能够编码类比实体间的关系，但在将关系应用于新实体时存在困难，成功案例显示结构对齐良好，失败则反映关系信息缺失或对齐错误。
...
Method: 使用比例类比和故事类比任务，分析LLMs在编码和应用关系概念时的表现，包括隐藏表示的修补和结构对齐分析。

Result: LLMs能够有效编码类比实体间的关系，但在应用关系至新实体时存在困难；通过策略性修补隐藏表示可在一定程度上促进信息传递；成功推理伴随强结构对齐，失败则反映关系信息缺失或对齐错误。
...
Method: 数据集包含435个独特的谜语，每个项目沿五个正交维度进行标注，并通过约束感知、AI辅助的流程自动转换为多项选择格式。评估了八个前沿LLM在零样本和少样本思维链提示下的表现。

Result: 揭示了在隐喻和文化特定推理方面的一致弱点。

Conclusion: BengaliFig既为评估LLM在低资源文化背景下的鲁棒性提供了诊断工具，也为包容性和传统感知的NLP评估迈出了一步。
...
TL;DR: 本文提出了一种新颖的面向任务的词干提取方法评估框架，通过SES、MPD和ANLD三个指标综合评估词干提取方法的效用、下游任务影响和语义相似度。
...
Method: 提出包含三个方面的评估框架：(1)使用词干提取有效性评分(SES)评估词干提取效用；(2)使用模型性能差异(MPD)评估词干提取对下游任务的影响；(3)使用平均归一化编辑距离(ANLD)评估词干提取词与原词的语义相似度。
