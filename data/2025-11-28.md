...
Motivation: 现有视觉语言模型在空间智能方面缺乏稳健性，主要原因是缺少从2D图像重建3D空间的视觉几何学习过程。
...
Conclusion: 通过将语义强大的视觉语言模型与低级3D视觉任务统一，G²VLM可为社区提供强大基准，并解锁更多未来应用如3D场景编辑。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在帧级保真度方面表现良好，但在运动连贯性、动态性和真实性方面存在不足，常产生抖动、重影或不合理的动态效果。标准的去噪MSE目标缺乏对时间一致性的直接监督。

Method: 基于3步蒸馏视频扩散模型，训练基于DiT的光流判别器来区分真实与生成的运动，结合分布匹配正则化器来保持视觉保真度。

Result: 在Wan2.1-T2V-1.3B上的实验显示，MoGAN显著提升了运动质量。在VBench上，运动分数比50步教师模型提升+7.3%，比3步DMD模型提升+13.3%。在VideoJAM-Bench上，运动分数分别提升+7.4%和+8.8%，同时保持相当或更好的美学和图像质量分数。人类研究进一步证实MoGAN在运动质量上更受青睐。

Conclusion: MoGAN在不牺牲视觉保真度或效率的情况下，提供了显著更真实的运动，为实现快速、高质量的视频生成提供了实用路径。
...
<details>
  <summary>Details</summary>
Motivation: 解决SAM在遥感图像上由于严重领域偏移和密集标注稀缺导致的性能不佳问题。

Method: 采用Refine-Requery-Reinforce循环：从初始点生成粗伪掩码（Refine），用自构建框提示改进（Requery），并通过嵌入对齐减少确认偏差（Reinforce）。
...
TL;DR: Qwen3-VL是Qwen系列中最强大的视觉语言模型，支持256K令牌的交错上下文，整合文本、图像和视频，在多种多模态基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够无缝整合文本、图像和视频，并支持长上下文理解的多模态模型，以适应现实工作流程中的图像推理、智能决策和多模态代码智能需求。

Method: 采用增强的交错MRoPE进行时空建模，集成DeepStack以利用多级ViT特征加强视觉语言对齐，以及基于文本的时间对齐技术来改进视频处理。

Result: 在密集和混合专家架构下，Qwen3-VL在纯文本理解、长上下文理解和多模态推理方面均表现出领先性能，特别是在MMMU和视觉数学基准测试中。
...
<details>
  <summary>Details</summary>
Motivation: AI模型在日常设备中广泛应用，但预测错误会降低用户体验。现有解决方案主要关注错误检测，缺乏高效的修正机制，特别是在资源受限设备上。

Method: 系统包含两个关键组件：(1) 服务器端管道利用知识蒸馏将基础模型的鲁棒特征表示转移到设备兼容架构；(2) 设备端机制通过原型适配实现超高效错误修正，使用原型更新而非模型重训练。

Result: 在Food-101和Flowers-102数据集上，单样本场景下实现超过50%的错误修正率，同时保持极低的遗忘率（小于0.02%）和可忽略的计算开销。通过Android演示应用验证了系统的实用性。

Conclusion: 该系统为资源受限设备提供了实用的AI错误修正解决方案，通过原型适配机制实现了高效、低开销的错误修正，在实际场景中具有良好应用前景。
...
TL;DR: CaFlow是一个用于长时动作质量评估的统一框架，结合了反事实去混淆和双向时间条件流，通过自监督方式分离因果特征和混淆特征，并利用双向时间建模提升长期表示的稳定性和连贯性。


<details>
  <summary>Details</summary>
Motivation: 长时动作质量评估（如花样滑冰、艺术体操）面临建模长期时间动态和抵抗上下文混淆的挑战。现有方法依赖昂贵标注或单向时间建模，容易受到虚假相关性和不稳定长期表示的影响。

Method: 提出CaFlow框架，包含因果反事实正则化（CCR）模块和双向时间条件流（BiT-Flow）模块。CCR通过反事实干预自监督分离因果和混淆特征，BiT-Flow通过前向和后向动态建模与循环一致性约束产生更平滑连贯的表示。

Result: 在多个长时动作质量评估基准测试上的广泛实验表明，CaFlow实现了最先进的性能。

Conclusion: CaFlow通过整合反事实去混淆和双向时间建模，有效解决了长时动作质量评估中的因果鲁棒性和时间连贯性问题，在多个基准测试上取得了优异表现。
...
TL;DR: Multi-Crit是一个用于评估多模态模型作为评判者能力的基准测试，重点关注模型遵循多样化、细粒度评估标准的能力，揭示了当前模型在多元标准遵循方面的局限性。
...
Method: 通过严格的数据筛选流程构建Multi-Crit基准，包含开放式生成和可验证推理任务，收集具有多标准人工标注的挑战性响应对，并引入三个新指标系统评估多元遵循性、标准切换灵活性和识别标准级别偏好冲突的能力。

Result: 对25个LMM的综合分析显示：1）专有模型在保持对多元标准的一致遵循方面仍有困难，特别是在开放式评估中；2）开源模型在灵活遵循多样化标准方面进一步落后；3）使用整体判断信号进行批评微调增强了视觉基础能力，但无法泛化到多元标准级别判断。

Conclusion: Multi-Crit为构建可靠和可引导的多模态AI评估奠定了基础，揭示了当前多模态评判者在遵循多样化评估标准方面的局限性。
...
TL;DR: ADVLA是一种针对视觉-语言-动作模型的对抗攻击框架，通过在视觉编码器投影到文本特征空间的特征上直接应用对抗扰动，在低幅度约束下有效破坏下游动作预测，具有高效率和隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 现有的对抗攻击方法需要昂贵的端到端训练，且通常生成明显的扰动补丁，为了解决这些局限性而提出ADVLA。

Method: ADVLA框架直接在视觉编码器投影到文本特征空间的特征上应用对抗扰动，采用注意力引导使扰动既集中又稀疏，引入三种策略增强敏感性、强制稀疏性和集中扰动。

Result: 在L∞=4/255约束下，ADVLA结合Top-K掩码修改少于10%的补丁，攻击成功率接近100%，扰动集中在关键区域，在整体图像中几乎不可察觉，单步迭代仅需约0.06秒，显著优于传统基于补丁的攻击。
...
TL;DR: 本文首次系统研究仅通过相机轨迹（而非像素）来感知视频内容的问题，发现相机轨迹是揭示视频内容的强信号。


<details>
  <summary>Details</summary>
Motivation: 探索仅通过相机运动轨迹能否理解视频内容，验证"如何移动"能否揭示"在做什么"或"在观察什么"。

Method: 提出对比学习框架训练CamFormer编码器，将相机姿态轨迹投影到联合嵌入空间，与自然语言对齐。

Result: 相机轨迹是揭示视频内容的强信号，CamFormer嵌入在跨模态对齐、分类和时间分析等任务中表现优异，且对不同相机姿态估计方法具有鲁棒性。
...
Method: 将多样控制信号编码为单一复合画布图像，通过多任务画布训练策略优化扩散模型，使其在统一学习范式中联合理解和整合异构控制到文本到图像生成中。

Result: 在包括多人组合、姿态控制组合、布局约束生成和多控制生成等具有挑战性的基准测试中，Canvas-to-Image在身份保持和控制遵循方面显著优于最先进的方法。

Conclusion: Canvas-to-Image通过统一的画布界面和多任务训练策略，实现了对多种控制信号的集成推理，在复杂多控制场景下表现出优异的泛化能力。
...
Method: 基于先前通过软提示训练识别"技能神经元"的工作，该方法将分析扩展到涉及多个技能的复杂场景。通过将神经元激活与外部标签和模型置信度等辅助指标相关联，无需手动标记聚合即可发现可解释的任务特定行为。

Result: 在开放式文本生成和自然语言推理等任务上进行了实证验证，证明该方法不仅能检测驱动已知技能的神经元，还能在BigBench的算术推理任务中揭示先前未识别的捷径。

Conclusion: 提出的方法能够有效识别大语言模型中编码特定技能的神经元，为理解模型内部机制提供了新的途径，并揭示了模型在复杂任务中可能使用的捷径策略。
...
TL;DR: 研究表明，在大型语言模型预训练中加入多种元数据（如文档质量指标）可以加速训练，其中细粒度信息编码是关键特征。通过元数据附加和可学习元标记等方法，可提高训练效率并改善学习效果。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅关注URL作为元数据的加速效果，本研究旨在探索更多元数据类型是否能在预训练中带来更大收益。

Method: 研究多种元数据类型，引入元数据附加作为辅助任务，使用可学习元标记配合掩码损失，并通过探针分析潜在表示。

Result: 发现细粒度文档质量指标等元数据能加速预训练，可学习元标记能恢复部分加速效果，元数据能塑造质量感知的潜在结构。

Conclusion: 提供了整合元数据以提升LLM预训练效率和效果的实用指南，细粒度信息编码是有效元数据的共同特征。
...
TL;DR: 研究表明，AI生成的捷克诗歌与人类创作的诗歌难以区分，参与者识别准确率仅为45.8%。审美评价存在作者偏见，当参与者认为诗歌是AI生成时评分更低，尽管实际上AI诗歌评分与人类诗歌相当或更高。


<details>
  <summary>Details</summary>
Motivation: 研究AI在形态复杂的低资源斯拉夫语言（捷克语）中创作诗歌的能力，以及读者对AI和人类诗歌的识别能力和审美评价差异。

Method: 让捷克母语者识别和评价AI与人类创作的诗歌，使用逻辑回归模型分析识别准确率与审美评价的关系。

Result: 参与者识别准确率接近随机水平（45.8%），AI诗歌与人类诗歌难以区分。审美评价显示强烈的作者偏见，认为AI创作的诗歌评分更低，尽管实际评分相当或更高。

Conclusion: AI能够在捷克语等形态复杂的低资源语言中创作出令人信服的诗歌，读者的作者信念与诗歌审美评价密切相关。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然功能强大，但在解决深度复杂问题（如Humanity's Last Exam）时仍面临概念挑战和计算成本高昂的问题

Method: 引入ToolOrchestra方法，使用强化学习训练小型编排器，包含结果感知、效率感知和用户偏好感知的奖励机制

Result: Orchestrator（8B模型）在HLE上获得37.1%的分数，优于GPT-5（35.1%）且效率提高2.5倍；在tau2-Bench和FRAMES上大幅超越GPT-5，仅使用约30%的成本
...
TL;DR: 本文通过系统评估大语言模型在不同任务难度下的泛化能力，发现跨难度泛化通常有限，无论使用简单还是困难数据进行训练都无法在所有难度范围内实现一致改进。


<details>
  <summary>Details</summary>
Motivation: 现有研究对于使用简单还是困难数据训练能获得更好结果存在争议，且不清楚这些收益是在简单还是困难测试数据上体现。本文旨在系统评估LLMs在不同难度下的泛化能力。

Method: 使用数千个不同LLMs的输出和项目反应理论（IRT）对六个数据集中的示例进行难度排序，仅基于LLMs的能力确定难度评级，排除人类对难度的主观判断。

Result: 跨难度泛化通常有限；使用简单或困难数据进行训练无法在整个难度范围内实现一致改进。

Conclusion: 结果表明在LLMs的训练和评估数据中包含各种难度级别的重要性，以及在难度方面走捷径存在风险。
