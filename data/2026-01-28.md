...
TL;DR: 提出一种针对目标检测模型的新型动态掩码后门攻击方法，利用数据集投毒技术嵌入恶意触发器，通过SAM模型生成动态掩码实现隐蔽攻击，在蘑菇检测数据集上验证了攻击有效性。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型在计算机视觉领域的广泛应用，模型面临各种对抗攻击的威胁，包括后门攻击。本文旨在展示针对目标检测模型的新型动态后门攻击方法，特别是在蘑菇检测等关键现实领域中的实际风险，强调外包实践中存在的安全隐患。

Method: 采用数据集投毒技术嵌入恶意触发器，利用SAM（Segment Anything Model）图像分割模型生成动态掩码进行触发器放置，创建动态、隐蔽的后门攻击方法。在YOLOv7目标检测模型上进行实验验证。

Result: 实验表明，该动态掩码后门攻击方法在干净数据上保持高准确率，同时在投毒样本上实现高攻击成功率。该方法超越了基于静态一致模式的传统后门注入方法，展示了更强的隐蔽性和有效性。

Conclusion: 本文提出的动态掩码后门攻击方法揭示了目标检测模型面临的新型安全威胁，特别是在关键现实应用领域。研究结果强调了开发鲁棒防御措施的紧迫性，以保护深度学习模型免受不断演变的对抗威胁。
...
TL;DR: 提出基于眨眼嵌入和哈希网格地标编码的动态神经辐射场方法，显著提升说话人像的嘴部运动准确性和生成质量


<details>
  <summary>Details</summary>
Motivation: 尽管动态神经辐射场在说话人像3D建模方面取得进展，但在准确高效捕捉嘴部运动方面仍存在挑战，需要提升说话人像的保真度
...
Conclusion: 提出的基于眨眼嵌入和哈希网格地标编码的动态神经辐射场方法能够显著提升说话人像的保真度，有效解决嘴部运动捕捉的挑战
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：3DMM方法难以实时捕捉整个头部（包括非面部区域和背景细节），而基于GAN的方法在重现细粒度头部细节（如皱纹和头发纹理）方面有限，且通常需要大量训练数据，很少关注仅使用简单自拍视频实现虚拟化身重演

Method: 结合3DMM与基于StyleGAN的生成器，提出详细重建模型，在对抗训练中采用混合损失函数进行前景重建和虚拟化身图像生成，以恢复高频细节

Result: 在自重演和交叉重演任务上的定性和定量评估表明，该方法相比现有方法实现了更优越的头部虚拟化身重建，具有更丰富和复杂的纹理

Conclusion: 该方法能够从简单的自拍视频中生成高质量的头部虚拟化身，有效解决了现有方法在细节重建和数据需求方面的局限性
...
TL;DR: 提出基于统计框架的AI生成图像检测方法，通过计算多个测试统计量的p值并进行聚合，获得可解释的概率分数
...
Method: 利用多个现有检测器的统计量，计算p值范围，使用经典统计集成方法聚合，评估与真实图像分布的匹配程度

Result: 该方法具有通用性、灵活性和无需训练的特点，适用于多样化和不断演化的场景中的鲁棒假图像检测

Conclusion: 提出的统计框架为假图像检测提供了严格、统计基础的方法，产生可解释的概率分数，增强了鲁棒性和适应性
...
TL;DR: 该研究通过大规模实证分析发现，在手术视觉基础模型中引入深度信息进行多模态预训练，能显著提升各项任务性能，且仅需25%标注数据即可超越纯RGB模型的全数据训练效果。


<details>
  <summary>Details</summary>
Motivation: 当前手术视觉基础模型主要依赖单模态RGB预训练，忽视了手术环境的复杂3D几何结构。虽然通用计算机视觉领域已有支持多模态或几何感知输入的架构，但在手术场景中深度信息的价值尚未充分探索。

Method: 研究比较了8种基于ViT的视觉基础模型，这些模型在预训练领域、学习目标和输入模态（RGB vs RGB-D）上有所不同。使用包含140万张机器人手术图像及深度图的数据集进行预训练，并在8个手术数据集上评估了冻结主干和端到端微调两种协议，涵盖目标检测、分割、深度估计和姿态估计任务。

Result: 采用显式几何标记化的模型（如MultiMAE）在所有任务上都显著优于单模态基线。几何感知预训练展现出卓越的数据效率：仅用25%标注数据微调的模型就能持续超越使用全数据集训练的纯RGB模型。这些优势在推理时无需架构或运行时改变，深度信息仅在预训练阶段使用。

Conclusion: 多模态预训练为构建更强大的手术视觉系统提供了可行路径，深度信息的引入能显著提升模型性能和数据效率，且易于实际部署应用。
...
<details>
  <summary>Details</summary>
Motivation: SplitFed学习在客户端部署最小化模型部分，但特征值、梯度更新和模型更新在通信信道传输时易受噪声影响，需要提高对信道噪声的鲁棒性

Method: 提出智能平均策略用于SplitFed学习，通过改进的聚合机制增强对通信信道噪声的抵抗力

Result: 在胚胎图像分割模型实验中，智能平均策略相比传统平均方法能容忍两个数量级更强的通信信道噪声，同时保持最终模型的准确性

Conclusion: 智能平均策略显著提高了SplitFed学习对通信信道噪声的鲁棒性，为分布式学习在噪声环境中的应用提供了有效解决方案
...
TL;DR: 提出RW-CP方法，通过随机游走扩散增强医学图像分割中的不确定性量化，在保持统计有效性的同时改善解剖结构的空间连贯性。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像分割中的不确定性量化方法（如标准共形预测）往往忽略解剖上下文，导致预测集呈现碎片化、空间不连贯和过度分割的问题，限制了临床实用性。

Method: 提出随机游走共形预测(RW-CP)框架，基于预训练视觉基础模型特征构建k近邻图，应用随机游走来扩散不确定性，正则化非共形分数，使预测集对共形校准参数λ更不敏感，从而生成空间连贯的解剖有效集。

Result: 在多模态公共数据集上的评估显示，在允许错误率α=0.1的情况下，相比标准CP基线方法，RW-CP在分割质量上提升了高达35.4%，同时保持了严格的边际覆盖率。

Conclusion: RW-CP是一个模型无关的框架，能够增强医学图像分割中的不确定性量化，在保持统计有效性的同时显著改善解剖结构的空间连贯性，提高了临床实用性。
...
<details>
  <summary>Details</summary>
Motivation: 慢性伤口监测和管理需要准确高效的伤口测量方法，传统手动测量方法存在主观性和不准确性

Method: 结合RGB-D里程计和B样条曲面重建技术，生成详细的3D伤口网格，自动计算伤口周长、表面积和尺寸等临床相关测量参数

Result: 在硅胶伤口模型上评估，3D重建精度达到亚毫米级，测量结果重复性低且与人工评估高度一致，优于现有最先进的物体中心RGB-D重建方法
...
TL;DR: 提出神经皮质地图作为皮质特征图的连续紧凑神经表示，替代传统离散网格结构，实现任意分辨率特征学习，在球面优化中比传统方法快30倍，并开发NC-Reg算法用于皮质表面刚性配准。


<details>
  <summary>Details</summary>
Motivation: 传统皮质特征图表示（如网格）存在离散性限制，无法灵活适应不同分辨率需求。需要一种连续、紧凑的神经表示方法，能够从任意大小的网格中学习，并在任意分辨率下提供学习特征，以提高皮质表面分析的效率和精度。

Method: 提出神经皮质地图（neural cortical maps）作为皮质特征图的连续神经表示。开发NC-Reg算法，结合神经皮质特征图、梯度下降优化和模拟退火策略，实现皮质表面的刚性配准。该方法支持从任意大小网格学习，并能提供任意分辨率的特征。

Result: 神经皮质地图在球面优化中比经典重心插值方法快30倍（相同迭代次数下）。NC-Reg算法在皮质表面刚性配准中实现亚度级精度（<1°偏离全局最优），作为临床环境中关键的稳健预对齐策略表现优异。

Conclusion: 神经皮质地图为皮质特征图提供了一种高效、灵活的连续神经表示方法，NC-Reg算法在皮质表面配准中展现出高精度和鲁棒性，有望成为临床应用中重要的预对齐工具。
...
TL;DR: NuiWorld是一个解决3D场景生成中可控性、可扩展性和效率问题的框架，通过生成式引导策略从少量图像生成训练数据，使用可变场景块和扁平化向量集表示来提升大场景的几何保真度和效率。
...
Result: 框架能够生成不同大小和布局的场景，提供足够的训练数据，通过伪草图标签实现可控性并展示对未见草图的泛化能力。可变场景块和扁平化向量集表示显著减少大场景的token长度，在不同场景大小下保持一致的几何保真度，同时提高训练和推理效率。

Conclusion: NuiWorld框架成功解决了3D场景生成中的可控性、可扩展性和效率挑战，通过生成式引导策略克服数据稀缺问题，使用创新的场景表示方法提升大场景的保真度和计算效率，为视频游戏、模拟和机器人等应用提供了有效的世界生成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MM-RAG系统缺乏内部策略来决定何时以及如何进行检索，且依赖模块化流水线（检测器、分割器、字幕生成器等）。需要一种能统一区域级感知和检索增强推理的端到端解决方案。

Method: 提出PixSearch模型：1）编码时发射<search>标记触发检索；2）选择查询模态（文本、图像或区域）；3）生成像素级掩码直接作为视觉查询；4）采用两阶段监督微调方案，通过搜索交错监督学习检索时机和查询选择，同时保留分割能力。

Result: 在自我中心和实体中心VQA基准测试中，PixSearch显著提升事实一致性和泛化能力，在CRAG-MM上相比整图检索获得19.7%的相对准确率提升，同时在各种VQA和纯文本QA任务上保持竞争力的推理性能。

Conclusion: PixSearch成功统一了区域级感知和检索增强推理，通过端到端设计消除了对模块化流水线的依赖，在需要细粒度感知和外部事实知识的VQA任务中表现出色。
...
TL;DR: m2sv是一个用于地图到街景空间推理的基准测试，要求模型通过对齐北向地图和同一交叉口的街景图像来推断相机视角方向。尽管现有视觉语言模型在多模态基准上表现良好，但在m2sv上仅达到65.2%准确率，远低于人类95%的水平。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多模态基准测试中表现良好，但在需要将抽象俯视图与第一人称视角对齐的空间推理任务上仍然脆弱。需要评估模型在真实世界空间推理任务上的能力，特别是地图与街景图像的对齐能力。

Method: 1. 引入m2sv基准测试，包含m2sv-20k（地理多样、控制模糊性的数据集）和m2sv-sft-11k（用于监督微调的结构化推理轨迹）；2. 评估现有视觉语言模型在m2sv上的表现；3. 使用监督微调和强化学习改进模型；4. 进行跨基准测试评估；5. 通过结构信号和人类努力系统分析推理难度；6. 对适应后的开源模型进行广泛的失败分析。

Result: 1. 最佳视觉语言模型在m2sv上仅达到65.2%准确率，远低于人类基线95%；2. 监督微调和强化学习带来一致改进；3. 跨基准测试显示有限的迁移能力；4. 分析揭示了在几何对齐、证据聚合和推理一致性方面的持续差距。

Conclusion: 研究揭示了视觉语言模型在接地空间推理方面的持续挑战，特别是在跨视角的几何对齐能力上。m2sv基准测试为评估和改进模型的空间推理能力提供了重要工具，未来工作需要关注跨视角的接地空间推理。
...
TL;DR: GF-Screen是一种基于强化学习的胰腺癌筛查框架，采用"一瞥一聚焦"策略，通过Glance模型定位病变区域，Focus模型精确分割病灶，利用分割结果通过强化学习奖励Glance模型，有效解决CT扫描中微小病灶检测的挑战。


<details>
  <summary>Details</summary>
Motivation: 大规模CT扫描中的胰腺癌筛查对现有AI方法具有挑战性，主要困难在于在大型CT体积中定位多样类型的微小病灶。极端的前景-背景不平衡阻碍模型关注病变区域，而对健康区域的冗余关注不仅降低效率，还增加假阳性。

Method: 提出GF-Screen框架，包含Glance模型和Focus模型。Glance模型从整个CT体积中裁剪一组子体积，学习选择包含病灶的子体积供Focus模型分割。由于选择操作对分割训练不可微分，提出使用分割结果通过强化学习奖励Glance模型。引入新颖的组相对学习范式，通过组内相对比较优先处理高优势预测并丢弃低优势预测。

Result: 在16个内部和7个外部数据集上对9种病灶类型进行了广泛实验，证明了GF-Screen的有效性。在MICCAI FLARE25胰腺癌挑战赛的公开验证排行榜上领先，大幅超越FLARE24冠军解决方案（DSC提升25.6%，NSD提升28.2%）。

Conclusion: GF-Screen首次有效扩展了前沿强化学习技术来解决胰腺癌筛查中的特定挑战，通过"一瞥一聚焦"策略显著提高了大规模CT扫描中微小病灶检测的效率和准确性。
...
TL;DR: Reg-TTR提出了一种测试时精炼框架，将深度学习与传统配准技术相结合，通过推理时精炼预训练模型的预测，在仅增加21%推理时间（0.56秒）的情况下显著提升配准精度。


<details>
  <summary>Details</summary>
Motivation: 传统图像配准方法稳健但迭代过程缓慢；深度学习加速了推理但难以应对领域偏移；新兴的配准基础模型在速度与稳健性间取得平衡，但通常无法达到在特定数据集上训练的专用模型的峰值精度。

Method: 提出Reg-TTR测试时精炼框架，协同结合深度学习和传统配准技术的互补优势，在推理时对预训练模型的预测进行精炼。

Result: 在两个不同任务上评估显示，Reg-TTR实现了最先进的性能，同时保持接近先前深度学习方法的推理速度，仅需增加21%的推理时间（0.56秒）。

Conclusion: 随着基础模型的持续涌现，该框架提供了一种高效策略，可缩小配准基础模型与在专用数据集上训练的最先进方法之间的性能差距。
...
TL;DR: FBSDiff/FBSDiff++：从频域视角将文本到图像扩散模型适配为图像到图像翻译框架，通过动态频率带替换实现可控的图像翻译，无需训练或微调。


<details>
  <summary>Details</summary>
Motivation: 随着大规模文本到图像扩散模型在开放域图像生成方面取得显著进展，研究者开始关注其自然扩展到文本驱动的图像到图像翻译领域，其中源图像除了文本提示外还提供视觉指导。

Method: 提出FBSDiff框架，从频域视角将现成的T2I扩散模型适配到I2I范式，通过动态频率带替换扩散特征实现可控的图像翻译。FBSDiff++进一步改进：1) 大幅加速推理速度(8.9倍)；2) 改进频率带替换模块以支持任意分辨率和宽高比的输入；3) 扩展功能实现局部图像操作和风格特定内容创建。

Result: 广泛的定性和定量实验验证了FBSDiff++在图像到图像翻译的视觉质量、效率、多功能性和可控性方面优于相关先进方法。

Conclusion: FBSDiff++提供了一个高效、灵活且功能强大的图像到图像翻译框架，通过频域特征操作实现了高度可控的图像翻译，无需额外的模型训练或优化。
...
<details>
  <summary>Details</summary>
Motivation: 开放世界目标检测面临领域不变表示的挑战，特别是隐含的非因果因子问题。现有基于领域对抗学习的方法过度关注学习领域不变信息，但忽视了潜在的非因果因子。本文发现两个关键原因：1）基于领域判别器的方法受限于极其稀疏的领域标签；2）非因果因子过于隐含，传统对抗学习方法难以识别。

Method: 提出GB-DAL方法：1）原型粒度球分割模块：从有限数据集中生成更密集的领域，类似于更细粒度的粒度球，揭示更多潜在非因果因子；2）模拟非因果因子模块：通过类似对抗扰动的数据增强，降低非因果因子的隐含性，促进GB-DAL训练。

Result: 在多个基准测试上的对比实验表明，该方法在新型环境下实现了更好的泛化性能。

Conclusion: GB-DAL方法通过更密集的领域生成和模拟非因果因子增强，有效解决了开放世界目标检测中的领域不变表示问题，显著提升了模型在新环境下的泛化能力。
...
TL;DR: QA-ReID提出了一种质量感知的双分支匹配方法，通过结合RGB特征和解析特征来处理换衣行人重识别问题，使用多模态注意力融合和查询自适应卷积来增强对服装变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统的行人重识别方法在服装变化场景下面临严重挑战，因为服装变化会引入显著的外观变化。换衣行人重识别需要同时建模全局外观特征和服装不变的结构线索。

Method: 提出了质量感知双分支匹配方法：1）联合利用RGB特征和解析特征；2）通过多模态注意力模块自适应融合异质特征；3）在匹配阶段设计了质量感知查询自适应卷积，包含像素级重要性加权和双向一致性约束。

Result: 在PRCC、LTCC和VC-Clothes等多个基准测试上取得了最先进的性能，在跨服装场景下显著优于现有方法。

Conclusion: QA-ReID通过结合外观和结构特征，并采用质量感知的匹配机制，有效解决了换衣行人重识别的挑战，为服装变化场景下的行人重识别提供了有效的解决方案。
...
TL;DR: SNR-Edit是一种无需训练的图像编辑框架，通过自适应噪声控制实现忠实潜在轨迹校正，解决现有基于流生成模型的图像编辑方法中固定高斯噪声导致的轨迹偏差问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于流生成模型的无反转图像编辑方法依赖于固定高斯噪声构建源轨迹，这会导致轨迹动态偏差，引起结构退化或质量损失。需要一种方法来解决轨迹漂移问题，同时保持高质量的结构保留。

Method: SNR-Edit采用结构感知噪声校正技术，将分割约束注入初始噪声中，将源轨迹的随机分量锚定到真实图像的隐式反转位置，从而减少源-目标传输过程中的轨迹漂移。这是一个轻量级修改，无需模型调优或反转操作。

Result: 在SD3和FLUX模型上的评估显示，SNR-Edit在PIE-Bench和SNR-Bench基准测试中，在像素级指标和VLM-based评分方面均表现出色，同时每张图像仅增加约1秒的开销。

Conclusion: SNR-Edit通过自适应噪声控制实现了更平滑的潜在轨迹和高保真结构保留，为基于流生成模型的无反转图像编辑提供了一种有效且高效的解决方案。
...
TL;DR: 提出CSR方法，通过频谱引导的对比目标优化校正扰动，在对抗攻击下显著提升视觉语言模型的鲁棒性，平均优于SOTA 18.1%
...
Method: 提出对比频谱校正（CSR）方法：利用频谱引导的对比目标优化校正扰动，将输入重新对齐到自然流形上，并采用输入自适应方式应用。

Result: 在16个分类基准测试中，CSR在强AutoAttack下平均优于SOTA 18.1%，推理开销适中，且在不同视觉任务中表现出广泛的适用性。

Conclusion: CSR通过利用对抗样本的频谱特性，提供了一种高效、鲁棒的测试时防御方法，显著提升了视觉语言模型在对抗攻击下的安全性。
...
TL;DR: UniPCB是首个用于PCB质量检测的统一视觉语言基准，通过系统化流程整合多源数据，并提出了基于渐进式课程学习的PCB-GPT模型，在细粒度缺陷定位上性能提升超过2倍。
...
Method: 1. 提出UniPCB基准：通过系统化流程整合和标准化来自不同来源的数据，涵盖三种标注场景；2. 开发PCB-GPT模型：基于新生成的指令数据集训练MLLM，采用新颖的渐进式课程学习方法，模拟人类专家的学习过程。

Result: 在UniPCB基准上的评估显示，现有MLLMs在领域特定任务上表现不佳，而PCB-GPT建立了新的基准。特别是在细粒度缺陷定位方面，PCB-GPT的性能比最强竞争对手提升超过2倍，在定位和分析方面具有显著优势。

Conclusion: UniPCB填补了PCB质量检测领域缺乏统一视觉语言基准的空白，PCB-GPT通过渐进式课程学习方法显著提升了PCB检测性能。该研究将发布指令数据、基准和模型，以促进未来研究。
...
TL;DR: SimpleSeg是一种简单而有效的多模态大语言模型分割方法，将分割重构为序列生成问题，模型直接预测描述物体边界的点序列，无需特殊架构设计。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要复杂的任务特定设计或辅助组件来实现像素级感知，作者希望探索多模态大语言模型是否具有固有的低层感知能力，能否通过简单方法解锁这种能力。

Method: 将分割重构为序列生成问题，模型直接预测描述物体边界的点序列（文本坐标）。采用两阶段训练流程：SF→RL，其中强化学习使用基于IoU的奖励来优化点序列以准确匹配真实轮廓。

Result: 在分割基准测试中，SimpleSeg的性能与依赖复杂任务特定设计的方法相当，甚至常常超越这些方法，表明标准MLLM架构具有强大的固有低层感知能力。
...
TL;DR: 本文提出了视频连接任务和VC-Bench基准，包含1579个高质量视频，涵盖15个主要类别和72个子类别，专注于视频质量、首尾一致性和过渡平滑度三个核心指标，评估发现现有模型在一致性和平滑度方面存在显著不足。
...
Method: 提出VC-Bench基准，包含1579个从公共平台收集的高质量视频，覆盖15个主要类别和72个子类别。设计了三个核心评估指标：视频质量分数(VQS)、首尾一致性分数(SECS)和过渡平滑度分数(TSS)，形成超越传统质量指标的全面评估框架。

Result: 在VC-Bench上评估了多个最先进的视频生成模型，实验结果显示现有模型在保持首尾一致性和过渡平滑度方面存在显著局限性，导致整体连贯性和流畅性较低。

Conclusion: VC-Bench作为视频连接任务的先驱性基准，有望激发和指导未来研究。评估指标和数据集已公开提供，将推动视频连接技术的发展。
...
TL;DR: 本文系统评估了手工特征描述符在CIFAKE数据集上检测合成图像的性能，发现结合多种手工特征并使用LightGBM分类器能获得最佳检测效果。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展使得合成图像高度逼真，引发了数字媒体真实性和可信度的担忧。虽然深度学习方法主导当前研究，但手工特征因其可解释性、效率和泛化能力仍然具有吸引力。

Method: 在CIFAKE数据集（5万训练样本，1万测试样本）上系统评估了多种手工特征描述符，包括原始像素、颜色直方图、DCT、HOG、LBP、GLCM和小波特征。使用七种分类器（从逻辑回归到梯度提升集成方法如LightGBM、XGBoost、CatBoost）进行基准测试。

Result: LightGBM在混合特征配置下表现最佳，获得PR-AUC 0.9879、ROC-AUC 0.9878、F1 0.9447和Brier分数0.0414。在三种配置（基线、高级、混合）中，性能单调提升，表明结合多样化的手工特征能带来显著收益。

Conclusion: 精心设计的特征工程和集成学习在检测合成图像方面仍然具有重要价值，特别是在需要可解释性和计算效率的场景中。手工特征与先进分类器的结合为合成图像检测提供了有效且实用的解决方案。
...
TL;DR: 本文提出了一种多视图一致性框架来解决半监督域适应中的类别预测偏差问题，通过去偏策略和伪负标签机制提升分类性能，在DomainNet和Office-Home数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在半监督域适应中，目标域标记样本有限导致特征空间中类别相似性高，即使在平衡数据集上训练也会产生有偏预测，需要解决这一局限性。

Method: 提出多视图一致性框架，包含两个视图：1）去偏策略根据模型预测性能校正类别预测概率；2）利用模型预测生成伪负标签。同时引入跨域亲和性学习来对齐不同域中相同类别的特征。
...
TL;DR: ProMist-5K是一个专门用于模拟电影摄影中Pro-Mist滤镜效果的5K分辨率数据集，包含20,000张高分辨率图像对，支持数字图像到电影风格的转换。


<details>
  <summary>Details</summary>
Motivation: Pro-Mist滤镜在电影摄影中能产生柔和的晕染、降低对比度并创造独特的氛围风格，但这些效果难以通过数字方式准确再现，因为光扩散行为复杂。需要专门的数据集来支持电影风格的数字模拟。

Method: 采用物理启发的管线在场景参考线性空间中构建数据集，包含20,000张高分辨率图像对，涵盖两种滤镜密度（1/2和1/8）和两种焦距（20mm和50mm）。使用多层模糊和精心调整的权重来模拟光学扩散的不同强度和扩散范围。
...
TL;DR: 提出了首个大规模真实世界人脸阴影去除数据集ASFW，包含1081对阴影/无阴影图像，通过专业Photoshop工作流程创建，填补了合成与真实数据之间的鸿沟。
...
Method: 创建ASFW数据集：通过专业Photoshop工作流程生成1081对真实世界的人脸阴影/无阴影图像；提出Face Shadow Eraser（FSE）方法来展示数据集的有效性。

Result: 在ASFW上训练的深度模型在真实世界条件下表现出更好的阴影去除效果；实验证明ASFW提升了人脸阴影去除模型的性能，为该任务设定了新标准。

Conclusion: ASFW数据集填补了人脸阴影去除领域真实世界配对数据的空白，提供了逼真的阴影变化和准确的地面真值，显著提升了模型在真实场景中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 单目相机在3D检测中存在深度模糊和恶劣条件下鲁棒性差的问题，雷达虽然对光照和天气变化具有鲁棒性，但其稀疏性和低分辨率限制了直接应用，因此需要有效的雷达-相机融合方法

Method: 1. InstaRadar：基于实例分割引导的雷达扩展方法，利用预训练分割掩码增强雷达密度和语义对齐；2. 将预训练的RCDPT深度估计模块集成到BEVDepth框架中，替换其深度模块

Result: InstaRadar在雷达引导的深度估计中达到最先进水平，RCDPT集成持续提升3D检测性能，相比基线BEVDepth模型获得稳定增益
...
TL;DR: Innovator-VL是一个科学多模态大语言模型，通过透明训练流程和数据高效方法，在科学理解和推理方面表现出色，同时保持通用视觉任务性能，无需大规模预训练。


<details>
  <summary>Details</summary>
Motivation: 当前科学多模态模型通常依赖大规模领域特定预训练和不透明的流程，本研究旨在证明通过原则性训练设计和透明方法，可以用更少数据实现强大的科学智能。

Method: 提供完全透明、端到端可复现的训练流程，包括数据收集、清洗、预处理、监督微调、强化学习和评估；采用数据高效策略，使用少于500万精选样本，无需大规模预训练。

Result: 在多种科学任务上取得有竞争力的性能，同时在通用视觉、多模态推理和科学基准测试中表现良好，表明科学对齐可以集成到统一模型中而不损害通用能力。

Conclusion: 高效、可复现、高性能的科学多模态模型可以在没有大规模数据的情况下构建，为未来研究提供了实用基础，强调原则性数据选择比无差别扩展更有效。
...
TL;DR: 提出区域课程学习策略，优先学习确定区域并逐步纳入不确定区域，减少梯度方差；引入帕累托一致损失平衡区域不确定性，模糊标记机制处理边界模糊性，在脑转移瘤分割中优于传统方法。
...
Method: 1) 区域课程学习策略：优先学习确定区域，逐步纳入不确定区域以减少梯度方差；2) 帕累托一致损失：自适应重塑损失景观，平衡区域不确定性之间的权衡；3) 模糊标记机制：在非边界区域保持二值置信度，在边界附近实现平滑过渡，稳定梯度并扩展损失表面的平坦区域。

Result: 在脑转移瘤和非转移性肿瘤分割实验中，该方法在多种配置下均表现出一致的改进，在所有肿瘤子区域中都优于传统的清晰集方法。

Conclusion: 通过区域课程学习、帕累托一致损失和模糊标记机制，有效解决了医学图像分割中不确定性分布不均匀的问题，提高了分割性能，特别是在边界模糊区域。
...
TL;DR: 开发了一个名为DermpathNet的大型开源皮肤病理学图像数据集，包含7,772张图像，涵盖166种诊断，通过混合工作流程从PubMed Central获取并分类


<details>
  <summary>Details</summary>
Motivation: 临床医生和皮肤病理学学员在获取高质量、开源皮肤病理学图像数据集用于学习和交叉参考方面面临挑战，需要建立全面的开源数据集用于教育、交叉参考和机器学习目的

Method: 采用混合工作流程：1) 使用特定关键词从PubMed Central提取相关图像；2) 结合深度学习图像模态分类和图像标题分析的新颖混合方法进行分类；3) 由认证皮肤病理学家审查和标注

Result: 在651张手动标注图像上验证：深度学习方法F-score为89.6%，关键词检索方法为61.0%，混合方法为90.4%。最终获得7,772张图像，涵盖166种诊断，创建了DermpathNet数据集

Conclusion: 开发了一个大型、同行评审、开源的皮肤病理学图像数据集DermpathNet，具有半自动化的策展工作流程，并发现当前OpenAI的图像分析算法不足以分析皮肤病理学图像
...
<details>
  <summary>Details</summary>
Motivation: 开发一个全面、免费的肺结节分析流程，整合多个公开数据集训练的开放访问模型，旨在提高敏感性同时减少标注者的候选负担

Method: 使用多个基于公共数据集训练的开放访问模型，开发了Tri-Reader三阶段工作流程：肺部分割、结节检测和恶性分类，形成统一管道

Result: 在多个内部和外部数据集上评估Tri-Reader，与专家标注和数据集提供的参考标准进行比较，确保准确性和跨不同实践的泛化能力

Conclusion: Tri-Reader是一个综合的免费肺结节分析管道，通过三阶段工作流程优先考虑敏感性，减少标注负担，并在多个数据集上验证了其准确性和泛化能力
...
TL;DR: 该论文提出了X-AIGD基准，用于可解释的AI生成图像检测，提供像素级分类标注，发现现有检测器很少依赖可感知的伪影特征，并提出通过显式对齐模型注意力来提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成图像检测方法主要依赖二元分类，缺乏可解释的证据来支持其决策。现有基准虽然包含大量合成图像，但缺乏伪影多样性的覆盖和详细的局部标注，限制了检测器的可解释性评估。

Method: 提出了X-AIGD基准，提供像素级、分类标注的可感知伪影，涵盖低级失真、高级语义和认知级反事实三个层次。使用该基准对现有AIGI检测器进行深入分析，并探索通过显式对齐模型注意力与伪影区域来提高可解释性的方法。

Result: 研究发现：(1)现有AIGI检测器对可感知伪影的依赖微乎其微，即使在最基本的失真层面；(2)虽然检测器可以被训练识别特定伪影，但判断仍主要基于不可解释的特征；(3)显式对齐模型注意力与伪影区域可以提高检测器的可解释性和泛化能力。

Conclusion: X-AIGD基准为可解释的AI生成图像检测提供了重要工具，揭示了现有检测器在可解释性方面的局限性，并展示了通过注意力对齐提高可解释性的潜力，为开发更可信的AIGI检测器奠定了基础。
...
TL;DR: 提出了一种结合CNN和SAM的双学生-教师框架（DSTCS），用于超声图像中耻骨联合和胎头分割，通过协同学习和专门的数据增强策略显著提升了分割精度。
...
Method: 对八种立体方法（涵盖迭代细化、基础模型、基于扩散和3D CNN范式）进行首次系统零样本评估，使用官方发布的预训练权重（在Scene Flow上训练），在四个标准基准（ETH3D、KITTI 2012/2015、Middlebury）和新的5,313对Canterbury Tree Branches数据集上进行评估。

Result: 结果显示场景依赖性模式：基础模型在结构化场景中表现出色（BridgeDepth在ETH3D上为0.23像素；DEFOM在Middlebury上为4.65像素），而迭代方法在不同基准间表现不一。在Tree Branches数据集上的定性评估确定DEFOM为植被深度估计的黄金标准基线，具有优异的跨域一致性（在所有基准中排名1-2位，平均排名1.75）。

Conclusion: 该研究填补了植被密集环境深度估计评估的空白，建立了DEFOM作为植被深度估计的基准标准，其预测结果将作为未来基准测试的伪地面真值。
...
TL;DR: Dyn-HSI是首个动态人-场景交互的认知架构，通过赋予虚拟人类视觉、记忆和控制三个类人组件，在动态场景中生成高质量的人-场景交互动作。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的场景是持续动态变化的，但现有的人-场景交互生成方法通常将场景视为静态，这与现实不符。为了解决这一局限性，需要开发能够处理动态场景的人-场景交互生成方法。

Method: Dyn-HSI采用认知架构，包含三个类人组件：1)视觉（动态场景感知导航）-持续感知环境变化并自适应预测下一个路径点；2)记忆（分层经验记忆）-存储和更新训练期间积累的经验数据，在推理时利用先验知识进行上下文感知的运动初始化；3)控制（人-场景交互扩散模型）-基于多模态输入生成高保真交互动作。同时构建了动态基准数据集Dyn-Scenes。

Result: 在Dyn-Scenes动态基准上的广泛定性和定量实验表明，Dyn-HSI在静态和动态设置下都优于现有方法，能够生成高质量的人-场景交互动作，并展现出更好的泛化能力。

Conclusion: Dyn-HSI是首个动态人-场景交互的认知架构，通过模拟人类的视觉、记忆和控制能力，成功解决了动态场景中的人-场景交互生成问题，在质量和泛化性方面都表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 视频token与语言token存在语义密度和冗余度的差异：视频token语义密度低、时空冗余度高，导致静态top-k/top-p采样策略在视频解码中效果不佳，要么在低不确定性区域引入不必要随机性，要么在高不确定性区域陷入早期错误累积

Method: 提出熵引导的k-Guard（ENkG）采样策略，根据每个token预测分布的熵值自适应调整候选集大小：低熵区域使用较少候选以抑制冗余噪声保持结构完整性，高熵区域使用更多候选以缓解错误累积

Result: 实验表明ENkG相比静态top-k/top-p策略在感知质量和结构稳定性方面有持续改进，且该方法模型无关、无需训练、计算开销可忽略

Conclusion: ENkG采样策略有效解决了视频生成中静态采样策略的局限性，通过自适应候选集大小平衡了结构保持和错误缓解的需求，提升了长序列视频生成质量
...
TL;DR: 针对SIGGRAPH Asia 3DGS快速重建挑战赛，开发了一个能在1分钟内完成高质量3D高斯重建的两阶段解决方案，分别处理SLAM噪声轨迹和COLMAP精确位姿两种场景，最终以PSNR 28.43获得比赛第一名。


<details>
  <summary>Details</summary>
Motivation: SIGGRAPH Asia 3DGS快速重建挑战赛要求在1分钟内完成3D高斯重建，但面临两个不同场景：初赛使用SLAM生成的噪声相机轨迹，决赛使用COLMAP的精确位姿。需要开发一个能同时处理这两种异构设置的鲁棒解决方案。

Method: 采用两阶段方法：初赛阶段针对SLAM噪声轨迹，使用反向逐高斯并行优化、紧凑前向光栅化、负载均衡分块、基于锚点的神经高斯表示（减少可学习参数）、单目深度和部分前馈3DGS模型初始化、全局位姿优化模块。决赛阶段针对精确COLMAP位姿，禁用位姿优化，从神经高斯回归标准3DGS（消除MLP推理开销），引入多视角一致性引导的高斯分裂，并使用深度估计器监督渲染深度。

Result: 该方法在严格的1分钟时间限制下实现了高质量重建，最终以PSNR 28.43的成绩在比赛中获得第一名。

Conclusion: 通过针对不同位姿质量场景的定制化两阶段策略，成功开发了一个能在1分钟内完成高质量3D高斯重建的快速管道，证明了在异构设置下实现快速收敛和高保真重建的可行性。
...
TL;DR: Cor2Vox是一个基于大脑皮层结构先验的生成框架，通过3D形状到图像的布朗桥扩散过程，实现解剖学上可信的脑MRI合成，在多个应用中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有神经影像生成模型主要依赖标签或文本等弱条件信号，缺乏解剖学基础，常产生生物学上不可信的输出。合成神经影像数据可以解决真实数据集中罕见表型稀缺、扫描仪间域偏移和纵向覆盖不足等关键限制。

Method: 提出Cor2Vox框架，利用高分辨率皮层表面引导3D形状到图像的布朗桥扩散过程，实现拓扑保真的合成和对底层解剖结构的精确控制。基于33,000多个UK Biobank扫描开发了大规模皮层形态统计形状模型。

Result: 在传统图像质量指标、高级皮层表面重建和全脑分割质量方面验证了Cor2Vox的保真度，优于多个基线方法。在三个应用中（解剖一致合成、渐进灰质萎缩模拟、内部额颞叶痴呆扫描与公共数据集协调）均表现出色。

Conclusion: Cor2Vox能够保持亚体素水平的细粒度皮层形态，对皮层几何形状和疾病表型的变化表现出显著的鲁棒性，无需重新训练即可实现解剖学上一致的脑MRI合成。
...
TL;DR: Pref-Restore是一个用于盲人脸修复的分层框架，通过整合离散语义逻辑和连续纹理生成来解决信息不对称问题，实现确定性、偏好对齐的修复。
...
Method: 采用两种互补策略：1) 通过自回归积分器将文本指令重构为密集潜在查询，增强输入密度；2) 将策略强化学习直接集成到扩散修复循环中，通过人类偏好惩罚随机偏差，修剪输出分布。

Result: 在合成和真实世界基准测试中达到最先进性能，偏好对齐策略显著降低解决方案熵，为可靠和确定性的盲修复建立了稳健路径。

Conclusion: Pref-Restore通过整合语义逻辑和纹理生成，有效解决了盲人脸修复中的信息不对称问题，实现了确定性、偏好对齐的修复，显著减少了随机不确定性和幻觉伪影。
...
TL;DR: 提出一种仅使用身体佩戴UWB传感器的稀疏成对距离测量来重建全身3D动作的新型运动捕捉系统，无需外部摄像头，适用于非受控户外环境。


<details>
  <summary>Details</summary>
Motivation: 传统光学或惯性运动捕捉系统受环境限制（如光照、磁场干扰），需要外部设备且对形状敏感。需要一种能在非受控户外环境中稳健运行、形状不变且成本低廉的运动捕捉方案。

Method: 使用身体佩戴的UWB传感器进行飞行时间测距，获取稀疏成对距离测量。核心是Wild-Poser（WiP），一种紧凑的实时Transformer架构，直接从噪声或损坏的距离测量预测3D关节位置，然后通过学习方法重建关节旋转。

Result: WiP能实时运行，实现低关节位置误差，准确重建人类和动物在野外环境中的3D动作。系统对不同形态的受试者（包括非人类物种）具有泛化能力，无需个体身体测量或形状拟合。

Conclusion: 该系统展示了在真实世界环境中实现可扩展、低成本、通用目的运动捕捉的潜力，为户外和非受控环境下的动作捕捉提供了创新解决方案。
...
TL;DR: S3LI Vulcano数据集是一个多模态SLAM和地点识别基准数据集，包含意大利火山岛上的多种环境数据，配有开源工具包用于生成真值位姿和标注样本。


<details>
  <summary>Details</summary>
Motivation: 为基于视觉和激光雷达的SLAM及地点识别算法提供开发和基准测试的多模态数据集，解决现有数据集在特定环境类型（如火山地形）覆盖不足的问题。

Method: 在意大利西西里岛埃奥利群岛的火山岛Vulcano上记录多个序列，采集包含玄武岩/富铁岩石、古老熔岩通道地质构造、干燥植被和水体等多种环境的数据，提供视觉和LiDAR模态数据。

Result: 发布了S3LI Vulcano数据集，包含多种环境纹理和地形的多模态序列数据，并配套开源工具包（s3li-toolkit）用于生成地面真值位姿和地点识别任务的标注样本准备。

Conclusion: 该数据集为SLAM和地点识别算法的开发和基准测试提供了有价值的多模态资源，特别针对火山地形等特定环境，配套工具包增强了数据集的实用性和可扩展性。
...
TL;DR: MaDiS是一个基于掩码扩散的语言模型，用于手语生成，通过双向依赖建模和并行多令牌生成解决了传统自回归模型的局限性，显著提升了生成质量和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有手语生成研究主要采用自回归语言模型框架，存在单向上下文建模和逐令牌推理速度慢的问题，需要更高效、表达能力更强的模型来改善手语生成质量。

Method: 提出MaDiS模型：1）基于掩码扩散的语言模型，支持双向依赖建模和并行多令牌生成；2）三层次跨模态预训练方案，联合学习令牌、潜在和3D物理空间目标；3）带时间检查点的解掩码策略，大幅降低解掩码顺序组合复杂度；4）混合部件嵌入层，通过可学习门控和优化码本融合不同部件令牌信息。

Result: 在CSL-Daily、Phoenix-2014T和How2Sign数据集上的实验表明，MaDiS在DTW误差和两个新指标SiBLEU、SiCLIP上均取得优越性能，同时推理延迟降低近30%。

Conclusion: MaDiS通过掩码扩散架构、三层次预训练和高效解掩码策略，显著提升了手语生成的质量和效率，为手语生成任务提供了新的解决方案。
...
TL;DR: QuaMo提出了一种基于四元数微分方程（QDE）的3D人体运动捕捉方法，解决了传统基于欧拉角方法的不连续性问题，通过状态空间模型和元PD控制器实现更准确、连续的运动估计。


<details>
  <summary>Details</summary>
Motivation: 传统3D姿态估计方法常忽略帧间时间一致性，导致运动不自然和抖动；基于运动学的方法虽能改善但依赖欧拉角，存在不连续性问题，尤其在在线设置中无法进行轨迹优化。四元数具有连续性优势，但现有方法未充分利用。

Method: 提出QuaMo方法：1）使用四元数微分方程（QDE）描述人体运动学；2）采用状态空间模型，以四元数为状态，QDE描述四元数速度；3）通过元PD控制器计算角加速度，并引入自适应调节控制信号的加速度增强机制；4）在四元数单位球约束下求解QDE。

Result: 实验表明，QuaMo的QDE加加速度增强方法能够准确估计3D人体运动学，无间断且最小化不合理性。在Human3.6M、Fit3D、SportsPose和AIST等多个数据集上优于现有最先进方法。

Conclusion: QuaMo通过四元数微分方程和加速度增强机制，成功解决了基于欧拉角的运动捕捉方法的不连续性问题，实现了更准确、连续的3D人体运动估计，为视频中的运动捕捉提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对自动驾驶场景的视觉语言模型评估基准，需要建立能够全面评估VLM在安全关键驾驶环境中能力的标准化测试框架。

Method: 基于ScenePilot-4K数据集（3,847小时驾驶视频）构建，包含多粒度标注（场景描述、风险评估、关键参与者识别、自车轨迹、相机参数），采用四轴评估套件（场景理解、空间感知、运动规划、GPT-Score）和安全感知指标。

Result: 对代表性VLM进行基准测试，提供了当前性能边界的实证分析，识别了面向驾驶推理的差距，建立了自动驾驶VLM评估的全面框架。

Conclusion: ScenePilot-Bench为评估和推进视觉语言模型在安全关键自动驾驶环境中的应用提供了全面的基准框架，有助于明确当前技术边界和未来研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 肉毒毒素注射是面部不对称和美学年轻化的金标准，但确定最佳剂量主要依赖直觉，常导致次优结果，需要更精确的注射规划方法

Method: 开发区域特异性潜在轴发现方法，在StyleGAN2潜在空间中学习局部肌肉松弛轨迹，建立剂量响应模型；比较直接度量回归与基于图像的生成模拟两种方法

Result: 在保留测试集上，框架显示出中度到强结构相关性，生成模型正确捕捉形态变化方向；引入"人在回路"混合工作流程，让临床医生交互式优化模拟

Conclusion: 虽然生物变异性限制了绝对精度，但该框架通过剂量响应建模和临床医生交互，为病理重建和美容规划之间的差距提供了桥梁
...
<details>
  <summary>Details</summary>
Motivation: 现有视频-音频联合嵌入方法（如CAVP）虽然能建模语义和时间对应关系，但对视频和音频信号的多尺度密集特性建模不足，对应关系跨越从细粒度到粗粒度的时空结构，现有框架未能充分利用这些特性。

Method: 提出GMS-CAVP框架：1）多尺度对比学习策略，捕捉不同粒度的语义和时间关系；2）超越传统对比学习，引入基于扩散的生成目标，实现视频和音频之间的模态转换和合成。这种统一的判别-生成式框架促进更深层次的跨模态理解。

Result: 在VGGSound、AudioSet和Panda70M数据集上的大量实验表明，GMS-CAVP在生成和检索任务上优于先前的方法。

Conclusion: GMS-CAVP通过结合多尺度对齐和扩散预训练目标，有效增强了视频-音频对应关系建模，为高保真生成任务铺平了道路，在跨模态理解和生成方面取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 差分隐私（DP）虽然能保护敏感数据，但通常会导致诊断性能显著下降。模型初始化被认为是缓解这种性能下降的关键因素，但现代自监督学习在全模型DP下的作用尚不清楚。本文旨在系统评估不同初始化策略在差分隐私医疗影像分析中的效果。

Method: 使用胸片分类作为代表性基准任务，包含超过80万张图像。采用最先进的ConvNeXt模型，在现实隐私保护机制下使用DP-SGD训练。比较三种初始化策略：1) 非领域特定的监督ImageNet初始化；2) 非领域特定的自监督DINOv3初始化；3) 领域特定的监督预训练（在MIMIC-CXR数据集上）。在五个外部数据集上进行评估，涵盖不同机构和采集设置。

Result: DINOv3初始化在DP下相对于ImageNet初始化能持续提升诊断效用，但仍不如领域特定的监督预训练。领域特定的监督预训练能达到最接近非隐私基线的性能。初始化选择强烈影响人口统计学公平性、跨数据集泛化能力，以及在隐私约束下对数据规模和模型容量的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有以对象为中心的感知范式和松散的视觉语言流程难以支持实际城市治理中所需的管理导向异常理解，需要填补这一空白。

Method: 1) 创建GovLA-10K基准数据集，围绕功能显著目标设计，提供基于观察的可操作管理建议；2) 开发GovLA-Reasoner框架，通过高效特征适配器隐式协调视觉检测器与大语言模型之间的判别性表示共享。

Result: 实验表明该方法显著提升了性能，同时避免了为任何任务特定组件进行微调的需求。

Conclusion: 该研究为未来管理感知的低空视觉语言系统提供了新的视角和基础。
...
Method: 首先分析模型参数空间中的知识保留机制，发现通用知识主要编码在主成分子空间，而任务特定知识编码在残差子空间。基于此发现，KeepLoRA通过将新任务的梯度投影到与预训练模型主成分子空间和先前任务特征主导方向正交的子空间，将LoRA参数更新限制在残差子空间，避免干扰先前学习的能力。

Result: 理论和实证分析证实KeepLoRA能够有效平衡三个目标，并实现了最先进的性能表现。

Conclusion: KeepLoRA提供了一种简单但有效的持续学习方法，通过参数空间分解策略成功平衡了预训练知识保留、任务知识保持和新知识获取的竞争需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像相似度度量方法不是感知度量，难以评估包含纹理失真的图像，且深度学习方法缺乏可解释性。
...
Conclusion: 该方法不仅提供更准确的感知相似度评分，还能提供可视化解释，使相似度评估更加透明和可解释。
...
Method: 提出SharpNet架构，通过引入辅助特征函数（定义为具有跳跃Neumann边界条件的泊松方程的解），使用高效局部积分评估，该积分对特征位置完全可微，从而能够联合优化特征位置和MLP参数。

Result: 在2D问题和3D CAD模型重建任务中，SharpNet准确恢复了尖锐边缘和角点，同时在远离这些特征的地方保持平滑行为，而现有方法倾向于平滑梯度不连续性。

Conclusion: SharpNet能够精确控制C^0连续性，在特征位置确保C^0连续性，在其他地方保持平滑，为表示具有尖锐特征的函数提供了有效解决方案。
...
Conclusion: Video-KTR通过简单的即插即用扩展提升了强化学习在复杂视频推理中的准确性和可解释性，为多模态大语言模型的推理增强提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有Vision Mamba模型虽然能有效处理长距离依赖关系，但在医学图像分割中主要关注复杂架构设计。本文旨在通过简单有效的自蒸馏方法提升VM-UNet性能，避免复杂结构设计

Method: 提出DSVM-UNet方法，采用双重自蒸馏技术，在全局和局部两个层次对齐特征表示，增强模型对语义特征的感知能力

Result: 在ISIC2017、ISIC2018和Synapse基准测试中实现了最先进的性能，同时保持了计算效率

Conclusion: 提出的双重自蒸馏方法简单有效，能够显著提升VM-UNet在医学图像分割中的性能，无需复杂架构设计，具有实用价值
...
TL;DR: SWEET是一个自监督框架，通过约束式预训练实现视觉任务中可变大小模型的可扩展初始化，使用共享权重模板和尺寸特定权重缩放器支持不同深度和宽度的架构适配。


<details>
  <summary>Details</summary>
Motivation: 现代模型参数规模和复杂性不断增加，预训练模型的重要性日益凸显。然而实际部署需要不同大小的架构，传统预训练和微调方法存在局限性，需要一种能够支持可变大小模型初始化的方法。

Method: 提出SWEET框架，采用基于Tucker分解的共享权重模板和尺寸特定权重缩放器学习。通过轻量级权重缩放器组合和重加权模板来初始化目标模型，并引入宽度方向随机缩放来增强宽度扩展的灵活性。

Result: 在分类、检测、分割和生成任务上的广泛实验表明，SWEET在初始化可变大小视觉模型方面达到了最先进的性能。

Conclusion: SWEET通过约束式预训练和模块化设计，有效解决了可变大小模型初始化问题，支持灵活的架构适配，为视觉任务提供了可扩展的初始化方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉Transformer中常用的矩形token会包含识别对象之外的信息，影响性能。耳朵生物识别中的形态变化与Transformer架构的位置敏感性之间存在不匹配问题。

Method: 提出PaW-ViT（基于补丁的扭曲视觉Transformer），通过解剖知识将耳朵图像归一化，将token边界准确对齐到检测到的耳朵特征边界，使特征边界与自然耳朵曲率对齐。

Result: 实验证实PaW-ViT在各种ViT模型（ViT-T、ViT-S、ViT-B、ViT-L）上有效，对形状、大小和姿态变化具有合理的对齐鲁棒性。

Conclusion: 该方法解决了耳朵生物识别形态变化与Transformer架构位置敏感性之间的脱节问题，为身份验证方案提供了可能途径。
...
TL;DR: GeoDiff3D是一个自监督的3D场景生成框架，使用粗糙几何作为结构锚点，结合几何约束的2D扩散模型生成纹理丰富的参考图像，无需严格的多视角一致性，能够高效生成高质量3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景生成方法存在两大问题：间接2D-to-3D重建和直接3D生成方法都受限于弱结构建模和对大规模真实监督数据的严重依赖，导致结构伪影、几何不一致和高频细节退化。需要一种更高效、更少依赖标注数据的解决方案。

Method: 1) 使用粗糙几何作为结构锚点；2) 采用几何约束的2D扩散模型生成纹理丰富的参考图像；3) 不要求扩散生成参考图像的多视角一致性，对噪声和不一致指导具有鲁棒性；4) 引入体素对齐的3D特征聚合和双重自监督机制；5) 低计算成本训练。

Result: 在具有挑战性的场景上进行广泛实验，相比现有基线方法展现出更好的泛化能力和生成质量，能够快速生成高质量的3D场景，显著减少对标注数据的依赖。

Conclusion: GeoDiff3D提供了一个实用、高效的3D场景构建解决方案，通过自监督框架结合几何约束和扩散模型，克服了现有方法的局限性，为游戏、影视特效和VR/AR等领域提供了可访问的高质量3D内容生成工具。
...
TL;DR: 该研究评估了扩散式耳部修复技术作为预处理辅助手段，用于缓解耳部饰品遮挡对基于Transformer的耳部识别系统性能的影响。


<details>
  <summary>Details</summary>
Motivation: 耳部饰品（如耳环、耳机）造成的遮挡会严重影响基于耳部的生物特征识别系统性能，特别是在非约束成像条件下。需要有效的方法来缓解这种遮挡问题。

Method: 使用扩散式耳部修复技术作为预处理辅助：给定输入耳部图像和自动生成的饰品掩码，修复模型通过合成缺失像素来重建干净且解剖学上合理的耳部区域，同时保持关键耳部结构（耳轮、对耳轮、耳甲、耳垂）的局部几何一致性。

Result: 实验表明，扩散式修复技术可以有效缓解耳部饰品遮挡问题，提高基于Transformer的耳部识别系统的整体识别性能。研究评估了多种视觉Transformer模型和不同补丁大小在多个基准数据集上的表现。

Conclusion: 扩散式耳部修复技术可以作为有效的预处理辅助手段，减轻耳部饰品遮挡对识别系统的影响，从而提升整体识别性能。
...
Conclusion: Youtu-VL为开发全面的通用视觉智能体建立了坚实基础，通过将优化目标从"视觉作为输入"转变为"视觉作为目标"，显著提升了模型的多模态理解能力
...
Method: 提出查询引导的时空频交互方法(QSTar)，结合问题引导线索，利用音频信号的频域特性以及时空感知；引入查询上下文推理块(QCR)，引导模型更精确关注语义相关的音视频特征

Result: 在多个AVQA基准测试中，相比现有的音频QA、视觉QA、视频QA和AVQA方法，取得了显著的性能提升

Conclusion: QSTar方法通过有效整合问题引导线索和音频频域特性，显著提升了音视频问答任务的性能，代码和预训练模型将在发表后发布
...
<details>
  <summary>Details</summary>
Motivation: 图像、文本和图等多模态数据通常包含层次和关系结构，这些结构在欧几里得几何中难以有效建模。双曲几何为表示此类结构提供了自然框架，因此研究如何将双曲几何融入视觉Transformer架构。

Method: 提出HexFormer双曲视觉Transformer，包含两种设计：纯双曲ViT（HexFormer）和混合变体（HexFormer-Hybrid），后者结合双曲编码器和欧几里得线性分类头。核心创新是基于指数映射聚合的新型注意力机制，相比标准质心平均方法能产生更准确稳定的聚合表示。
...
Result: 在ARCTIC和EgoExo4D数据集上实验显示，相比最先进方法取得一致性能提升。同时展示了真实世界的泛化能力，并通过重建手部作为视觉提示改善了EgoVLM的手物交互推理。
...
TL;DR: SONIC是一种连续光谱参数化方法，使用少量共享的定向选择性组件建模卷积算子，实现全局感受野和跨分辨率自适应滤波，在多个任务中优于传统卷积、注意力机制和光谱架构，参数数量少一个数量级。


<details>
  <summary>Details</summary>
Motivation: CNN依赖固定大小核扫描局部区域，难以捕获全局上下文或长程依赖；ViT提供全局连接但缺乏空间归纳偏置，依赖显式位置编码且受初始补丁大小限制。需要一种既结构化又全局的表示方法。

Method: 引入SONIC（光谱定向神经不变卷积），一种连续光谱参数化方法，使用少量共享的定向选择性组件建模卷积算子。这些组件在完整频率域定义平滑响应，产生全局感受野和跨分辨率自然适应的滤波器。

Result: 在合成基准测试、大规模图像分类和3D医学数据集上，SONIC对几何变换、噪声和分辨率变化表现出更好的鲁棒性，匹配或超越卷积、注意力机制和先前光谱架构，参数数量少一个数量级。

Conclusion: 连续、定向感知的光谱参数化为传统空间和光谱算子提供了原则性且可扩展的替代方案，能够同时实现结构化和全局表示。
...
<details>
  <summary>Details</summary>
Motivation: 改进VGGT-SLAM系统，解决其存在的高维15自由度漂移和平面退化问题，同时处理相机内参未知时VGGT的重建模糊性，并提升图像检索验证的准确性。

Method: 1. 设计新的因子图结构来消除高维漂移和平面退化；2. 研究VGGT的注意力层，发现其中一个层适合用于图像检索验证，无需额外训练即可拒绝误匹配并完成更多闭环；3. 在多种环境中进行实验验证。

Result: 1. 系统可在Jetson Thor上实现地面机器人的在线实时运行；2. 在杂乱室内公寓、办公室场景和4200平方英尺的谷仓等环境中表现良好；3. 在TUM数据集上达到最高精度，位姿误差比VGGT-SLAM减少约23%；4. 可轻松适配开放集物体检测。

Conclusion: VGGT-SLAM 2.0通过改进因子图设计和利用VGGT注意力层，显著提升了SLAM系统的精度和鲁棒性，在多种真实环境中实现了实时高性能的定位与建图。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在处理阿拉伯文字，特别是艺术化和风格化的书法形式方面能力不足，缺乏专门的评估基准来测试模型对复杂阿拉伯书写系统的理解。

Method: 创建DuwatBench数据集，包含1272个精心策划的样本，涵盖约1475个独特词汇，覆盖六种古典和现代书法风格，每个样本都配有句子级检测标注。使用该数据集评估了13个领先的阿拉伯语和多语言多模态模型。

Result: 评估显示，虽然现有模型在处理干净文本时表现良好，但在处理书法变体、艺术扭曲和精确的视觉-文本对齐方面存在困难，表明当前模型对阿拉伯书法形式的理解有限。

Conclusion: DuwatBench的发布旨在推动基于文化的多模态研究，促进阿拉伯语言和视觉遗产在AI系统中的公平包容，并支持该领域的持续进步。数据集和评估套件已公开可用。
...
TL;DR: 论文提出基于语言家族共享连接器的多语言ASR策略，减少参数数量同时提升跨领域泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的ASR系统为每种语言训练单独连接器，忽略了语言之间的相关性，导致参数效率低下

Method: 提出基于语言家族成员关系的连接器共享策略，每个语言家族使用一个连接器，连接冻结的语音编码器和预训练LLM

Result: 在两个多语言LLM和两个真实世界语料库上的实验表明，家族式连接器减少参数数量同时提升跨领域泛化能力

Conclusion: 基于语言家族的连接器共享策略为多语言ASR部署提供了实用且可扩展的解决方案
...
TL;DR: 提出新的校准探测框架评估语言模型关系知识的可靠性，发现多数模型（特别是掩码预训练模型）过度自信，最佳校准来自考虑陈述重述不一致性的置信度估计。


<details>
  <summary>Details</summary>
Motivation: 现有知识探测方法仅通过准确率等指标评估模型能力，未能考虑模型可靠性（反映在其置信度校准上），需要新的框架来评估关系知识的校准性。

Method: 提出关系知识校准探测框架，涵盖三种模型置信度模态：内在置信度、结构一致性和语义基础，分析了10个因果模型和6个掩码语言模型。

Result: 大多数模型（特别是掩码预训练模型）过度自信；最佳校准得分来自考虑陈述重述不一致性的置信度估计；即使最大的预训练模型也无法准确编码语言置信度表达式的语义。

Conclusion: 需要超越传统准确性指标来评估语言模型的知识可靠性，校准探测框架揭示了模型置信度校准的重要缺陷，特别是掩码预训练模型的过度自信问题。
...
<details>
  <summary>Details</summary>
Motivation: 推测解码通常需要在大数据集上训练草稿模型，但并非所有训练样本对接受率贡献相同。研究发现目标模型预测分布较平坦的token更有价值，这启发了基于数据中心的优化方法。

Method: 提出平坦度指标量化token的预测分布特性，开发样本级平坦度数据集蒸馏方法SFDD，筛选保留最有价值的训练样本，减少训练数据量同时保持模型性能。

Result: 在EAGLE框架上的实验表明，SFDD仅使用50%训练数据即可实现超过2倍的训练加速，最终模型的推理加速效果与全数据集基线相比仅损失4%。

Conclusion: 该工作提出了一种有效的数据中心方法，显著提升了推测解码的训练效率，通过数据筛选优化实现了训练加速与性能保持的良好平衡。
...
TL;DR: 研究人员开发了BabyReasoningBench基准测试，用于评估在儿童发展数据上训练的"婴儿语言模型"的推理能力，发现这些模型在因果推理方面表现较好，但在心理理论和语用敏感任务上仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型推理评估主要针对成人设计，基于广泛世界知识、复杂指令遵循和成熟语用能力，这与在儿童发展数据上训练的"婴儿语言模型"不匹配，无法准确评估在儿童发展约束下哪些推理能力能够出现。

Method: 开发了BabyReasoningBench基准测试，包含19个基于发展心理学经典范式的推理任务，涵盖心理理论、类比关系推理、因果推理和干预选择、以及核心推理原语等任务类别。在两个基于GPT-2架构、分别在1000万和1亿儿童导向语音文本上预训练的婴儿语言模型上进行测试。

Result: 婴儿语言模型整体表现较低但不均衡：模型规模扩大改善了多个因果和物理推理任务的表现，但信念归因和语用敏感任务仍然具有挑战性。不同任务家族之间存在明显差异。

Conclusion: BabyReasoningBench为分析儿童发展数据训练分布支持的推理类型提供了发展心理学基础框架，可用于测试关于这些能力如何出现的机制假设，揭示了在儿童发展约束下不同推理能力的出现模式。
...
TL;DR: LLMs在程序终止性预测任务上表现出色，在SV-Comp 2025终止性类别中接近顶尖工具性能，但无法提供有效证明且性能随程序长度增加而下降。


<details>
  <summary>Details</summary>
Motivation: 图灵停机问题的不可判定性使得传统验证工具只能近似处理终止性问题，且通常受限于特定架构和编程语言。随着LLMs的成功，研究其能否可靠预测程序终止性成为一个重要问题。

Method: 使用国际软件验证竞赛(SV-Comp) 2025终止性类别中的多样化C程序集，评估多种LLMs（包括GPT-5、Claude Sonnet-4.5和Code World Model）在程序终止性预测任务上的表现。

Result: LLMs在预测程序终止性方面表现优异：GPT-5和Claude Sonnet-4.5的性能仅次于排名第一的工具（使用测试时缩放），CWM则仅次于排名第二的工具。然而，LLMs通常无法提供有效的证明见证，且性能随程序长度增加而下降。

Conclusion: LLMs能够有效预测程序终止性，但在提供证明方面存在局限，且受程序长度影响。这些发现为进一步研究LLMs在不可判定问题推理方面的潜力提供了动力。
...
TL;DR: LLMs可通过绕过安全防护、重新解释NLP论文中的研究工具来生成有害研究提案，但不同LLM评估结果存在严重分歧，表明目前LLM尚不能作为恶意风险评估的可靠裁判。
...
Method: 开发了一个端到端管道：1）通过说服式越狱绕过LLM安全防护；2）重新解释NLP论文以识别和重新利用其研究工具（数据集、方法、工具），利用其漏洞；3）使用三维评估框架（危害性、误用可行性、技术合理性）评估这些提案的安全性。

Result: LLMs能够通过重新利用伦理设计的开放资源生成有害提案；但不同LLM评估结果存在显著分歧：GPT-4.1评分较高（显示更大潜在危害、更高技术合理性和误用可行性），Gemini-2.5-pro明显更严格，Grok-3介于两者之间。

Conclusion: LLMs目前还不能在恶意评估设置中作为可靠裁判，人类评估对于可信的双重用途风险评估至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 传统推理方法存在效率低下和可靠性不足的问题，需要一种能够识别并消除推理异常值的方法来优化推理过程。

Method: 提出推理异常值的概念，设计基于注意力的机制来识别和移除这些异常值，在句子级别消除异常值的同时保持和增强模型的推理能力。

Result: 在四个基准测试上使用Phi-4-Reasoning和GPT-OSS-20B模型验证，优于TALE和ThinkLess等最先进方法，平均减少69.68%的token使用，准确率提升26.70%，最大无穷范数降低15.97%，平均峰度降低91.09%。
...
TL;DR: 首个针对语音对话系统的多奖励RLAIF框架，结合语义、音频质量和情感一致性奖励，通过DPO目标改善多轮对话和增量解码模型的质量。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF/RLAIF在语音对话系统中主要局限于单语义奖励和话语级别应用，忽视了对话质量的多维多模态特性（语义连贯性、音频自然度、说话人一致性、情感对齐、轮转行为），且与增量生成响应的双工语音对话系统不匹配。

Method: 提出首个多奖励RLAIF框架，结合语义、音频质量和情感一致性奖励。采用轮级偏好采样，在DPO目标中聚合每块的log概率，以对齐话语级别偏好与增量块解码。

Result: 单奖励RLAIF能选择性改善其目标指标，而联合多奖励训练在语义质量和音频自然度方面均获得一致提升。实验表明多奖励对齐对实用对话系统的重要性。

Conclusion: 多奖励RLAIF框架能有效改善语音对话系统的综合质量，强调了整体多奖励对齐对实用对话系统的重要性，并发布了多奖励DPO数据集支持可重复研究。
...
Method: 提出PsyProbe系统，采用PPPPPI框架（呈现、易感性、诱发、维持、保护、影响）结合认知错误检测。系统包含：状态构建器提取结构化心理档案，记忆构建跟踪信息缺口，策略规划器使用动机访谈行为编码，响应生成器包含问题构思和批评/修订模块。

Result: 在27名参与者的真实韩国心理咨询场景中评估。自动评估显示完整PsyProbe模型始终优于基线和消融模式。用户评估显示参与意愿显著增加，自然度改善。专家评估表明PsyProbe显著改善核心问题理解，问题生成率与专业咨询师相当。

Conclusion: PsyProbe验证了系统性状态建模和主动提问在治疗性探索中的有效性，能够生成与专业咨询师相当的问题，提高用户参与度和自然度。
...
TL;DR: TeMLM是一个临床语言模型的透明度优先发布框架，包含可机检的发布包、文档工具和合规检查清单，并在一个大规模合成临床数据集上进行了验证。


<details>
  <summary>Details</summary>
Motivation: 为了解决临床语言模型发布中缺乏透明度、可追溯性和治理标准的问题，作者提出了一个统一的透明度框架，确保模型的可信度和可审计性。

Method: 设计了TeMLM框架，包含三个核心文档工具（TeMLM-Card、TeMLM-Datasheet、TeMLM-Provenance）和一个轻量级合规检查清单。该框架将来源追溯、数据透明度、建模透明度和治理统一到一个可机检的发布包中。在Technetium-I合成临床数据集上实例化该框架，并使用ProtactiniumBERT模型进行验证。

Result: 成功创建了包含498,000条临床记录、770万个人健康信息实体标注和ICD-9-CM诊断标签的大规模合成数据集。在PHI去识别化（token分类）和top-50 ICD-9代码提取（多标签分类）任务上获得了参考结果，验证了框架的有效性。

Conclusion: TeMLM为临床语言模型提供了标准化的透明度框架，合成基准对工具和流程验证有价值，但模型在部署前仍需在真实临床数据上进行验证以确保可靠性。
...
TL;DR: 该研究揭示了视觉语言模型在文本误导信息面前的脆弱性，发现即使有明确的视觉证据，模型也容易被冲突的文本提示所误导，性能平均下降超过48.2%。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在多模态推理方面表现出色，但其对文本误导信息的鲁棒性尚未得到充分研究。现有研究主要关注纯文本领域的误导信息，但尚不清楚视觉语言模型如何处理不同模态之间的冲突信息。

Method: 首先提出了CONTEXT-VQA数据集，包含图像-问题对以及系统生成的与视觉证据冲突的说服性提示。然后设计了全面的评估框架，对11个最先进的视觉语言模型进行基准测试，评估它们对这些冲突多模态输入的敏感性。

Result: 实验表明，当前的视觉语言模型确实容易受到误导性文本提示的影响，通常会优先考虑冲突的文本而忽视清晰的视觉证据。仅经过一轮说服性对话后，模型的平均性能下降超过48.2%。

Conclusion: 研究揭示了当前视觉语言模型的一个关键局限性，强调了提高对文本操纵的鲁棒性的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 理解语言模型中语义关联（如"鸟"和"飞"之间的联系）如何从自然语言数据中学习形成，这对于连接深度学习与语言学理论、建立大语言模型的机制基础至关重要。

Method: 利用梯度的主导项近似，推导出训练早期阶段的权重闭式表达式，分析Transformer各组件权重如何由三种基础函数（二元组、标记可互换性和上下文映射）的简单组合构成。

Result: 理论权重表征与真实世界LLM学习到的权重高度匹配，定性分析表明该定理有助于解释Transformer中学习到的语义关联。

Conclusion: Transformer权重可以通过三种统计基础函数的组合来表征，这揭示了语义关联如何在语言模型中形成，为理解大语言模型的机制提供了理论基础。
...
TL;DR: 提出了DREAMSTATE框架，利用条件扩散变换器直接建模RWKV状态的概率流形，实现状态生成和编辑，并进一步提出结合RNN局部优势和全局上下文适应性的混合架构。
...
Method: 首先提出DREAMSTATE框架，使用条件扩散变换器（DiT）直接建模状态的概率流形；然后提出混合架构，结合RNN局部优势和全局上下文适应性，通过并行DiT处理变长全局上下文来动态生成和调整核心循环模块的WKV参数。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的RAG方法存在三个主要问题：1）依赖语义无关的路径采样，与KG推理目标弱对齐；2）直接将检索路径输入推理器，缺乏组织，影响小模型利用知识的能力；3）主要依赖大模型（如ChatGPT/GPT-4），对7B参数以下的小模型研究不足。

Method: RPO-RAG框架包含三个关键创新：1）查询路径语义采样策略，提供信息丰富的监督信号；2）关系感知偏好优化，使训练与KG推理中间信号（如关系）对齐；3）答案中心提示设计，以可解释格式组织实体和推理路径。

Result: 在两个KGQA基准数据集上的实验表明：在WebQSP上F1提升高达8.8%，在CWQ上实现了8B参数以下模型中的新SOTA结果（Hit和F1）。即使参数低于3B，RPO-RAG也能显著提升小模型的推理能力。

Conclusion: RPO-RAG有效缩小了小模型与大模型之间的性能差距，展示了小模型在资源高效和实用设备端KGQA应用中的潜力，为小型LLM的知识密集型任务提供了有效的解决方案。
...
TL;DR: DiaDem是一个视听视频描述模型，专注于生成更精确的对话描述，同时保持整体性能，并通过两阶段GRPO策略和专门基准进行系统评估。


<details>
  <summary>Details</summary>
Motivation: 现有视听视频描述模型在生成忠实对话描述方面存在困难，这影响了下游理解和生成任务的质量，需要专门的方法来提升对话描述的准确性。

Method: 1) 合成高质量SFT数据集；2) 采用难度分区的两阶段GRPO策略增强对话描述能力；3) 构建DiaDemBench基准系统评估对话描述能力。

Result: DiaDem在对话描述准确性上超越了Gemini系列模型，在通用视听视频描述基准上也表现出竞争力，显示出商业模型在对话感知描述方面仍有很大改进空间。

Conclusion: DiaDem通过专门的数据集、训练策略和评估基准，有效提升了视听视频描述中的对话描述准确性，为对话感知的视频描述提供了有效的解决方案。
...
TL;DR: 论文提出了一个创建和评估类比谜语的简单流程，包含事实三元组构建、语义映射、谜语生成和验证四个模块，并用此研究大语言模型能否完整识别谜语的所有可能答案。


<details>
  <summary>Details</summary>
Motivation: 谜语作为一种需要解释线索、识别模式并推理答案的创造性表达形式，可以作为研究语言模型推理覆盖范围和歧义处理能力的轻量级工具。

Method: 开发了一个包含四个组件的系统：1) 三元组创建器 - 构建概念的结构化事实；2) 语义映射器 - 选择适合类比的属性；3) 风格化生成器 - 将属性转化为谜语线索；4) 验证器 - 收集谜语可能指向的所有答案。

Result: 案例研究表明，虽然大语言模型经常能猜出主要预期答案，但常常会遗漏其他有效的解释，这表明谜语能有效揭示语言模型在推理覆盖和歧义处理方面的局限性。

Conclusion: 谜语可以作为评估语言模型推理能力和歧义处理的有效工具，揭示了当前模型在完整识别所有可能答案方面的不足，为语言模型评估提供了新的研究方向。
...
TL;DR: DART是一种基于并行生成的推测解码方法，通过单次前向传播预测多个未来位置的logits，消除自回归展开，显著降低草稿阶段延迟，实现2.03-3.44倍的端到端解码加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法（如EAGLE3）虽然提高了准确性，但需要多步自回归推理，导致草稿阶段延迟高，成为性能瓶颈。需要一种既能保持高准确性又能显著降低草稿延迟的方法。

Method: 1. 基于目标模型隐藏状态，在单次前向传播中并行预测多个未来掩码位置的logits，消除草稿模型的自回归展开；2. 引入高效的树剪枝算法，构建具有N-gram语义连续性的高质量草稿标记树。

Result: 实验结果显示，DART在多个数据集上实现了2.03-3.44倍的端到端解码加速，平均比EAGLE3快30%，显著降低了草稿阶段开销同时保持了高草稿准确性。

Conclusion: DART通过并行生成和树剪枝算法，有效解决了现有推测解码方法中草稿阶段延迟高的问题，提供了一个实用的推测解码框架，显著提升了LLM推理速度。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法大多将LLM作为任务无关的EHR编码器或补全模块，未能充分利用预测任务信号，限制了任务性能。需要一种能更好整合预测任务信息的框架。

Method: 提出ReToP框架，包含EHR重写器和临床预测器的端到端训练。通过临床驱动特征选择生成合成伪标签来训练重写器，使用新颖的Classifier Supervised Contribution (CSC)评分使重写器生成临床相关重写。

Result: 在MIMIC-IV的三个临床任务上超越了强基线模型。分析显示ReToP具有良好的泛化能力，能适应未见数据集和任务，同时保持忠实重写并强调任务相关预测特征。

Conclusion: ReToP通过端到端训练将EHR重写与预测任务对齐，有效提升了临床预测性能，展示了在EHR数据稀疏性挑战下的优越表现和泛化能力。
...
TL;DR: F-1 Prompting是一种两阶段方法，首先从问题描述中提取控制方程，然后根据生成的方程自适应选择CoT、PoT或直接计算策略，在单一LLM调用中完成，显著提升应用数学问题的解决能力。


<details>
  <summary>Details</summary>
Motivation: 当前CoT和PoT等提示技术在数学推理中主要关注自然语言或代码的结构化中间步骤，但应用数学问题（如金融、物理、密码学）通常需要回忆或推导控制方程，这是现有方法未能充分利用的关键步骤。

Method: 提出Formula-One Prompting (F-1)两阶段方法：1) 从问题描述中提取控制方程作为中间表示；2) 根据生成的方程自适应选择解决策略（CoT、PoT或直接计算），所有步骤在单一LLM调用中完成。
...
Conclusion: F-1通过将数学方程作为中间表示并自适应选择解决策略，显著提升了应用数学问题的解决效果，特别是在需要控制方程的领域表现优于现有方法。
...
TL;DR: DeconIEP是一个在评估阶段通过输入嵌入空间的小扰动来减轻测试集污染影响的去污染框架，能在保持基准测试完整性的同时有效降低污染带来的性能虚高。


<details>
  <summary>Details</summary>
Motivation: 基准测试评估是评估大语言模型的标准方法，但其可靠性受到测试集污染的严重威胁。现有方法要么需要修改测试集本身，要么在评估时干扰正常推理导致性能下降。需要一种既能有效去污染又不显著影响正常性能的方法。

Method: DeconIEP在评估阶段完全操作，通过在输入嵌入空间应用有界的小扰动。该方法利用相对较少污染的参考模型作为指导，学习一个实例自适应的扰动生成器，引导被评估模型远离记忆驱动的捷径路径。

Result: 在多个开源权重LLM和基准测试上的广泛实验表明，DeconIEP实现了强大的去污染效果，同时在良性效用上仅造成最小程度的性能下降。

Conclusion: DeconIEP提供了一种有效的评估时去污染框架，能够在保持基准测试完整性的同时，显著减轻测试集污染对模型性能评估的影响，为更可靠的LLM评估提供了新方法。
...
TL;DR: 本文提出了一种基于交叉审问框架（CEF）的无参考文本生成评估方法，通过将源文本和生成文本视为独立知识库，生成可验证问题并进行交叉审问，得到覆盖率、一致性和连贯性三个可解释分数。
...
Result: 在翻译、摘要和临床笔记生成任务上验证，CEF能够识别标准指标遗漏的关键错误。无参考模式与有参考模式强相关，验证了无需黄金参考的可靠性。人类专家验证显示CEF不匹配问题与语义错误的关联度高于非语义错误，特别擅长识别实体和关系扭曲。

Conclusion: CEF提供了一种可靠的无参考多维度评估框架，能够有效捕捉生成文本中的语义错误，特别是在识别实体和关系扭曲方面表现优异，为文本生成任务提供了更全面的评估工具。
...
TL;DR: 本文提出了一种结合二元标记级分类、语言特征集成和数据增强的多词表达识别方法，使用DeBERTa-v3-large模型在CoAM数据集上达到69.8% F1分数，比Qwen-72B提升12个百分点，参数量减少165倍。


<details>
  <summary>Details</summary>
Motivation: 解决多词表达识别任务中现有方法（特别是大型语言模型）在结构化NLP任务上的不足，探索如何在资源受限环境下通过精心设计的小型模型超越大型模型性能。
...
Conclusion: 精心设计的小型模型可以在结构化NLP任务上显著超越大型语言模型，这对于资源受限的部署具有重要启示，证明了通过任务特定的架构设计和特征工程可以实现更高效、更准确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在翻译方面表现出色，但它们在文档级自动后编辑任务中的有效性尚未得到充分理解，特别是在利用文档上下文方面

Method: 采用简单的文档级提示设置，系统比较专有和开源大语言模型，分析APE质量、上下文行为、鲁棒性和效率

Result: 专有LLMs即使使用简单的一次性提示也能达到接近人类水平的APE质量，但对数据投毒攻击的鲁棒性较高，却无法有效利用文档级上下文进行错误修正；标准自动指标不能可靠反映质量改进

Conclusion: 专有LLMs在APE任务中表现出色但成本过高不实用，且无法有效利用文档上下文，需要更高效的长上下文建模方法来实现翻译精炼
...
TL;DR: KG-CRAFT：一种基于知识图谱对比推理的自动声明验证方法，通过LLM结合知识图谱构建的对比问题提升事实核查性能


<details>
  <summary>Details</summary>
Motivation: 自动事实核查系统中的声明验证需要依赖可靠证据源，现有方法在证据利用和推理方面仍有改进空间，需要更有效地利用结构化知识来提升LLM的事实核查能力

Method: 首先从声明和相关报告中构建知识图谱，然后基于知识图谱结构制定上下文相关的对比问题，这些问题指导基于证据的报告提炼，最终合成简洁摘要供LLM进行真实性评估

Result: 在两个真实世界数据集（LIAR-RAW和RAWFC）上的广泛评估表明，该方法在预测性能上达到了新的最先进水平，综合分析验证了基于知识图谱的对比推理方法在提升LLM事实核查能力方面的有效性

Conclusion: KG-CRAFT通过知识图谱增强的对比推理显著提升了自动声明验证的性能，为LLM在事实核查任务中的应用提供了有效的结构化知识利用方法
...
TL;DR: ClaimPT是一个欧洲葡萄牙语新闻文章数据集，包含1,308篇文章和6,875个标注，专注于事实核查中的主张识别任务，旨在解决葡萄牙语缺乏标注数据的问题。
...
Method: 通过与葡萄牙新闻社LUSA合作收集新闻文章，采用新提出的标注方案，由两名训练有素的标注员对每篇文章进行标注，并由一名策展人验证所有标注，确保标注质量。

Result: 创建了ClaimPT数据集，包含1,308篇欧洲葡萄牙语新闻文章和6,875个个体标注，专注于新闻内容而非社交媒体或议会记录。提供了主张检测的基线模型，建立了初步基准。

Conclusion: ClaimPT数据集的发布旨在推进低资源语言的事实核查研究，增强对新闻媒体中错误信息的理解，并为未来的NLP和信息检索应用提供支持。
...
TL;DR: GradPruner：一种在微调早期阶段基于梯度信息剪裁LLM层的方法，能在保持精度的同时显著减少参数和提升训练/推理效率
...
Method: 在微调初期使用参数累积梯度计算初始梯度信息累积矩阵(IGIA-Matrix)评估层重要性并进行剪枝；基于IGIA-Matrix稀疏化剪裁层并与剩余层合并，仅合并符号相同的元素以减少符号变化干扰

Result: 在两个LLM和八个下游数据集（包括医疗、金融和通用基准任务）上的实验表明，GradPruner实现了40%的参数减少，精度仅下降0.99%

Conclusion: GradPruner能同时提升下游任务微调的训练和推理效率，通过早期梯度引导的层剪枝实现了参数显著减少而精度损失极小
...
TL;DR: Yunque DeepResearch是一个分层、模块化、鲁棒的深度研究框架，通过多智能体编排、动态上下文管理和主动监督来解决现有深度研究中的上下文噪声、脆弱性和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究在自主智能体应用中面临三个关键限制：长时程任务中上下文噪声不断累积、脆弱性导致级联错误、以及缺乏模块化可扩展性，阻碍了深度研究在复杂开放任务中的充分发挥。

Method: 提出Yunque DeepResearch框架，包含三个核心组件：1) 集中式多智能体编排系统，将子任务路由到原子能力池的工具和专用子智能体；2) 动态上下文管理机制，将完成的子目标结构化为语义摘要以减轻信息过载；3) 主动监督模块，通过主动异常检测和上下文修剪确保鲁棒性。

Result: 在多个智能体深度研究基准测试中达到最先进性能，包括GAIA、BrowseComp、BrowseComp-ZH和Humanity's Last Exam。框架、可复现实现和应用案例已开源。

Conclusion: Yunque DeepResearch通过分层、模块化和鲁棒的架构设计，有效解决了深度研究中的关键挑战，为自主智能体在复杂开放任务中的深度研究能力提供了强大框架支持。
...
TL;DR: PIP提出了一种并行推理范式，用于视觉丰富文档的关键信息提取，通过使用掩码标记同时生成所有目标值，相比自回归模型实现了5-36倍的推理加速，性能损失可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM和MLLM的KIE方法依赖自回归推理，需要顺序生成输出，在处理需要提取多个语义独立字段的任务时存在显著的效率瓶颈，限制了实际应用的可扩展性。

Method: 提出PIP并行推理范式，使用"[mask]"标记作为所有目标值的占位符，在单次前向传播中同时生成所有字段。为此开发了专门的掩码预训练策略，并构建了大规模监督数据集。

Result: 实验结果表明，PIP模型相比传统自回归基础模型实现了5-36倍的推理加速，同时性能下降可忽略不计，显著提升了KIE任务的效率。
...
TL;DR: 本文提出了一种简单有效的方法来解决扩散语言模型中的移动sink现象，通过引入一个额外的sink token来稳定注意力机制，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然具有并行文本生成的优点，但存在移动sink现象这一关键不稳定性问题。sink token在Transformer值空间中表现出低范数表示，移动sink现象是DLMs防止过度信息混合的保护机制，但其在扩散步骤中不可预测的位置会破坏推理的鲁棒性。

Method: 提出了一种简单但有效的额外sink token方法，通过修改注意力掩码实现。具体引入一个特殊token，该token只能关注自身，但对所有其他token全局可见。这种方法稳定了注意力sink，且其有效性独立于token位置，语义内容可忽略。

Result: 实验结果表明，引入单个额外token能够稳定注意力sink，显著提高模型性能。进一步分析证实该token的有效性与其位置无关，且语义内容可忽略，验证了其作为稳健专用结构sink的作用。

Conclusion: 通过引入一个简单的额外sink token可以有效解决扩散语言模型中的移动sink现象，提高模型稳定性和性能，该方法简单有效且具有鲁棒性。
...
TL;DR: SynCABEL是一个利用大语言模型生成生物医学实体链接合成训练数据的框架，解决了标注数据稀缺问题，在多语言基准测试中达到SOTA，减少60%人工标注需求，并引入LLM评估协议验证临床有效性。
...
Method: SynCABEL框架利用大语言模型为目标知识库中的所有候选概念生成上下文丰富的合成训练示例，无需人工标注即可提供广泛的监督。结合仅解码器模型和引导推理，并在评估中引入LLM作为评判者的协议来评估临床有效性。

Result: 在三个多语言基准测试（英语MedMentions、法语QUAERO、西班牙语SPACCC）上达到新的最先进水平；数据效率方面，相比完整人工监督，减少60%标注数据需求；LLM评估显示SynCABEL显著提高了临床有效预测率。

Conclusion: SynCABEL通过合成数据生成有效解决了生物医学实体链接的标注数据稀缺问题，大幅减少对昂贵专家标注的依赖，同时提升模型性能，并通过LLM评估协议更好地评估临床有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型微调方法内存消耗高，虽然已有激活优化方法，但数据无关的特性导致微调效果不稳定且效率低下。

Method: 提出TokenSeek方法，通过实例感知的token寻找和丢弃机制，动态选择重要token，减少激活内存占用。

Result: 在Llama3.2 1B等模型上实现显著内存节省（仅需14.8%内存），性能相当或更好，且token寻找过程具有可解释性。

Conclusion: TokenSeek为各种transformer模型提供高效的微调解决方案，其可解释的token选择机制为未来token效率研究提供宝贵见解。
...
TL;DR: 提出交互式医疗咨询评估框架EviMed，通过模拟患者和报告器建模咨询过程，引入信息覆盖率(ICR)量化证据收集完整性，发现诊断推理强不等于信息收集有效，并提出REFINE策略改善交互性能
...
Method: 构建基于证据的基准EviMed，使用模拟患者和报告器建模咨询过程，提出信息覆盖率(ICR)指标，并设计REFINE策略利用诊断验证引导智能体主动解决不确定性

Result: 评估10个不同推理能力的模型发现，强大的诊断推理能力不能保证有效的信息收集，这是限制交互性能的主要瓶颈；REFINE策略在多个数据集上一致优于基线，并能促进模型协作，使小型智能体在强推理监督下获得更好性能

Conclusion: 交互式医疗咨询需要专门关注证据收集过程，REFINE策略通过诊断验证引导主动不确定性解决，能有效提升交互性能，为医疗AI系统开发提供了新的评估框架和改进方法
...
TL;DR: 提出零样本立场检测新任务DGTA，通过动态目标生成和多目标适应，在无先验目标知识下从文本中自动识别多个目标-立场对，构建中文社交媒体数据集并设计多维度评估指标。


<details>
  <summary>Details</summary>
Motivation: 现实社交媒体场景中，目标既非预定义也非静态，而是复杂动态的。现有立场检测研究通常基于给定目标和文本进行预测，无法适应真实世界的动态需求。

Method: 提出DGTA任务框架，包含动态目标生成和多目标适应。构建中文社交媒体立场检测数据集，设计多维度评估指标。探索集成和两阶段微调策略，评估多种基线模型，包括Qwen2.5-7B和DeepSeek-R1-Distill-Qwen-7B等大语言模型。

Result: 微调后的大语言模型在该任务上表现优异：两阶段微调的Qwen2.5-7B获得最高综合目标识别分数66.99%；集成微调的DeepSeek-R1-Distill-Qwen-7B获得立场检测F1分数79.26%。

Conclusion: 提出的DGTA任务有效解决了现实社交媒体中动态目标的立场检测问题，微调后的大语言模型在该任务上表现出色，为实际应用提供了可行方案。
...
TL;DR: 迭代检索增强生成(RAG)在科学多跳推理任务中显著优于静态RAG，即使与理想黄金上下文相比也有高达25.6%的性能提升，但仍有覆盖不完整、干扰项锁定等失败模式。


<details>
  <summary>Details</summary>
Motivation: 研究迭代检索-推理循环何时能真正超越静态RAG，特别是在科学领域存在多跳推理、稀疏领域知识和异构证据的情况下。这是首个机制层面的诊断研究，旨在确定同步迭代检索和推理是否能超越理想化的静态RAG上限。

Method: 在化学领域的ChemKGMultiHopQA数据集上，对11个最先进的LLM进行三种范式测试：无上下文（测量参数记忆依赖）、黄金上下文（一次性提供所有证据）、迭代RAG（无训练控制器，交替进行检索、假设细化和证据感知停止）。通过诊断指标分析检索覆盖差距、锚点携带丢失、查询质量、组合保真度和控制校准等行为。

Result: 迭代RAG在所有模型中一致优于黄金上下文，提升幅度高达25.6个百分点，特别是对于非推理微调模型。分阶段检索减少了后期跳失败、缓解了上下文过载，并能动态纠正早期假设漂移。但仍有失败模式：跳覆盖不完整、干扰项锁定轨迹、早期停止校准错误，以及即使完美检索也存在高组合失败率。

Conclusion: 分阶段检索通常比仅仅存在理想证据更具影响力；研究为在专业科学环境中部署和诊断RAG系统提供了实用指导，并为构建更可靠、可控的迭代检索-推理框架奠定了基础。
...
Result: 在10个数学和编程基准测试中表现一致改进，包括在AIME-24和AIME-25上获得超过13%的提升。AdaRAS展现出跨数据集的强可迁移性和对更强模型的扩展性，优于后训练方法。

Conclusion: AdaRAS通过选择性干预推理关键神经元，提供了一种轻量级、高效的推理可靠性提升方案，无需额外训练或采样成本，具有实际应用价值。
...
TL;DR: 本文提出了一种基于提示的反射式翻译框架，通过让模型生成初始翻译、进行结构化自我批评，然后利用反思生成改进翻译，在低资源语言翻译中取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 像isiZulu和isiXhosa这样的低资源语言在机器翻译中面临持续挑战，主要原因是有限的平行数据和语言资源。虽然大型语言模型的自反思能力在推理质量和事实一致性方面显示出潜力，但如何将其应用于低资源语言翻译仍待探索。

Method: 提出反射式翻译框架：1) 模型生成初始翻译；2) 生成结构化自我批评；3) 利用反思生成改进翻译。该方法无需微调，模型无关，在英语-isiZulu和英语-isiXhosa翻译任务上使用OPUS-100和NTREX-African数据集进行评估，测试多种提示策略和置信度阈值。

Result: 结果显示第一轮和第二轮翻译在BLEU和COMET分数上均有持续改进，平均提升分别达到+0.22 BLEU和+0.18 COMET。配对非参数统计检验证实这些改进具有稳健性。此外，该方法还创建了反射增强数据集，可用于未来的监督或分析驱动工作。

Conclusion: 结构化自我反思是提高低资源环境下翻译质量的一种实用有效机制。该方法模型无关、无需微调，为低资源语言翻译提供了有前景的解决方案，并创建了可用于未来研究的数据资源。
...
TL;DR: ONCOTIMIA是一个集成生成式AI的临床工具，通过LLMs自动完成肺癌多学科肿瘤委员会表格，减少文档负担


<details>
  <summary>Details</summary>
Motivation: 多学科肿瘤委员会在肿瘤学决策中起核心作用，但需要大量手动处理异构临床信息，造成沉重的文档负担

Method: 开发ONCOTIMIA系统，结合多层数据湖、混合关系/向量存储、检索增强生成和规则驱动自适应表单模型，将非结构化临床文档转化为结构化标准化肿瘤委员会记录

Result: 评估6个通过AWS Bedrock部署的LLMs在10个肺癌病例上的表现，最佳配置实现80%正确字段完成率，大多数LLMs具有临床可接受的响应时间，更大更新的模型表现最好且延迟可接受

Conclusion: LLM辅助的自动表单完成在技术上是可行的，在操作上是可行的，有潜力显著减少文档负担同时保持数据质量
