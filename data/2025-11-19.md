...
TL;DR: 本文提出一个解决开放集识别中过自信问题的框架，通过参数扰动估计预测不确定性，并利用两阶段分类器改进已知与未知类别的区分。


<details>
  <summary>Details</summary>
Motivation: 开放集识别中，当未知样本与已知类别语义相似时，特征空间中的类间重叠会导致模型对未知样本赋予过高置信度，造成过自信现象，从而模糊已知与未知类别的决策边界。

Method: 框架包含两个组件：基于扰动的预测不确定性估计模块，通过可控参数扰动生成多样化预测；以及具有独立学习分类器的未知检测模块，采用两阶段程序利用估计的不确定性改进已知与未知类别区分。

Result: 在三个公共数据集上的实验结果表明，所提框架优于现有的开放集识别方法。

Conclusion: 该框架通过显式缓解由类间重叠引起的过自信问题，有效提升了开放集识别性能。
...
TL;DR: 本文提出了一种新颖的少样本视频目标检测方法，通过对象感知的时间建模和选择性特征传播机制，解决了视频中目标检测的时序一致性和少样本泛化问题。


<details>
  <summary>Details</summary>
Motivation: 传统目标检测方法需要大量训练数据，而少样本视频目标检测面临时序一致性保持和少样本泛化的挑战，特别是在遮挡和外观变化的情况下。

Method: 采用对象感知的时间建模方法，包含过滤机制选择性传播高置信度目标特征，结合少样本训练的检测和分类头，实现高效特征传播。

Result: 在5-shot设置下，AP提升分别为：FSVOD-500（3.7%）、FSYTV-40（5.3%）、VidOR（4.3%）、VidVRD（4.5%），在1-shot、3-shot和10-shot配置下也表现出改进。

Conclusion: 该方法在不依赖显式目标管提议的情况下实现了鲁棒的时序一致性，为少样本视频目标检测提供了有效的解决方案。
...
TL;DR: 提出了一种基于流匹配的图像融合方法，通过直接概率传输从源模态到融合图像分布，提高采样效率。使用多模型融合结果作为先验，结合任务感知选择和融合精炼模块提升伪标签质量，并引入弹性权重巩固和经验回放机制支持多任务学习。


<details>
  <summary>Details</summary>
Motivation: 当前多模态图像融合方法依赖任务特定模型，训练成本高且可扩展性有限。生成方法采样效率低，缺乏高质量融合图像监督数据。

Method: 基于流匹配范式，将图像融合建模为从源模态到融合图像的直接概率传输。收集多模型融合结果作为先验，使用任务感知选择函数筛选可靠伪标签，引入融合精炼模块分解和增强退化组件。多任务场景下集成弹性权重巩固和经验回放机制。

Result: 在多种融合任务中取得有竞争力的性能，显著提高采样效率，保持轻量级模型设计。

Conclusion: 该方法提供了一种高效统一的多模态图像融合解决方案，平衡了性能、效率和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 实时碰撞检测对于开发主动安全管理策略和提高交通效率至关重要，但传统方法受限于轨迹获取和车辆跟踪的限制。

Method: 第一阶段使用基于扩散的Mapfusion模型进行路段地图生成，通过噪声到正常的过程逐步添加噪声；第二阶段通过比较监控路段地图与扩散模型生成结果来实现碰撞检测。

Result: Mapfusion成功生成了基于学习运动模式的真实路段演化地图，并在不同采样间隔下保持鲁棒性，在真实世界碰撞实验中准确检测到碰撞。
...
<details>
  <summary>Details</summary>
Motivation: 由于人工智能技术发展的紧急性和同质化，基于transformer的基础模型已革新了科学应用，但地震数据具有独特特征，需要专门的处理技术。现有的视觉变换器(ViTs)忽略了地震数据的内在模式，无法有效捕捉高频和低频地震信息。

Method: 采用自适应双网格基础模型训练策略，通过频谱分解分离高频和低频分量，使用分层希尔伯特编码有效表示数据，并基于频率原则提出自适应训练策略，先关注粗粒度信息，再逐步细化到细粒度特征。

Result: 广泛的实验证明了该训练方法的有效性和效率，能够显著提升视觉地震基础模型的预训练效果。

Conclusion: 本研究强调了基于地震图像高频和低频特征的数据编码和训练策略的重要性，为增强视觉地震基础模型预训练做出了贡献。
...
TL;DR: 该研究开发了一种基于面部微动态分析的被动式痴呆筛查方法，仅使用短时摄像头视频，无需语言或文本，通过分析眨眼动态、嘴部运动、注视变异性等面部微运动来检测早期神经认知变化。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆筛查方法主要依赖语音或脚本化访谈，限制了在临床环境外的应用，且预测结果与语言和转录紧密耦合。本研究旨在开发一种语言无关、可扩展的被动筛查方法，能够在自然环境中分析面部行为。

Method: 通过稳定面部信号，将微运动转换为可解释的面部微动态时间序列，平滑处理后总结为紧凑的片段级统计特征。分析运动在多个通道中的分布而非幅度，使每个通道的效果透明化。

Result: 在YT DemTalk数据集上，消融实验确定注视不稳定性和嘴部/下颌动态为最有效线索，轻量级分类器达到AUROC 0.953、平均精度0.961、F1分数0.851和准确率0.857的痴呆预测性能。

Conclusion: 面部微动态分析为痴呆筛查提供了一种有效的语言无关方法，可在自然环境中实现大规模被动筛查，且具有良好的跨设备、跨主题和跨文化适应性。
...
TL;DR: Gen-ViRe是一个基于认知科学的视觉推理基准测试框架，用于评估视频生成模型作为推理器的能力，填补了现有基准无法评估链式帧推理的空白。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成基准主要关注保真度或对齐性，无法评估链式帧推理的核心认知能力，如多步规划、算法逻辑和抽象模式外推，这阻碍了对模型能力的系统理解和改进指导。

Method: 将链式帧推理分解为六个认知维度和24个子任务，通过多源数据整理、最小提示协议和基于VLM的混合评估方法，提供首个视频模型作为推理器的定量评估。

Result: 实验显示最先进系统在视觉质量和实际推理深度之间存在显著差距，建立了基线和诊断工具来推进真正的世界模拟器发展。

Conclusion: Gen-ViRe框架为评估视频模型的推理能力提供了系统方法，揭示了当前模型在视觉质量与推理深度之间的不匹配，为开发真正的世界模拟器提供了重要指导。
...
TL;DR: 该论文提出了一种基于排序的损失函数来解决基于热图的人体姿态估计中的三个主要问题：MSE损失对峰值定位不敏感、热图的空间和类别不平衡、以及损失函数与评估指标之间的不一致。


<details>
  <summary>Details</summary>
Motivation: 解决传统热图方法中的三个核心问题：(1) MSE损失无法有效提升关节定位精度；(2) 热图的空间和类别不平衡；(3) 损失函数与评估指标mAP之间的不一致性。

Method: 提出基于排序的损失函数，通过理论分析和实验验证，提高置信度分数与定位质量之间的相关性，从而改善NMS过程中的实例选择和mAP性能。

Result: 在COCO-val数据集上，RSPose使用ViTPose-H模型达到79.9 mAP，优于先前最佳方法；同时将SimCC Resnet-50在COCO-val上的AP提升了1.5，达到73.6 AP。

Conclusion: 提出的排序损失函数首次实现了与评估指标mAP对齐的损失设计，在多个数据集和不同热图模式下均表现出优越性能，显著提升了人体姿态估计的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知能力启发，希望从碰撞声音中识别产生声音的物体，特别是在第一人称视角视频中，声音清晰但视觉场景杂乱。

Method: 使用弱监督的音频条件分割方法，结合基础模型CLIP和SAM2，并融入第一人称视角线索(如手中物体)来识别潜在的碰撞声源物体。

Result: 在两个新基准EPIC-CS3和Ego4D-CS3上，mIoU指标分别比竞争基线方法高出3倍和4.7倍。

Conclusion: 提出的CS3任务和方法能够有效从碰撞声音中分割出声源物体，在第一人称视角视频中表现优异。
...
TL;DR: 本文提出了一种几何基础的位置回归方法，将绝对位置回归重新构建为几何表示回归，通过预测解耦的几何表示来估计相机位姿。


<details>
  <summary>Details</summary>
Motivation: 传统的绝对位置回归方法作为黑盒直接回归6自由度位姿，容易记忆训练视图而非理解3D场景几何。本文旨在引入几何先验，提供更鲁棒的位置估计方法。

Method: 将APR重新构建为几何表示回归，预测两个解耦的几何表示：射线束方向估计相机旋转，对应点图估计相机平移，使用可微分确定性求解器恢复最终位姿。

Result: 在7-Scenes和Cambridge Landmarks数据集上实现了最先进的性能，验证了建模逆渲染过程对通用绝对位置估计的有效性。

Conclusion: 通过解耦旋转和平移预测，引入几何先验，为绝对位置估计提供了更鲁棒和可泛化的方法。
...
TL;DR: 本文提出了一种用于膀胱癌复发预测的H-CNN-ViT模型，该模型通过分层门控注意力机制融合CNN和ViT的局部与全局特征，在多序列MRI数据集上取得了78.6%的AUC，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高达78%，术后监测至关重要。多序列增强MRI是常用检测手段，但术后组织变化使判读困难。AI辅助诊断工具虽有潜力，但缺乏专门的多序列MRI数据集限制了该领域发展。

Method: 首先构建了专门用于膀胱癌复发预测的多序列多模态MRI数据集，然后提出H-CNN-ViT模型，采用分层门控注意力多分支架构，独立处理每个模态，通过门控机制选择性加权CNN（局部）和ViT（全局）路径的特征。

Result: 在自建数据集上评估，H-CNN-ViT模型达到78.6%的AUC，超越了现有最先进模型。

Conclusion: 提出的H-CNN-ViT模型在膀胱癌复发预测任务中表现出色，为术后监测提供了有效的AI辅助工具，同时构建的数据集为未来研究提供了基准。
...
<details>
  <summary>Details</summary>
Motivation: CLIP的文本编码器最多只能处理77个token，无法有效表示信息丰富的长格式放射学报告。现有的领域特定编码器如PubMedBERT或ClinicalBERT虽然有所改进，但仍受限于512个token的输入长度和相对浅层的语义理解。
...
<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在高维医学图像中存在局限性，无法充分提取详细解剖特征，需要克服这些约束以提升分类性能。

Method: 采用混合方法整合EfficientNet和VGG19，结合自定义的伪牛顿增强层和稀疏诱导特征减少层，形成多层框架，优化特征权重和减少冗余。
...
Method: 利用VLM的强大场景理解能力为现有视觉基准添加结构化、语言丰富的场景描述，引入多模态交互架构融合视觉和语言特征，并设计后处理细化模块提升预测可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 医学解剖分割领域的数据缩放规律研究不足，而深度学习在医学图像分析中需要大量标注数据，因此探索如何通过数据增强策略提高数据利用效率具有重要意义。

Method: 分析了15个语义任务和4种成像模态的缩放规律，评估了随机弹性变形和配准引导变形两种增强策略的影响，并提出了一种基于图像配准生成微分同胚映射的新颖可扩展图像增强方法。

Result: 实验结果表明，配准和生成的变形增强方法显著提高了数据利用效率，提出的生成变形方法实现了更优的性能和加速收敛，超越了标准幂律缩放趋势。
...
<details>
  <summary>Details</summary>
Motivation: 现有数字血液病理学方法（包括单任务、视觉语言、WSI优化或单细胞血液学模型）无法提供跨复杂血液病理学场景的统一多任务多模态推理，需要开发能够整合多种任务的统一模型。

Method: 基于Hema-Former多模态模块构建Uni-Hema模型，该模块在层次级别桥接视觉和文本表示，支持不同粒度的检测、分类、分割、形态学、掩码语言建模和视觉问答任务，利用46个公共数据集（超过70万张图像和2.1万个问答对）。

Result: 大量实验表明，Uni-Hema在多种血液学任务上达到或优于单任务单数据集模型的性能，同时在单细胞级别提供可解释的形态学相关见解。

Conclusion: Uni-Hema为多任务多模态数字血液病理学建立了新标准，能够统一处理恶性疾病、感染性疾病和非恶性红细胞疾病等多种血液病理学场景。
...
TL;DR: 本文提出了首个弱监督管道用于检测瞬时沟壑，利用视觉语言模型减少人工标注负担，并发布了首个半监督检测数据集。


<details>
  <summary>Details</summary>
Motivation: 瞬时沟壑是农业领域最受关注的土壤侵蚀问题之一，但由于其短暂的时间周期和缺乏准确的标注数据，传统的计算机视觉方法和机器学习方法难以自动检测。

Method: 使用视觉语言模型嵌入的知识，构建教师-学生模型，教师从VLM的噪声标签中学习，学生通过弱监督使用教师生成的标签和噪声感知损失函数进行学习。

Result: 实验结果表明，该方法在弱监督训练学生模型时，相比VLM和标签模型本身表现出更优越的性能。

Conclusion: 该方法有效解决了瞬时沟壑自动检测的挑战，通过弱监督方法显著减少了人工标注需求，并为该领域提供了首个公开数据集。
...
TL;DR: 本文提出了一种基于压缩视频流中运动向量的可扩展框架，用于评估生成视频的时间真实性。通过比较真实视频和生成视频的运动向量统计差异，揭示了生成模型在运动表现上的系统性缺陷，并展示了运动向量与RGB融合能显著提升下游分类任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型在时间真实性方面存在明显弱点，大多数评估指标主要关注空间外观，对运动的敏感性有限。需要开发能够有效评估视频时间行为的框架。
...
Result: 实验发现生成视频与真实视频在运动上存在系统性差异：基于熵的散度显示Pika和SVD最接近真实视频，MV-sum统计偏好VC2和Text2Video-Zero，而CogVideo在所有指标上都表现出最大偏差。运动向量与RGB融合显著提升了分类性能，ResNet-18/34达到97.4%准确率，I3D达到99.0%准确率。

Conclusion: 压缩域运动向量为诊断生成视频中的运动缺陷和增强判别模型中的时间推理提供了有效的时序信号，是评估和改进生成视频时间真实性的有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有MCVT研究过于关注精度而忽视实时性和可扩展性，这在城市规模应用中至关重要，因为摄像头数量增加会带来挑战。

Method: 系统包含多个边缘设备与中央工作站。边缘端处理RTSP视频流，进行目标检测、跟踪、地理映射和特征提取，仅传输轻量元数据；中央端基于自监督相机链接模型计算跨摄像头关联。
...
<details>
  <summary>Details</summary>
Motivation: 道路安全规范对人类和自动驾驶车辆的AI系统都至关重要，需要评估多模态大语言模型对道路安全概念的理解能力。
...
TL;DR: 本文提出CuRPO方法，通过基于课程学习的相对策略优化解决CoT推理在视觉定位任务中的性能退化问题。
...
Method: 提出基于课程的相对策略优化(CuRPO)，利用CoT长度和广义交并比奖励作为复杂度指标，从简单到困难逐步组织训练数据。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上的实验表明，CuRPO显著优于现有方法，在RefCOCO上提升高达+12.52 mAP。

Conclusion: CuRPO方法在视觉定位任务中表现出卓越的效率和鲁棒性，特别是在少样本学习场景下，对具有模糊复杂文本描述的任务尤为有效。
...
TL;DR: 提出基于聚类的帧选择策略来缓解视频衍生帧数据集中的信息泄露问题，通过先对视觉相似帧进行分组再划分数据集，生成更具代表性、平衡性和可靠性的数据集分区。
...
Method: 采用聚类方法对视觉相似的帧进行分组，然后在聚类结果的基础上进行数据集划分，避免相似帧被分配到不同集合中。

Result: 该方法能够产生更具代表性、平衡性和可靠性的数据集分区，有效减少信息泄露风险。

Conclusion: 基于聚类的帧选择策略是缓解视频衍生帧数据集中信息泄露问题的有效方法，能够提升数据集划分的质量和机器学习模型的评估可靠性。
...
TL;DR: 通过使用程序生成的无视觉语义内容数据预训练视觉Transformer，可以显著提高数据效率、收敛速度和下游任务性能。仅用1%的训练预算就能在ImageNet-1k上提升1.7%的准确率。


<details>
  <summary>Details</summary>
Motivation: 探索为视觉Transformer注入通用归纳偏置的新方法，通过程序生成的无视觉语义内容数据进行预训练，以提升模型的泛化能力和数据效率。

Method: 使用形式语法等简单算法生成程序化数据，这些数据与自然或合成图像无关。在预训练阶段绕过视觉补丁嵌入机制，让模型内化抽象计算先验知识，然后进行标准的基于图像的训练。

Result: 在ImageNet-1k上，仅用1%的训练预算进行程序化数据预训练，就能将最终准确率提高超过1.7%。从性能效果来看，1%的程序生成数据相当于28%的ImageNet-1k数据。

Conclusion: 这种方法为实现新的数据高效和领域无关的预训练策略提供了有前景的路径。
...
Method: EchoAgent在大语言模型控制下协调专业视觉工具，执行时间定位、空间测量和临床解释。关键贡献是测量可行性预测模型，用于确定解剖结构在每帧中是否可靠可测量，从而实现自主工具选择。
...
TL;DR: CrossTrainer提出了一种跨运动技能评估方法，通过发现跨运动边界的技能属性（如平衡、控制、手部位置），训练多模态语言模型为视频生成可操作的反馈和熟练度评级，在跨运动和同运动设置下相比现有技术提升达60%。


<details>
  <summary>Details</summary>
Motivation: 解决现有模型局限于单一运动、专家级监督成本高且稀缺的问题，探索可迁移的视频表示用于技能评估。

Method: 发现跨运动边界的技能属性，训练多模态语言模型生成针对新视频的可操作反馈和熟练度评级。

Result: 在多个数据集上的跨运动和同运动设置中，相比现有技术提升达60%，通过抽象出指示人类技能的共享行为，视频表示比现有技术泛化能力更强。

Conclusion: 通过抽象共享的人类技能行为，所提出的视频表示比现有技术泛化能力更强，丰富了当今的多模态大语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决室内场景理解中光学传感器存在的遮挡和隐私风险问题，利用毫米波雷达的穿透性和隐私保护特性，但需要克服其低空间分辨率的挑战。

Method: 提出双角度多径增强方法，显式建模到达角和出发角来恢复二次反射；采用模拟到现实的分层扩散框架，将碎片化的雷达响应转化为完整的布局重建和物体检测。

Result: 在布局重建方面将Chamfer距离降低60%（降至16厘米）；首次实现基于毫米波的物体检测，达到58%的IoU；构建了包含50,000帧数据的大规模基准数据集。

Conclusion: RISE为使用单个静态雷达实现几何感知和隐私保护的室内场景理解建立了新的基础，证明了多径反射在几何推理中的价值。
...
TL;DR: 开发了一个用于自动识别MRI切片解剖平面（轴向、冠状、矢状）的2.5D上下文感知分类器，显著提高了平面方向元数据的生成准确性，并在脑肿瘤检测任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 自动化系统难以准确识别MRI切片的解剖平面方向，缺失的平面方向元数据会复杂化分析、增加领域偏移并降低诊断分类器的准确性。

Method: 采用2.5D上下文感知模型，利用多切片信息避免孤立切片的模糊性，并在3D切片序列和静态2D图像上进行训练。

Result: 2.5D方法将准确率从2D参考模型的98.74%提升至99.49%，错误减少60%；在脑肿瘤检测任务中，通过基于不确定性的门控策略，准确率从97.0%提升至98.0%，误诊减少33.3%。

Conclusion: 2.5D上下文对于MRI平面方向识别至关重要，生成的元数据能显著提升下游诊断任务的性能，已集成到开源交互式Web应用中。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，传统主动学习需要专家标注感兴趣区域，但由于医学图像边界模糊和模糊，标注过程仍然具有挑战性、劳动密集且认知要求高。语言指导提供了一种有效的替代方案，需要最少的专家努力，同时绕过精确边界描绘的认知要求任务。

Method: LINGUAL框架接收专家的自然语言指令，通过上下文学习将其转化为可执行程序，并自动执行相应的子任务序列，无需任何人工干预。

Result: 在主动领域自适应中，LINGUAL实现了与主动学习基线相当或更优的性能，同时估计标注时间减少了约80%。

Conclusion: LINGUAL框架通过语言指导有效解决了医学图像分割中的标注挑战，显著减少了专家工作量，同时保持了高性能。
...
TL;DR: WaRPAD是一种无需训练、基于自监督模型的AI生成图像检测算法，通过分析图像嵌入对高频扰动的敏感性来检测AI生成图像。


<details>
  <summary>Details</summary>
Motivation: 随着视觉生成模型的快速发展，AI生成图像检测变得至关重要。研究无需训练、不依赖特定数据集知识的检测方法，利用自监督模型在随机裁剪等数据增强中学习到的表示一致性。

Method: 基于自监督模型对RandomResizedCrop增强的鲁棒性，定义基础评分函数量化图像嵌入对Haar小波分解提取的高频方向扰动的敏感性。将图像缩放到模型输入尺寸的倍数，分割成小块，计算每个块的基础评分，最终通过平均所有块的评分得到检测分数。

Result: 在真实数据集和23种不同生成模型生成的图像上验证，WaRPAD始终达到竞争性性能，并对测试时损坏表现出强鲁棒性。

Conclusion: WaRPAD是一种有效的训练免费AI生成图像检测方法，由于RandomResizedCrop不变性是自监督模型的常见训练方案，该方法可跨自监督模型应用。
...
TL;DR: FashionMAC是一个基于扩散模型的免变形框架，用于生成高质量可控的时尚展示图像，通过直接外推分割出的服装来保留细节，并提出区域自适应解耦注意力机制实现细粒度控制。
...
Method: 提出免变形框架直接外推分割服装，设计区域自适应解耦注意力机制和链式掩码注入策略，实现细粒度外观控制。

Result: 实验验证了该方法在视觉保真度和可控性方面优于现有最先进方法。

Conclusion: FashionMAC框架能够高质量地生成可控的时尚展示图像，有效保留服装细节并实现细粒度外观控制。
...
TL;DR: SCAR是一种基于自回归模型的语义上下文驱动图像编辑方法，通过压缩语义预填充和语义对齐指导来解决现有AR模型在图像编辑中条件控制弱、效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像生成方面表现出强大潜力，但在扩展到通用图像编辑时面临条件控制弱、效率低的问题，导致指令遵循性差和视觉伪影。
...
Conclusion: SCAR克服了基于矢量量化的预填充方法的语义限制和高成本问题，在保持灵活性和通用性的同时，显著提升了自回归模型在图像编辑中的性能。
...
TL;DR: CORE是一种新的视觉令牌压缩范式，通过对象分割掩码作为语义先验，将视觉令牌合并为紧凑的对象中心表示，并采用质心引导排序机制保持空间顺序，在高效LVLM处理中表现出色。
...
Method: 使用高效分割解码器生成对象掩码作为语义先验，指导视觉令牌合并为对象中心表示；引入质心引导排序机制恢复合并令牌的空间顺序。

Result: 在六个权威基准测试中达到固定率压缩的最新水平，在自适应率设置下实现显著效率提升；即使在仅保留2.2%视觉令牌的极端压缩下，仍能保持97.4%的基线性能。

Conclusion: 对象中心表示在高效有效的LVLM处理中具有优越性，CORE为视觉令牌压缩提供了新的解决方案。
...
Method: 提出语义引导参数合成器(SGPS)，利用多模态任务信息(单张图像和临床文本描述)直接合成轻量级分类器的全部权重参数。

Result: 在ISIC 2018皮肤病变数据集和自定义罕见疾病数据集上，1-shot和5-shot分类任务中显著优于现有few-shot和zero-shot学习方法。
...
TL;DR: 提出了DEER-3D框架，通过错误驱动的3D场景编辑生成针对性反事实数据，解决3D-LLMs在视觉空间元素语言接地方面的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在将语言准确接地到3D环境的视觉和空间元素方面存在限制，主要原因是训练数据侧重于语言推理而非空间理解，且缺乏3D资源导致固有的接地偏见未解决。

Method: 提出3D场景编辑作为关键机制生成精确视觉反事实，引入DEER-3D框架，采用"分解、诊断评估、编辑和重新训练"的工作流程，针对模型特定弱点进行最小化、谓词对齐的3D场景编辑。

Result: 在多个3D接地和场景理解基准测试中评估编辑流程，通过迭代精炼在所有评估数据集上一致显示出改进效果。
...
TL;DR: 提出BCE3S方法，通过二元交叉熵的三部分协同学习解决长尾识别问题，包括联合学习、对比学习和均匀学习，在多个长尾数据集上达到SOTA性能。
...
Method: BCE3S包含三个组件：1) BCE联合学习通过多个Sigmoid解耦特征与不平衡分类器向量的度量；2) BCE对比学习进一步提升特征的类内紧密度；3) BCE均匀学习平衡分类器向量间的可分离性。

Result: 实验表明，BCE3S训练的长尾识别模型不仅实现了更高的特征紧密度和可分离性，还平衡了分类器的可分离性，在CIFAR10-LT、CIFAR100-LT、ImageNet-LT和iNaturalist2018等数据集上达到SOTA性能。

Conclusion: BCE3S通过三元协同学习方法有效解决了长尾识别中的特征学习和分类器平衡问题，显著提升了模型性能。
...
TL;DR: FAPE-IR是一个用于全合一图像恢复的频率感知规划和执行框架，使用冻结的多模态大语言模型作为规划器生成频率感知的恢复计划，通过LoRA-MoE模块在扩散模型执行器中动态选择高频或低频专家，实现统一且可解释的图像恢复。


<details>
  <summary>Details</summary>
Motivation: 现有的全合一图像恢复方法通常依赖任务特定设计或潜在路由策略，难以适应现实世界中各种复杂退化情况，需要更统一和自适应的解决方案。

Method: 使用冻结的MLLM作为规划器分析退化图像并生成频率感知恢复计划；采用LoRA-MoE模块在扩散执行器中动态选择高频/低频专家；引入对抗训练和频率正则化损失来减少伪影。
...
TL;DR: 本文提出了推理视频编辑任务和RIVER模型，能够通过多跳推理解析隐式查询来编辑视频内容，无需精确的空间位置和时间边界描述。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要用户提供精确的空间位置和时间边界描述，这在用户希望通过语义属性或对象关系的隐式查询来概念化编辑时变得不切实际。

Method: RIVER模型通过数字孪生表示解耦推理和生成，使用大语言模型进行多跳推理，然后输出结构化指令指导基于扩散的编辑器执行像素级修改，采用强化学习训练。

Result: RIVER在提出的RVEBenchmark上表现最佳，并在VegGIE和FiVE两个额外视频编辑基准上达到最先进性能，超越了六个基线方法。

Conclusion: RIVER通过推理驱动的视频编辑方法，成功解决了隐式查询的视频编辑问题，为复杂语义理解下的视频编辑提供了有效解决方案。
...
TL;DR: RTS-Mono是一种实时自监督单目深度估计方法，采用轻量级编码器-解码器架构，在KITTI数据集上以极低参数数量（3M）实现了最先进性能，并在Nvidia Jetson Orin上达到49 FPS的实时推理速度。
...
Conclusion: RTS-Mono在保持高性能的同时实现了实时推理，为自监督单目深度估计在现实世界中的部署提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有最优传输聚合方法对源和目标边缘分布进行对称处理，当图像特征和聚类中心分布差异较大时效果受限，需要非对称匹配机制来适应视觉地点识别中的分布差异。

Method: 采用行-列归一化平均与单独边缘校准实现非对称匹配，通过可学习坐标嵌入融入几何约束，计算与特征相似性融合的兼容性分数，促进空间邻近特征分配到同一聚类。

Result: 在MSLS、NordLand和Pittsburgh数据集上的实验结果表明，该方法在匹配准确性和鲁棒性方面表现出优越性能。

Conclusion: 所提出的A²GC-VPR方法通过非对称聚合和几何约束有效提升了视觉地点识别的性能，验证了该方法在改善匹配精度和鲁棒性方面的有效性。
...
TL;DR: 提出了一种轻量级视觉Transformer架构CViT，通过新颖的CCFFN设计提高参数和FLOP效率，在保持准确性的同时显著降低计算量和能耗。
...
Method: 设计了Cascaded-ViT架构，采用新颖的Cascaded-Chunk Feed Forward Network（CCFFN），通过分割输入特征来提高效率。

Result: 在ImageNet-1K上，CViT-XL达到75.5% Top-1准确率，相比EfficientViT-M5减少15% FLOPs和3.3%能耗。CViT系列在各种模型尺寸下都表现出最低的能耗。

Conclusion: CViT架构在保持高准确性的同时显著提升了计算效率，特别适合部署在电池受限设备上，如手机和无人机。
...
TL;DR: Coffee是一种通过语言描述来指定不希望学习的概念，从而在文本到图像扩散模型微调过程中进行正则化的方法，无需额外训练即可防止模型学习不需要的概念。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型在微调过程中学习不需要的概念以及与用户提示纠缠的问题，这对于偏见缓解、防止恶意适应、属性解缠和扩散策略的通用微调等下游任务至关重要。

Method: 通过保持用户提示的嵌入与不希望学习的概念不对齐来实现正则化，核心在于使用语言描述来指定不希望学习的概念，无需额外训练，且可以通过修改文本描述来灵活调整不希望学习的概念。

Result: 实验结果表明，Coffee能够有效防止文本到图像模型在微调过程中学习指定的不希望学习的概念，并且在性能上优于现有方法。

Conclusion: Coffee提供了一种有效的方法来在文本到图像扩散模型微调过程中控制不希望学习的概念，具有灵活性和无需额外训练的优势。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视频系统只能检测事故发生但无法分析行人行为认知阶段的问题，利用视觉语言模型潜力进行更深入的事故分析。

Method: 采用四阶段统一框架：事件触发多视角视频采集、行人行为阶段分割、阶段特定多视角推理、分层合成和诊断推理，使用两个专用VLM进行行为阶段分割和多视角分析。

Result: TG-VLM行为阶段分割mIoU达0.4881，PhaVR-VLM多视角分析captioning score达33.063，问答准确率最高64.70%，在Woven Traffic Safety数据集上验证有效。

Conclusion: MP-PVIR成功将多视角视频数据转化为可操作见解，推进了车路协同系统中AI驱动的交通安全分析。
...
Result: 在多个草图任务（对象定位、计数、图像检索、视觉问答）上的综合评估显示，O3SLM在草图理解和推理方面显著优于现有LVLMs，达到了最先进的性能。

Conclusion: 通过专门的数据集和模型设计，成功提升了大型视觉语言模型对抽象视觉输入（特别是手绘草图）的理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够重建被完全遮挡物体的3D形状的能力，这在机器人、增强现实和物流等领域具有重要应用价值。毫米波信号能够穿透常见遮挡物并从隐藏物体反射，为解决这一问题提供了可能。

Method: 采用三阶段流水线：提出候选几何表面、使用专门为毫米波信号设计的基于transformer的形状补全模型、执行熵引导的表面选择。该方法结合了毫米波信号的物理特性与基于视觉的形状补全技术的最新进展。

Result: 与最先进基线方法相比，Wave-Former将召回率从54%提升到72%，同时保持85%的高精度。该方法仅使用合成点云进行训练，但在真实世界数据上表现出良好的泛化能力。

Conclusion: Wave-Former通过引入物理感知的形状补全模型，成功解决了过去毫米波重建方法覆盖范围有限和噪声高的问题，实现了对完全遮挡物体的高精度3D形状重建。
...
<details>
  <summary>Details</summary>
Motivation: 多模态人脸反欺诈方法在跨域部署时比单模态方法性能下降更严重，主要由于两个被忽视的风险：模态表示不变风险（表示在域偏移下是否保持泛化性）和模态协同不变风险（模型过拟合到域特定的模态间相关性）。

Method: 提出RiSe框架：1）使用非对称不变风险最小化在径向空间学习不变球形决策边界以适应非对称分布；2）采用多模态协同解耦通过跨样本混合和解耦增强内在、可泛化的模态特征。

Result: 理论分析和实验验证了RiSe的有效性，在跨域性能上达到了最先进水平。

Conclusion: RiSe框架通过解决多模态泛化的两个关键风险，显著提升了多模态人脸反欺诈在未知域中的性能表现。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入对大型视觉语言模型鲁棒性影响的基准测试，包含1,248个专家标注的VQA实例，涵盖视觉概念、属性和关系三个层次，并提出了细粒度评估指标MVI-Sensitivity。
...
TL;DR: 本文提出了一种面向多模态大语言模型的对象级令牌压缩策略，通过自适应令牌合并减少图像令牌数量，仅使用10%令牌即可达到原模型96%性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统补丁级令牌化导致的二次增长问题，该问题增加了计算和内存负担，且与人类视觉认知系统不匹配，导致幻觉和计算冗余。

Method: 提出对象级令牌合并策略进行自适应令牌压缩，该方法与人类视觉系统保持一致。

Result: 在多个综合基准测试中，平均仅使用10%令牌即可达到原模型96%性能，在压缩比和性能平衡方面优于相关工作。

Conclusion: 该方法有效解决了多模态大语言模型中图像令牌过多的问题，在保持性能的同时显著降低了计算和内存需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作针对软效应退化问题开发了高度专业化的模型，缺乏可扩展性且未能利用这些恢复问题的共享本质。通用模型需要详细提示且在这些细粒度任务上表现不佳。

Method: 构建包含380万对图像的大规模数据集，包含新颖的物理合理数据填补公共基准的关键空白，并定制训练流程微调扩散变换器，从多样化数据中学习鲁棒恢复先验，集成细粒度掩码和强度控制。

Result: UniSER显著优于专业模型和通用模型，在真实场景中实现鲁棒、高保真的图像恢复。

Conclusion: 通过利用软效应的共同本质（半透明遮挡）和协同方法，UniSER能够在一个统一框架内有效处理多种图像退化问题，展示了基础性通用模型的潜力。
...
TL;DR: 本文提出了一种全局视角分词器(GloTok)，利用全局关系信息建模更均匀的语义分布，通过代码本直方图关系学习和残差学习模块，在无需预训练模型直接访问的情况下实现高质量图像重建和生成。
...
Method: 提出全局视角分词器(GloTok)，包含代码本直方图关系学习方法和残差学习模块，前者将预训练模型在整个数据集上建模的语义转移到语义代码本，后者恢复细粒度细节以最小化量化重建误差。

Result: 在标准ImageNet-1k基准测试中，该方法实现了最先进的重建性能和生成质量。

Conclusion: GloTok通过全局关系建模实现了更均匀分布的语义潜在表示，促进了自回归模型训练，无需在训练过程中直接访问预训练模型即可生成高质量图像。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集大多由人类驾驶模式收集，只能用于自动驾驶车辆的感知和预测早期训练，无法评估黑盒控制下自动驾驶车辆的真实行为安全性。

Method: 收集多个量产自动驾驶车辆模型的真实世界数据，将原始数据分割为32,727个关键帧，每个关键帧包含同步摄像头图像和高精度GNSS/IMU数据，并提供车辆轨迹和详细的2D标注。

Result: 数据集包含丰富的场景级属性，包括驾驶员意图、区域类型、光照条件、天气、路面状况等。使用端到端运动规划模型在自动驾驶帧上实现1.4米的平均位移误差。
...
TL;DR: 该论文提出了一种基于课程学习策略的分层语义学习方法，用于主动脉血管分割。通过分形softmax和两阶段推理策略，显著提高了分割精度和效率，特别解决了血管结构中的类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 主动脉作为人体最大的动脉，容易出现夹层、动脉瘤和动脉粥样硬化等病理变化，需要及时干预。现有的方法往往忽略分层解剖关系，同时难以处理血管结构中固有的严重类别不平衡问题。

Method: 采用课程学习策略，利用新颖的分形softmax进行分层语义学习。该方法模仿人类认知过程，从简单到复杂逐步学习解剖约束。通过两阶段推理策略实现高达五倍的加速。

Result: 在验证集上，分层语义损失将nnU-Net ResEnc M的Dice分数提高了11.65%。在测试集上，所提模型的Dice分数比基线高出5.6%。实验结果显示分割精度和效率均有显著提升。

Conclusion: 该框架在分割准确性和效率方面都取得了显著改进，适用于实时临床应用。提出的课程学习策略有效解决了类别不平衡问题，加速了模型收敛。
...
TL;DR: DetGain是一种专门用于目标检测的在线数据筛选方法，通过估计每张图像对数据集级平均精度(AP)的边际扰动来选择信息丰富的训练样本。
...
TL;DR: InstantViR是一个基于预训练视频扩散先验的超快速视频重建框架，通过将双向视频扩散模型蒸馏为因果自回归学生模型，实现单次前向传播的实时视频恢复，在保持高质量重建的同时达到35 FPS以上的处理速度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散先验方法要么使用临时正则化器导致时间伪影，要么依赖迭代后验采样速度过慢，无法满足实时视频流、AR/VR等应用的严格延迟要求。

Method: 将强大的双向视频扩散模型（教师）蒸馏为因果自回归学生模型，仅需教师扩散模型和已知退化算子，无需外部配对数据；采用LeanVAE替换VAE骨干网，通过教师空间正则化蒸馏方案实现低延迟潜在空间处理。
...
TL;DR: 提出了一种自回归扩散模型，用于语音驱动的3D面部动画，通过流式处理解决长音频序列的性能和延迟问题。
...
Conclusion: 提出的流式处理方法有效解决了长音频序列的性能和延迟问题，实现了实时高质量的语音驱动3D面部动画。
...
<details>
  <summary>Details</summary>
Motivation: 随着基础模型在单目深度估计中的零样本泛化能力显著提升，研究如何利用下游任务数据进一步改进这些模型的性能成为自然需求。

Method: 采用密集自训练目标作为结构自监督的主要来源；引入语义感知的分层归一化，利用实例级分割图进行更稳定的多尺度结构归一化；使用成对序数深度标注作为成本效益高的弱监督；应用权重正则化损失来锚定LoRA更新。

Result: 在多样化和具有挑战性的场景下，对现实和损坏的分布外数据集进行广泛实验，表明WeSTAR持续改善泛化能力，并在广泛基准测试中达到最先进性能。

Conclusion: WeSTAR框架通过弱监督自训练适应与正则化，有效增强了单目深度估计基础模型在未见领域的鲁棒性和泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统GNSS定位在GNSS拒止环境中经常失效，导致协作感知中的特征对齐困难，需要开发不依赖GNSS的鲁棒协作感知方法。

Method: 1) 提出轻量级姿态生成器PGC估计紧凑姿态和置信度表示；2) 开发姿态感知时空对齐变换器PASTAT，进行置信度感知的空间对齐并捕获关键时间上下文；3) 创建V2VLoc仿真数据集支持定位和协作检测任务。

Result: 在V2VLoc数据集上的广泛实验表明，该方法在GNSS拒止条件下达到最先进性能，在真实世界V2V4Real数据集上的扩展实验验证了PASTAT的有效性和泛化能力。

Conclusion: 该框架为GNSS拒止环境下的协作感知提供了有效的解决方案，通过LiDAR定位和时空对齐技术实现了鲁棒的协作感知性能。
...
TL;DR: 提出了ManipBench大规模图像篡改检测基准和ManipShield统一检测模型，针对AI编辑图像的检测挑战，包含45万张篡改图像和10万张带标注图像，实现检测、定位和解释的一体化解决方案。
...
Method: 构建包含45万张由25个最先进图像编辑模型生成的篡改图像的ManipBench基准；提出基于多模态大语言模型的ManipShield模型，采用对比LoRA微调和任务特定解码器实现统一检测。

Result: 在ManipBench和多个公开数据集上的广泛实验表明，ManipShield实现了最先进的性能，并对未见过的篡改模型表现出强泛化能力。

Conclusion: ManipBench和ManipShield为解决AI编辑图像检测挑战提供了有效的基准和解决方案，两者将在发表后发布。
...
TL;DR: 提出基于高斯泼溅的低秩张量表示框架GSLR，用于多维图像的紧凑连续表示，解决了传统t-SVD方法在捕捉空间局部高频信息方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统张量奇异值分解方法存在两个关键限制：潜在张量近似粗糙无法准确捕捉空间局部高频信息；变换矩阵由固定基原子组成无法精确捕捉模式-3纤维的局部高频信息。
...
Conclusion: GSLR框架通过高斯泼溅技术有效解决了传统t-SVD方法的局限性，为多维图像提供了紧凑连续的表示方法，在图像恢复任务中表现出优越性能。
...
TL;DR: VLM3D是一个通用框架，利用大型视觉语言模型作为可微分的语义和空间评判器，通过双查询评判信号同时评估语义保真度和几何一致性，显著提升了文本到3D生成的质量。
...
Method: 提出VLM3D框架，重新利用大型视觉语言模型作为强大的可微分语义和空间评判器，核心贡献是基于VLM的是/否对数概率的双查询评判信号。

Result: 在优化型流程中作为奖励目标显著优于现有方法；在前馈型流程中作为测试时指导模块，能够主动纠正严重空间错误。
...
Conclusion: NeuralBoneReg在解剖结构和模态间表现出强大的泛化能力，为CAOS提供鲁棒且准确的跨模态配准。
...
TL;DR: Gen3d是一种从单张图像生成高质量、宽范围通用3D场景的新方法，通过RGBD图像创建初始点云，维护和扩展世界模型，并优化高斯泼溅表示来生成3D场景。
...
Result: 在多个数据集上的广泛实验表明，该方法在生成世界模型和合成高保真度、一致的新视角方面具有强大的泛化能力和优越性能。

Conclusion: Gen3d方法能够从单张图像有效生成高质量的3D场景，为具身AI和世界模型的发展提供了重要支持。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临数据隐私和专家标注成本限制，联邦半监督学习(FSSL)存在伪标签可靠性依赖本地模型能力、客户端设备计算资源有限导致模型架构紧凑或异构的问题。

Method: 提出SAM-Fed框架，结合双重知识蒸馏和自适应一致性机制来精炼像素级监督，利用高容量分割基础模型指导轻量级客户端训练。

Result: 在皮肤病变和息肉分割任务上的实验表明，SAM-Fed在同质和异构设置下均优于最先进的FSSL方法。

Conclusion: SAM-Fed通过利用基础模型指导轻量级客户端，有效解决了联邦半监督学习中伪标签质量和稳定性的挑战。
...
TL;DR: 本文提出了Diff-NAF框架，用于解决多源静态CT在超稀疏视角采样下的重建质量下降问题，通过结合神经衰减场表示和双分支条件扩散模型，实现迭代式投影完善和高质量CT重建。


<details>
  <summary>Details</summary>
Motivation: 多源静态CT在时间敏感应用中具有快速重建优势，但实际系统常受限于超稀疏视角采样，导致重建质量显著下降。传统方法在超稀疏视角条件下表现不佳，插值不准确，重建结果不理想。

Method: Diff-NAF结合神经衰减场表示与双分支条件扩散模型，采用迭代框架：首先使用超稀疏视角投影训练初始NAF；然后通过角度先验引导的投影合成策略生成新投影；再利用扩散驱动的重用投影细化模块进行精炼；最后将精炼投影作为伪标签加入训练集进行下一轮迭代。

Result: 在多个模拟3D CT体积和真实投影数据上的实验结果表明，Diff-NAF在超稀疏视角条件下实现了最佳性能，显著提升了投影完整性和重建保真度。

Conclusion: Diff-NAF通过迭代精炼过程，在超稀疏视角条件下逐步增强投影完整性和重建质量，最终获得高质量的CT重建结果，为解决多源静态CT在极端稀疏采样下的重建挑战提供了有效解决方案。
...
TL;DR: Dental3R是一个用于稀疏口腔内照片的3D重建管道，通过几何感知配对策略和基于小波的3D高斯溅射，解决了传统方法在远程正畸中的挑战。
...
Method: 提出几何感知配对策略(GAPS)选择高价值图像对，然后使用基于小波正则化的3D高斯溅射进行训练，以保持精细的牙釉质边界和邻接边缘。

Result: 在950个临床病例和195个视频测试集上验证，Dental3R能有效处理稀疏无姿态输入，在牙齿咬合可视化方面优于现有方法。

Conclusion: Dental3R为远程正畸提供了一种稳健、高保真度的3D重建解决方案，能够从稀疏口腔照片中恢复高质量的牙齿几何结构。
...
TL;DR: ArchMap是一个无需训练的知识引导框架，用于从3D口腔扫描中实现结构化牙齿理解，通过几何感知的牙弓展平模块和牙科知识库来解决现有方法的泛化性问题。
...
TL;DR: DentalSCR是一个用于口腔扫描模型与侧位头颅X光片精确配准的稳定轮廓引导框架，通过构建统一解剖坐标系和分层优化策略，解决了传统配准方法在临床条件下的稳定性问题。
...
Method: 首先构建U-Midline Dental Axis建立统一跨弓解剖坐标系，然后通过表面DRR公式生成X光片样投影，最后使用对称双向Chamfer距离在分层粗到细策略下优化2D相似变换。

Result: 在34个专家标注的临床病例上评估，显示显著降低了标志点误差（特别是后牙区域），下颌骨误差分布更集中，曲线级别的Chamfer和Hausdorff距离表现优异。

Conclusion: DentalSCR能够稳健处理真实世界头颅X光片，提供高保真度的3D-2D对齐结果，优于传统基线方法，具有临床可检查性。
...
TL;DR: ARC-Chapter是首个基于百万级长视频章节数据训练的大规模视频分段模型，通过双语、时序定位和分层标注，在视频章节划分任务上实现了显著性能提升。
...
Method: 通过结构化流水线构建双语英中章节数据集，统一ASR转录、场景文本和视觉描述为多级标注，从短标题到长摘要；设计了GRACE评估指标，结合多对一段落重叠和语义相似度。

Result: ARC-Chapter在F1分数上比之前最佳方法提升14.0%，在SODA分数上提升11.3%，建立了新的最先进水平；在YouCook2等下游任务上也表现出优异的迁移能力。

Conclusion: 数据规模（数据量和标注密度）的扩展带来明显性能改进，ARC-Chapter证明了大规模训练在视频章节划分任务中的有效性，并展示了良好的迁移性能。
...
TL;DR: 本文提出了一种基于图像的高斯溅射方法，通过结合标准3DGS渲染的基础颜色和从邻近训练图像学习到的残差，显著提升了渲染质量，特别是高频细节和视角相关效果。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯溅射方法使用低阶球谐函数，难以捕捉空间变化的颜色和视角相关效果（如镜面高光）。现有方法要么使用全局纹理图（难以处理复杂场景），要么使用每个高斯点的纹理图（存储开销大）。

Method: 提出图像基高斯溅射方法，将每个像素颜色建模为标准3DGS渲染的基础颜色与从邻近训练图像学习到的残差的组合，利用高分辨率源图像进行精细细节和视角特定颜色建模。

Result: 在标准新视角合成基准测试中，该方法在渲染质量上显著优于先前的高斯溅射方法，且不增加存储占用。

Conclusion: 该方法通过有效利用源图像信息，实现了高质量的新视角合成，同时保持了存储效率。
...
TL;DR: Bapp是一款基于Flutter框架和Google ML Kit开发的移动应用，用于实时分析眼睑运动。在45个真实患者视频的临床验证中，该应用在眨眼检测方面达到了98.4%的精确度、96.9%的召回率和98.3%的整体准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的眼睑运动评估工具存在复杂性、成本高和临床应用有限的问题，需要开发便携、可访问且客观的监测工具。

Method: 使用Flutter框架和Google ML Kit开发移动应用Bapp，在设备上进行实时眼睑运动分析。验证使用45个真实患者视频，由眼科专家手动标注作为真实基准。

Result: Bapp在眨眼检测方面表现优异：精确度98.4%、召回率96.9%、整体准确率98.3%。

Conclusion: Bapp被证实为一种可靠、便携、可访问的客观工具，可用于监测正常和异常眼睑运动，为传统手动眨眼计数提供了有前景的替代方案。
...
TL;DR: 提出了一种肩部姿态评估算法，用于自动评估生物特征身份文档中肩部姿态的合规性，填补了现有质量评估方法在此方面的空白。


<details>
  <summary>Details</summary>
Motivation: 国际生物特征身份文档标准要求严格符合姿态要求，包括肩部的正面展示，但现有自动化质量评估方法缺乏对此特定属性的定量评估方法。

Method: 使用常见姿态估计框架提供的两个肩部标志点的3D坐标，量化肩部偏航角和滚转角，开发了肩部姿态评估算法。

Result: 在121张肖像图像数据集上评估，SPE得分与人工标注显示出强Pearson相关性（r≈0.80），通过改进的Error-versus-Discard方法验证了其在识别不合规样本方面的有效性。
...
TL;DR: 本文提出首个针对自动驾驶中立体匹配模型的纹理化物理对抗攻击方法，使用3D全局伪装纹理而非局部2D补丁，确保立体相机不同视角下的视觉一致性和攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击多采用2D补丁且主要针对单目感知，立体双目深度估计中的物理对抗样本有效性尚未充分探索，需要开发针对立体匹配模型的物理攻击方法。

Method: 提出3D物理对抗样本，采用全局伪装纹理；设计3D立体匹配渲染模块处理相机视差效应；提出新颖的融合攻击，通过细粒度优化将目标无缝融入环境。

Result: 广泛评估表明，所提出的物理对抗样本能成功欺骗立体模型产生错误的深度信息，相比现有隐藏攻击具有显著增强的隐蔽性和杀伤力。

Conclusion: 该方法首次实现了针对立体匹配模型的纹理化物理对抗攻击，在保持视觉一致性的同时有效破坏深度估计，为自动驾驶系统的安全性评估提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在图像坐标系中估计人体运动，但许多应用需要世界坐标系下的运动估计，特别是在同时存在人体和相机运动时。该领域进展受到缺乏具有真实人体和相机运动地面真值视频数据的限制。

Method: 开发BEDLAM2.0数据集，通过增加更多样化和真实的相机、相机运动、身体形状、动作、服装、头发、3D环境和鞋子等元素来改进BEDLAM数据集。

Result: BEDLAM2.0在训练估计世界坐标系下人体运动的方法时显著优于BEDLAM。比较最先进方法在BEDLAM和BEDLAM2.0上的训练结果，发现BEDLAM2.0显著提高了准确性。

Conclusion: BEDLAM2.0是一个重要的数据集改进，特别适用于训练估计世界坐标系下人体运动的方法，提供了渲染视频、地面真值身体参数和相机运动等研究资源。
...
Method: 使用序数回归框架，结合绿通道提取、噪声掩蔽和CLAHE等预处理方法，从APTOS-2019眼底图像数据集中提取最相关的特征进行分类。
...
TL;DR: LAVA是一个新颖的领域增量学习框架，通过基于文本的参考锚点实现相对对齐，解决现有方法在统一视觉空间和隔离参数之间的两难困境。


<details>
  <summary>Details</summary>
Motivation: 解决领域增量学习中统一视觉空间导致的域间干扰和语义扭曲问题，以及隔离参数导致的知识碎片化问题。

Method: 使用语言锚定的视觉对齐方法，通过文本参考锚点驱动相对对齐，保持一致的相对几何结构，该结构基于类名之间的语义相似性定义。

Result: 在标准领域增量学习基准测试中，LAVA相比现有最先进方法实现了显著的性能提升。

Conclusion: LAVA通过语言锚定的相对对齐方法，有效解决了领域增量学习中的知识保留和干扰问题，为跨领域知识重用提供了新思路。
...
TL;DR: 本文提出Cranio-ID框架，通过YOLO-pose模型自动标注2D头骨X射线扫描图像和光学图像中的标志点，然后使用图表示和跨模态注意力机制进行头骨到面部的匹配，在法医科学中实现可靠的跨域识别。
...
Conclusion: Cranio-ID框架提供了一种可靠且准确的颅骨标志点自动标注和跨模态匹配方法，在法医识别中具有重要应用价值。
...
TL;DR: 该研究通过模拟婴儿视觉发育过程（灰度到彩色、模糊到清晰、保持时间连续性）训练自监督学习模型，发现这种阶段性视觉训练能增强模型的鲁棒性，并产生与生物发育相似的模式。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿视觉发育过程的生态优势，理解早期视觉经验如何促进鲁棒视觉智能的涌现。

Method: 使用CATDiet方法训练自监督学习模型，模拟婴儿视觉约束条件：灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)。建立包含10个数据集的综合基准进行评估。

Result: 所有CATDiet变体在物体识别方面表现出增强的鲁棒性，模型显示出与生物发育一致的模式，包括类似猕猴V1区突触密度的神经可塑性变化和类似婴儿视觉悬崖反应的行为。

Conclusion: 早期婴儿视觉经验的发育进程为理解机器中鲁棒视觉智能的涌现提供了一个强大的逆向工程框架。
...
TL;DR: AVI是一个无需训练的视频理解框架，通过检索-感知-审查的三阶段推理过程、结构化视频知识库和开源模型集成，实现了竞争性性能和高可解释性。
...
Method: 提出三阶段推理过程（检索-感知-审查）、基于实体图的结构化视频知识库、多粒度集成工具，以及结合推理LLM和轻量级CV模型的开源模型集成。

Result: 在LVBench、VideoMME-Long、LongVideoBench和Charades-STA等基准测试中取得了竞争性性能。

Conclusion: AVI框架通过系统级设计和优化，实现了无需依赖专有API或强化学习训练的高效视频理解，同时提供优越的可解释性。
...
TL;DR: DIR-TIR框架通过对话精炼和图像精炼两个模块，实现交互式对话文本到图像检索，通过多轮对话逐步优化搜索精度，显著提升目标图像命中率。


<details>
  <summary>Details</summary>
Motivation: 解决传统单查询文本到图像检索方法在控制性和容错性方面的不足，通过交互式对话提供更精确的图像搜索体验。

Method: 采用双模块架构：对话精炼模块主动询问用户提取关键信息生成精确描述；图像精炼模块识别生成图像与用户意图之间的感知差距，减少视觉语义差异。

Result: 在多个图像数据集上的综合实验表明，该方法显著优于仅使用初始描述的基线方法，模块协同集成实现了更高的检索精度和更好的交互体验。

Conclusion: DIR-TIR框架通过多轮对话机制提供了优越的控制性和容错性，在交互式文本到图像检索任务中表现出色，为相关应用提供了有效解决方案。
...
TL;DR: 提出了一种物理引导的残差学习框架，通过预测BedMachine先验上的床厚残差，从观测表面重建冰川床地形，在格陵兰冰盖区域实现了优于传统方法的精度和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 冰川床地形的准确测量对冰盖建模至关重要，但雷达观测数据稀疏且分布不均，需要开发能够处理稀疏数据并保持物理合理性的重建方法。

Method: 使用DeepLabV3+解码器和ResNet-50编码器，结合多尺度质量守恒、流向总变分、拉普拉斯阻尼、厚度非负性、斜坡先验一致性项以及基于置信度图的掩码Huber拟合等轻量级物理和数据项进行训练。

Result: 在格陵兰两个子区域，该方法在测试核心区域实现了强精度和高结构保真度，优于U-Net、Attention U-Net、FPN和普通CNN等基准方法。

Conclusion: 残差-先验设计结合物理约束能够产生空间相干、物理合理的床地形，适用于领域偏移下的操作制图。
...
TL;DR: Gaussian Spatial Transport (GST) 是一个利用高斯泼溅从图像坐标空间概率测度到标注图的传输框架，通过贝叶斯概率计算传输计划，避免了传统最优传输方案在训练期间的迭代计算，显著提高了效率。
...
TL;DR: SEED-SR是一种新的农业边界分割方法，通过在分割感知的潜在空间中进行超分辨率处理，能够以20倍尺度因子生成分割图，显著提升了小农户农场边界分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决小农户农场边界分割的挑战，特别是在高分辨率图像重访频率低的情况下，需要结合高分辨率和低分辨率图像进行更频繁的监测。

Method: 使用条件潜在扩散模型和大规模多光谱、多源地理空间基础模型，绕过像素空间的显式超分辨率任务，在分割感知的潜在空间中进行超分辨率处理。

Result: 在两个大型真实数据集上的实验表明，相比基于最先进Ref-SR方法的方法，在实例分割和语义分割指标上分别实现了25.5%和12.9%的相对改进。

Conclusion: SEED-SR方法能够以前所未有的20倍尺度因子生成分割图，有效解决了农业应用中农场边界分割的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前自动驾驶系统在泛化能力方面的限制，特别是处理未见场景或不同传感器配置的能力，同时避免混合系统的不一致性和端到端方法的高计算成本。
...
Conclusion: RSD通过风险注意力蒸馏有效增强了自动驾驶系统的泛化能力，使其更接近人类驾驶行为，在复杂动态环境中表现更优。
...
TL;DR: 提出了一种基于解码器的新框架PAMM，利用状态空间模型的丰富参数来增强多任务密集预测中的任务互联性，通过双状态空间参数专家整合任务特定参数先验，并在NYUD-v2和PASCAL-Context基准测试中验证了有效性。
...
Method: 提出参数感知Mamba模型(PAMM)，利用状态空间模型的丰富可扩展参数增强任务互联性。采用双状态空间参数专家整合任务特定参数先验，通过结构化状态空间序列模型(S4)全局整合任务先验，并使用多方向希尔伯特扫描方法构建多角度特征序列。

Result: 在NYUD-v2和PASCAL-Context基准测试上进行了广泛实验，证明了所提方法的有效性。

Conclusion: PAMM框架通过状态空间模型有效增强了多任务密集预测中的任务互联性，双参数专家机制能够捕获任务内在特性，多方向特征序列构建提升了2D数据的感知能力。
...
TL;DR: D-PerceptCT是一种受人类视觉系统启发的LDCT图像增强架构，通过整合语义先验和多尺度特征来保留诊断关键结构，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: LDCT图像质量因低剂量辐射而下降，现有方法过度平滑导致关键细节丢失，需要开发能保留感知相关特征的增强方法。

Method: 提出D-PerceptCT架构，包含视觉双路径提取器（整合DINOv2语义先验）和全局-局部状态空间块，并设计了基于人类对比敏感度的深度感知相关性损失函数。

Result: 在Mayo2016数据集上的实验表明，D-PerceptCT在LDCT增强方面优于现有SOTA方法，能更好地保留结构和纹理信息。

Conclusion: D-PerceptCT通过模拟人类视觉系统原理，成功提升了LDCT图像质量，为放射科医生提供了感知可见的关键解剖结构和病理细节。
...
Conclusion: DeCo-VAE通过内容解耦实现了紧凑的视频潜在表示，显著提升了视频重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 虽然隐式函数在单个形状或场景上表现良好，但在分析多个具有高保真几何细节的SDF时存在障碍，主要由于潜在空间编码信息有限和几何细节丢失的问题。

Method: 采用结合泛化学习和过拟合学习策略的框架，充分利用两种策略的优势；引入新的采样策略来采样训练查询，提高训练效率并消除其他SDF影响导致的伪影。

Result: 在广泛使用的基准测试上进行了数值和视觉评估，验证了设计有效性，在表示能力和紧凑性方面优于最新方法。

Conclusion: 该方法能够用更紧凑的潜在表示恢复更多高保真几何细节，解决了多SDF分析中的几何细节丢失问题。
...
TL;DR: 本文提出了一种无需物体先验知识的手-物体交互场景几何和外观建模方法，通过动态3D高斯泼溅技术解决复杂交互中的遮挡和边缘模糊问题，实现了最先进的动态手-物体交互重建性能。


<details>
  <summary>Details</summary>
Motivation: 解决在没有物体先验知识的情况下，同时建模手-物体交互场景的几何和外观的挑战性问题，特别是处理复杂交互中的相互遮挡和边缘模糊。

Method: 采用动态3D高斯泼溅方法，引入交互感知的手-物体高斯模型和可优化参数，使用分段线性假设实现更清晰的结构表示；将手部信息整合到物体变形场中，构建交互感知动态场来建模灵活运动；采用渐进式策略分步处理动态区域和静态背景；设计显式正则化来稳定手-物体表示。

Result: 实验表明，该方法超越了现有的基于动态3D-GS的方法，在动态手-物体交互重建方面达到了最先进的性能。

Conclusion: 提出的方法成功解决了无先验知识下的手-物体交互场景建模问题，通过交互感知的高斯模型和动态场、渐进式策略以及正则化设计，实现了高质量的动态交互重建。
...
Method: 提出三模态框架：RGB分支（ConvNeXt-tiny）提取全局视觉不一致性；纹理分支（Swin Transformer-tiny）检测细粒度融合伪影；频率分支（CNN+SE）识别周期性频谱噪声。采用注意力机制进行时序池化和自适应融合。

Result: 在Celeb-DF (v2)数据集上，AUC达到0.9752，F1-Score为0.9408，准确率为0.9208，优于单流基线方法。消融实验验证了分支协同作用，Grad-CAM确认了检测焦点。

Conclusion: 通过全面的特征融合，该方法对细微伪造具有更强的鲁棒性，为深度伪造检测提供了有效的多模态解决方案。
...
Method: 首先识别关键音频令牌，计算音频保留分数捕捉信息密度，动态指导视频令牌剪枝，并使用交错时空方案压缩视频令牌。

Result: 实现了3.42倍推理加速和1.4倍内存减少，同时保持性能且无需训练。

Conclusion: OmniZip有效解决了音视频令牌压缩的计算瓶颈问题，在保持性能的同时显著提升了推理效率。
...
### [93] [Deep Learning-Based Regional White Matter Hyperintensity Mapping as a Robust Biomarker for Alzheimer's Disease](https://arxiv.org/abs/2511.14588)
*Julia Machnio,Mads Nielsen,Mostafa Mehdipour Ghazi*

Main category: cs.CV

TL;DR: 提出深度学习框架进行WMH分割和定位，区域WMH体积比全局病灶负担在疾病分类中表现更好，结合脑萎缩指标可进一步提高性能。


<details>
  <summary>Details</summary>
Motivation: 现有WMH自动分割方法大多只提供全局病灶负荷，忽视了不同白质区域的空间分布，需要开发能准确定位和量化区域WMH的方法。

Method: 使用深度学习框架进行稳健的WMH分割和定位，在公共数据集和ADNI队列中评估，量化解剖定义区域内的WMH负荷并与脑结构体积结合。

Result: 预测病灶负荷与参考WMH估计一致，区域WMH体积在疾病分类中持续优于全局病灶负担，结合脑萎缩指标AUC可达0.97，前白质束等特定区域与诊断状态可重复相关。
...
### [94] [CCSD: Cross-Modal Compositional Self-Distillation for Robust Brain Tumor Segmentation with Missing Modalities](https://arxiv.org/abs/2511.14599)
*Dongqing Xie,Yonghuang Wu,Zisheng Ai,Jun Min,Zhencun Jiang,Shaojin Geng,Lei Wang*

Main category: cs.CV

TL;DR: 提出CCSD框架解决多模态MRI中模态缺失问题，通过跨模态组合自蒸馏技术提升脑肿瘤分割的鲁棒性和泛化能力。
...
Method: 采用共享-特定编码器-解码器架构，结合层次模态自蒸馏和渐进模态组合蒸馏两种策略，在训练中模拟模态缺失。

Result: 在公共脑肿瘤分割基准测试中，CCSD在各种模态缺失场景下均达到最先进性能，具有强泛化性和稳定性。

Conclusion: CCSD框架能灵活处理任意输入模态组合，有效解决临床多模态MRI数据不完整的问题。
...
TL;DR: XAttn-BMD是一个多模态深度学习框架，通过髋部X射线图像和临床元数据预测股骨颈骨密度，采用双向交叉注意力机制实现跨模态特征融合，在骨密度估计方面优于基线模型。
...
Method: 提出XAttn-BMD多模态深度学习框架，使用新颖的双向交叉注意力机制动态整合图像和元数据特征，采用加权平滑L1损失函数处理骨密度不平衡问题并优先考虑临床显著病例。

Result: 在Hertfordshire队列研究数据上的实验表明，该模型在回归泛化性和鲁棒性方面优于基线模型。交叉注意力融合比简单特征拼接减少MSE 16.7%、MAE 6.03%，提高R2分数16.4%。在临床相关骨密度阈值下的二元分类也显示了良好的筛查性能。

Conclusion: XAttn-BMD框架通过交叉注意力机制有效整合多模态数据，在股骨颈骨密度估计方面表现出色，具有在真实世界场景中应用的潜力。
...
TL;DR: HoloTea是一个3D感知的流匹配框架，通过从H&E染色图像中推断斑点级基因表达，利用相邻切片信息来提升3D组织转录组学分析的准确性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有的预测算法大多独立处理每个切片而忽略3D结构，而现有的3D感知方法不可生成且扩展性差，需要开发能够充分利用3D结构信息的可扩展方法。
...
TL;DR: 本研究提出了BioST-GCN模型，结合姿态和生物力学信息进行跌倒预测，在模拟数据上表现良好但存在模拟-现实差距问题。
...
Method: 使用双流Biomechanical Spatio-Temporal Graph Convolutional Network (BioST-GCN)模型，结合姿态和生物力学信息，采用交叉注意力融合机制。
...
TL;DR: 本文提出了一种名为S-Net的方法，通过立体几何-纹理对齐和伪特征增强几何一致性，在稀疏视图场景下实现更准确和详细的表面重建，同时保持高质量的新视角合成性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏视图场景下容易过拟合，导致重建质量下降。扁平化高斯原语虽然能更好地拟合表面几何，但其固有的各向异性在稀疏视图下会加剧过拟合问题。
...
Conclusion: 该方法能够有效缓解稀疏监督引起的过拟合问题，同时提升表面重建和新视角合成的质量。
...
Method: 提出SLAM-AGS框架，联合优化弱监督相似性目标（在幻灯片负补丁上）和自监督对比目标（在幻灯片正补丁上），使用自适应梯度手术解决任务梯度冲突，防止模型崩溃。

Result: 在公开骨髓细胞学数据集上，模拟目击率从10%降至0.5%，SLAM-AGS在袋级F1分数和前400阳性细胞检索方面优于其他预训练方法，在低目击率下增益最大。

Conclusion: 解决梯度干扰能够实现稳定预训练，并在下游任务上获得更好性能，特别是在低目击率场景下。
...
### [100] [Improving segmentation of retinal arteries and veins using cardiac signal in doppler holograms](https://arxiv.org/abs/2511.14654)
...
<details>
  <summary>Details</summary>
Motivation: 传统视网膜血管分割方法仅关注空间信息，忽略了多普勒全息数据的时间丰富性，无法充分利用动态血流信息。

Method: 使用标准分割架构，结合专门的脉搏分析流程提取的特征，让传统U-Net能够利用时间动态信息进行动脉-静脉分割。

Result: 该方法实现了与更复杂的注意力或迭代模型相当的性能，证明了时间分辨预处理能够释放深度学习在多普勒全息中的潜力。

Conclusion: 时间分辨预处理可以为多普勒全息术的深度学习应用开辟新前景，促进视网膜血流动力学的定量探索。
...
### [101] [Signature vs. Substance: Evaluating the Balance of Adversarial Resistance and Linguistic Quality in Watermarking Large Language Models](https://arxiv.org/abs/2511.13722)
*William Guo,Adaku Uchendu,Ana Smith*

Main category: cs.CL

TL;DR: 本文评估了多种水印技术对对抗攻击的鲁棒性，发现这些技术能保持语义但会偏离原始文本的写作风格，且容易受到对抗攻击，特别是回译攻击。


<details>
  <summary>Details</summary>
Motivation: 为了减轻大型语言模型生成文本的潜在危害，研究者提出了水印技术，但现有技术存在降低文本质量和易受对抗攻击的问题，导致LLM创建者对广泛采用水印产生抵制。

Method: 通过比较改写和回译（英语→其他语言→英语）攻击，评估多种水印技术的鲁棒性；使用语言指标来评估水印技术保持文本质量和写作风格的能力。

Result: 水印技术能够保持语义，但会偏离未加水印文本的写作风格，并且容易受到对抗攻击，特别是回译攻击。

Conclusion: 当前的水印技术在保持语义方面表现良好，但在保护写作风格和抵抗对抗攻击方面存在不足，特别是对回译攻击的脆弱性阻碍了其广泛采用。
...
### [102] [Refine Thought: A Test-Time Inference Method for Embedding Model Reasoning](https://arxiv.org/abs/2511.13726)
*Guangzhi Wang,Kai Li,Yinghao Jiao,Zhi Liu*

Main category: cs.CL

TL;DR: RT（Refine Thought）是一种通过多次前向传递来增强文本嵌入模型语义推理能力的方法，在语义推理任务上取得显著改进，同时在通用语义理解任务上保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 提升文本嵌入模型的语义推理能力，激活预训练期间学到的语义推理潜力。

Method: 通过多次前向传递运行文本嵌入模型，获得最终的语义表示。

Result: 在BRIGHT和PJBenchmark1语义推理任务上取得显著改进，在C-MTEB等通用语义理解任务上保持稳定性能。

Conclusion: RT作为测试时推理方法，能够有效激活解码器专用文本嵌入模型的语义推理能力。
...
### [103] [Can QE-informed (Re)Translation lead to Error Correction?](https://arxiv.org/abs/2511.13884)
*Govardhan Padmanabhan*

Main category: cs.CL

TL;DR: 本文提出了两种无需训练的方法用于WMT 2025质量评估指导的机器翻译错误修正任务：QE-informed重翻译和基于QE解释的错误子串替换方法。第一种方法通过选择不同LLM生成的最佳质量翻译获胜。


<details>
  <summary>Details</summary>
Motivation: 虽然联合训练QE和APE系统能提升性能，但APE系统存在过度修正问题，导致性能下降。本文旨在研究无需训练的简单方法来避免过度修正问题。

Method: 提出了两种无需训练的方法：1）QE-informed重翻译：从不同LLM生成的多个候选中选择最高质量的翻译；2）基于QE解释的错误子串替换：根据提供的QE解释指导LLM替换错误子串，并使用条件启发式最小化编辑次数。

Result: 两种方法分别获得了0.0201和-0.0108的Delta COMET分数，第一种方法在子任务排行榜上获胜。

Conclusion: 无需训练的QE-informed重翻译方法在质量评估指导的错误修正任务中表现最佳，避免了APE系统的过度修正问题。
...
TL;DR: 论文研究了大型语言模型在长文本上下文中的'迷失在中间'现象，提出了GM-Extract基准数据集和评估系统，评估了7-8B参数模型在多文档任务中的表现，并分析了缓解方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长范围上下文时存在'迷失在中间'现象，这给基于检索的LLM应用带来了重大挑战，需要在实际应用场景中研究这一现象的影响。

Method: 提出了GM-Extract基准数据集，使用两个指标（空间检索能力和语义检索能力）进行诊断，系统评估了7-8B参数模型在多文档任务中的表现，并进行了缓解方法的文献调研和应用测试。

Result: 发现仅通过改变数据在上下文窗口中的表示方式就能显著改变检索性能，虽然未始终观察到明显的U形曲线，但发现了清晰的性能模式，并与困惑度得分相关。缓解方法的效果具有高度复杂性，在某些场景下能改善性能，在某些情况下会产生负面影响。

Conclusion: 该研究为理解LLM在长上下文中的表现提供了全面视角，强调了缓解方法在实际应用中的复杂性和情境依赖性，为改进检索型LLM应用提供了重要见解。
...
TL;DR: 论文提出了一种框架，利用LLM解析电商搜索查询中的最高级表达（如“最好”、“最流行”），通过提取结构化提示来改进搜索性能，并开发了将解释转移到轻量级模型的效率优化方法。


<details>
  <summary>Details</summary>
Motivation: 电商搜索中的最高级查询需要跨多个维度比较候选商品，这要求语言理解和领域知识。现有方法难以有效解析这些查询的潜在意图。

Method: 开发了一个框架，将查询分解为属性-值提示，这些提示与检索过程同时生成，并集成到排序管道中。为了解决LLM直接重排序的延迟问题，将最高级解释转移到轻量级模型。

Result: 方法在MAP上提升了10.9个百分点，在MRR上提升了5.9个百分点，优于基线方法。
...
Method: 提出HiEAG分层证据增强生成框架，将外部一致性检查分解为检索、重排序和重写的综合引擎流程，使用AESP进行证据重排序，AEGP进行证据重写。
...
TL;DR: 本文构建了一个平衡的多标签情感数据集，并开发了一个结合FastText嵌入、CNN、BiLSTM和注意力机制的增强模型，显著提升了多标签情感分类的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多标签情感数据集（如GoEmotions）存在严重的类别不平衡问题，这影响了模型性能，特别是对于代表性不足的情感类别。

Method: 通过整合GoEmotions原始数据、使用RoBERTa-base-GoEmotions模型标注的Sentiment140样本以及GPT-4 mini生成的手动标注文本，构建平衡数据集；开发包含FastText嵌入、CNN、BiLSTM、注意力机制和Sigmoid输出层的多标签分类模型，并采用混合精度训练提高效率。

Result: 实验结果显示，与在不平衡数据上训练的模型相比，该方法在准确率、精确率、召回率、F1分数和AUC方面均有显著提升。

Conclusion: 所提出的平衡数据集和增强模型方法有效解决了多标签情感分类中的类别不平衡问题，显著提高了分类性能。
...
TL;DR: 提出了一种名为Stealth Fine-Tuning的新型攻击方法，通过分段级干扰和自生成输出作为监督微调数据，能够以低成本（499样本，3小时A100）有效绕过RVLMs的安全对齐防御，攻击成功率比IDEATOR高38.52%。


<details>
  <summary>Details</summary>
Motivation: 虽然RVLMs依赖安全对齐来防止有害行为，但其暴露的思维链轨迹引入了新的攻击面，需要研究如何突破这种安全对齐机制。

Method: 采用分段级干扰引发有害推理轨迹，将自生成输出作为监督微调数据，使用轮次加权损失设计实现轻量级、分布一致的微调方法。

Result: 在AdvBench和多个通用基准测试中，Stealth Fine-Tuning以低成本高效地绕过了对齐防御，同时保持了模型的通用推理能力。

Conclusion: Stealth Fine-Tuning是一种低成本且高效的绕过对齐防御的方法，暴露了RVLMs安全对齐机制的脆弱性。
...
TL;DR: 本文提出了一种数据中心的框架，通过生成高质量合成出院摘要来解决ICD编码中的极端长尾分布问题，显著改善了罕见和零样本ICD代码的预测性能。


<details>
  <summary>Details</summary>
Motivation: 临床文本自动ICD编码在医疗NLP中至关重要，但由于诊断代码的极端长尾分布而受到阻碍。数千个罕见和零样本ICD代码在MIMIC-III等数据集中严重不足，导致宏观F1分数较低。

Method: 构建基于罕见代码的现实多标签代码集，利用真实世界的共现模式、ICD描述、同义词、分类法和相似临床笔记生成结构化提示，生成90,000个合成笔记覆盖7,902个ICD代码。

Result: 在原始和扩展数据集上微调PLM-ICD和GKI-ICD模型，实验显示该方法适度提高了宏观F1，同时保持了强大的微观F1，优于先前的最先进方法。
...
TL;DR: 该研究结合基于Transformer的关系提取和知识图谱匹配，应用于回答多项选择题，同时保持输出过程的可追溯性。通过将问题句子转换为关系图并验证其真实性来回答问题。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱构建成本高且被视为静态数据库，而基于Transformer的关系提取方法能够动态生成知识图谱，为表示输入句子的含义提供了可能性。

Method: 提出一种方法，通过(i)使用关系提取方法将句子转换为关系图，(ii)在封闭世界假设下验证其与事实正确知识图谱的一致性，来回答填空式多项选择题。

Result: 实验结果表明，该方法能够正确回答约70%的问题，同时提供程序的可追溯性。问题类别对准确率有很大影响。

Conclusion: 该方法成功结合了动态知识图谱生成和验证，有效回答了多项选择题，并强调了问题类别对性能的重要影响。
...
TL;DR: 本文提出了一种选择性弱到强泛化框架，通过训练二元分类器识别强模型能够回答的问题，并使用自生成标签进行对齐，同时通过图平滑方法优化弱标签，在三个基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决超人类模型只能获得弱监督数据的问题，现有弱到强泛化方法在鲁棒性方面存在问题，部分弱标签对模型有害。
...
TL;DR: 本文提出了首个基于符号语言知识的幻觉定位框架，通过分析符号触发词（如修饰语、否定、数字等）在模型各层的处理过程，揭示了幻觉的根本原因是符号语言处理失败而非一般生成问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在处理符号触发词时仍存在幻觉问题，但缺乏对这些符号幻觉来源的清晰理解，需要系统性地处理这些触发词并定位幻觉在模型内部的产生位置。

Method: 提出基于符号语言和语义知识的定位框架，使用HaluEval和TruthfulQA分析五个模型，重点关注模型如何处理符号触发词。

Result: 研究发现符号语言元素的注意力方差在早期层（2-4层）爆炸性增长至临界不稳定状态，否定触发词导致灾难性方差水平；尽管模型规模增大，幻觉率仍保持高位（78.3%-83.7%），深层中符号语义触发词的注意力急剧下降。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只在实体链接的孤立阶段使用大语言模型，未能充分利用其在整个流程中的潜力，且单独消歧实体无法达到最优性能。

Method: 提出DeepEL框架，将LLMs集成到实体链接的每个阶段，并引入自验证机制，利用全局上下文信息让LLMs能够纠正自身预测并识别同一句子中实体间的连贯关系。

Result: 在10个基准数据集上的评估显示，DeepEL显著优于现有最先进方法，整体F1分数平均提升2.6%，在领域外数据集上提升达4%。
...
### [114] [ArbESC+: Arabic Enhanced Edit Selection System Combination for Grammatical Error Correction Resolving conflict and improving system combination in Arabic GEC](https://arxiv.org/abs/2511.14230)
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语具有复杂的形态和句法结构，比其他语言更具挑战性。虽然现代神经模型已有很大改进，但大多数先前尝试使用单一模型，没有考虑组合不同系统的潜在优势。

Method: 提出ArbESC+系统，使用多个模型（AraT5、ByT5、mT5、AraBART等）收集修正建议，将其表示为数值特征，通过分类器确定并实施适当修正，并使用支持技术过滤重叠修正和评估决策可靠性。

Result: 在QALB-14测试数据上F0.5达到82.63%，在QALB-15 L1数据上达到84.64%，在QALB-15 L2数据上达到65.55%，优于单一模型。

Conclusion: 这是首个整合语言错误校正的阿拉伯语尝试，改进现有模型为开发先进的阿拉伯语文本处理工具提供了实际步骤，将有益于用户和研究人员。
...
### [115] [MuCPT: Music-related Natural Language Model Continued Pretraining](https://arxiv.org/abs/2511.14245)
*Kai Tian,Yirong Mao,Wendong Bi,Hanjie Wang,Que Wenhui*

Main category: cs.CL

TL;DR: 本文针对大语言模型在音乐领域专业化应用受限的问题，提出了一个包含40B标记的音乐相关自然语言语料库，采用领域优先的数据管道和多源音乐文本整合方法，并引入基于参考模型的标记级软评分进行质量控制，最终开发了MusicSimpleQA基准进行事实性评估。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现优异，但在音乐等专业领域仍受限制，主要挑战在于语料库规模、纯净度以及数据与训练目标的匹配度。

Method: 构建大规模音乐相关自然语言语料库，采用领域优先数据管道（轻量级分类器过滤加权、多阶段清洗去重、隐私保护掩码），整合多源音乐文本与元数据，引入基于参考模型的标记级软评分进行质量控制和动态优化。

Result: 开发了MusicSimpleQA基准用于事实性评估，通过系统比较数据组成轴线的性能，验证了所提框架的有效性。

Conclusion: 本研究在音乐领域推进了正确的语料库和训练目标，提供了可扩展的数据训练框架和可重用的评估工具，为构建领域专用大语言模型奠定了基础。
...
### [116] [Towards Authentic Movie Dubbing with Retrieve-Augmented Director-Actor Interaction Learning](https://arxiv.org/abs/2511.14249)
*Rui Liu,Yuan Zhao,Zhenqi Jia*

Main category: cs.CL

TL;DR: 提出Authentic-Dubber模型，通过检索增强的导演-演员交互学习方案实现真实电影配音，包含多模态参考素材库、情感相似性检索增强和渐进式图基语音生成三个新机制。


<details>
  <summary>Details</summary>
Motivation: 现有方法模拟简化的配音流程，演员直接配音而无需准备，忽略了关键的导演-演员交互。真实流程中导演会指导演员内化情感等上下文线索。

Method: 构建多模态参考素材库模拟导演提供的学习素材；提出情感相似性检索增强策略；开发渐进式图基语音生成方法逐步融入检索到的多模态情感知识。

Result: 在V2C Animation基准数据集上的主观和客观评估验证了有效性，在情感表现力方面实现了全面改进。

Conclusion: Authentic-Dubber能够忠实复制真实的配音工作流程，显著提升配音的情感表现力。
...
### [117] [AfriSpeech-MultiBench: A Verticalized Multidomain Multicountry Benchmark Suite for African Accented English ASR](https://arxiv.org/abs/2511.14255)
*Gabrial Zencha Ashungafac,Mardhiyah Sanni,Busayo Awobade,Alex Gichamba,Tobi Olatunji*

Main category: cs.CL

TL;DR: AfriSpeech-MultiBench是首个针对非洲英语口音的领域特定评估套件，覆盖10多个国家的100多种非洲英语口音和7个应用领域，评估了多种语音识别系统在非洲语境下的表现。
...
Result: 开源ASR在自发语音中表现良好但在嘈杂非母语对话中退化；多模态LLM对口音更鲁棒但处理领域特定命名实体困难；专有模型在清晰语音中准确率高但随国家和领域变化显著；非洲英语微调模型在延迟方面具有实用优势。
...
### [118] [Entropy-Guided Reasoning Compression](https://arxiv.org/abs/2511.14258)
*Hourun Zhu,Yang Gao,Wenlong Fei,Jiawei Li,Huashan Sun*

Main category: cs.CL

TL;DR: 本文提出了一种熵引导的训练框架来解决大型推理模型在压缩过程中的熵冲突问题，将推理长度压缩到原始的20%同时保持或提升准确率。
...
### [119] [Don't Miss the Forest for the Trees: In-Depth Confidence Estimation for LLMs via Reasoning over the Answer Space](https://arxiv.org/abs/2511.14275)
*Ante Wang,Weizhi Ma,Yang Liu*

Main category: cs.CL

TL;DR: 本文提出了一种通过预测语言化概率分布来增强LLM置信度估计的方法，该方法要求模型考虑答案空间中的所有候选答案并仔细分配置信度分数，在不同模型和任务中均显示出优势。
...
### [120] [AraLingBench A Human-Annotated Benchmark for Evaluating Arabic Linguistic Capabilities of Large Language Models](https://arxiv.org/abs/2511.14295)
...
TL;DR: AraLingBench是一个完全人工标注的基准测试，用于评估大语言模型的阿拉伯语语言能力，包含语法、形态、拼写、阅读理解和句法五个核心类别，共150个专家设计的多项选择题。


<details>
  <summary>Details</summary>
Motivation: 当前阿拉伯语和双语大语言模型在知识型基准测试中得分很高，但缺乏真正的语言掌握能力，需要通过专门的基准测试来评估其深层语法和句法推理能力。

Method: 开发了包含150个专家设计的多项选择题的基准测试，涵盖五个语言核心类别，评估了35个阿拉伯语和双语大语言模型。

Result: 评估显示当前模型在表面水平上表现出色，但在深层语法和句法推理方面存在困难，许多模型通过记忆或模式识别而非真实理解来获得成功。

Conclusion: AraLingBench揭示了知识型基准测试高分与真正语言掌握之间的差距，为开发阿拉伯语大语言模型提供了诊断框架，评估代码已在GitHub公开。
...
### [121] [ConInstruct: Evaluating Large Language Models on Conflict Detection and Resolution in Instructions](https://arxiv.org/abs/2511.14342)
*Xingwei He,Qianru Zhang,Pengfei Chen,Guanhua Chen,Linlin Yu,Yuan Yuan,Siu-Ming Yiu*

Main category: cs.CL

TL;DR: ConInstruct基准测试评估大语言模型在用户指令包含冲突约束时的检测和解决能力，发现专有模型表现较好但很少明确通知用户冲突。
...
Result: 专有LLMs具有强冲突检测能力，DeepSeek-R1和Claude-4.5-Sonnet分别获得91.5%和87.3%的平均F1分数；但LLMs很少明确通知用户冲突或请求澄清。
...
### [122] [ATLAS: A High-Difficulty, Multidisciplinary Benchmark for Frontier Scientific Reasoning](https://arxiv.org/abs/2511.14366)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试存在性能饱和、学科范围狭窄、答案格式简化、数据污染等问题，无法有效评估模型在真实科学探究中的能力，需要开发更可靠的评估工具。

Method: 由领域专家开发原创问题，采用多阶段专家同行评审和对抗性测试进行质量控制，使用LLM评委小组进行自动化复杂答案评估。

Result: 初步结果显示ATLAS能有效区分领先模型的先进科学推理能力。

Conclusion: ATLAS计划发展成为长期、开放、社区驱动的平台，为人工通用智能的进展提供可靠评估标准。
...
### [123] [Mitigating Label Length Bias in Large Language Models](https://arxiv.org/abs/2511.14385)
*Mario Sanz-Guerrero,Katharina von der Wense*

Main category: cs.CL

TL;DR: 论文提出标准化上下文校准(NCC)方法，解决大语言模型中多标记类别标签导致的标签长度偏差问题，在多个数据集和模型上显著提升性能。
...
Result: NCC在多个数据集和模型上取得统计显著改进，F1分数提升高达10%，结合上下文学习时对少样本示例选择更不敏感，需要更少示例获得竞争性表现，产生更可靠的置信度估计。

Conclusion: 缓解完整标签偏差对于提高基于大语言模型方法的性能和鲁棒性至关重要，特别是在类别标签自然包含多个标记的实际应用中。
...
### [124] [Unified Defense for Large Language Models against Jailbreak and Fine-Tuning Attacks in Education](https://arxiv.org/abs/2511.14423)
*Xin Yi,Yue Li,Dongsheng Shi,Linlin Wang,Xiaoling Wang,Liang He*

Main category: cs.CL

TL;DR: 本文提出了EduHarm基准和TSSF框架，用于评估和防御教育场景中LLM的安全漏洞，包括越狱和微调攻击。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注通用安全评估，对教育场景的独特安全需求关注不足。LLM在教育应用中容易受到越狱和微调攻击，导致安全对齐失效和有害输出。

Method: 构建EduHarm基准，包含五个教育场景的安全-不安全指令对；提出三阶段防护框架(TSSF)：安全感知注意力重对齐、分层安全判断、防御驱动双路由。

Result: 在八种越狱攻击策略下，TSSF有效增强安全性且避免对良性查询的过度拒绝；在三个微调攻击数据集上，保持对有害查询的鲁棒防御同时维持良性微调的效用增益。

Conclusion: TSSF框架能够同时缓解越狱和微调攻击，为教育LLM提供有效的安全防护，在保持安全性的同时不影响正常教育功能。
...
### [125] [MedBench v4: A Robust and Scalable Benchmark for Evaluating Chinese Medical Language Models, Multimodal Models, and Intelligent Agents](https://arxiv.org/abs/2511.14439)
...
TL;DR: MedBench v4是一个全国性的云基准测试平台，包含70多万个专家策划的医疗任务，评估了15个前沿模型在LLM、多模态和智能体方面的表现。
...
Method: 构建包含24个主要专科和91个次要专科的70多万个任务基准，通过多阶段精炼和多轮临床医生评审，使用LLM作为评分员校准人类评分。
...
Conclusion: MedBench v4揭示了基础模型在多模态推理和安全方面存在持续差距，但治理意识的智能体编排可以显著提升临床准备度，为医院、开发者和政策制定者提供实用参考。
...
### [126] [Tell Me: An LLM-powered Mental Well-being Assistant with RAG, Synthetic Dialogue Generation, and Agentic Planning](https://arxiv.org/abs/2511.14445)
...
<details>
  <summary>Details</summary>
Motivation: 解决心理健康资源获取障碍，利用AI技术提供可访问的心理支持，同时解决治疗数据稀缺问题，促进NLP研究者与心理健康专业人士的跨学科合作。

Method: 系统集成三个组件：基于检索增强生成的个性化对话助手、基于用户档案的合成治疗对话生成器、以及使用CrewAI实现的AI健康规划器生成周度自我护理计划和冥想音频。

Result: 在精心设计的健康场景中评估RAG助手，使用自动LLM判断和人类用户研究，展示了对话助手如何降低支持门槛、补充现有护理并扩大心理健康资源获取。

Conclusion: 这项工作强调了NLP研究者与心理健康专业人士跨学科合作的机会，推动人机交互在健康领域的负责任创新，展示了AI系统如何作为情感处理空间而非治疗替代品。
...
### [127] [Agent-R1: Training Powerful LLM Agents with End-to-End Reinforcement Learning](https://arxiv.org/abs/2511.14460)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在复杂问题解决中应用强化学习仍处于早期阶段，缺乏专门针对LLM智能体的RL方法探索和灵活可扩展的训练框架。

Method: 1. 系统化扩展马尔可夫决策过程框架来定义LLM智能体的关键组件；2. 提出Agent-R1模块化训练框架，支持跨任务场景和环境适配。

Result: 在多跳问答基准任务上的实验为所提方法和框架的有效性提供了初步验证。

Conclusion: 提出的方法和框架有助于推进RL在LLM智能体训练中的应用，为这一新兴领域提供了系统化解决方案。
...
### [128] [LiveRAG: A diverse Q&A dataset with varying difficulty level for RAG evaluation](https://arxiv.org/abs/2511.14531)
...
Method: 从SIGIR'2025 LiveRAG挑战赛中提取基准，并补充了挑战期间未公开的真实答案、支持证据，应用项目反应理论模型计算难度和区分度分数。

Result: 分析显示基准问题具有多样性、广泛的难度范围，能有效区分系统能力差异。
...
### [129] [Examining the Metrics for Document-Level Claim Extraction in Czech and Slovak](https://arxiv.org/abs/2511.14566)
*Lucia Makaiová,Martin Fajčík,Antonín Jarolím*

Main category: cs.CL

TL;DR: 本文研究文档级声明提取的评估方法，通过对齐两组声明并计算相似度分数来评估模型提取性能，在捷克和斯洛伐克新闻评论数据集上进行实验。
...
Method: 探索声明集对齐方法，通过识别最佳对齐和评估方法来计算相似度分数，作为模型提取性能评估指标和标注者间一致性度量。
...
### [130] [A Method for Characterizing Disease Progression from Acute Kidney Injury to Chronic Kidney Disease](https://arxiv.org/abs/2511.14603)
*Yilu Fang,Jordan G. Nestor,Casey N. Ta,Jerard Z. Kneifati-Hayek,Chunhua Weng*

Main category: cs.CL

TL;DR: 使用电子健康记录数据动态追踪急性肾损伤患者的临床演变，识别15种不同的AKI后临床状态，发现17%的患者发展为慢性肾病，建立了基于多状态模型的风险预测方法


<details>
  <summary>Details</summary>
Motivation: 急性肾损伤患者有高风险发展为慢性肾病，但识别最高风险患者仍然具有挑战性，需要开发数据驱动的方法来支持早期检测和干预

Method: 使用电子健康记录数据，通过纵向医疗代码和肌酐测量值聚类识别AKI后临床状态，采用多状态模型估计状态间转移概率和CKD进展风险，通过生存分析识别不同亚群中的CKD风险因素

Result: 在20,699名AKI患者中，3,491人（17%）发展为CKD；识别出15种不同的AKI后状态，每种状态具有不同的CKD发展概率；75%的患者在研究期间保持单一状态或仅进行一次状态转换；发现了传统和新型CKD风险因素在不同临床状态中的不同影响
...
### [131] [Bridging Human and Model Perspectives: A Comparative Analysis of Political Bias Detection in News Media Using Large Language Models](https://arxiv.org/abs/2511.14606)
*Shreya Adrita Banik,Niaz Nafi Rahman,Tahsina Moiukh,Farig Sadeque*

Main category: cs.CL

TL;DR: 本文比较了人类标注与多种大语言模型在政治偏见检测上的表现，发现RoBERTa与人类标注最一致，GPT在零样本设置下表现最佳，强调了需要结合人类可解释性和模型可扩展性的混合评估框架。


<details>
  <summary>Details</summary>
Motivation: 尽管自然语言处理技术已能自动分类政治偏见，但大语言模型与人类判断的一致性程度仍未被充分探索和理解，因此需要建立一个比较框架来评估这种一致性。

Method: 构建手动标注的新闻文章数据集，评估标注一致性、偏见极性和模型间一致性，量化人类与模型在偏见感知上的差异，比较GPT、BERT、RoBERTa和FLAN等模型的表现。

Result: 在传统基于Transformer的模型中，RoBERTa与人类标签的一致性最高；在零样本设置下，GPT等生成模型与人类标注的总体一致性最强；微调后的RoBERTa模型在所有基线中获得了最高准确率和最强的人类标注对齐度。

Conclusion: 人类和LLMs在感知政治倾向时存在系统性差异，强调在自动化媒体偏见检测中需要结合人类可解释性和模型可扩展性的混合评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 提升自主科学发现系统的端到端性能，通过视觉语言模型作为评判者来实时评估和纠正智能体的错误推理路径。

Method: 使用视觉语言模型作为评判者，基于动态生成的领域特定评分标准评估图表，使智能体能够实时纠正错误并指导探索性数据分析。

Result: 在10个数据驱动发现任务的基准测试中，VLM增强系统达到0.7-0.8的pass@1分数，显著优于代码基线(0.2-0.3)和代码加文本基线(0.4-0.5)。

Conclusion: 视觉语言模型引导的多智能体系统能够有效提高自主科学发现的性能，提供可审计的推理轨迹，并在宇宙学和天体化学案例中展示了从错误推理路径恢复和适应新数据集的能力。
...
TL;DR: RareSeek R1是一个针对罕见病诊断的AI系统，通过领域专业临床语料库和分阶段指令调优开发，在多中心电子健康记录和公共基准测试中达到最先进准确率，性能与经验丰富的医生相当。


<details>
  <summary>Details</summary>
Motivation: 罕见病影响全球数亿人，但诊断通常需要数年时间。传统诊断流程存在证据提取与推理诊断脱节的问题，而通用/医学大语言模型面临真实世界电子健康记录稀缺、领域知识陈旧和幻觉问题。

Method: 构建大型领域专业临床语料库和临床医生验证的推理集，通过分阶段指令调优、思维链学习和图基础检索开发RareSeek R1系统。

Result: 在多中心电子健康记录叙述和公共基准测试中达到最先进准确率，具有稳健的泛化能力和噪声或重叠表型下的稳定性。增强检索在叙述与优先变异配对时效果最佳，通过解决歧义和对齐候选机制获得最大收益。

Conclusion: 这项工作推进了以叙述为先、知识整合的推理范式，缩短诊断历程，并提供可审计、临床可转化的决策支持，透明推理突出了支撑正确诊断的决定性非表型证据（如影像学、干预措施、功能测试）。
...
TL;DR: 本研究通过将统计语言模型与人类行为数据相结合，开发了一个定量模型来预测比较性幻觉的强度，支持了句子理解中的噪声通道理论。


<details>
  <summary>Details</summary>
Motivation: 研究比较性幻觉现象，验证句子理解中的噪声通道理论，解释为什么人们会系统性地误解某些句子。

Method: 结合统计语言模型和人类行为数据，构建后验概率定量模型，直接预测幻觉强度，并分析代词与完整名词短语的影响。

Result: 模型不仅解释了比较性幻觉效应的精细梯度，还解释了之前未解释的代词与完整名词短语主语的影响效应。

Conclusion: 研究结果支持噪声通道理论作为语言处理现象的统一计算层面理论，证实该理论能够对比较性幻觉做出新颖预测并获得实证支持。
