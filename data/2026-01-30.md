...
TL;DR: 该论文针对对话式图像生成中的非马尔可夫问题，提出数据构建、训练框架和优化方法，显著提升多轮对话中图像生成的一致性和指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在对话式图像生成中存在局限性，大多数多轮基准和训练方法本质上是马尔可夫的，即下一输出主要依赖最近图像，导致模型忽略长程历史。用户可能引用早期状态、撤销更改或引用多轮前引入的实体，需要解决非马尔可夫设置下的挑战。

Method: 提出三种方法：(1) 非马尔可夫多轮数据构建策略，包括回滚式编辑和基于名称的多轮个性化；(2) 历史条件训练和推理框架，使用令牌级缓存防止多轮身份漂移；(3) 高保真图像重建和可编辑个性化改进，包括基于重建的DiT解令牌器和多阶段微调课程。

Result: 实验表明，明确针对非马尔可夫交互进行训练能显著提升多轮一致性和指令遵循能力，同时保持强大的单轮编辑和个性化性能。

Conclusion: 该工作形式化并解决了对话式图像生成中的非马尔可夫挑战，通过系统性的数据、训练和优化方法，实现了在多轮交互中更好地保持视觉一致性和遵循复杂指令的能力。
...
<details>
  <summary>Details</summary>
Motivation: PET图像在医学诊断中至关重要，但常受到复杂噪声干扰，影响诊断信息。图像质量受多种因素影响，包括扫描仪硬件、图像重建、示踪剂特性、剂量/计数水平和采集时间等。

Method: 提出文本引导的去噪方法，利用预训练CLIP模型提取特征，结合U-Net架构的去噪模型，能够在单一模型中增强不同计数水平的PET图像。

Result: 实验结果表明，所提模型在定性和定量评估方面均有显著改进，模型灵活性显示出处理更复杂去噪需求或减少采集时间的潜力。

Conclusion: 该方法为PET图像去噪提供了有效的文本引导解决方案，具有处理多种噪声水平和临床应用的潜力。
...
<details>
  <summary>Details</summary>
Motivation: CT探测器中的低性能像素会导致环形和条纹伪影，现有监督深度学习方法需要昂贵的专用数据集，且仅关注图像域或投影域，忽略了CT几何前向操作的内在相关性

Method: 提出基于合成数据的展开双域方法，利用自然图像生成合成数据，通过CT几何前向操作的内在相关性，在投影域和图像域之间建立联系进行伪影校正

Result: 在模拟1-2%探测器缺陷的实验条件下，该方法大幅优于现有最先进方法，能够有效校正低性能像素伪影

Conclusion: 该方法无需收集真实临床数据即可校正低性能像素伪影，且能适应不同扫描仪设置的软件应用，具有实用价值
...
TL;DR: 基于前列腺活检切片训练的AI模型能预测根治性前列腺切除术后生化复发风险，在外部验证中表现良好，结合临床变量可改善预后分层，但需审慎评估其相对于简单模型的增量价值。
...
Result: 图像方法在三个外部队列中分别获得0.64、0.70和0.70的5年时间依赖性AUC；整合临床变量增加了补充预后价值，实现了统计学显著的风险分层；相比指南推荐的CAPRA-S，AI模型改善了术后预后预测。

Conclusion: 基于活检训练的组织病理学AI模型能够跨标本类型泛化，支持术前和术后决策，但AI多模态方法相对于简单预测模型的增量价值需要在进一步研究中审慎评估。
...
TL;DR: BadDet+是一个针对目标检测的惩罚框架，统一了区域误分类攻击和物体消失攻击，具有位置尺度不变性和物理鲁棒性。
...
Method: 引入BadDet+惩罚框架，使用对数障碍惩罚来抑制触发输入的真实类别预测，统一了区域误分类攻击和物体消失攻击。该方法具有位置尺度不变性和增强的物理鲁棒性。

Result: 在真实世界基准测试中，BadDet+相比现有的RMA和ODA基线实现了更好的合成到物理迁移性能，同时保持了干净样本的性能。理论分析证实惩罚在触发特定特征子空间中起作用。

Conclusion: 目标检测存在显著漏洞，需要专门的防御机制。BadDet+框架揭示了这些漏洞，并为后门攻击研究提供了统一方法。
...
TL;DR: ActionVLM：一种缓解时序动作定位中模态偏见的视觉语言聚合框架，通过去偏重加权模块和残差聚合策略，保持视觉主导信号，仅在有益时利用语言信息，在THUMOS14上优于SOTA方法3.2% mAP。


<details>
  <summary>Details</summary>
Motivation: 现有时序动作定位方法在使用视觉语言模型时过度强调语言先验，导致明显的模态偏见，损害了视觉性能。需要一种能系统缓解这种偏见的方法。

Method: 提出ActionVLM框架：1）去偏重加权模块，估计语言相对于纯视觉预测的优势，并动态调整语言模态权重；2）残差聚合策略，将语言视为补充性细化而非主要驱动因素。

Result: 在THUMOS14数据集上，ActionVLM比现有最优方法提升了高达3.2%的平均精度（mAP），有效缓解了模态偏见，减少了语言先验的过度自信，增强了时序推理能力。

Conclusion: ActionVLM通过保持视觉主导信号并自适应利用语言信息，成功缓解了时序动作定位中的模态偏见问题，实现了更好的性能平衡。
...
TL;DR: Shape-of-Thought (SoT) 是一个视觉思维链框架，通过渐进式形状组装解决文本到图像生成中的组合结构约束问题，无需外部引擎，在组件计数和结构拓扑方面显著优于纯文本基线。
...
Method: 提出SoT视觉思维链框架，通过连贯的2D投影实现渐进式形状组装，无需推理时的外部引擎。训练统一的多模态自回归模型生成交错的文本计划和渲染的中间状态，帮助模型捕捉形状组装逻辑而不产生显式几何表示。

Result: 在SoT-26K数据集上微调后，在组件计数方面达到88.4%，在结构拓扑方面达到84.8%，比纯文本基线提高了约20%。建立了透明、过程监督的组合生成新范式。
...
<details>
  <summary>Details</summary>
Motivation: 微血管吻合术是显微外科的基本技能，传统评估依赖专家主观判断，存在评分者间差异大、标准不统一、认知偏见、耗时等问题，迫切需要客观、可靠、自动化的评估系统。

Method: 提出四模块AI框架：1)基于YOLO的器械检测模块；2)基于DeepSORT的器械追踪模块；3)基于形状描述符的器械尖端定位模块；4)基于专家标注数据的监督分类模块评估操作熟练度。

Result: 实验结果显示框架有效，器械检测精度达到97%，平均精度(mAP)为96%（IoU阈值50%-95%）。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像具有地物分布密集、边界复杂的特点，对几何定位和语义预测要求更高。现有的无需训练开放词汇语义分割方法通常采用"单向注入"和"浅层后处理"策略，难以满足这些要求。

Method: 1. 跨模型注意力融合模块：通过相互注入自注意力图指导协作推理；2. 双向交叉图扩散优化模块：通过迭代随机游走扩散增强双分支分割分数的可靠性；3. 凸优化超像素协作预测机制：结合低级超像素结构进一步优化对象边界。

Result: 在多个遥感语义分割基准测试中，该方法优于现有方法。消融研究进一步证实，利用超像素结构的传统基于对象的遥感图像分析方法在深度学习框架中仍然有效。

Conclusion: 提出的SDCI框架通过空间正则化感知的双分支协作推理，有效解决了高分辨率遥感图像开放词汇语义分割中的几何定位和语义预测挑战，展示了传统超像素结构在深度学习中的持续价值。
...
TL;DR: GeoDiff-LF是一个基于扩散模型的框架，用于增强水下4-D光场成像质量，通过结合几何线索和扩散先验来减少水下场景的颜色失真。


<details>
  <summary>Details</summary>
Motivation: 水下成像面临颜色失真等挑战，传统方法难以获取高质量的水下图像。4-D光场成像具有空间-角度结构信息，但如何有效利用这些几何线索来提升水下成像质量是一个未充分探索的问题。
...
Result: 大量实验表明，GeoDiff-LF在视觉保真度和定量性能方面均优于现有方法，显著提升了水下成像的质量，特别是在减少颜色失真方面表现出色。

Conclusion: GeoDiff-LF通过整合扩散先验和光场几何信息，有效提升了水下4-D光场成像的质量，推动了水下成像技术的发展。代码将公开提供以促进进一步研究。
...
TL;DR: GRDR提出了一种新的生成式检索方法，通过多语义ID分配和联合训练提升召回质量，在保持准确性的同时大幅降低存储和计算成本


<details>
  <summary>Details</summary>
Motivation: 传统两阶段视频检索中，召回模型性能受限。生成式检索虽然具有恒定复杂度的优势，但存在语义模糊和跨模态对齐问题，需要改进以提升召回质量

Method: 提出GRDR框架：1) 使用查询引导的多视角分词器为每个视频分配多个语义ID；2) 通过共享码本联合训练分词器和生成式检索器；3) 推理时使用trie约束解码生成紧凑候选集，再由密集模型重排序

Result: 在TVR基准测试中，GRDR在准确性上与强密集检索器相当，同时将索引存储降低一个数量级，全库检索速度提升高达300倍

Conclusion: GRDR通过解决生成式检索的语义模糊和跨模态对齐问题，实现了高效高质量的视频检索，为大规模实时应用提供了实用解决方案
...
Method: 1. 构建大规模机器人感知和推理数据集，包含自我视角视频、视觉定位、空间理解和思维链数据；2. 提出简单有效的联合输入方法，同时使用关键帧和完整视频序列来增强模型视频理解能力

Result: 在任务规划领域两个最常用的基准数据集上取得了最先进的结果

Conclusion: Thinker模型通过专门的数据集设计和输入策略，有效解决了视觉语言模型在机器人应用中的关键问题，为具身智能提供了更好的基础模型
...
TL;DR: LAMP是一种针对多模态大语言模型的黑盒攻击方法，通过学习通用对抗扰动，通过注意力约束和跨图像传染约束破坏模型的多图像信息聚合能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多图像处理方面取得了显著进展，但其安全漏洞尚未被充分探索。现有对抗攻击主要针对单图像场景且多为白盒攻击，这在现实场景中不实用。需要研究针对多图像MLLM的黑盒攻击方法。

Method: LAMP方法包含三个核心组件：1) 基于注意力的约束，防止模型有效聚合跨图像信息；2) 跨图像传染约束，使扰动token影响干净token，无需修改所有输入；3) 索引注意力抑制损失，实现鲁棒的位置不变攻击。

Result: 实验结果表明，LAMP在多个视觉语言任务和模型上超越了现有最先进的基线方法，取得了最高的攻击成功率。

Conclusion: LAMP成功展示了多模态大语言模型在多图像场景下的安全漏洞，为黑盒对抗攻击提供了有效的解决方案，强调了多图像MLLM安全性的重要性。
...
TL;DR: PTQ4ARVG是一个针对自回归视觉生成模型的训练后量化框架，解决了通道级异常值、令牌级动态激活和样本级分布不匹配三大挑战，实现了8位和6位量化。


<details>
  <summary>Details</summary>
Motivation: 自回归视觉生成模型虽然与语言模型架构兼容且性能接近扩散模型，但量化技术在该领域的应用仍未被充分探索。现有量化方法无法有效推广到ARVG模型，存在通道级异常值、令牌级动态激活和样本级分布不匹配三大挑战。

Method: 提出PTQ4ARVG框架，包含三个核心组件：1) 增益投影缩放通过泰勒展开量化激活-权重量化的缩放增益，推导最优缩放因子；2) 静态令牌级量化利用ARVG的固定令牌长度和位置不变分布特性；3) 分布引导校准选择对分布熵贡献最大的样本。

Result: 实验表明PTQ4ARVG能有效将ARVG系列模型量化为8位和6位，同时保持有竞争力的性能。

Conclusion: PTQ4ARVG解决了ARVG模型量化的关键挑战，为自回归视觉生成模型的部署提供了有效的训练后量化解决方案。
...
TL;DR: NFCDS通过控制反向扩散噪声的频率来改善保真度-感知权衡，低通噪声导致模糊而高通噪声促进细节生成，使用傅里叶域滤波器抑制低频噪声保留高频内容


<details>
  <summary>Details</summary>
Motivation: 基于扩散采样的Plug-and-Play方法虽然能产生高感知质量的图像，但常常牺牲数据保真度，主要原因是反向扩散过程中引入的噪声。需要解决这种保真度与感知质量之间的权衡问题。

Method: 提出噪声频率控制扩散采样（NFCDS），这是一种反向扩散噪声的频谱调制机制。基于噪声频率分析：低频分量导致模糊和保真度下降，高频分量驱动细节生成。设计傅里叶域滤波器，逐步抑制低频噪声并保留高频内容，将数据一致性先验直接注入采样过程。

Result: NFCDS能够快速收敛到既高保真又感知可信的结果，无需额外训练。作为PnP模块，它可以无缝集成到现有的基于扩散的修复框架中，并在多种零样本任务中改善保真度-感知平衡。

Conclusion: 通过控制反向扩散噪声的频率特性，NFCDS有效解决了扩散采样中保真度与感知质量之间的冲突，为图像修复任务提供了一种无需训练的高效解决方案。
...
TL;DR: Hypersolid方法将表示学习重新解释为离散填充问题，使用短程硬球排斥防止局部碰撞，在细粒度和低分辨率分类任务上表现出色


<details>
  <summary>Details</summary>
Motivation: 解决自监督学习中表示崩溃的挑战，现有方法依赖全局正则化，本文提出从离散填充问题角度重新理解表示学习

Method: 将表示学习重新解释为离散填充问题，通过保持单射性来保留信息，使用短程硬球排斥防止局部碰撞，创建高分离几何机制

Result: Hypersolid方法能够保持增强多样性，在细粒度和低分辨率分类任务上表现优异

Conclusion: 通过离散填充视角重新思考表示学习，使用局部硬球排斥约束可以防止表示崩溃，在特定任务上优于全局正则化方法
...
<details>
  <summary>Details</summary>
Motivation: 现有2D视频压缩技术无法保留细粒度几何和外观细节，而NeRF等隐式神经渲染方法计算成本过高，难以满足实时3D视频会议对高保真、低码率的需求

Method: 集成FLAME参数化建模与3DGS神经渲染，仅传输关键面部元数据；提出紧凑表示和压缩方案，包括高斯属性压缩和MLP优化，提升传输效率

Result: 实验结果显示该方法在极低码率下实现优异的率失真性能，提供高质量面部渲染，适合实时3D视频会议应用
...
TL;DR: 本文提出了首个地理定位推理链基准测试，发现虽然大型视觉语言模型在位置预测上接近人类专家水平，但在生成可审计推理链方面仍远落后于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在地理定位预测上表现优秀，但解释其预测依据的能力很差，经常产生幻觉性的场景属性来支持位置预测。缺乏评估模型地理定位推理能力的基准。

Method: 创建首个地理定位推理链基准，基于GeoGuessr游戏中的Google街景图像。与专家玩家（包括世界冠军）合作，为500个查询场景生成800个真实推理链，涵盖数百个区分性视觉属性。评估LLM-as-a-judge和VLM-as-a-judge策略来评分模型生成的推理链。

Result: Qwen 3 LLM-as-a-judge与人类评分相关性最好。大型闭源VLM（如Gemini和GPT 5）在位置预测上接近人类专家，但在生成可审计推理链方面仍落后。开源VLM（如Llama和Qwen）在基准测试中表现灾难性失败，仅略优于基于位置信息但无视觉输入的幻觉推理链基线。

Conclusion: 人类专家与VLM在此任务上的差距表明VLM在从高分辨率图像中提取细粒度视觉属性方面存在局限性。该基准揭示了VLM在可解释地理定位推理方面的不足。
...
TL;DR: WorldBench是一个新的视频基准测试，专门用于解耦的、概念特定的物理理解评估，能够隔离测试单个物理概念或定律，为世界模型提供更精确的诊断能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于物理的视频基准测试存在纠缠问题，同时评估多个物理定律和概念，限制了其诊断能力。需要一种能够隔离评估单个物理概念的方法，以更精确地评估生成式基础模型（世界模型）的物理保真度。

Method: 设计了WorldBench基准测试，包含两个层次：1）直观物理理解评估（如物体恒存性、尺度/透视）；2）低层物理常数和材料属性评估（如摩擦系数、流体粘度）。通过概念特定的解耦评估，能够隔离测试单个物理概念。
...
TL;DR: 提出Mam-App模型，一种基于Mamba架构的参数高效苹果叶病害分类模型，在保持高精度的同时仅需0.051M参数，适合部署在资源受限设备上。


<details>
  <summary>Details</summary>
Motivation: 全球人口增长和技术进步加剧了粮食生产需求，苹果作为重要水果，病害导致大量产量损失。现有深度学习模型参数过多，训练和推理时间长，而轻量级模型性能又不足，需要在效率和性能之间取得平衡。

Method: 提出Mam-App模型，基于Mamba架构设计参数高效的叶病害分类模型。使用PlantVillage苹果叶病害数据集进行训练和评估，并在玉米和马铃薯叶病害数据集上进行泛化性测试。

Result: 在苹果叶病害数据集上达到99.58%准确率、99.30%精确率、99.14%召回率和99.22% F1分数，仅使用0.051M参数。在玉米数据集上达到99.48%准确率，在土豆数据集上达到98.46%准确率，证明了模型的鲁棒性和泛化能力。

Conclusion: Mam-App模型在保持高精度的同时大幅减少参数数量，解决了效率与性能的权衡问题，适合部署在无人机、移动设备等资源受限平台上，为农业病害检测提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有将3D网格token化为1D序列的自回归方法存在资源利用不足的问题，导致推理速度慢且只能处理小规模序列，严重限制了可表达的结构细节。
...
Conclusion: LANE为高质量3D网格生成提供了有效解决方案，通过紧凑自回归依赖和高效推理策略克服了传统方法的局限性。
...
TL;DR: 视觉基础模型在离散采样的病理图像上训练后，其表征能隐式编码疾病的连续进展过程，轨迹保真度可作为表征质量的补充指标。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究视觉基础模型是否能在表征空间中编码连续的疾病进展过程。在计算病理学中，能够隐式捕获疾病连续进展的模型可能更好地反映生物学基础，支持更稳健的泛化，并实现对疾病转变相关特征的定量分析。

Method: 使用扩散伪时间（一种从单细胞转录组学推断发育轨迹的方法）来探测基础模型是否在表征空间中沿着连贯的进展方向组织疾病状态。研究分析了四种癌症进展和六种模型，评估模型恢复轨迹排序的能力。

Result: 所有病理特异性模型恢复的轨迹排序都显著超过零基线，其中纯视觉模型达到最高的保真度（CRC-Serrated上τ>0.78）。模型在参考疾病上的轨迹保真度排名强烈预测了在保留疾病上的少样本分类性能（ρ=0.92）。探索性分析显示细胞类型组成沿着推断的轨迹平滑变化，模式与已知的基质重塑一致。

Conclusion: 视觉基础模型可以从独立的静态观察中隐式学习表示连续过程，轨迹保真度提供了超越下游性能的表征质量补充度量。虽然研究在病理学领域进行，但该框架可应用于其他通过静态快照观察连续过程的领域。
...
Result: 在多个类增量学习基准测试上的实验表明，DAF达到了最先进的性能水平。

Conclusion: DAF通过动态融合机制构建单一鲁棒全局适配器，有效解决了类增量学习中的知识迁移和灾难性遗忘问题，在保持高性能的同时降低了参数维护成本。
...
TL;DR: SGDS通过语义引导的动态稀疏化，在激活空间中塑造类特定稀疏子空间，有效缓解类增量学习中的干扰问题，无需对参数空间施加刚性约束。


<details>
  <summary>Details</summary>
Motivation: 现有类增量学习方法通常冻结预训练模型并使用轻量级适配器，并通过正交约束参数来防止任务间干扰。但作者认为这种参数约束方法会损害模型的可塑性。

Method: 提出语义引导的动态稀疏化(SGDS)，通过定向稀疏化主动引导激活空间，控制其子空间的方向和秩。鼓励相似类共享紧凑的激活子空间以促进知识迁移，同时为不相似类分配非重叠的激活子空间以防止干扰。

Result: 在多个基准数据集上的广泛实验表明，SGDS取得了最先进的性能表现。

Conclusion: 通过在激活空间中塑造类特定稀疏子空间，SGDS能够有效缓解干扰问题，而无需对参数空间施加刚性约束，从而在保持可塑性的同时实现更好的类增量学习性能。
...
TL;DR: 本文提出GIQT方法解决无人机-地面行人重识别中的几何畸变问题，通过几何诱导的查询-键变换修正相似度空间，提升极端视角下的匹配性能。


<details>
  <summary>Details</summary>
Motivation: 无人机-地面行人重识别面临极端视角和距离差异导致的严重几何畸变，现有方法假设注意力机制中的点积相似度在极端视角下仍然可靠，但这一假设并不成立。几何畸变会系统性地扭曲查询-键相似度空间，降低基于注意力的匹配性能。

Method: 提出几何诱导查询-键变换(GIQT)，这是一个轻量级低秩模块，通过将查询-键交互条件化于相机几何来显式修正相似度空间。不修改特征表示或注意力公式本身，而是调整相似度计算以补偿主导的几何诱导各向异性畸变。在此基础上，进一步结合几何条件提示生成机制，提供直接从相机几何导出的全局、视图自适应表示先验。

Result: 在四个无人机-地面行人重识别基准测试上的实验表明，该框架在极端和未见过的几何条件下持续提升鲁棒性，同时相比最先进方法引入最小的计算开销。

Conclusion: 通过显式修正相似度空间而非仅仅调整特征表示，GIQT方法有效解决了无人机-地面行人重识别中的几何畸变问题，在极端视角变化下实现了更鲁棒的匹配性能。
...
TL;DR: UniMRG提出了一种通过多表征生成任务来增强统一多模态模型理解能力的后训练方法，显著提升了模型的细粒度感知、空间理解和生成能力。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型旨在实现理解与生成的相互促进循环，但现有方法主要利用理解来增强生成，而利用生成来提升理解的研究仍然不足。

Method: UniMRG是一种架构无关的后训练方法，通过训练模型生成图像的多种内在表征（像素重建、深度几何、分割结构），结合标准的视觉理解目标，使模型能够捕捉外观、空间关系和结构布局的互补信息。

Result: 在多种UMM架构上的广泛实验表明，该方法显著增强了细粒度感知、减少了幻觉、改善了空间理解，同时提升了生成能力。
...
TL;DR: 提出一种检测AI生成视频的双路径框架，通过静态流形偏差分支检测空间异常，通过微时态波动分支分析连续帧间的结构化投影波动来识别高保真伪造视频。


<details>
  <summary>Details</summary>
Motivation: 尽管当前视频生成模型（如Veo和Wan）的视觉质量已经很高，宏观语义错误和时间不一致性不再明显，但AI生成视频本质上是通过流形拟合过程产生的，而非物理录制。这种差异导致AI视频中连续相邻帧的像素组合逻辑呈现出结构化且同质化的特征，称为"流形投影波动"（MPF）。

Method: 提出分层双路径框架作为顺序过滤过程：1）静态流形偏差分支：利用大规模视觉基础模型（VFMs）的精细感知边界捕捉偏离自然真实世界流形的空间异常或物理违规；2）微时态波动分支：作为二级精细过滤器，分析即使在视觉完美序列中仍然存在的结构化MPF。

Result: 该框架能够确保无论伪造表现为全局真实世界流形偏差还是细微的计算指纹，都能被检测出来。即使高保真视频成功驻留在流形上并逃避空间检测，也能通过分析微时态波动来暴露伪造。

Conclusion: AI生成视频与真实视频之间存在可追踪的差异，特别是通过流形投影波动这一特征。提出的分层双路径框架能够有效检测高保真AI生成视频，为视频真实性验证提供了新方法。
...
TL;DR: 该研究揭示了隐式密度场在重度遮挡场景下的内在几何退化问题，提出基于稀疏体素栅格化的显式几何方法，显著提高了密集自遮挡场景中的实例恢复率。


<details>
  <summary>Details</summary>
Motivation: NeRF在多视图重建中表现出色，但在密集自遮挡场景中用于定量3D分析的可靠性尚不清楚。研究者发现隐式密度场在重度遮挡下存在内在几何退化问题，导致实例计数不足。

Method: 提出基于稀疏体素栅格化的显式几何流程：从SfM特征几何初始化，将2D实例掩码投影到显式体素网格上，通过递归分割强制几何分离，保持物理实体性。

Result: 在密集场景中，最先进的掩码监督NeRF仅能恢复约89%的实例，而提出的SVRaster方法达到95.8%的恢复率。在分割掩码质量下降的情况下，基于SfM的显式几何比隐式基线多恢复43%的实例。

Conclusion: 显式几何先验是高度自遮挡3D场景中进行可靠定量分析的必要条件。隐式密度场在重度遮挡下会重建空心或碎片化结构，而显式几何方法能保持物理实体性，提高实例恢复的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度学习存在预训练与微调之间的根本性差距：预训练已从单模态转向多模态学习，增强了视觉理解能力，但微调主要仍是单模态的，这限制了预训练丰富表示的优势。
...
TL;DR: Spava是一个序列并行框架，通过分布式近似注意力机制和多GPU加速长视频推理，解决了大型多模态模型预填充阶段计算密集的问题，在保持性能的同时实现了显著加速。
...
Method: 提出Spava序列并行框架，通过分布式近似注意力机制在多GPU上加速长视频推理。该方法减少计算并增加并行性，无需压缩即可高效处理更多视觉嵌入。系统级优化包括负载均衡和融合前向传播。

Result: 相比FlashAttn、ZigZagRing和APB，Spava分别实现了12.72倍、1.70倍和1.18倍的加速，且没有明显的性能损失。

Conclusion: Spava通过序列并行和优化注意力机制，有效解决了长视频推理的计算瓶颈，在保持性能的同时实现了显著加速，为处理更长、更复杂的视频内容提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 度量学习对检索任务至关重要，但其对嵌入几何和优化动态的影响尚未被充分理解。研究者希望系统分析不同损失函数如何影响嵌入表示的特性，为实际应用提供指导。

Method: 引入VARIANCE（类内/类间方差）和GREEDINESS（活跃比率和梯度范数）诊断框架，比较了7种代表性损失函数（Contrastive、Triplet、N-pair、InfoNCE、ArcFace、SCL、CCL）在5个图像检索数据集上的表现。

Result: Triplet和SCL能保持更高的类内方差和更清晰的类间边界，在细粒度检索中表现更好；Contrastive和InfoNCE通过大量小更新快速压缩嵌入，加速收敛但可能过度简化类别结构；N-pair实现较大的平均分离但间距不均匀。

Conclusion: 研究揭示了效率与粒度之间的权衡：当需要保持多样性和困难样本区分时，应优先选择Triplet/SCL；当需要更快的嵌入压缩时，应选择Contrastive/InfoNCE。这些发现为度量学习损失函数的选择提供了实用指导。
...
TL;DR: RT-DeepLoc是一种基于重建的弱监督时序深度伪造定位框架，通过仅使用真实数据训练的MAE模型检测伪造片段，结合非对称视频内对比损失实现高性能定位。


<details>
  <summary>Details</summary>
Motivation: 现代深度伪造技术已发展为局部化和间歇性操作，需要细粒度的时间定位。由于帧级标注成本过高，需要依赖视频级标签的弱监督方法。

Method: 提出RT-DeepLoc框架：1) 使用仅基于真实数据训练的掩码自编码器学习时空模式，通过重建误差识别伪造片段；2) 引入非对称视频内对比损失，通过重建线索引导真实特征紧凑性，建立稳定决策边界。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成式AI方法通常将图像生成和编辑分开处理，导致效率低下，难以保持空间一致性和语义连贯性。缺乏对对象关系和空间布局的结构化控制是主要障碍。

Method: 提出SimGraph统一框架，集成基于场景图的图像生成和编辑。框架结合基于token的生成和基于扩散的编辑，在单个场景图驱动模型中实现精确的对象交互、布局和空间一致性控制。
...
<details>
  <summary>Details</summary>
Motivation: 全合一图像恢复面临的主要挑战是如何协调异构退化之间的冲突优化目标。现有方法通常受限于粗粒度控制机制或固定映射调度，导致次优适应能力。

Method: 提出不确定性感知扩散桥模型（UDBM），引入松弛扩散桥公式替代严格的终端约束，建模退化不确定性并理论上解决标准扩散桥中的漂移奇异性。采用双重调制策略：噪声调度将多样退化对齐到共享高熵潜在空间，路径调度基于熵正则化的粘性动力学自适应调节传输轨迹。

Result: UDBM通过有效校正传输几何和动力学，在单一推理步骤内实现了跨多种恢复任务的最先进性能。

Conclusion: 该方法通过将全合一图像恢复重新表述为随机传输问题，并引入不确定性感知机制和双重调制策略，成功解决了异构退化优化冲突问题，为图像恢复提供了新的理论框架和实用解决方案。
...
TL;DR: HydroSense是一个低成本物联网水质监测系统，集成了pH、溶解氧、温度、TDS、氮含量和水位六个关键参数，采用双微控制器架构，相比商业系统成本降低85%，实现了专业级的水质评估。
...
Result: 90天实验验证显示出色性能：pH精度±0.08单位（0-14范围），DO测量稳定性±0.2 mg/L，TDS精度±1.9%（0-1000 ppm），云数据传输可靠性99.8%。总实现成本32,983 BDT（约300美元），相比商业系统成本降低85%。
...
<details>
  <summary>Details</summary>
Motivation: 现有水印评估方法存在显著局限性：缺乏统一框架同时处理残差和语义水印、结果缺乏可解释性、忽视全面的安全考虑、对语义水印使用不恰当的评估指标。

Method: 提出WMVLM框架，重新定义两类水印的质量和安全指标：残差水印评估伪影强度和擦除抵抗能力，语义水印评估潜在分布偏移。采用三阶段训练策略，逐步实现分类、评分和可解释文本生成。

Result: 实验表明WMVLM在多个数据集、扩散模型和水印方法上优于现有最先进的视觉语言模型，展现出强大的泛化能力。

Conclusion: WMVLM是首个统一且可解释的扩散模型图像水印评估框架，通过重新定义评估指标和创新的训练策略，有效解决了现有评估方法的局限性。
...
Method: 1) 构建PathReasoner数据集：利用医学知识图谱，将结构化病理发现和临床推理与诊断对齐，生成超过20K高质量指令样本；2) 开发PathReasoner-R1模型：结合轨迹掩码监督微调和推理导向强化学习；3) 设计知识感知多粒度奖励函数，包含与知识图谱严格对齐的实体奖励机制。

Result: PathReasoner-R1在PathReasoner数据集和公共基准测试中均达到最先进性能，在不同图像尺度上表现优异，为病理模型提供了透明、临床可信的推理能力。

Conclusion: 该研究通过构建首个大规模WSI推理数据集和开发推理强化模型，成功解决了病理诊断中缺乏可验证推理的问题，显著提升了临床信任度和模型透明度，为计算病理学发展提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有文献表明大型模型倾向于收敛到相似的"通用"表示，但本研究想探究模型是否也通过相同的中间步骤和操作达到这些表示

Method: 量化不同模型在不同处理阶段的表示距离，跟踪表示距离在整个处理过程中的演变，识别模型间差异最大的处理步骤

Result: 发现相似位置的层具有最相似的表示，但仍有显著差异；分类器模型会丢弃低层图像统计信息；CNN和Transformer模型行为不同，Transformer的表示变化更平滑

Conclusion: 这些发现阐明了模型表示收敛的层次和性质，使对图像模型底层过程的定性描述成为可能
...
TL;DR: 研究发现潜在扩散模型中自编码器评估存在系统性偏差：过度关注生成友好性指标（如gFID）而忽视重建质量，这种偏差在可控生成任务中会导致条件漂移问题，影响可控性。


<details>
  <summary>Details</summary>
Motivation: 当前ImageNet规模的自编码器研究存在评估偏差，过度偏向生成友好性指标（gFID），而忽视重建质量评估。这种偏差在扩展到可控扩散模型时可能带来风险，因为自编码器可能引发条件漂移，限制条件对齐效果。

Method: 1. 理论分析gFID主导偏好的原因及其在可控扩散中的风险；2. 设计多维条件漂移评估协议，反映可控生成任务需求；3. 实证研究多个近期ImageNet自编码器；4. 通过ControlNet实验验证可控性与条件保持的关系。

Result: 研究发现：1. gFID仅弱预测条件保持能力；2. 重建导向指标与条件保持显著更相关；3. ControlNet实验证实可控性跟踪条件保持而非gFID；4. 实例级重建指标能更好指示可控性。

Conclusion: ImageNet中心的自编码器评估与可扩展可控扩散需求存在差距，需要更平衡的评估框架。重建保真度（特别是实例级指标）对可控生成更为重要，为可靠的基准测试和模型选择提供实用指导。
...
Method: 提出RSGround-R1框架：1) 使用合成生成的RSVG推理数据进行思维链监督微调建立显式位置感知；2) 应用强化微调，配合新设计的位置奖励提供连续、距离感知的定位指导；3) 引入空间一致性引导优化方案，基于空间一致性动态调整策略更新

Result: 在RSVG基准测试中表现出优越的性能和泛化能力

Conclusion: 提出的推理引导、位置感知的后训练框架能有效增强MLLM在遥感视觉定位中的空间理解能力
...
TL;DR: OCRVerse：首个端到端的统一文本中心OCR和视觉中心OCR方法，通过两阶段SFT-RL多域训练，在文本密集和视觉密集图像上均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有OCR方法主要关注从图像或扫描文档中识别文本元素（文本中心OCR），忽略了从图表、网页和科学图表等视觉信息密集图像源中识别视觉元素（视觉中心OCR）。这些视觉密集图像在互联网上广泛存在，具有重要的实际应用价值。

Method: 提出OCRVerse，首个端到端的统一文本中心OCR和视觉中心OCR方法。构建全面的数据工程覆盖文本中心文档和视觉中心渲染复合图像。采用两阶段SFT-RL多域训练方法：SFT阶段直接混合跨域数据训练建立初始域知识；RL阶段为每个域设计个性化奖励策略，定制灵活的奖励信号以改善跨域融合并避免数据冲突。

Result: 实验结果表明OCRVerse的有效性，在文本中心和视觉中心数据类型上均取得竞争性结果，甚至可与大规模开源和闭源模型相媲美。

Conclusion: OCRVerse是首个端到端的统一文本中心OCR和视觉中心OCR方法，通过创新的数据工程和两阶段训练策略，成功解决了传统OCR方法在处理视觉密集图像时的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有抑郁症检测的深度学习方法大多依赖有限特征类型，忽视显式跨模态交互，使用简单拼接或静态权重融合，需要更有效的多模态融合方法。

Method: 提出CAF-Mamba框架，基于Mamba架构，通过模态注意力机制显式和隐式捕捉跨模态交互，动态调整不同模态的贡献度，实现更有效的多模态融合。

Result: 在两个真实世界基准数据集LMVD和D-Vlog上的实验表明，CAF-Mamba持续优于现有方法，实现了最先进的性能。

Conclusion: CAF-Mamba通过显式和隐式跨模态交互捕捉以及动态模态贡献调整，为抑郁症检测提供了更有效的多模态融合解决方案。
...
TL;DR: 冰川崩解前缘分割模型在基准测试中表现接近人类水平，但在新研究站点实际应用时精度不足，通过少量样本域适应、空间先验知识和夏季参考图像等方法，将误差从1131.6米降至68.7米。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的冰川崩解前缘分割模型在基准测试中表现良好，但在实际应用到新研究站点时精度不足，无法满足科学分析需求，需要解决模型在新域（out-of-distribution）的泛化问题。

Method: 采用少量样本域适应策略，结合空间静态先验知识，并在输入时间序列中包含夏季参考图像，这些方法不需要修改模型架构。

Result: 通过提出的方法，冰川崩解前缘的分割误差从1131.6米显著降低到68.7米，大幅提升了模型在新研究站点的性能。

Conclusion: 这些方法学进展为将基于深度学习的冰川崩解前缘分割应用于新研究站点建立了框架，使全球尺度的冰川崩解前缘监测成为可能。
...
TL;DR: 该论文提出了一个轻量级的几何感知正则化框架，通过约束中间嵌入的几何结构来解决多模态学习中的表示几何病理问题，包括模态内表示塌缩和样本级跨模态不一致性。
...
Conclusion: 通过引入几何感知正则化框架，显式调节表示几何是多模态学习中的一个有效控制维度，能够缓解模态权衡问题，提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有图表编辑方法大多采用基于流水线的设计，使用自然语言或代码作为中间表示，限制了执行复杂编辑的能力，需要端到端的直接编辑解决方案。

Method: 构建ChartE³基准测试，包含两个互补的编辑维度：局部编辑（字体、颜色等细粒度外观调整）和全局编辑（数据过滤、趋势线添加等整体数据转换）。通过精心设计的数据流水线人工构建1200多个高质量样本，每个样本包含图表图像、底层代码和多模态编辑指令。

Result: 对最先进的多模态大语言模型进行广泛基准测试，发现在全局编辑任务上存在显著性能差距，突显了当前端到端图表编辑能力的严重局限性。

Conclusion: ChartE³基准测试揭示了当前图表编辑模型的不足，特别是处理全局编辑任务的能力，为未来端到端图表编辑研究提供了重要评估框架。
...
Method: 采用两阶段范式：1）通过将参考外观和运动线索融合到统一的潜在空间，弥合输入模态差距，使模型能够利用基础模型的生成先验共同推理空间身份和时间动态；2）引入自引导数据合成管道，策划伪跨身份训练对，实现从姿态依赖控制到直接端到端RGB驱动动画的无缝过渡。

Result: DreamActor-M2实现了最先进的性能，提供了卓越的视觉保真度和强大的跨域泛化能力。研究还引入了AW Bench基准测试，涵盖广泛的角色类型和运动场景。

Conclusion: DreamActor-M2通过重新构想运动条件为上下文学习问题，解决了现有角色动画方法的局限性，实现了更好的身份保持与运动一致性平衡，并显著提高了对不同角色和运动场景的泛化能力。
...
TL;DR: 该论文提出了Granularity-Modulated Correlation (GMC)方法，用于更精细地评估图像质量评估(IQA)模型性能，克服传统全局相关性指标(如PLCC和SRCC)的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统IQA评估主要依赖全局相关性指标(PLCC和SRCC)，这些指标将性能简化为单一标量值，无法捕捉局部质量谱上的排名一致性变化。两个IQA模型可能获得相同的SRCC值，但一个在高质量图像上表现更好，另一个在质量差异小的图像对上区分能力更强。此外，这些指标对测试样本质量分布敏感，导致跨测试集比较不稳定。

Method: 提出了Granularity-Modulated Correlation (GMC)方法，包括两个核心组件：1) Granularity Modulator：应用高斯加权相关性，条件于绝对MOS值和成对MOS差异(|ΔMOS|)，以检查局部性能变化；2) Distribution Regulator：正则化相关性以减轻非均匀质量分布带来的偏差。最终生成相关性曲面，将相关值映射为MOS和|ΔMOS|的联合函数，提供IQA性能的3D表示。
...
TL;DR: DGNav框架通过动态拓扑导航解决VLN-CE中的"粒度刚性"问题，使用场景感知自适应策略和动态图变换器动态调整地图密度和连接性，在R2R-CE和RxR-CE基准上表现出优越的导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有拓扑规划方法存在"粒度刚性"问题，依赖固定的几何阈值采样节点，无法适应不同环境复杂度，导致简单区域过度采样造成计算冗余，而高不确定性区域采样不足增加碰撞风险并影响精度。

Method: 提出DGNav动态拓扑导航框架，包含两个核心创新：1) 场景感知自适应策略，基于预测路径点分散度动态调整图构建阈值，实现"按需密集化"；2) 动态图变换器，融合视觉、语言和几何线索到动态边权重中，重建图连接性，过滤拓扑噪声。

Result: 在R2R-CE和RxR-CE基准测试中，DGNav展现出优越的导航性能和强大的泛化能力。消融研究证实该框架在导航效率和安全探索之间实现了最佳平衡。

Conclusion: DGNav通过动态调整拓扑地图密度和连接性，有效解决了VLN-CE中的"粒度刚性"问题，实现了更高效、更安全的视觉语言导航，代码已开源。
...
Method: 采用三阶段流水线：1)大规模数据收集与标准化；2)思维链原理生成；3)基于推理质量和难度感知的综合选择。基于该数据集微调Qwen3-VL-Instruct得到不同规模模型。

Result: MMFineReason-4B成功超越Qwen3-VL-8B-Thinking，MMFineReason-8B甚至超越Qwen3-VL-30B-A3B-Thinking并接近Qwen3-VL-32B-Thinking，展现了显著的参数效率。发现"少即是多"现象：仅7%数据子集（12.3万样本）即可达到完整数据集性能。

Conclusion: MMFineReason数据集有效提升了开源视觉语言模型的推理能力，揭示了推理导向数据组合能同时提升通用能力，为高质量多模态推理数据集的构建提供了有效方法。
...
Method: 重新解释扩散过程为结构化潜在空间中的随机轨迹演化，通过设计初始噪声及其几何对齐使背景生成自然避开前景区域；将风格控制与文本条件解耦，引入缓存风格方向作为潜在空间中的持久向量，约束扩散轨迹到共享风格子空间

Result: 无需训练，与现有扩散模型兼容，能够生成视觉连贯、前景保留的结果，在多页面生成中保持风格一致性，无需重复指定风格提示

Conclusion: 通过将扩散重新框架为潜在空间中的轨迹设计，为一致性和结构化生成建模提供了原则性方法，消除了显式约束和辅助机制的需求
...
TL;DR: 该论文为Classifier-free Guidance（CFG）提供了一种基于优化的理论解释，揭示了CFG是平滑距离函数梯度的近似，并提出了一种基于流形约束的同伦优化采样方法，通过Anderson加速提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 尽管CFG在扩散和流模型中广泛用于可控生成并取得了经验成功，但它依赖于启发式的线性外推，通常对引导尺度敏感。缺乏理论理解限制了其稳定性和性能。
...
Result: 提出的方法是免训练的，能一致地提高生成保真度、提示对齐和对引导尺度的鲁棒性。在DiT-XL-2-256、Flux和Stable Diffusion 3.5等大规模模型上的多样化基准测试中验证了有效性，显示出显著改进。
...
TL;DR: 本文提出PaFu-KV方法，通过轻量级显著性估计头来优化KV缓存策略，解决长视频生成中因忽略token重要性差异导致的关键时空信息丢失问题，实现更好的质量-效率平衡。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频生成方法依赖启发式KV缓存策略，忽略了token在长视频生成中的重要性差异，导致关键时空信息丢失和冗余缓存积累，从而降低视频生成质量和效率。

Method: 提出Past- and Future-Informed KV Cache Policy (PaFu-KV)，引入从双向教师模型蒸馏的轻量级显著性估计头来评估token重要性分数，使KV缓存能保留信息丰富的token而丢弃相关性较低的token。

Result: 在基准测试上的广泛实验表明，该方法在保持高保真视频生成质量的同时，通过缩小KV缓存容量和减少推理时内存占用，实现了加速推理，支持更高效的长时视频生成。

Conclusion: PaFu-KV方法通过智能KV缓存管理解决了长视频生成中的质量-效率权衡问题，为高效长时视频生成提供了有效解决方案。
...
TL;DR: TraceRouter是一个路径级防御框架，通过追踪和断开有害语义的因果传播电路来增强大基础模型的对抗鲁棒性，相比现有局部防御方法在安全性和实用性之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 当前大基础模型的防御方法主要基于"局部性假设"，通过抑制孤立的神经元或特征来对抗对抗性攻击。然而，有害语义实际上是分布式、跨层的电路，这使得局部干预方法既脆弱又损害模型实用性。需要一种能够追踪和断开有害语义因果传播路径的新方法。

Method: TraceRouter采用三阶段路径级框架：1) 通过分析注意力差异定位敏感起始层；2) 利用稀疏自编码器和差分激活分析来解耦和隔离恶意特征；3) 通过基于归零干预的特征影响分数将这些特征映射到下游因果路径。通过选择性抑制这些因果链，物理上切断有害信息流，同时保持正交计算路径完整。

Result: 大量实验表明，TraceRouter显著优于现有最先进的基线方法，在对抗鲁棒性和通用实用性之间实现了优越的平衡。

Conclusion: TraceRouter通过追踪和断开有害语义的因果传播电路，为大基础模型提供了一种有效的路径级防御框架，相比局部防御方法在保持模型实用性的同时显著提升了对抗鲁棒性。
...
TL;DR: 提出PST金字塔Shapley-Taylor学习框架，通过逐层关节和片段对齐实现细粒度运动-语言检索，显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有运动-语言检索方法主要关注全局对齐，忽略了局部运动片段与身体关节和文本标记之间的细粒度交互，导致检索性能不理想。受人类运动感知的金字塔过程启发（从关节动态到片段连贯性，再到整体理解），需要更细粒度的对齐方法。

Method: 提出金字塔Shapley-Taylor学习框架，将人体运动分解为时间片段和空间身体关节，通过渐进式的关节级和片段级对齐以金字塔方式学习跨模态对应关系，有效捕捉局部语义细节和层次结构关系。

Result: 在多个公开基准数据集上的广泛实验表明，该方法显著优于现有最先进方法，实现了运动片段、身体关节与对应文本标记之间的精确对齐。

Conclusion: 通过金字塔式的细粒度对齐框架，能够更好地捕捉运动与语言之间的局部语义细节和层次结构关系，为运动-语言检索提供了更有效的解决方案。
...
TL;DR: VideoAesBench是一个用于评估大型多模态模型视频美学质量理解能力的综合基准，包含1804个多样化视频和多种问题格式，测试了23个模型后发现当前LMMs仅具备基本的视频美学感知能力。


<details>
  <summary>Details</summary>
Motivation: 虽然大型多模态模型在各种视觉感知任务中表现出色，但视频美学质量评估这一人类基本能力在LMMs中尚未得到充分探索，因此需要建立一个专门的基准来评估LMMs在这方面的能力。

Method: 创建VideoAesBench基准，包含1804个来自多个来源的视频（用户生成、AI生成、压缩、机器人生成和游戏视频），采用多种问题格式（单选、多选、判断、开放式描述），涵盖视觉形式、视觉风格和视觉情感三个维度的美学问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的零样本图像修复与增强方法在应用于视频时会导致严重的时间闪烁问题，需要保持更好的时间一致性
...
Conclusion: 该方法是无需训练的，可应用于任何基于扩散的图像修复与增强方法，为视频修复与增强提供了有效的时间一致性解决方案
...
<details>
  <summary>Details</summary>
Motivation: 深度视觉特征在视觉系统中应用日益广泛，需要描述特征特性和控制特征质量。将JND扩展到深度特征空间，可以为机器感知提供任务对齐的容错边界，在资源受限条件下控制特征质量。

Method: 提出FeatJND，一种任务对齐的JND公式，预测每个特征的最大可容忍扰动图；在标准化分割点设计FeatJND估计器；在图像分类、检测和实例分割任务上验证；应用于token-wise动态量化。

Result: 在相同失真强度下，FeatJND失真比非结构化高斯扰动能更好地保持任务性能；归因可视化显示FeatJND能抑制非关键特征区域；在动态量化应用中，FeatJND引导的步长分配优于随机步长排列和全局均匀步长。

Conclusion: FeatJND为深度视觉特征提供了任务对齐的容错边界，能有效控制特征质量并保持下游任务性能，在资源受限的视觉系统中具有实用价值。
...
TL;DR: BookNet是首个端到端深度学习框架，专门用于双页书籍图像校正，通过双分支架构和跨页注意力机制处理左右页面的不对称几何畸变。
...
Method: 提出BookNet框架，采用双分支架构和跨页注意力机制，能够同时估计单个页面和完整书籍展开页的扭曲流，显式建模左右页面之间的相互影响。还创建了Book3D合成数据集用于训练和Book100真实世界基准用于评估。

Result: 大量实验表明，BookNet在书籍图像校正方面优于现有的最先进方法。

Conclusion: BookNet是首个专门为双页书籍图像校正设计的端到端深度学习框架，通过建模页面间几何关系显著提升了校正效果，并提供了专门的数据集和基准。
...
TL;DR: PaddleOCR-VL-1.5模型在OmniDocBench v1.5上达到94.5%的SOTA准确率，并提出了Real5-OmniDocBench基准来评估物理失真鲁棒性，同时扩展了印章识别和文本检测功能，保持0.9B超紧凑VLM的高效性。


<details>
  <summary>Details</summary>
Motivation: 为了提升文档理解模型在真实世界物理失真条件下的鲁棒性，包括扫描、倾斜、扭曲、屏幕拍摄和光照变化等常见场景，需要更全面的评估基准和更强的模型能力。

Method: 提出了Real5-OmniDocBench基准来系统评估模型对物理失真的鲁棒性；开发了PaddleOCR-VL-1.5模型，在保持0.9B参数超紧凑架构的同时，扩展了印章识别和文本检测功能。

Result: PaddleOCR-VL-1.5在OmniDocBench v1.5上达到94.5%的SOTA准确率；在新提出的Real5-OmniDocBench基准上也取得了SOTA性能；模型保持了0.9B参数的紧凑架构和高效率。

Conclusion: PaddleOCR-VL-1.5在文档理解任务上实现了新的SOTA性能，并通过Real5-OmniDocBench基准提供了更全面的物理失真鲁棒性评估，同时扩展了功能并保持了模型的高效性。
...
<details>
  <summary>Details</summary>
Motivation: 视频世界建模结合视觉语言预训练为机器人学习提供了新的独立基础，视频世界模型能够通过理解动作与视觉动态之间的因果关系来预测近期未来。
...
Conclusion: 视频世界建模与视觉语言预训练相结合为机器人学习提供了新的基础，LingBot-VA框架展示了在复杂机器人任务中的有效性和泛化能力，代码和模型已公开。
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶使用自监督视频预训练学习规划表示，但现有视频世界模型对场景理解的改进有限。驾驶场景存在固有的模糊性，每个场景通常只提供单一的人类轨迹，难以学习多模态行为

Method: 1. 将V-JEPA适配到端到端驾驶，在大规模驾驶视频上预训练ViT编码器，生成与轨迹规划对齐的预测性表示；2. 提出以提议为中心的规划器，蒸馏模拟器生成的多样化轨迹和人类轨迹，采用动量感知选择机制促进稳定安全行为

Result: 在NAVSIM评估中，V-JEPA表示结合简单的基于transformer的解码器，在无感知设置下比先前方法提升3 PDMS。完整的Drive-JEPA框架在v1上达到93.3 PDMS，在v2上达到87.8 EPDMS，创造了新的最先进水平

Conclusion: Drive-JEPA通过整合视频联合嵌入预测架构和多模态轨迹蒸馏，有效解决了驾驶场景的模糊性问题，显著提升了端到端自动驾驶的规划性能
...
<details>
  <summary>Details</summary>
Motivation: 现有神经表面重建方法在稀疏视角条件下存在几何模糊和不稳定性问题，而大规模城市遥感中航空影像获取受飞行路径、地形和成本限制

Method: 提出融合3D SAR点云与航空影像的框架，将雷达空间约束集成到基于SDF的神经表面重建主干中，指导结构感知光线选择和自适应采样

Result: 实验表明，融合3D SAR显著提高了重建精度、完整性和鲁棒性，特别是在高度稀疏和倾斜视角条件下优于单模态基线

Conclusion: 该方法为利用先进机载和星载光学-SAR传感实现可扩展高保真城市重建提供了可行路径，并构建了首个带配准3D SAR点云和航空影像的基准数据集
...
TL;DR: PLANING是一种高效的在线单目图像序列重建框架，通过混合表示松散耦合显式几何基元与神经高斯，实现几何与外观解耦建模，在保证高质量渲染的同时获得准确几何。


<details>
  <summary>Details</summary>
Motivation: 现有单目图像序列流式重建方法通常只能在高质量渲染和准确几何之间二选一，难以同时兼顾两者，需要一种能同时实现高质量渲染和准确几何的在线重建方法。
...
Conclusion: PLANING通过几何与外观解耦的混合表示实现了高质量的流式重建，结构清晰且计算高效，适用于大规模场景建模和具身AI的仿真就绪环境等下游应用。
...
TL;DR: Metric Anything是一个用于度量深度估计的简单可扩展预训练框架，通过稀疏度量提示从多样化的3D数据中学习，无需手动设计提示或特定相机建模，首次展示了度量深度领域的缩放趋势。


<details>
  <summary>Details</summary>
Motivation: 尽管缩放推动了视觉基础模型的进步，但将其扩展到度量深度估计仍然面临挑战，包括异构传感器噪声、相机相关偏差以及跨源3D数据中的度量模糊性。需要一种能够从噪声多样的3D数据中学习度量深度的通用方法。

Method: 提出稀疏度量提示方法，通过随机掩码深度图创建通用接口，将空间推理与传感器和相机偏差解耦。使用约2000万张图像-深度对，涵盖重建、捕获和渲染的3D数据，跨越10000个相机模型。

Result: 首次展示了度量深度领域的清晰缩放趋势。预训练模型在深度补全、超分辨率和雷达-相机融合等提示驱动任务中表现出色，其蒸馏的无提示学生模型在单目深度估计、相机内参恢复、单/多视角度量3D重建和VLA规划中达到最先进水平。

Conclusion: 度量深度估计可以从驱动现代基础模型的相同缩放定律中受益，为可扩展和高效的现实世界度量感知建立了新路径。该框架还显著提升了多模态大语言模型的空间智能能力。
...
### [65] [UEval: A Benchmark for Unified Multimodal Generation](https://arxiv.org/abs/2601.22155)
*Bo Li,Yida Yin,Wenhao Chai,Xingyu Fu,Zhuang Liu*

Main category: cs.CV

TL;DR: UEval是一个评估统一模型（能同时生成图像和文本的模型）的基准测试，包含1000个专家策划的问题，涵盖8个真实世界任务，需要模型输出图像和文本。该基准采用基于量规的评分系统，包含10,417个经过验证的评分标准，能够进行细粒度的自动评分。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方法难以准确评估开放式多模态生成任务，简单的LLM-as-a-judge方法可能忽略细微差别。需要一种能够同时评估图像和文本生成质量，并能处理复杂多模态理解任务的评估基准。

Method: UEval包含1000个专家策划的问题，涵盖8个真实世界任务。采用基于量规的评分系统：首先由多模态大语言模型根据参考图像和文本生成初始评分量规，然后由人类专家进行细化和验证。最终包含10,417个经过验证的评分标准，实现可扩展的细粒度自动评分。

Result: UEval对当前统一模型具有挑战性：GPT-5-Thinking仅得66.4分（满分100），最佳开源模型仅得49.1分。研究发现推理模型通常优于非推理模型，将推理轨迹从推理模型转移到非推理模型能显著缩小性能差距。

Conclusion: 推理能力对于需要复杂多模态理解和生成的任务可能很重要。UEval为评估统一模型提供了一个具有挑战性的基准，其基于量规的评分系统能够实现细粒度的自动评估。
...
TL;DR: 该论文提出了一种通过对抗训练改进无监督因子分解的方法，使用鉴别器区分单源样本与跨源因子重组样本，从而提升因子发现和组合生成质量。


<details>
  <summary>Details</summary>
Motivation: 将复杂数据分解为因子化表示可以揭示可重用组件，并通过组件重组合成新样本。在无因子级监督的情况下学习因子化潜在空间，需要改进因子发现和组合生成质量。

Method: 引入对抗训练信号：训练一个鉴别器来区分单源样本和跨源因子重组生成的样本，通过优化生成器欺骗鉴别器，鼓励重组结果的物理和语义一致性。

Result: 在CelebA-HQ、Virtual KITTI、CLEVR和Falcor3D数据集上优于先前基线方法，获得更低的FID分数和更好的解缠结性能（MIG和MCC指标）。在机器人视频轨迹应用中，通过重组学习到的动作组件生成多样化序列，显著增加了LIBERO基准测试中的状态空间覆盖率。

Conclusion: 提出的对抗训练方法有效提升了无监督因子分解的质量和组合生成能力，在多个视觉数据集上表现优异，并在机器人视频轨迹应用中展示了实用价值。
...
TL;DR: Vision-DeepResearch提出了一种新的多模态深度研究范式，通过多轮、多实体、多尺度的视觉和文本搜索，在强噪声环境下实现更鲁棒的搜索引擎交互，显著提升了复杂问题的解决能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要大量事实信息的任务上表现有限，传统方法通常假设单一图像查询和少量文本查询就足以检索关键证据，这在现实世界的强噪声场景中不切实际。此外，现有方法在推理深度和搜索广度上存在局限，难以解决需要聚合多样化视觉和文本证据的复杂问题。

Method: 提出Vision-DeepResearch，采用多轮、多实体、多尺度的视觉和文本搜索策略，支持数十个推理步骤和数百次引擎交互。通过冷启动监督和强化学习训练，将深度研究能力内化到MLLM中，构建端到端的多模态深度研究MLLM。

Result: Vision-DeepResearch显著优于现有的多模态深度研究MLLMs，并且在性能上超越了基于GPT-5、Gemini-2.5-pro和Claude-4-Sonnet等强大闭源基础模型构建的工作流程。

Conclusion: 该研究提出了一种创新的多模态深度研究范式，通过更鲁棒的搜索策略和深度推理能力，有效解决了现实世界中复杂多模态问题，为MLLMs在需要广泛事实检索的任务上提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: Segment Anything Model (SAM) 虽然具有零样本分割能力，但依赖手动提示阻碍了全自动部署。现有将目标检测器作为提示生成器的方案存在两个根本限制：1) 目标不匹配 - 检测器优化的几何定位目标与SAM所需的最优提示上下文不对应；2) 对齐过拟合 - 标准联合训练中检测器只是记忆特定训练样本的提示调整，而非学习可泛化的策略。

Method: 提出BLO-Inst统一框架，通过双层优化对齐检测与分割目标。将对齐问题表述为在不相交数据分割上的嵌套优化问题：下层在子集D1上微调SAM以最大化给定当前检测提议的分割保真度；上层在子集D2上更新检测器，使其生成的边界框显式最小化微调后SAM的验证损失。这使检测器转变为分割感知的提示生成器，优化边界框不仅为了定位精度，更为了下游掩码质量。

Result: 大量实验表明，BLO-Inst在通用和生物医学领域任务上均取得优异性能，优于标准基线方法。

Conclusion: BLO-Inst通过双层优化有效解决了SAM自动化部署中检测器提示生成的关键问题，实现了检测与分割目标的对齐，使检测器成为分割感知的提示生成器，在多个领域表现出优越性能。
...
TL;DR: 提出3D资产参考扩散模型，通过双分支感知架构利用3D资产的多视角RGB图像和点云图，实现生成图像与3D参考的精确一致性


<details>
  <summary>Details</summary>
Motivation: 现有基于参考的图像生成方法仅限于单图像参考，无法利用3D资产，限制了实际应用的多功能性

Method: 提出跨域扩散模型，采用双分支感知架构，联合建模3D资产的色彩和规范空间坐标，通过空间对齐的双分支生成架构和域解耦生成机制，同时生成空间对齐但内容解耦的RGB图像和点云图

Result: 实验表明该方法能有效使用3D资产作为参考，生成与给定资产一致的图像
...
TL;DR: SINA是一个开源自动化电路原理图转网表生成器，通过深度学习、CCL、OCR和VLM技术，实现96.47%的网表生成准确率，比现有方法高2.72倍。


<details>
  <summary>Details</summary>
Motivation: 当前电路原理图图像转机器可读网表的方法在元件识别和连接性推断方面存在困难，需要更准确和自动化的解决方案。

Method: SINA集成了深度学习用于精确元件检测、连通分量标记(CCL)用于连接性提取、OCR用于元件参考标识符检索，并采用视觉语言模型(VLM)进行可靠的参考标识符分配。

Result: 实验表明，SINA实现了96.47%的整体网表生成准确率，比最先进方法高出2.72倍。

Conclusion: SINA提供了一个高效、准确的开源自动化电路原理图转网表生成解决方案，显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有创意图像生成方法通常依赖于手动混合概念或排除子类别，缺乏系统性的创造力定义和量化方法。需要一种能够自动生成新颖、高质量创意图像的原则性框架。

Method: 提出基于扩散模型的创意生成框架：1) 将创造力定义为CLIP嵌入空间中图像存在概率的倒数；2) 计算生成图像的概率分布并驱动其向低概率区域移动；3) 引入回拉机制，在保持高创造力的同时不牺牲视觉保真度。

Result: 在文本到图像扩散模型上的大量实验表明，该框架能够有效且高效地生成独特、新颖、引人深思的图像，实现了高创造力与视觉质量的平衡。

Conclusion: 该工作为生成模型中的创造力提供了新的视角，提供了一种原则性的方法来促进视觉内容合成的创新，推动了创意图像生成领域的发展。
...
TL;DR: EditYourself是一个基于DiT的音频驱动视频编辑框架，能够通过修改文字脚本对说话人视频进行编辑，包括添加、删除和重新调整视觉内容，同时保持运动、时间一致性、说话人身份和准确的唇部同步。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型擅长从文本和图像提示生成新内容，但在编辑现有预录视频方面存在关键空白。当需要对口语脚本进行微小修改时，需要保持运动、时间一致性、说话人身份和准确的唇部同步，现有模型难以满足这些需求。

Method: EditYourself是一个基于DiT的音频驱动视频到视频编辑框架。它在通用视频扩散模型基础上，通过音频条件化和区域感知、编辑聚焦的训练扩展来增强V2V能力。使用时空修复技术实现精确的唇部同步和时间一致的重构，包括在新添加的片段中合成逼真的人类运动。

Result: 该框架能够实现基于文字脚本的说话人视频修改，包括无缝添加、删除和重新调整视觉内容。能够保持长时间内的视觉保真度和身份一致性，实现精确的唇部同步和时间一致的重构。

Conclusion: 这项工作代表了生成视频模型作为专业视频后期制作实用工具的基础性步骤，为解决现有预录视频编辑中的关键挑战提供了有效方案。
...
TL;DR: 开发了名为ePAI的AI系统，用于早期检测胰腺导管腺癌（PDAC），在内部测试中AUC达0.939-0.999，灵敏度95.3%，特异性98.7%；在外部测试中AUC达0.918-0.945，灵敏度91.5%，特异性88.0%。该系统能检测到临床诊断前3-36个月被放射科医生忽略的病变，在多读者研究中显著优于30名认证放射科医生。


<details>
  <summary>Details</summary>
Motivation: 胰腺导管腺癌（PDAC）是最致命的实体恶性肿瘤之一，通常在晚期和不可手术阶段才被发现。回顾性分析显示，当专家放射科医生知道患者后来发展为PDAC时，经常能在之前的CT扫描中发现被忽略的病变。因此需要开发自动化系统来帮助早期检测这些病变。
...
Result: ePAI在内部测试中对小于2cm的PDAC检测AUC为0.939-0.999，灵敏度95.3%，特异性98.7%，能精确定位小至2mm的病变。在外部测试中AUC为0.918-0.945，灵敏度91.5%，特异性88.0%，能精确定位小至5mm的病变。系统成功检测到临床诊断前3-36个月被放射科医生忽略的病变，在159名患者中检测到75例，中位提前时间为347天。多读者研究显示ePAI灵敏度显著优于30名放射科医生50.3%，同时保持95.4%的可比特异性。

Conclusion: ePAI系统在早期胰腺癌检测方面表现出优异性能，能够检测到放射科医生忽略的早期病变，显著提高了检测灵敏度。这些发现表明ePAI有潜力作为辅助工具改善胰腺癌的早期检测。
...
TL;DR: PI-Light是一个两阶段物理启发的扩散模型框架，用于全图像重光照，通过物理约束和批量感知注意力提升真实场景泛化能力。


<details>
  <summary>Details</summary>
Motivation: 全图像重光照面临三大挑战：大规模结构化配对数据收集困难、物理合理性难以保持、数据驱动先验导致的泛化能力有限。现有方法在合成到真实场景的转换上仍不理想。

Method: 提出两阶段物理启发扩散框架，包含：1)批量感知注意力提升内在预测一致性；2)物理引导神经渲染模块确保物理合理的光传输；3)物理启发损失函数规范训练动态；4)精心策划的多样化物体和场景数据集。

Result: PI-Light能够在各种材质上合成镜面高光和漫反射，相比现有方法在真实世界场景中展现出更优越的泛化能力。
...
TL;DR: 本文提出了VI-Probe框架，通过可控的视觉错觉实验发现大型视觉语言模型对视觉变化的感知存在多种机制，而非单一原因。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在面对经典视觉错觉时，即使错觉因素被反转，模型仍给出相同回答，这引发了一个根本问题：模型是真正感知视觉变化，还是仅仅回忆记忆模式？虽然已有研究注意到这一现象，但其根本原因尚不清楚。

Method: 本文引入了VI-Probe框架，包含分级扰动和匹配的视觉控制（无错觉诱导因素），以区分视觉基础感知和语言驱动回忆。不同于先前关注平均准确率的工作，我们使用极性翻转一致性、模板固定指数和相对于匹配控制的错觉乘数来测量稳定性和敏感性。

Result: 实验发现不同模型家族表现出不同的响应持续性机制：GPT-5表现出记忆覆盖，Claude-Opus-4.1显示感知-记忆竞争，而Qwen变体则暗示视觉处理限制。这些发现挑战了单一原因的观点。

Conclusion: 研究结果表明需要基于探测的评估方法，既能测量知识，又能测量对受控视觉变化的敏感性。这为理解视觉语言模型的感知机制提供了新视角。
...
TL;DR: 本文提出了语音识别评估的多项改进：支持多参考标注的字符串对齐算法、收集俄语长语音数据集、开发流式语音识别评估工具，并发现模型会适应数据集特定标注导致指标虚高。


<details>
  <summary>Details</summary>
Motivation: 当前语音识别评估存在局限性，特别是在非拉丁语言、词汇形态丰富的语言以及杂乱或长语音场景下。现有评估方法对多参考标注支持不足，对齐算法不够精确，且缺乏针对流式语音识别的评估工具。

Method: 1. 提出支持多参考标注、任意长度插入和更好词对齐的字符串对齐算法；2. 收集并标注DiverseSpeech-Ru俄语长语音数据集；3. 对流行俄语测试集进行多参考重新标注；4. 开发流式语音识别评估工具和可视化对比工具；5. 为多个离线和流式语音识别模型提供统一封装。

Result: 1. 改进的对齐算法特别适用于非拉丁语言和词汇形态丰富的语言；2. 发现模型会适应数据集特定标注，导致指标改进的假象；3. 提供了全面的评估工具集和模型封装，代码将公开可用。

Conclusion: 本文提出的改进方法显著提升了语音识别评估的准确性和实用性，特别是在多参考标注、非拉丁语言处理和流式识别评估方面。研究揭示了数据集特定标注对模型评估的影响，为更可靠的语音识别评估提供了工具和方法基础。
...
TL;DR: 研究发现，在自监督学习语音模型中，使用MSE损失进行前端语音增强微调时，模型会过度利用位置嵌入而非内容信息，提出了两种位置不变微调策略来改善这一问题。


<details>
  <summary>Details</summary>
Motivation: 当前将前端语音增强模型与自监督学习语音模型结合时，通常使用MSE损失在增强语音和干净语音之间进行微调。但研究发现MSE损失容易过度利用SSL模型中的位置嵌入，使目标函数通过位置相关性而非内容相关信息最小化，这限制了模型性能。

Method: 提出了两种位置不变微调策略：1）零填充方法（在SSL预训练中已有探索，但在微调场景中重新审视）；2）速度扰动结合软DTW损失的方法。这些方法旨在减少模型对位置嵌入的依赖，增强对内容信息的关注。

Result: 实验表明，基于软DTW的方法实现了更快的收敛速度和更好的下游任务性能。这验证了在基于SSL的语音建模中，位置不变微调的重要性。

Conclusion: 该研究揭示了自监督表示微调中MSE损失的局限性，提出了有效的解决方案。软DTW方法在收敛速度和性能上都优于传统方法，强调了位置不变微调在SSL语音建模中的关键作用。
...
TL;DR: ChunkWise LoRA：一种动态自适应方法，根据token复杂度将序列划分为可变长度的块，并为每个块分配定制的低秩配置，显著降低延迟和内存使用。
...
Result: 在Wikitext-103和SQuAD基准测试中，相比基线LoRA，ChunkWise LoRA实现了延迟降低34%、内存减少38%，同时保持或改进了BLEU、EM和困惑度等任务性能指标。
...
TL;DR: 比较了两种创建小型多任务代码LLM的方法：数据混合与模型合并，发现在7B规模下模型合并效果更好，在2B规模下数据混合更优，并提出权重分析技术理解任务对参数的影响。


<details>
  <summary>Details</summary>
Motivation: 当前研究提倡在智能体框架中部署小型专业化代码LLM，需要探索高效的多任务学习策略来平衡性能、约束和成本。

Method: 比较数据混合与模型合并两种方法，在Qwen Coder和DeepSeek Coder两个模型家族的2B和7B规模上进行实验，针对代码生成和代码摘要任务进行微调，并在HumanEval、MBPP和CodeXGlue基准上评估，引入权重分析技术理解任务对参数的影响。

Result: 在7B规模下，模型合并获得最佳整体性能，在代码生成任务上保留96%的专业模型性能；合并模型甚至能超越单独微调的模型，Qwen Coder 2.5 7B在HumanEval上达到92.7% Pass@1（相比单独微调的90.9%）；在2B规模下数据混合是更优策略。

Conclusion: 精心设计的合并和混合策略能有效结合任务特定能力而不会显著降低性能，适合资源受限的部署场景。
...
TL;DR: LLM可从姓名推断种族/民族，准确率超传统BISG方法，无需额外训练数据，适用于美国以外地区及不同分类体系


<details>
  <summary>Details</summary>
Motivation: 传统BISG方法存在局限性：仅适用于美国，依赖人口普查数据，对富裕社区少数族裔存在系统性误分类为白人，且无法适应不同国家的分类体系

Method: 使用六种LLM模型（包括Gemini 3 Flash、GPT-4o、DeepSeek v3.2、GLM-4.7等）从姓名推断种族/民族，采用分层抽样验证，测试扩展推理和元数据（如党派登记）的影响，并在多个国家（印度、乌干达、尼泊尔、亚美尼亚、智利、哥斯达黎加）进行验证

Result: LLM分类准确率达84.7%，显著优于BISG的68.2%；扩展推理可提升1-3个百分点；包含党派登记元数据可达86.7%；减少BISG中的收入偏见；在黎巴嫩（宗教派别64.3%）、印度（保留选区议员99.2%、种姓分类74.0%）等国际验证中表现良好；微调的小型Transformer模型在保持准确性的同时实现本地零成本部署

Conclusion: LLM提供了一种强大、灵活且成本效益高的种族/民族推断方法，超越传统BISG，减少偏见，适用于全球范围，通过微调小型模型可实现大规模应用的本地部署
...
TL;DR: EnsembleLink是一种无需训练标签的高精度记录链接方法，利用预训练语言模型的语义理解能力，在多种基准测试中表现优于需要大量标注的方法。


<details>
  <summary>Details</summary>
Motivation: 记录链接在社会科学研究中至关重要，但现有方法要么精度低，要么需要大量标注数据，且研究者通常将其视为预处理步骤，没有量化链接错误对后续分析的不确定性影响。

Method: EnsembleLink利用预训练语言模型，这些模型从大规模文本语料库中学习了语义关系（如"South Ozone Park"是"New York City"的社区），无需任何训练标签即可实现高精度记录匹配。

Result: 在城市名称、人名、组织、多语言政党和书目记录等多种基准测试中，EnsembleLink的表现与需要大量标注的方法相当或更好，且能在本地开源模型上运行，无需外部API调用，几分钟内完成典型链接任务。

Conclusion: EnsembleLink为社会科学研究提供了一种无需标注数据的高精度记录链接方法，解决了现有方法在准确性和数据需求方面的局限性，同时保持了计算效率和本地化运行的优势。
...
TL;DR: OS-Search将LLM生成转化为端点搜索，通过外层循环在编码器定义的3D输出空间中选择目标点，训练检索增强策略生成接近该目标的输出，实现并行扫描和黑盒优化
...
Method: 1. 定义冻结编码器的3D输出空间Z；2. 外层循环在Z中选择目标点z*；3. 训练检索增强策略（使用序列级强化学习）生成输出，使其在标准自回归解码下坐标接近z*；4. 在Z空间进行并行扫描和贝叶斯优化

Result: 在故事生成中，在Z空间扫描比提示链方法获得3.1倍更高的LLM评分多样性；在代码生成中，对Z空间进行贝叶斯优化在相同推理预算下改进了控制器未知的目标函数，同时保持有效性

Conclusion: OS-Search成功将LLM生成转化为输出空间搜索问题，实现了并行扫描和黑盒优化，在文本和代码生成任务中均表现出优越性能
...
TL;DR: 研究探讨了哪些统计条件支持从线性输入中学习层次结构，重点关注功能词的分布特性，发现频率、结构关联和边界对齐三个属性共同促进神经学习者的语言习得。


<details>
  <summary>Details</summary>
Motivation: 功能词在语言习得中扮演关键角色，但支持从线性输入学习层次结构的统计条件尚不明确。本研究旨在探究功能词的分布特性（高频率、与句法结构的可靠关联、与短语边界的对齐）如何支持语言习得。

Method: 1. 跨语言语料库分析：研究186种语言中功能词的三个分布特性；2. 反事实语言建模和消融实验：比较不同属性组合的语言变体对神经学习者的习得难度；3. 探测和消融分析：探究不同学习条件下对功能词的依赖机制差异。

Result: 1. 所有186种语言中功能词都具备三个分布特性；2. 同时保留三个属性的语言变体更容易被神经学习者习得，其中频率和结构关联的贡献比边界对齐更强；3. 不同学习条件导致对功能词的依赖机制不同，相似表现可能源于不同的内部机制。

Conclusion: 功能词的分布特性（特别是高频率和与句法结构的可靠关联）为从线性输入学习层次结构提供了关键的统计支持，这些跨语言的普遍特性有助于解释语言习得的机制。
...
Method: 提出多语言音素生成评估框架，整合通用音素识别和语言特定音素解释，使用对比音系特征距离进行音素到音位映射和序列对齐。框架产生三个指标：音素错误率、音系特征错误率和新的对齐无关指标——音素覆盖率。

Result: 在英语、西班牙语、意大利语和泰米尔语上的分析显示：音素错误率受益于映射和对齐的结合，音系特征错误率仅受益于对齐，音素覆盖率受益于映射。进一步分析表明该框架能捕捉与构音障碍语音已知观察一致的有临床意义的可懂度退化模式。

Conclusion: 提出的多语言音素生成评估框架能够有效评估跨语言的构音障碍语音可懂度，捕捉临床相关模式，为多语言语音病理学评估提供了实用工具。
...
TL;DR: 本文研究了LLMs在推理步数超出训练分布时的性能下降问题，发现错误集中在少数关键错误类型的token位置，源于内部竞争机制，并提出了一种动态识别和停用错误处理头的测试时校正方法。


<details>
  <summary>Details</summary>
Motivation: 尽管CoT推理已成为LLMs解决复杂问题的标准范式，但最近研究发现，在推理步数超出训练分布时性能急剧下降，而这种现象的内部机制尚未被充分理解。

Method: 通过跨多个领域任务的系统研究，发现错误集中在特定token位置，源于错误处理头（ep heads）的内部竞争机制。提出测试时推理校正方法，在推理过程中动态识别和停用这些ep heads。

Result: 实验表明，在推理过程中移除单个ep heads通常能恢复正确预测。提出的测试时校正方法在不同任务和LLMs上都能持续改善推理步数泛化性能。

Conclusion: 研究揭示了LLMs在推理步数泛化失败的内在机制，并提出了一种轻量级干预方法，为理解LLMs推理失败机制和改进其泛化能力提供了新视角。
...
TL;DR: 本文提出了一个更稳健的LLM诚实性评估基准数据集，并利用公开的预训练数据构建更诚实的LLM的新方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能够回答问题，但常常不清楚自己的知识边界，导致在缺乏足够知识的主题上产生事实错误的回答（幻觉）。模型应该更诚实，在不知道时回答"我不知道"。现有方法评估不够稳健，没有考虑模型在预训练中已吸收的知识。

Method: 1. 利用Pythia（真正开源且预训练数据公开的LLM）构建更稳健的LLM诚实性评估基准数据集；2. 提出一种新颖方法，利用预训练数据构建更诚实的LLM。

Result: 提出了一个考虑预训练知识的更稳健评估基准，以及利用预训练数据提高LLM诚实性的新方法。

Conclusion: 通过利用公开的预训练数据，可以构建更稳健的LLM诚实性评估基准，并开发更有效的方法来提高语言模型的诚实性，减少幻觉问题。
...
TL;DR: SHARP框架通过多维风险分析评估LLMs的社会危害，发现相似平均风险的模型在尾部风险和波动性上存在显著差异，表明需要超越标量平均值的评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs评估基准将复杂社会风险简化为均值中心的标量分数，掩盖了分布结构、跨维度交互和最坏情况行为，而LLMs在关键领域部署需要更全面的风险评估。

Method: 提出SHARP框架，将危害建模为多元随机变量，整合偏见、公平、伦理和认知可靠性的显式分解，采用风险敏感分布统计（CVaR95作为主要指标），使用901个社会敏感提示评估11个前沿LLMs。

Result: 相似平均风险的模型在尾部暴露和波动性上存在两倍以上差异；不同危害维度的尾部行为呈现系统性差异：偏见维度尾部严重性最强，认知和公平风险居中，伦理错配最低；揭示了标量基准无法捕捉的异质性、模型依赖的失败结构。

Conclusion: LLMs的负责任评估和治理需要从标量平均值转向多维、尾部敏感的风险分析，SHARP框架为此提供了系统方法。
...
TL;DR: MoCo是一个用于大规模执行、基准测试和比较模型协作算法的Python库，包含26种协作方法和25个评估数据集，实验显示协作策略在61%的情况下优于单模型，最高提升25.8%
...
Method: 开发MoCo库，集成26种模型协作方法（涵盖路由、文本、logit和模型参数等不同层次的信息交换），整合25个评估数据集，支持用户自定义数据，进行大规模实验比较

Result: 大多数协作策略在61.0%的（模型，数据）设置中平均优于无协作模型，最有效的方法性能提升达25.8%；分析了协作策略的扩展性、训练/推理效率，发现协作系统能解决单模型难以处理的问题

Conclusion: MoCo作为有价值的工具包，将促进开放、模块化、去中心化和协作式AI未来的发展，为模型协作研究提供统一平台
...
TL;DR: Qwen3-ASR系列包含两个全功能语音识别模型和一个创新的非自回归语音强制对齐模型，支持多语言识别，在开源模型中达到SOTA性能，并在效率与精度间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有的ASR模型在开源基准测试中差异不大，但在实际场景中质量差异显著，需要更全面的评估。同时，社区需要更高效、多功能的语音理解和强制对齐工具来加速研究。

Method: 基于Qwen3-Omni基础模型的强大音频理解能力，利用大规模语音训练数据开发了Qwen3-ASR-1.7B和Qwen3-ASR-0.6B两个ASR模型，支持52种语言和方言的语音识别与语言识别。同时开发了基于LLM的非自回归强制对齐模型Qwen3-ForcedAligner-0.6B，用于11种语言的文本-语音对齐。

Result: 1.7B版本在开源ASR模型中达到SOTA性能，与最强的专有API竞争；0.6B版本提供最佳精度-效率平衡，平均TTFT低至92ms，并发128时可在1秒内转录2000秒语音。强制对齐模型在时间戳准确率上超越三个最强的强制对齐模型，在效率和多功能性方面更具优势。

Conclusion: Qwen3-ASR系列模型在语音识别和强制对齐任务上表现出色，特别是0.6B版本在效率与精度间取得良好平衡。这些模型以Apache 2.0许可证发布，将加速ASR和音频理解社区的研究进展。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型主要通过收集昂贵数据集和多阶段微调来确保生成质量，但这种方法无法纠正预训练阶段学习到的错误模式。需要在预训练阶段就解决这些问题，因为预训练塑造了模型的核心行为，防止不安全或幻觉输出被深度嵌入
...
Conclusion: 该方法从基础层面构建了更高质量、更安全、更事实的模型，通过在预训练阶段直接集成质量改进机制，有效解决了传统方法无法纠正预训练模式的问题
...
TL;DR: LLMs在代码评估中存在系统性漏洞：过度遵循指令导致忽略代码逻辑，即使代码功能错误也给出高分认证


<details>
  <summary>Details</summary>
Motivation: 当前教育评估中广泛使用LLMs，但存在一个未经验证的假设：指令遵循能力能直接转化为客观评判能力。作者质疑这一假设，认为模型可能为了满足隐藏指令而忽略代码质量评估

Method: 提出SPACI框架和AST-ASIP协议，利用语法-语义鸿沟，在抽象语法树的无关节点中嵌入对抗性指令。对9个SOTA模型进行大规模评估，涉及25,000份Python、C、C++、Java代码提交

Result: 发现灾难性失败率（>95%），特别是DeepSeek-V3等高容量开源模型，系统性地优先考虑隐藏格式约束而非代码正确性。通过解耦概率、分数差异和教学严重性三个指标量化了"虚假认证"现象
...
TL;DR: 本文提出证据排序新任务，旨在优先呈现充足信息以最小化用户阅读负担，同时保留所有可用证据供顺序验证。通过对比单次排序和增量排序方法，构建统一基准评估框架，实验表明增量排序策略能更好捕捉互补证据，LLM方法优于浅层基线，证据排序相比证据选择能减少阅读负担并提升验证效果。


<details>
  <summary>Details</summary>
Motivation: 当前自动系统和大型语言模型在信息可靠性评估中面临归因和事实核查的挑战。这些系统通常为用户提供不足或过度冗余的证据信息，导致验证过程低效且容易出错。需要一种能优先呈现充足信息同时最小化用户阅读负担的方法。

Method: 提出证据排序新任务，比较两种方法：单次排序和增量排序。构建受信息检索指标启发的新评估框架，通过聚合现有事实核查数据集创建统一基准。使用多样化模型进行广泛实验，包括LLM方法和浅层基线方法。

Result: 实验表明增量排序策略能更好捕捉互补证据，LLM方法优于浅层基线方法，但在平衡充分性和冗余性方面仍面临挑战。通过受控用户研究证明，相比证据选择，证据排序既能减少阅读负担又能提升验证效果。

Conclusion: 证据排序任务为实现更可解释、高效且用户对齐的信息验证系统提供了基础性步骤。该方法通过优先呈现充足信息同时最小化用户阅读负担，改善了事实核查过程的效率和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 训练大语言模型处理创造性写作、对话、伦理推理等不可验证任务时缺乏真实标签，而LLM-as-Judge方法受限于评估者自身质量，无法识别优质解决方案，且存在评估偏见（如偏好冗长而非质量）。

Method: CoNL框架通过多智能体自博弈统一生成、评估和元评估。多个共享相同策略的智能体进行结构化对话：提出方案、批评、修订方案。能够帮助改进解决方案的批评获得诊断奖励，为元评估提供明确监督，实现生成和评估能力的联合优化。

Result: 在五个基准测试上的实验表明，CoNL相比自奖励基线实现了持续改进，同时保持训练稳定性。

Conclusion: CoNL框架通过多智能体自博弈解决了评估者质量限制问题，在没有外部评估者或真实标签的情况下，实现了生成和评估能力的联合优化，为不可验证任务的LLM训练提供了新方法。
...
TL;DR: SOUP框架通过令牌级别的单样本混合策略训练，统一了离线和在线强化学习，解决了语言模型后训练中探索不足和早期饱和的问题。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型后训练中广泛使用的在线强化学习方法（如GRPO）存在探索有限和早期饱和的问题，而现有的离线数据混合方法会导致策略不匹配和不稳定性。

Method: 提出SOUP框架，在令牌级别统一离线和在线学习：序列前缀从历史策略中采样（离线），而延续部分在线生成，通过令牌级别的重要性比率有效利用离线信息。

Result: 大量实验表明，SOUP在性能上一致优于标准的在线训练和现有的离线扩展方法，能够同时改善探索和最终性能。

Conclusion: 细粒度的单样本混合策略训练框架SOUP能够有效解决语言模型强化学习中的探索不足问题，提升训练稳定性和最终性能。
...
TL;DR: DimStance是首个包含情感维度（效价-唤醒度）标注的立场检测资源，涵盖5种语言、2个领域的11,746个目标方面，支持细粒度立场分析。


<details>
  <summary>Details</summary>
Motivation: 传统立场检测仅使用分类标签（支持、中立、反对），无法捕捉立场表达背后的细微情感状态。研究者希望利用情感科学框架，通过连续的情感维度（效价和唤醒度）来实现更细粒度的立场分析。

Method: 引入DimStance资源，包含11,746个目标方面的效价-唤醒度标注，涵盖5种语言和2个领域。提出维度立场回归任务，评估预训练模型和大语言模型在回归和提示设置下的表现。

Result: 微调后的LLM回归器表现优异，但在低资源语言中仍面临挑战，基于token的生成方法存在局限性。跨语言VA模式分析揭示了不同语言间的差异。

Conclusion: DimStance为多语言、情感感知的立场分析和基准测试提供了基础，推动了从分类立场检测向连续情感维度分析的转变。
...
TL;DR: 研究人员创建了MURAD数据集，包含96,243个阿拉伯语词-定义对，涵盖多个专业领域，支持计算语言学和词典学研究。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语词汇丰富，涵盖科学、宗教和文学等多个领域，但大规模连接阿拉伯语词汇与精确定义的数据集仍然有限，需要填补这一空白。

Method: 采用混合提取流程，整合了直接文本解析、光学字符识别和自动重建技术，从可信的参考著作和教育资源中提取数据。

Result: 成功构建了MURAD数据集，包含96,243个词-定义对，每个记录都包含目标词、标准化阿拉伯语定义以及标识源领域的元数据，涵盖语言学、伊斯兰研究、数学、物理学、心理学和工程学等多个领域。
...
TL;DR: 本文提出Landmark (LMK) pooling方法，通过将序列分块并在块间插入地标token，然后对地标token嵌入进行平均池化，解决了传统[CLS]和平均池化在长上下文任务中的系统性弱点。


<details>
  <summary>Details</summary>
Motivation: 现有序列编码器通常使用[CLS] token或平均池化将变长token序列压缩为单个向量，但这些方法存在系统性弱点：[CLS]倾向于将信息集中在序列初始位置，可能无法充分表示分布式证据；平均池化可能稀释显著的局部信号，在短上下文任务中表现更差。

Method: 提出Landmark (LMK) pooling方法：1) 将序列划分为多个块；2) 在块之间插入地标token；3) 通过对地标token的嵌入进行平均池化形成最终表示。这种方法在引入少量特殊token的成本下，改善了长上下文外推能力而不牺牲局部显著特征。

Result: 实证研究表明，LMK pooling在短上下文检索任务上与现有方法表现相当，在长上下文任务上带来显著改进，使其成为现有池化方法的实用且可扩展的替代方案。

Conclusion: LMK pooling通过简单机制解决了传统池化策略的系统性弱点，在保持短上下文性能的同时显著提升了长上下文任务的表现，为序列表示学习提供了更有效的池化方法。
...
TL;DR: 本文提出inversedMixup框架，将Mixup的可控性与LLM的可解释性结合，通过三阶段训练对齐任务模型与LLM的嵌入空间，生成可读的增强文本，并首次实证文本Mixup中的流形入侵现象。
...
Method: 提出inversedMixup框架，采用三阶段训练程序对齐任务特定模型的输出嵌入空间与LLM的输入嵌入空间。成功对齐后，可将具有可控混合比例的混合嵌入重构为人类可解释的增强句子。

Result: 实验证明该方法在少样本和全监督场景中都有效且具有泛化性。首次提供了文本Mixup中流形入侵现象的实证证据，并提出了简单有效的缓解策略。

Conclusion: inversedMixup成功结合了Mixup的可控性和LLM增强的可解释性，通过嵌入空间对齐实现了可读文本增强，并解决了流形入侵问题，在多种场景下表现出色。
...
TL;DR: Note2Chat框架通过将医疗记录转换为高质量医患对话，采用三阶段微调策略和单轮推理范式，显著提升LLM在临床病史采集和诊断中的表现。
...
Result: 该方法显著改善了临床推理能力，相比GPT-4o提升了+16.9 F1分数和+21.0 Top-1诊断准确率。

Conclusion: Note2Chat框架通过利用广泛可得的医疗记录而非稀缺敏感的对话数据，有效提升了LLM在动态临床病史采集和诊断任务中的表现，同时增强了可解释性和样本效率。
...
TL;DR: KromHC使用小双随机矩阵的Kronecker积来参数化残差矩阵，在保证双随机性的同时将参数复杂度从O(n³C)或O(nC·n!)降低到O(n²C)，性能优于现有mHC变体。


<details>
  <summary>Details</summary>
Motivation: 超连接（HC）存在训练不稳定和可扩展性受限的问题，mHC通过将残差连接空间投影到Birkhoff多面体来缓解这些问题，但仍面临两个挑战：1）迭代SK算法不一定产生精确的双随机残差矩阵；2）参数复杂度高达O(n³C)。mHC-lite虽然保证双随机性，但参数复杂度呈阶乘爆炸O(nC·n!)。

Method: 提出KromHC方法，使用小双随机矩阵的Kronecker积来参数化mHC中的残差矩阵。通过对张量化残差流的每个模态上的因子残差矩阵施加流形约束，KromHC保证残差矩阵的精确双随机性，同时将参数复杂度降低到O(n²C)。

Result: 综合实验表明，KromHC在性能上匹配甚至优于最先进的mHC变体，同时需要显著更少的可训练参数。

Conclusion: KromHC通过Kronecker积参数化有效解决了mHC的双随机性和参数复杂度问题，在保持性能优势的同时大幅减少了参数数量，为超连接的实际应用提供了更高效的解决方案。
...
TL;DR: 使用语言模型作为受控统计学习者，系统模拟跨语言影响并分离其驱动因素，验证了语言优势和熟练度对跨语言影响的关键作用。


<details>
  <summary>Details</summary>
Motivation: 人类研究中关于跨语言影响的结果常常相互矛盾，这是由于实验方差导致的。研究旨在通过语言模型作为受控统计学习者来解决这些不一致性，系统模拟跨语言影响并分离其潜在驱动因素。

Method: 使用语言模型模拟跨语言影响，通过控制L2暴露年龄来操纵L2语言熟练度，研究不同句法距离的L1预训练影响，采用跨语言启动范式分析L1结构激活如何影响L2处理。

Result: 结果与心理语言学证据一致：语言优势和熟练度是跨语言影响的强预测因子；语法结构启动是双向的，但不合语法结构的启动对语言优势敏感；L1在L2处理过程中被共同激活并直接影响L2的神经回路。

Conclusion: 语言模型可以作为计算框架来支持人类跨语言影响理论，为跨语言影响提供了机制性证据，展示了L1在L2处理中的共同激活和直接影响。
...
TL;DR: 本文提出ILRR框架，一种无需训练的方法，通过单参考序列指导离散扩散语言模型生成，实现属性控制，计算开销小且效果显著。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型在文本生成方面有潜力，但推理时控制机制研究不足。现有方法包括采样级指导或轨迹优化，需要更有效的控制方法。

Method: 提出ILRR框架：1) 在去噪过程中动态对齐生成序列与参考序列的内部激活；2) 引入空间调制指导，用短参考指导长文本生成；3) 可调节指导强度实现灵活属性控制。

Result: 在LLaDA和MDLM架构上验证，ILRR仅需每个去噪步骤增加一次并行前向传播，计算开销小。在相同计算预算下，属性准确率比基线提高10-60%，同时保持高质量生成。

Conclusion: ILRR为离散扩散语言模型提供了有效的推理时控制框架，通过单参考序列实现属性指导，计算高效且效果显著，为可控文本生成提供了新方法。
...
TL;DR: 提出一种后训练词汇表适配方法，通过选择性替换低效用标记来优化特定领域或语言的子词分词效果


<details>
  <summary>Details</summary>
Motivation: 标准BPE分词器在所有文本数据上统一处理，但在应用到特定领域或语言时存在效率问题，通用标记集会导致编码效率低下

Method: 提出后训练适配策略，基于适配语料库中标记的频率，选择性替换低效用标记为更相关的标记，算法识别出在给定目标词汇表大小下最有效编码适配语料库的标记清单

Result: 在多种语言的生成和分类任务上的广泛实验表明，适配后的分词器在相同词汇表大小下比基线方法更有效地压缩测试语料库

Conclusion: 该方法作为一种轻量级适配机制，类似于词汇表微调过程，能够为特定领域或任务优化分词效果
...
<details>
  <summary>Details</summary>
Motivation: 语言通过一系列语义状态演进，但其底层动力学机制尚不清楚。研究旨在理解文本语义进展的动力学特性，并建立区分人类认知与算法模型生成模式的框架。

Method: 将文本语义进展视为高维状态空间中的随机轨迹，使用精密计量学中的Allan偏差工具，将有序句子嵌入视为位移信号来分析语义稳定性。

Result: 发现两个不同的动力学机制：短时幂律标度（区分创意文学与技术文本）和长时交叉到稳定性限制的噪声基底。大型语言模型能成功模仿人类文本的局部标度统计，但表现出稳定性视野的系统性减少。

Conclusion: 语义连贯性是可测量的物理属性，该框架能够区分人类认知的细微动力学与算法模型生成模式，为理解文本语义演进提供了新的量化方法。
...
TL;DR: RSE（回收搜索经验）是一种无需训练的自引导策略，将测试时搜索从孤立试验转变为累积过程，通过重用中间结论和失败模式来减少计算冗余，提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法在探索解决方案空间时，通常将每次推理过程视为一次性样本，丢弃了有价值的中间洞察。这种系统性无记忆性导致大量计算冗余，模型会重复推导已发现的结论并重新访问已知的死胡同。

Method: 提出RSE策略，通过主动将原始推理轨迹提炼到共享经验库中，实现正回收（重用中间结论以简化冗余推导）和负回收（利用失败模式修剪遇到的死胡同），将测试时搜索从孤立试验转变为累积过程。

Result: 理论分析形式化了RSE的效率增益，验证了其在解决复杂推理任务中相对于独立采样的优势。在HMMT24、HMMT25、IMO-Bench和HLE等数据集上的实验表明，RSE在计算成本相当的情况下持续优于强基线方法，实现了最先进的扩展效率。

Conclusion: RSE通过回收搜索经验，有效解决了测试时扩展中的计算冗余问题，将推理过程从无记忆的重复探索转变为累积学习过程，显著提高了大型语言模型的推理效率和扩展性能。
...
TL;DR: TAPPA框架从时间连续性角度统一解释LLM中的各种注意力模式，将其分为可预测和不可预测两类，并基于此指导KV缓存压缩和模型剪枝等推理加速任务。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然识别了检索头、汇聚头、对角线轨迹等注意力模式，但这些观察是碎片化的，缺乏统一的解释框架。需要从数学层面统一理解这些模式，以指导模型优化和推理加速。

Method: 提出TAPPA（时间注意力模式可预测性分析）框架，从时间连续性角度分析注意力模式的数学公式。将注意力模式分为可预测（有明显规律）和不可预测（看似随机）两类，并通过查询在时间维度上的自相似性来解释这种区分。特别分析了查询、键和RoPE位置编码联合作用下的三种代表性情况。

Result: TAPPA的见解被应用于KV缓存压缩和LLM剪枝任务，基于TAPPA启发的简单指标在这些任务中持续优于基线方法。

Conclusion: TAPPA为理解LLM中的注意力模式提供了统一框架，不仅加深了对注意力行为的理解，还能有效指导推理加速方法的设计。
...
TL;DR: TACLer是一个基于课程强化学习的框架，通过渐进式训练和混合思考/非思考推理模式，在减少计算成本的同时提升LLMs在复杂数学问题上的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在复杂推理任务中通常需要大规模强化学习训练来生成长链式思维，但这会导致过度思考、冗余中间步骤和计算效率低下。需要一种既能提高学习推理效率又能保持甚至增强性能的方法。

Method: 提出TACLer框架，包含两个核心组件：1) 定制化课程学习，根据模型在不同阶段的知识缺失情况渐进增加数据复杂度；2) 混合思考/非思考推理范式，通过启用或禁用思考模式来平衡准确性和效率。

Result: TACLer在学习和推理方面带来双重优势：1) 减少计算成本，训练计算量比长思考模型减少50%以上，推理令牌使用比基础模型减少42%以上；2) 提升准确性，在基础模型上提高9%以上，在四个数学数据集上持续优于最先进的非思考和思考基线模型。

Conclusion: TACLer通过课程强化学习和混合推理范式，有效解决了LLMs在复杂推理任务中的效率与准确性平衡问题，为高效的大语言模型训练和推理提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: ESG可持续性报告对评估至关重要，但存在"漂绿"和模糊声明问题，现有NLP模型缺乏对这些实践的鲁棒性，通常依赖表层模式且泛化能力差
...
Conclusion: 提出的框架能有效提升ESG报告可靠性评估的鲁棒性，为可持续性披露分析提供了更可靠的NLP解决方案
...
TL;DR: 论文提出在语言模型预训练中先使用抽象结构化数据（如形式语言生成的程序数据）进行训练，能显著提升模型算法能力，加速后续自然语言预训练，达到相同效果只需更少数据。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型主要在网页规模语料上进行预训练，但人类学习通常从简单逻辑和数学开始，再学习复杂语义知识。研究探索是否让模型先接触抽象结构化数据能促进后续语义知识获取。

Method: 使用形式语言和其他简单算法生成的程序数据作为抽象结构化数据源。首先诊断不同程序数据能提升的算法技能；然后研究在更大模型（达13亿参数）上的效果；最后探索程序预训练在注意力层和MLP层中建立的机制。

Result: 程序预训练显著提升算法能力：在上下文回忆任务上，使用Dyck序列（平衡括号）预训练后准确率从10%提升到98%。仅使用0.1%程序数据预训练就优于标准自然语言、代码和数学预训练。程序预训练使模型只需原数据量的55-86%就能达到相同损失值。程序预训练在注意力层和MLP层都建立了非平凡结构。

Conclusion: 程序预训练是改进语言模型性能和加速预训练的简单轻量方法，表明在大型语言模型中分离知识获取和推理具有潜力，类似于人类从简单逻辑开始的学习过程。
...
TL;DR: 提出CE-GOCD方法，通过建模学术知识图谱中的语义子结构来增强LLM在科学问答中的表现，优于现有检索增强基线


<details>
  <summary>Details</summary>
Motivation: 现有检索增强方法通常依赖孤立的文本块或概念，忽视了论文间更深层的语义联系，这限制了LLM对科学文献的理解，影响了回答的全面性和针对性

Method: CE-GOCD方法：1) 以论文标题为中心实体进行定向子图检索；2) 通过子图剪枝和补全增强隐式语义发现；3) 应用社区检测提炼具有共同主题的连贯论文群组

Result: 在三个NLP文献问答数据集上评估，结果显示该方法优于其他检索增强基线方法，证实了框架的有效性

Conclusion: 通过显式建模和利用学术知识图谱中的语义子结构，CE-GOCD方法能够有效增强LLM在科学问答中的表现，解决了现有方法忽视深层语义联系的问题
...
TL;DR: ChatGPT在医学信息抽取任务上表现不如微调模型，但能提供高质量解释，存在过度自信问题，对原始文本忠实度高，生成不确定性影响信息抽取结果。


<details>
  <summary>Details</summary>
Motivation: 评估ChatGPT在医学信息抽取任务中的整体能力，包括性能、可解释性、置信度、忠实度和不确定性等方面，以了解其在医疗NLP应用中的潜力与局限。

Method: 在6个基准数据集上对ChatGPT进行4种不同医学信息抽取任务的系统性评估，测量其性能、可解释性、置信度、忠实度和不确定性等指标。

Result: 1) ChatGPT在MedIE任务上的性能得分低于微调基线模型；2) ChatGPT能提供高质量决策解释但存在过度自信问题；3) 大多数情况下对原始文本忠实度高；4) 生成不确定性导致信息抽取结果不确定性，可能阻碍其在MedIE任务中的应用。

Conclusion: ChatGPT在医学信息抽取任务中表现有限，虽然具有高质量解释能力和文本忠实度，但性能不足、过度自信和生成不确定性限制了其在医疗信息抽取领域的实际应用价值。
...
TL;DR: Zonkey是一种分层扩散模型，通过完全可训练的从原始字符到文档级表示的流程，解决了传统LLM中固定不可微分词器的限制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型受限于固定、不可微的分词器（如BPE），这阻碍了端到端优化和对噪声或领域特定数据的适应性。

Method: 引入可微分分词器（Segment Splitter）学习概率性序列开始决策，通过概率注意力机制实现软掩码，使用分层压缩序列到更高抽象级别，并通过去噪扩散混合模型进行重建。

Result: 在Wikipedia上端到端训练的Zonkey能够从噪声生成连贯、可变长度的文本，展示了涌现的分层结构，与基于熵的可学习分词器相比有更好的定性对齐。

Conclusion: 该方法朝着完全基于梯度的LLM迈进，具有更好的领域适应性和可扩展生成潜力。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统结合密集检索器和语言模型，但在高风险领域部署时，组件间交互的不透明性带来挑战。需要理解检索器和生成器如何相互作用，以确保输出质量。

Method: 提出RAG-E框架：1) 采用集成梯度方法分析检索器；2) 引入PMCSHAP（蒙特卡洛稳定的Shapley值近似）进行生成器归因；3) 提出WARG（加权归因-相关性差距）指标，衡量生成器文档使用与检索器排名的对齐程度。

Result: 在TREC CAsT和FoodSafeSum数据集上的实证分析显示：47.4%-66.7%的查询中，生成器忽略检索器排名最高的文档；48.1%-65.9%的查询中，生成器依赖相关性较低的文档。这表明RAG输出质量不仅取决于单个组件性能，更取决于它们的相互作用。

Conclusion: RAG-E框架能够审计RAG系统中检索器与生成器之间的对齐问题，揭示了关键的对齐失败模式，为高风险领域部署提供了重要的可解释性工具。
...
TL;DR: DARE方法通过利用完整的经验rollout分布而非单一多数投票结果，提供更丰富、更稳健的奖励估计，显著提升了测试时强化学习在推理任务中的性能。
...
Conclusion: 利用完整的rollout分布而非单一多数投票结果进行奖励估计，能够提供更丰富、更稳健的学习信号，从而有效提升测试时强化学习在无监督输入上的自我改进能力。
...
TL;DR: MilSCORE是首个基于军事规划场景的专家编写多跳问答数据集，用于评估LLM在长上下文、多模态信息下的空间推理和规划能力。


<details>
  <summary>Details</summary>
Motivation: 随着LLM处理更长更复杂任务的需求增长，需要真实的长上下文基准测试来评估模型在选择性阅读和整合异构多模态信息方面的能力，特别是在需要快速准确推理地图、命令、情报报告等分布式数据的军事规划等地理空间规划问题中。

Method: 提出了MilSCORE数据集，包含基于复杂模拟军事规划场景的专家编写多跳问题，涵盖七类问题类型，针对事实回忆和多步推理（约束、策略、空间分析），并提供了评估协议和多种当代视觉语言模型的基线结果。

Result: 基线结果显示当前系统在MilSCORE上仍有很大提升空间，表明现有模型在处理真实场景级长上下文规划方面存在困难，MilSCORE成为未来工作的挑战性测试平台。

Conclusion: MilSCORE填补了长上下文多模态基准测试的空白，特别是针对高风险的军事规划决策，为评估LLM在复杂地理空间环境中的推理能力提供了重要工具，揭示了当前模型的局限性并指明了未来研究方向。
...
TL;DR: 提出一种基于重写的自适应距离学习算法，用于检测LLM生成内容，在100多个实验设置中优于基线方法


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成高度类人文本引发对虚假信息和学术诚信的担忧，需要可靠算法检测LLM生成内容

Method: 首先提出几何方法解析基于重写的检测算法，然后引入自适应学习原始文本与重写文本之间距离的新算法

Result: 理论证明自适应距离函数比固定距离更有效；实验显示在多数场景中优于基线，对GPT、Claude、Gemini等目标LLM相对改进57.8%-80.6%

Conclusion: 自适应学习距离的基于重写检测算法能有效识别LLM生成内容，为解决虚假信息和学术诚信问题提供可靠方案
...
TL;DR: SONIC是一个基于学习的KV缓存压缩框架，通过将历史对话段压缩为紧凑的Nexus令牌来解决多轮对话中KV缓存线性增长的问题，支持动态预算训练以适应不同内存约束。


<details>
  <summary>Details</summary>
Motivation: 多轮LLM部署中KV缓存的线性增长是主要瓶颈，现有KV缓存压缩方法通常忽略多轮对话的结构特性，依赖启发式淘汰策略可能导致关键上下文丢失。

Method: 提出SONIC学习框架，将历史对话段压缩为紧凑且语义丰富的Nexus令牌，采用动态预算训练机制，无需重新训练即可灵活适应不同内存约束。

Result: 在80%和50%的压缩比下，SONIC在四个多轮基准测试中持续优于H2O和StreamingLLM等基线方法；在MTBench101基准上，平均得分比最先进基线提高35.55%；整体推理过程加速50.1%。

Conclusion: SONIC通过基于学习的KV缓存压缩有效维持多轮对话的连贯性，显著提升部署效率，为多轮LLM部署提供了高效的解决方案。
...
TL;DR: 提出基于GPT的选择性微调架构，用于临床文本分类，通过冻结大部分参数仅训练最后一层，在减少计算成本的同时保持对复杂临床语言的建模能力。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中非结构化临床叙述的增加为疾病表征、队列识别和临床决策支持提供了新机会，但建模长领域特定临床文本面临标注数据有限、类别严重不平衡以及大型预训练语言模型适应成本高的挑战。

Method: 提出基于GPT的临床文本分类架构，采用选择性微调策略：冻结GPT-2主干的大部分参数，仅训练最后的Transformer块、最后一层归一化和轻量级分类头，大幅减少可训练参数的同时保留复杂临床语言的表征能力。

Result: 在MIMIC-IV-Note数据集的放射学报告上进行评估，使用基于报告文本的不确定性感知CheXpert风格标签。实验涵盖多种问题表述，包括放射学发现的多标签分类、不同不确定性假设下的二元标签分类以及聚合疾病结果预测。在不同数据集规模下，模型表现出稳定的收敛行为和强大的分类性能，特别是在非提及和否定发现占主导的场景中。
...
Result: 在Web问答和数学推理任务上的实验表明，OVD显著优于现有方法：Web Q&A任务平均EM提升高达+12.9%，数学基准测试提升高达+25.7%(仅使用一个随机样本训练)，同时表现出优越的训练效率。

Conclusion: OVD通过轨迹匹配和离散语言评分有效解决了传统知识蒸馏的内存瓶颈和探索限制问题，为高效知识转移提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生幻觉，生成与输入不一致的内容。现有的RAG和RLHF方法需要大量资源，而解码方法虽然轻量但缺乏明确的幻觉控制机制。

Method: 提出Token-Guard方法，包含三个核心机制：1) 在每个推理步骤进行内部验证，检测幻觉token；2) 在潜在空间中对候选片段进行显式幻觉风险评分；3) 通过迭代剪枝和再生动态纠正检测到的错误。

Result: 在HALU数据集上的实验表明，Token-Guard显著减少了幻觉并提高了生成准确性，为可靠的LLM输出提供了可扩展的模块化解决方案。

Conclusion: Token-Guard提供了一种轻量级、可扩展的幻觉控制方法，通过token级自检和动态修正机制，有效提升LLM输出的可靠性，代码已公开。
...
TL;DR: MDA框架使用影响函数将可解释电路溯源到训练数据，通过干预高影响力样本验证了可解释头的因果起源，发现重复性结构数据是机制催化剂，并建立了归纳头与上下文学习的功能联系。


<details>
  <summary>Details</summary>
Motivation: 尽管机制可解释性已在LLMs中识别出可解释电路，但这些电路在训练数据中的因果起源仍然难以捉摸。需要建立从模型内部机制到训练数据的可追溯框架。

Method: 提出机制数据归因（MDA）框架，利用影响函数将可解释单元溯源到特定训练样本。在Pythia模型家族上进行实验，通过定向干预（移除或增强高影响力样本）验证因果关系。

Result: 定向干预显著调节可解释头的出现，而随机干预无效果。发现重复性结构数据（如LaTeX、XML）是机制催化剂。干预归纳头形成会同时改变模型的上下文学习能力，为归纳头与ICL的功能联系提供了直接因果证据。

Conclusion: MDA框架成功建立了从模型内部机制到训练数据的因果联系，提出的机制数据增强管道能持续加速不同规模模型的电路收敛，为引导LLMs发展轨迹提供了原则性方法。
...
TL;DR: 本文提出了一种针对LLM应用的评估驱动工作流（定义、测试、诊断、修复），并介绍了最小可行评估套件（MVES），通过实验展示了通用提示模板可能在不同任务间产生性能权衡。


<details>
  <summary>Details</summary>
Motivation: LLM应用评估与传统软件测试不同，因为其输出具有随机性、高维性，且对提示和模型变化敏感。需要一种系统化的评估方法来应对这些挑战。

Method: 提出了评估驱动的工作流程（定义、测试、诊断、修复）和最小可行评估套件（MVES），包含通用LLM应用、检索增强生成（RAG）和智能体工具使用工作流的评估组件。使用自动化检查、人工评分和LLM作为评判者等方法，并在本地实验中测试了不同提示模板的效果。

Result: 实验发现通用"改进"提示模板可能在不同行为间产生权衡：在Llama 3上，用通用规则替换任务特定提示后，提取通过率从100%降至90%，RAG合规性从93.3%降至80%，但指令遵循能力有所改善。

Conclusion: 研究结果支持评估驱动的提示迭代和谨慎的声明校准，而非通用的提示配方。所有测试套件、工具和结果都已开源以确保可复现性。
...
TL;DR: CARD（因果自回归扩散）是一个新框架，将ARMs的训练效率与扩散模型的高吞吐量推理统一起来，通过因果注意力掩码实现密集的单次前向监督，支持动态并行解码。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练效率和推理吞吐量之间存在权衡：自回归模型（ARMs）训练高效但推理慢，扩散模型推理快但训练成本高。需要一种能同时获得训练效率和推理速度优势的框架。

Method: CARD在严格因果注意力掩码内重新表述扩散过程，实现单次前向传递的密集监督。引入软尾掩码方案保留局部上下文，以及基于信噪比原理的上下文感知重加权机制。支持动态并行解码，利用KV缓存自适应生成变长标记序列。

Result: CARD在性能上优于现有离散扩散基线，同时相比块扩散方法减少了3倍的训练延迟。实现了ARM级别的数据效率，同时获得了并行生成的延迟优势。

Conclusion: CARD建立了一个稳健的下一代高效LLMs范式，统一了训练效率和推理吞吐量，为高效语言模型提供了有前景的解决方案。
...
Method: 提出Leviathan架构，使用连续嵌入生成器替代传统的离散查找表，在等参数设置下进行评估

Result: 在Pile数据集上，Leviathan持续优于标准LLaMA风格架构；通过经验幂律拟合，Leviathan展现出显著优越的有效参数容量，相当于密集模型参数量的1.47到2.11倍

Conclusion: Leviathan架构通过更优的参数分配策略，在小语言模型中实现了更高的参数效率，挑战了传统Transformer参数可互换的假设
...
<details>
  <summary>Details</summary>
Motivation: 指令跟随旨在通过指定明确约束来对齐大语言模型与人类意图，但研究发现这反而会干扰模型的任务解决能力，需要量化这种反直觉现象

Method: 提出SUSTAINSCORE指标，测量在指令中插入自明约束（从原始成功输出中提取）后任务性能的下降；在数学、多跳QA和代码生成任务上测试当前LLMs；分析失败模式并研究干扰机制

Result: 添加自明约束导致性能显著下降，即使是Claude-Sonnet-4.5等先进模型也受影响；失败案例对约束分配了显著更多注意力；干扰现象在不同约束类型和规模上具有普遍性

Conclusion: 指令跟随可能干扰LLMs的任务解决能力，SUSTAINSCORE可用于评估不同后训练范式对干扰的影响，为当前对齐策略提供实证观察
...
TL;DR: MasalBench是一个评估大语言模型对波斯谚语上下文理解和跨文化理解的基准，发现模型在识别波斯谚语方面表现良好（准确率>0.90），但在识别等效英语谚语方面表现较差（最佳模型0.79），揭示了当前LLM在文化知识和类比推理方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型已成为日常生活的重要组成部分，需要掌握对话语言规则以有效与用户沟通。虽然先前研究评估了LLM在高资源语言中比喻语言的理解能力，但它们在低资源语言中的表现仍未被充分探索。

Method: 引入MasalBench基准，用于评估LLM对波斯谚语的上下文和跨文化理解。评估了八个最先进的LLM，测试它们在识别波斯谚语上下文和识别等效英语谚语方面的能力。

Result: LLM在识别波斯谚语上下文方面表现良好，准确率超过0.90；但在识别等效英语谚语方面表现显著下降，最佳模型仅达到0.79准确率。

Conclusion: 研究结果突显了当前LLM在文化知识和类比推理方面的局限性，并为评估其他低资源语言的跨文化理解提供了框架。MasalBench基准已开源可用。
...
Conclusion: VTC-R1作为一种可扩展的推理密集型应用解决方案，通过视觉-文本压缩显著提高了推理效率，同时保持了性能优势
...
TL;DR: 提出Fed-MedLoRA框架，通过联邦学习在医疗领域适配大语言模型，解决传统联邦学习在医疗LLM应用中的通信开销和数据异质性问题。


<details>
  <summary>Details</summary>
Motivation: 当前医疗大语言模型通常基于单一机构数据训练，存在泛化性和安全性限制。传统联邦学习应用于医疗LLM面临两大挑战：1) 传输完整模型参数导致通信开销巨大；2) 假设数据同质性，而实际医疗数据高度异质。

Method: 提出Fed-MedLoRA框架：1) 仅传输低秩适配器参数，减少通信和计算开销；2) Fed-MedLoRA+进一步引入自适应、数据感知的聚合机制，提升在跨站点异质性下的收敛性能。应用于临床信息抽取任务。

Result: 在五个患者队列中评估准确性，与BERT、LLaMA-3、DeepSeek-R1、GPT-4o等模型比较。评估设置包括：1) 域内训练和测试；2) 独立队列的外部验证；3) 使用耶鲁纽黑文医疗系统真实临床笔记的低资源新站点适应场景。

Conclusion: Fed-MedLoRA框架为医疗大语言模型的联邦学习提供了一种模型无关且参数高效的解决方案，能够有效应对通信开销和数据异质性挑战，在临床信息抽取任务中展现出良好性能。
...
TL;DR: PIR提出主动交互式推理新范式，让LLM从被动求解转变为主动询问，通过用户交互解决前提和意图层面的不确定性，显著提升推理性能并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有CoT提示的LLMs存在"盲目自我思考"的根本局限：即使在关键信息缺失或模糊时仍进行大量内部推理。需要让LLM能够主动与用户交互澄清不确定性。

Method: 提出PIR范式，包含两个核心组件：1) 不确定性感知的监督微调，赋予模型交互推理能力；2) 基于用户模拟器的策略优化框架，使用复合奖励对齐用户意图。

Result: 在数学推理、代码生成和文档编辑任务上，PIR显著优于基线方法，准确率提升达32.70%，通过率提升22.90%，BLEU提升41.36，同时减少近一半的推理计算和不必要的交互轮次。

Conclusion: PIR将LLM从被动求解器转变为主动询问者，有效解决了前提和意图层面的不确定性，在多个任务上展现出强大的泛化能力和鲁棒性，为LLM推理提供了新范式。
...
TL;DR: 该论文提出FineInstructions方法，通过将互联网规模的预训练文档转化为数十亿个合成指令-答案训练对，使LLM可以从头开始仅使用指令调优目标进行预训练，从而更好地对齐下游使用场景。


<details>
  <summary>Details</summary>
Motivation: 由于监督训练数据有限，大型语言模型通常先通过自监督的"预测下一个词"目标在大量无结构文本上进行预训练，然后再用较少的指令调优数据进行训练。为了克服监督数据不足的问题，需要一种方法能将预训练文档中的知识转化为大规模的合成指令训练数据。

Method: 提出FineInstructions方法：1）从真实用户查询和提示中创建约1800万个指令模板；2）将这些指令模板与无结构预训练语料库中的人类撰写源文档进行匹配和实例化；3）生成大规模的合成监督训练数据，使LLM可以仅使用指令调优目标从头开始预训练。

Result: 在控制性的token-for-token训练实验中，使用FineInstructions进行预训练的表现优于标准预训练和其他提出的合成预训练技术，在衡量自由形式响应质量的标准基准测试中取得更好结果。

Conclusion: FineInstructions方法能够有效利用预训练文档中的知识生成大规模合成指令数据，使LLM预训练更加符合下游使用场景，从而提升模型在响应质量方面的表现。
...
TL;DR: 提出HALO蒸馏流程将Transformer转换为RNN-注意力混合模型，以及HypeNet混合架构，通过少量数据（2.3B tokens）实现高效转换，保持性能的同时获得更好的长上下文处理能力和效率


<details>
  <summary>Details</summary>
Motivation: 现有的混合Transformer架构（结合softmax注意力和RNN）在长上下文建模中展现出良好的性能-吞吐量权衡，但大规模从头预训练成本过高。现有的转换方法需要大量训练数据（超过100亿tokens），且生成的混合模型在长上下文场景下表现不佳，而这正是混合模型相比纯Transformer模型具有显著推理加速优势的场景

Method: 1. 提出HALO（Hybrid Attention via Layer Optimization）蒸馏流程，用于将Transformer模型蒸馏为RNN-注意力混合模型；2. 提出HypeNet混合架构，采用新颖的位置编码方案HyPE和各种架构修改，实现优越的长度泛化能力；3. 使用HALO将Qwen3系列转换为HypeNet模型

Result: 仅需2.3B tokens（不到原始预训练数据的0.01%）即可完成转换，转换后的混合模型性能与原始Transformer模型相当，同时具有优越的长上下文性能和效率

Conclusion: HALO蒸馏流程和HypeNet架构提供了一种高效的方法，能够以极低的训练成本将现有Transformer模型转换为混合模型，在保持性能的同时显著提升长上下文处理能力和推理效率
