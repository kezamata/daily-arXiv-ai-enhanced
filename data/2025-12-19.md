...
TL;DR: 提出了一种结合规则掩码变形与GAN图像转换的两步生成数据增强框架，用于解决遮挡人脸检测与识别中的数据稀缺和分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 数据稀缺和分布偏移是遮挡人脸检测与识别面临的主要挑战，需要生成超越纯合成变换的现实遮挡人脸样本。

Method: 采用两步生成数据增强框架：1) 规则掩码变形；2) 使用GAN进行非配对图像到图像转换。引入非掩码保持损失和随机噪声注入来稳定训练并增强样本多样性。

Result: 相比单独使用规则变形，该方法在定性上获得一致改进，并补充了现有的GAN基遮挡人脸生成方法（如IAMGAN）。实验验证了所提组件的有效性。

Conclusion: 该框架为面向人脸识别任务的数据中心增强提供了有效方法，并指出了未来改进方向。
...
TL;DR: JARVIS是一个基于JEPA框架的自监督视觉增强方法，用于提升多模态大语言模型的视觉理解能力，通过整合I-JEPA学习范式来减少对语言监督的依赖。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基础视觉推理任务上表现有限，主要原因是它们主要从文本描述中学习视觉理解，而文本描述是主观且不完整的监督信号。此外，多模态指令调优的规模相对较小，导致模型过度依赖语言先验而忽略视觉细节。

Method: 引入JARVIS框架，将I-JEPA学习范式整合到标准视觉-语言对齐流程中。使用冻结的视觉基础模型作为上下文和目标编码器，训练预测器（实现为LLM的早期层）从图像中学习结构和语义规律，而不完全依赖语言监督。

Result: 在标准MLLM基准测试上的广泛实验表明，JARVIS能够持续提升不同LLM家族在视觉中心基准上的性能，同时不会降低多模态推理能力。

Conclusion: JARVIS通过自监督视觉增强有效解决了MLLMs过度依赖语言监督的问题，显著提升了视觉理解能力，为多模态模型的发展提供了新方向。
...
TL;DR: 本文提出了稀疏接地视觉导航任务和CityNav基准，用于评估多模态大语言模型在真实城市导航中的顺序决策能力，发现现有方法表现不佳，并提出Verbalization of Path方法显著提升导航成功率。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在具身智能体开发方面具有潜力，但现有评估基准主要关注语言中心或模拟环境，缺乏对实际复杂场景中知识密集型推理能力的测试。需要填补这一空白，评估MLLMs在真实世界环境中的顺序决策能力。

Method: 提出了稀疏接地视觉导航任务，并构建了CityNav基准，涵盖四个全球城市，要求智能体仅依赖视觉输入和内部多模态推理在50+决策点上顺序导航。为解决现有方法表现不佳的问题，提出了Verbalization of Path方法，通过从MLLMs中提取显式认知地图（关键地标和朝向目的地的方向）来明确接地智能体的内部推理。

Result: 通过广泛评估发现，当前最先进的多模态大语言模型和标准推理技术（如思维链、反思）在这一挑战性设置中表现显著不佳。提出的Verbalization of Path方法能够显著提升导航成功率。

Conclusion: 稀疏接地视觉导航任务和CityNav基准为评估MLLMs在真实世界环境中的顺序决策能力提供了重要工具。现有方法在这一挑战性任务中表现不足，而Verbalization of Path方法通过显式接地推理过程，能够有效提升导航性能，为开发更强大的具身智能体提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 受人类四维感知推理能力的启发，研究者希望让视觉语言模型具备类似的结构化内部表示能力，能够编码语义、空间布局和时间动态，从而实现回忆过去事件、推断未观察状态和上下文相关推理。

Method: R4框架持续构建4D知识数据库，将对象级语义描述锚定在度量空间和时间中，形成持久的世界模型。推理时，自然语言查询被分解为语义、空间和时间键来检索相关观察，然后整合到视觉语言模型的推理过程中。

Result: 在具身问答和导航基准测试中，R4在时空信息检索和推理方面显著优于基线方法，展示了无需训练的4D推理能力。

Conclusion: R4为动态环境中的具身4D推理提供了新范式，通过结构化终身记忆系统使视觉语言模型能够进行情节式和协作式推理。
...
TL;DR: 该论文提出了"感知观测站"框架，用于系统评估多模态大语言模型的视觉感知能力，超越传统的端到端任务准确率评估，关注模型在扰动下的鲁棒性、归因保真度和推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然能力不断增强，但其感知能力缺乏系统表征。大多数模型家族在扩展语言组件的同时重用几乎相同的视觉编码器，这引发了一个关键问题：模型的进步是真正基于视觉基础，还是主要依赖互联网规模的文本世界知识。现有评估方法过于强调端任务准确率，忽视了鲁棒性、归因保真度和受控扰动下的推理能力。

Method: 提出了"感知观测站"框架，从多个垂直维度评估MLLMs：(1)简单视觉任务，如人脸匹配和视觉文本理解能力；(2)局部到全局理解，包括图像匹配、网格指向游戏和属性定位，测试一般视觉基础。每个维度都使用真实数据集（人脸和文字），并通过基于像素的增强和基于扩散的风格化幻觉进行系统性扰动。

Result: 该框架超越了排行榜准确率评估，提供了关于MLLMs如何在扰动下保持感知基础和关系结构的深入见解，为分析当前和未来模型的优势和弱点提供了原则性基础。

Conclusion: 感知观测站框架为系统评估多模态大语言模型的视觉感知能力提供了新的方法论，能够更全面地理解模型在视觉基础方面的真实能力，而不仅仅是依赖语言知识的表面性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单人类场景的自我中心视角行为预测，但许多机器人应用需要从第三人称视角理解多人类行为，缺乏相关数据集和方法。

Method: 提出CAMP-VLM框架，整合视觉输入的上下文特征和场景图的空间感知；使用光真实感模拟器生成合成数据，通过监督微调(SFT)和直接偏好优化(DPO)进行训练。

Result: CAMP-VLM在预测准确率上比最佳基线方法提升高达66.9%，在合成和真实世界序列上都表现出良好的泛化能力。

Conclusion: CAMP-VLM框架有效解决了从第三人称视角预测多人类行为的挑战，通过整合视觉上下文和空间感知，在缺乏合适数据集的情况下展现了良好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测在自动驾驶和监控等安全关键应用中至关重要，但标注数据的稀缺限制了深度检测器的训练。文本类别信息可以作为有价值的语义监督源，而视觉语言模型在计算机视觉中的成功启发了其在少样本多光谱检测中的应用探索。

Method: 将两种代表性的VLM检测器（Grounding DINO和YOLO-World）适配到多光谱输入，提出有效机制整合文本、视觉和热模态信息，利用大规模VLM学习到的语义先验知识。

Result: 在FLIR和M3FD两个多光谱图像基准测试中，VLM检测器在少样本场景下显著优于使用可比数据训练的专用多光谱模型，在全监督设置下也达到竞争性或更优的结果。

Conclusion: 大规模视觉语言模型学习到的语义先验知识能够有效迁移到未见的光谱模态，为数据高效的多光谱感知提供了强大途径，展示了VLM在少样本多光谱目标检测中的巨大潜力。
...
TL;DR: 当前视觉语言模型在农业分类任务中表现不佳，零样本性能远低于专用监督模型，多选提示效果优于开放提示，评估方法显著影响结果排名


<details>
  <summary>Details</summary>
Motivation: 评估视觉语言模型作为通用解决方案在农业决策支持中的可靠性，了解其在农业分类任务中的实际表现

Method: 在AgML集合的27个农业分类数据集（162个类别）上对开源和闭源VLM进行基准测试，比较零样本性能与专用监督模型，分析不同提示策略和评估方法的影响

Result: VLM在所有任务中表现远低于专用监督模型YOLO11；多选提示下最佳模型准确率约62%，开放提示准确率低于25%；LLM语义评判可提升开放提示准确率；开源模型中Qwen-VL-72B表现最佳；植物和杂草分类比病虫害识别更容易

Conclusion: 当前现成的VLM尚不适合作为独立的农业诊断系统，但可作为辅助组件与约束接口、明确标签本体和领域感知评估策略结合使用
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：两阶段流水线限制了跨模态信息紧密耦合共享，或者依赖单模态扩散模型适应联合分布无法充分利用预训练视频知识。视频扩散模型通常缺乏动作标注，限制了其在机器人策略学习中的应用。
...
Conclusion: 该方法为利用大规模视频数据进行机器人学习提供了一个可扩展的框架，通过自动提供动作标注克服了视频扩散模型在机器人策略学习中的应用限制
...
<details>
  <summary>Details</summary>
Motivation: 单壳低角分辨率dMRI估计FOD精度有限，多壳高角分辨率dMRI需要长时间扫描，扩散模型在从LAR-FOD估计HAR-FOD方面有潜力，但SH系数数量大导致高效生成困难

Method: 提出3D多通道补丁扩散模型，设计FOD补丁适配器引入先验脑解剖知识，体素级条件协调模块增强全局理解，SH注意力模块学习SH系数的复杂相关性

Result: 实验结果显示该方法在HAR-FOD预测中取得最佳性能，优于其他最先进方法

Conclusion: 提出的3D多通道补丁扩散模型能够有效从LAR-FOD预测HAR-FOD，解决了SH系数数量大带来的挑战，在性能和效率上均有优势
...
TL;DR: 本文提出了一种自动词汇3D目标检测方法(AV3DOD)，无需用户指定类别即可自动生成检测对象的类别名称，在ScanNetV2和SUNRGB-D数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇3D目标检测方法虽然在训练时能处理未见类别，但在训练和推理时仍依赖用户指定的类别。本文旨在研究真正的自动词汇3D目标检测，即无需任何用户输入就能自动生成检测对象的类别名称。

Method: 提出了语义评分(SS)来评估生成类别名称的质量，并开发了AV3DOD框架。该框架利用2D视觉语言模型通过图像描述、伪3D框生成和特征空间语义扩展来生成丰富的语义候选。

Result: AV3DOD在ScanNetV2和SUNRGB-D数据集上实现了最先进的定位性能(mAP)和语义质量(SS)。在ScanNetV2上，相比最先进的CoDA方法，整体mAP提升了3.48，SS相对提升了24.5%。

Conclusion: 本文提出的AV3DOD框架在自动词汇3D目标检测方面取得了突破性进展，不仅实现了准确的3D定位，还能自动生成高质量的语义类别名称，为真正的开放词汇3D场景理解奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成方面取得了显著进展，但生成过程计算密集且耗时，限制了实际应用。需要开发高效的加速框架来降低计算成本并提高生成效率。
...
Result: 在多个视频生成模型上的实验表明，TurboDiffusion在单张RTX 5090 GPU上实现了100-200倍的加速，同时保持了可比的视频质量。

Conclusion: TurboDiffusion是一个高效的视频生成加速框架，通过创新的注意力机制、蒸馏技术和量化方法，显著提升了扩散模型的生成速度，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 相机标定是摄影测量和3D视觉应用中的关键步骤，现有方法需要复杂的标定环境和相机运动，限制了标定的灵活性和效率。本文旨在开发一种更可靠、可控且灵活的相机标定方法。
...
Conclusion: 提出的基于准直仪系统的相机标定方法通过角度不变性约束简化了标定过程，实现了无需相机运动的灵活快速标定，为摄影测量和3D视觉应用提供了新颖的标定解决方案。
...
TL;DR: C-DGPA是一种用于无监督域适应的类中心双对齐生成提示适应方法，通过双分支架构同时优化边际分布对齐和条件分布对齐，在多个基准测试中取得最先进结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的提示调优方法在无监督域适应任务中主要关注边际分布对齐，但忽略了条件分布差异，导致类原型错位和语义判别性下降等问题。

Method: 提出C-DGPA方法，包含两个分支：1）边际分布对齐分支采用动态对抗训练框架；2）条件分布对齐分支引入类映射机制来标准化语义提示理解并防止源域过拟合。通过协同优化将领域知识整合到提示学习中。

Result: 在OfficeHome、Office31和VisDA-2017三个基准数据集上进行广泛实验，C-DGPA在所有基准测试中都取得了新的最先进结果。

Conclusion: C-DGPA通过双对齐策略有效整合领域知识到提示学习中，确保获得领域不变且语义可判别的表示，显著提升了无监督域适应性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统相机在端到端驾驶中存在领域差距问题，当训练数据条件与部署环境不匹配时性能大幅下降，特别是日夜间光照差异问题。

Method: 提出使用事件相机替代传统相机，研究其在日夜间光照差异领域差距中的表现，并与灰度帧进行对比分析。

Result: 事件相机在不同光照条件下保持更一致的性能，其领域偏移惩罚通常与灰度帧相当或更小，在跨领域场景中提供更优越的基线性能。

Conclusion: 事件相机作为传统相机的潜在替代方案，能够有效应对自动驾驶中的日夜间光照差异领域差距问题，无需额外调整即可在不同光照条件下保持稳定性能。
...
TL;DR: Avatar4D是一个可迁移的合成人体运动数据集生成管道，专注于领域特定应用，提供对姿态、外观、视角和环境的细粒度控制，无需人工标注。作者创建了Syn2Sport体育数据集验证其效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注日常通用动作，灵活性有限，难以满足特定领域（如体育）的需求。体育运动中特定的人体动作和运动模式对运动理解提出了独特挑战，需要可定制、可扩展的合成数据集。
...
Conclusion: Avatar4D展示了在不依赖领域特定真实数据的情况下，生成可扩展、可控且可迁移的人体数据集用于多样化领域特定任务的潜力，为特定应用提供了灵活的合成数据解决方案。
...
TL;DR: VALOR提出了一种基于强化学习的后对齐框架，通过两阶段训练改善医学视觉语言模型，提升放射学报告生成的视觉基础和临床准确性。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型在生成放射学报告时存在视觉基础不足和临床准确性不高的问题，主要源于视觉与语言表示之间的跨模态对齐不佳，导致幻觉现象。现有方法依赖大规模标注数据、昂贵的任务特定偏好数据或检索方法，但未能有效解决跨模态对齐问题。

Method: VALOR采用基于强化学习的后对齐框架，使用组相对近端优化（GRPO）。训练分为两个阶段：第一阶段通过文本奖励改进医学视觉语言模型，鼓励使用临床精确术语；第二阶段将文本基础模型的视觉投影模块与疾病发现对齐，引导注意力集中在与诊断任务最相关的图像区域。

Result: 在多个基准测试上的广泛实验表明，VALOR显著提高了事实准确性和视觉基础性，在放射学报告生成任务上相比最先进方法取得了显著的性能提升。

Conclusion: VALOR通过创新的强化学习后对齐框架有效解决了医学视觉语言模型在放射学报告生成中的视觉基础和临床准确性问题，为自动化医疗工作流程提供了更可靠的解决方案。
...
TL;DR: 该论文提出了一种基于信息几何的3D点云形状分析方法，通过将点云表示为统计流形上的高斯混合模型，并引入具有理论保证上下界的修正对称KL散度，解决了传统几何度量和现有KL近似方法在点云比较中的局限性。


<details>
  <summary>Details</summary>
Motivation: 三维点云在多个领域有重要应用，但点云比较面临挑战：点云的非结构化特性、复杂几何表面表示困难。传统几何度量（如Hausdorff和Chamfer距离）无法捕捉全局统计结构且对异常值敏感，而现有的高斯混合模型KL散度近似可能产生无界或数值不稳定的值。

Method: 1. 将点云表示为统计流形上的高斯混合模型；2. 证明GMM空间构成统计流形；3. 提出修正对称KL散度，具有理论保证的上下界，确保所有GMM比较的数值稳定性。

Result: 在人体姿态判别（MPI-FAUST数据集）和动物形状比较（G-PCD数据集）的实验中，MSKL提供了稳定且单调变化的数值，直接反映几何变化，性能优于传统距离和现有KL近似方法。

Conclusion: 提出的信息几何框架为3D点云形状分析提供了理论基础，修正对称KL散度解决了传统方法的局限性，为点云比较提供了数值稳定且几何意义明确的度量方法。
...
<details>
  <summary>Details</summary>
Motivation: 图像在互联网中占据重要比例，高效压缩对于减少存储和带宽需求至关重要。研究旨在探索奇异值分解和低秩矩阵近似在图像压缩中的潜力。
...
TL;DR: ARMFlow是一个基于MeanFlow的自回归框架，用于3D人体反应生成，解决了高保真度、实时推理和在线场景自回归适应性三大挑战，在性能和速度上都超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 3D人体反应生成面临三个主要挑战：高运动保真度、实时推理能力以及在线场景的自回归适应性。现有方法无法同时满足这三个要求，需要一种能够兼顾性能、速度和适应性的新方法。
...
Result: ARMFlow在在线生成中，单步推理比现有在线方法在InterHuman和InterX数据集上的FID指标提升超过40%。同时匹配离线最先进方法的性能，尽管只使用了部分序列条件。ReMFlow在离线方法中达到最先进性能，且推理速度最快。

Conclusion: ARMFlow成功解决了3D人体反应生成的三大挑战，通过全局上下文编码器增强语义对齐，单步推理实现高精度和低延迟，BSCE减少累积误差。该方法在在线和离线场景中都表现出色，为实时交互应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多视角人群计数旨在解决大范围场景中的严重遮挡问题，但由于多视角图像采集和标注困难，现有数据集规模有限。为应对数据不足问题，本文提出半监督方法减少对标注数据的依赖。
...
Conclusion: 通过排序不同视角数量融合模型的预测结果或模型不确定性，可以在有限标注数据下有效训练多视角人群计数模型，为解决多视角数据稀缺问题提供了有效的半监督解决方案。
...
TL;DR: TextEditBench：首个专注于图像中文本区域编辑的评估基准，强调推理密集型编辑场景，并提出语义期望（SE）新评估维度，发现当前模型在上下文推理、物理一致性和布局感知方面仍存在困难。


<details>
  <summary>Details</summary>
Motivation: 文本渲染已成为视觉生成中最具挑战性的前沿领域，但图像中的文本编辑仍未被充分探索。现有方法需要生成清晰可读的字符，同时保持语义、几何和上下文的一致性，这需要新的评估基准来推动该领域发展。

Method: 提出了TextEditBench评估基准，专注于图像中的文本中心区域，强调推理密集型编辑场景。引入新的评估维度——语义期望（SE），用于衡量模型在文本编辑过程中保持语义一致性、上下文连贯性和跨模态对齐的推理能力。

Result: 通过对最先进的编辑系统进行广泛实验发现，当前模型能够遵循简单的文本指令，但在上下文相关推理、物理一致性和布局感知集成方面仍存在困难。TextEditBench为推进文本引导的图像编辑和多模态生成中的推理能力建立了新的测试平台。

Conclusion: TextEditBench填补了文本编辑评估的空白，通过专注于这一长期被忽视但基本的能力，为推进文本引导的图像编辑和多模态生成中的推理建立了新的测试基准，有望推动该领域的发展。
...
TL;DR: MACL方法通过标签感知采样、频率敏感加权和动态温度缩放，解决了遥感图像多标签检索中的语义重叠、标签分布不平衡和复杂共现模式问题，在多个数据集上优于现有对比学习方法。


<details>
  <summary>Details</summary>
Motivation: 遥感图像多标签检索面临三大挑战：1）土地覆盖类别间的语义重叠；2）高度不平衡的标签分布；3）复杂的类别间共现模式。这些因素导致传统方法难以学习平衡且有效的表示。

Method: 提出多标签自适应对比学习（MACL），扩展了对比学习框架，包含三个关键技术：1）标签感知采样策略；2）频率敏感加权机制；3）动态温度缩放技术，以实现对常见和稀有类别的平衡表示学习。

Result: 在DLRSD、ML-AID和WHDLD三个基准数据集上的广泛实验表明，MACL始终优于基于对比损失的基线方法，有效缓解了语义不平衡问题，并在大规模遥感档案中提供了更可靠的检索性能。

Conclusion: MACL通过自适应对比学习框架成功解决了遥感图像多标签检索中的关键挑战，为大规模遥感档案的高效检索提供了有效解决方案，代码和预训练模型将在接受后开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型具备图像输出能力，但现有图像生成基准主要关注美学质量而非细粒度生成能力。需要一种客观方法来评估模型在像素精度上的细粒度生成智能。

Method: 提出PixelArena基准，使用语义分割任务来客观检验多模态大语言模型的细粒度生成能力。在零样本设置下评估模型生成语义掩码的能力，并对不同模型进行定性和定量比较分析。

Result: Gemini 3 Pro Image展现出新兴的图像生成能力，在零样本设置下能生成高保真语义掩码，表现出前所未有的视觉智能和在新图像生成任务中的真正泛化能力。研究还展示了与其他模型的比较结果和失败案例。

Conclusion: 这些发现不仅标志着该领域的令人兴奋进展，还为未来多模态、推理、可解释性和基准测试相关研究提供了重要见解。
...
TL;DR: 提出基于岭估计的融合定位方法，结合序列图像场景信息和激光测距精度，解决有限观测条件下设计矩阵多重共线性导致的病态问题，提高定位精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在无人机多传感器目标跟踪中，远距离、小交会角、大倾角等有限观测条件下，最小二乘估计的设计矩阵存在严重多重共线性，导致病态问题、不稳定性和低鲁棒性。
...
Conclusion: 基于岭估计的融合定位方法能够有效解决有限观测条件下的多重共线性问题，提高无人机多传感器目标定位的精度和鲁棒性，特别适用于远距离、小交会角等挑战性场景。
...
<details>
  <summary>Details</summary>
Motivation: 非专用车载移动群智感知系统面临覆盖率、可靠性和车辆动态参与度相互关联的挑战，需要在不依赖专用基础设施的情况下实现低成本高质量的城市监测。

Method: 提出QUIDS系统，引入聚合感知质量(ASQ)指标量化信息质量，开发相互辅助的信念感知车辆调度算法，在不确定性下估计感知可靠性并分配激励。

Result: 在真实世界部署中，QUIDS比无调度场景提升ASQ 38%，比现有最优方法提升10%，重建地图误差降低39-74%。

Conclusion: 通过质量感知的激励机制联合优化覆盖率和可靠性，QUIDS实现了无需专用基础设施的低成本高质量城市监测，适用于交通和环境感知等智慧城市场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统部署中，边缘设备将视觉数据传输到服务器进行VLM推理时，为匹配视觉编码器输入分辨率而调整原始图像大小会丢失细粒度细节，导致精度下降。需要一种既能保持精度又能减少通信成本的方法。

Method: 设计了两阶段框架：第一阶段服务器在全局图像上进行推理，利用VLM内部注意力识别感兴趣区域，计算输出令牌的最小熵作为置信度度量；若最小熵超过预设阈值，则请求边缘设备发送RoI的细节保留局部图像；第二阶段服务器联合利用全局和局部图像进行细化推理。

Result: 在多种VLM架构上的实验表明，该框架在保持推理精度的同时显著降低了通信成本。

Conclusion: 提出的协作边缘到服务器推理框架通过选择性重传策略，有效地在减少通信开销的同时保持了视觉语言模型的推理精度，解决了传统部署中因图像调整而丢失细节的问题。
...
TL;DR: GMODiff是一个基于增益图的单步扩散框架，用于多曝光HDR重建，通过将HDR重建重新定义为条件引导的增益图估计任务，解决了传统潜在扩散模型在HDR应用中的动态范围限制、高推理成本和内容幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 预训练的潜在扩散模型（LDMs）在低层视觉任务中显示出强大的感知先验，但在多曝光HDR重建中直接应用面临三个主要挑战：8位潜在压缩导致的动态范围有限、多步去噪的高推理成本以及生成性质固有的内容幻觉问题。

Method: 1. 将HDR重建重新定义为条件引导的增益图估计任务，增益图编码扩展的动态范围同时保持与LDR图像相同的位深度；2. 从信息丰富的基于回归的估计而非纯噪声初始化去噪过程，实现单步生成高质量增益图；3. 利用回归先验引导LDM的去噪过程和潜在解码，抑制幻觉同时保持结构准确性。

Result: GMODiff在性能上优于多个最先进方法，并且比之前基于LDM的方法快100倍，在保持结构准确性的同时有效抑制了内容幻觉。

Conclusion: GMODiff通过增益图驱动的单步扩散框架，成功解决了传统潜在扩散模型在HDR重建中的关键限制，实现了高效、高质量的HDR重建，平衡了内容保真度和感知质量。
...
TL;DR: EverybodyDance提出了一种解决多角色动画中身份对应问题的系统方案，通过身份匹配图建模角色关系，优化身份对应正确性，显著提升多角色动画质量。


<details>
  <summary>Details</summary>
Motivation: 现有姿态驱动角色动画在单角色场景已取得显著进展，但扩展到多角色场景（特别是涉及位置交换时）存在挑战。核心问题在于确保参考帧和生成帧之间角色的正确身份对应关系。

Method: 提出身份匹配图建模角色关系，使用掩码查询注意力计算角色间亲和度；将身份对应正确性形式化为图结构度量并在训练中优化；提出身份嵌入引导、多尺度匹配策略和预分类采样等针对性策略。

Result: 构建了专门的身份对应评估基准，大量实验表明EverybodyDance在身份对应正确性和视觉保真度方面显著优于现有最先进基线方法。

Conclusion: EverybodyDance为解决多角色动画中的身份对应问题提供了系统解决方案，通过图结构建模和针对性优化策略，在多角色动画生成中取得了优异性能。
...
TL;DR: FVG将文本到视频生成分解为三个专门阶段：推理（LLM重写提示描述初始场景）、构图（T2I模型合成高质量锚定帧）、时序合成（视频模型专注于动画化场景），显著提升视频质量并加速采样。
...
Result: FVG在T2V CompBench基准测试中达到新的最先进水平，并在VBench2上显著改进所有测试模型。视觉锚定允许将采样步骤减少70%而不损失性能，实现采样速度的大幅提升。

Conclusion: 因子化视频生成为更高效、鲁棒和可控的视频合成提供了一条简单而实用的路径，通过解耦初始场景构建和时序动画化任务，解决了现有文本到视频模型的根本局限性。
...
TL;DR: 利用3D神经表示从少量未标定人脸图像构建统一解释，通过高斯泼溅和语义分割实现中性姿态重建，将高斯泼溅转换为纹理空间神经纹理，支持标准图形管线使用。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要大量校准图像或长视频才能重建人脸3D模型，且难以与标准图形管线兼容。本文旨在从少量未标定图像中重建高质量人脸几何和纹理，使其能在标准图形流程中使用。

Method: 1) 使用高斯泼溅而非NeRF，因其更显式且易于约束；2) 利用语义分割标注对齐人脸区域，仅需11张图像重建中性姿态；3) 将高斯泼溅软约束到三角网格表面，提供结构化重建；4) 将高斯泼溅转换为纹理空间的神经纹理；5) 使用可重光照高斯模型分离纹理和光照，获得高分辨率反照率纹理。

Result: 成功从少量未标定图像重建高质量人脸几何和纹理，生成的三角网格和纹理可直接用于标准图形管线。系统支持不同光照条件下的训练图像，并展示了在文本驱动资产创建流程中的应用效果。

Conclusion: 该方法实现了从少量未标定人脸图像到标准图形管线可用资产的端到端转换，将高斯泼溅的高视觉保真度与标准图形工作流相结合，为3D内容创建提供了新途径。
...
TL;DR: BrepLLM：首个让大语言模型能够直接处理3D边界表示(Brep)数据的框架，通过两阶段训练实现3D几何与自然语言的跨模态对齐，在3D物体分类和描述任务上达到SOTA性能。
...
Method: 采用两阶段训练：1)跨模态对齐预训练：通过自适应UV采样将Brep转换为图表示，设计分层BrepEncoder提取几何和拓扑特征，通过对比学习将全局token与CLIP文本嵌入对齐；2)多阶段LLM微调：集成BrepEncoder到LLM，采用三阶段渐进训练策略：语义映射训练、LLM微调、混合查询专家增强几何多样性建模。
...
TL;DR: CountZES是一个无需训练、基于零样本示例选择的对象计数框架，通过三阶段协同机制从文本描述中逐步发现多样化的示例，在复杂场景中实现精确的对象计数。


<details>
  <summary>Details</summary>
Motivation: 现有零样本对象计数方法存在两个主要问题：基于开放词汇检测器的方法通常产生多实例候选，而基于随机补丁采样的方法无法准确描绘对象实例。需要一种能够精确识别单个实例示例的方法来提升零样本计数的准确性。

Method: 提出CountZES框架，包含三个协同阶段：1) 检测锚定示例(DAE)：精炼开放词汇检测结果以隔离精确的单实例示例；2) 密度引导示例(DGE)：引入密度驱动的自监督范式，识别统计一致且语义紧凑的示例；3) 特征共识示例(FCE)：通过特征空间聚类增强视觉一致性。这三个阶段共同产生多样化、互补的示例集。

Result: 在多样化数据集上的实验表明，CountZES在零样本对象计数方法中表现出优越性能，同时在自然、航空和医学领域都能有效泛化。

Conclusion: CountZES通过三阶段协同的零样本示例选择机制，成功解决了复杂场景中零样本对象计数的挑战，实现了文本基础、计数一致性和特征代表性的平衡，为跨领域对象计数提供了有效的解决方案。
...
TL;DR: 本文提出了一种无需训练的方法，通过几何角度精炼文本嵌入来抑制不需要的语义，解决文本到图像扩散模型中多输出间主体一致性问题。
...
Method: 提出一种简单有效的无需训练方法，从几何角度通过精炼文本嵌入来抑制不需要的语义，解决语义纠缠问题。该方法不依赖模型微调，直接操作文本嵌入空间。

Result: 大量实验证明，该方法在主体一致性和文本对齐方面显著优于现有基线方法，有效解决了语义泄漏和文本错位问题。

Conclusion: 该方法提供了一种计算效率高的解决方案，无需训练即可改善文本到图像扩散模型在视觉叙事中的主体一致性，为多图像生成任务提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: VAR模型虽然通过下一尺度预测实现了高质量图像生成，但在大规模步骤下计算复杂度和运行时间急剧增加。现有加速方法依赖手动步骤选择，且忽略了生成过程中不同阶段的重要性差异。
...
Conclusion: 阶段感知设计是高效视觉自回归图像生成的有力原则，StageVAR通过系统分析不同生成阶段的重要性，实现了显著加速且保持高质量输出。
...
TL;DR: 该研究提出了一种让模型获得人类感知-推理能力的盲图像质量评估方法，通过强化学习使用人类标注作为奖励信号，使模型能够进行自我一致推理，在预测性能和人类解释相似度方面都取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 人类通过感知-推理级联来评估图像质量，结合感官线索和隐式推理形成自一致的判断。当前盲图像质量评估模型缺乏这种人类式的推理能力，需要开发既能预测质量分数又能提供人类可解释推理的模型。

Method: 1. 收集捕捉人类感知-推理过程多个方面的人类评估数据；2. 采用强化学习，使用人类标注作为奖励信号，引导模型学习人类式感知和推理；3. 设计特殊奖励机制，使模型能够从自我生成的描述中推断图像质量，从而内化自我一致的推理能力。

Result: 1. 在Pearson和Spearman相关系数等通用指标上，预测性能与最先进的盲图像质量评估系统相当；2. 在1000多个人类标注样本上，模型达到ROUGE-1分数0.512（基线为0.443），表明模型生成解释与人类感知-推理链有显著覆盖，在人类可解释推理方面取得进展。

Conclusion: 该方法成功使模型获得了人类式的感知-推理能力，不仅在质量评分预测上表现优异，更重要的是在提供人类可解释的推理方面取得了实质性进展，为盲图像质量评估向更人性化、可解释的方向迈出了重要一步。
...
TL;DR: 本文提出EMER数据集和EMERT模型，通过引入眼动行为作为重要情感线索来弥合面部表情识别(FER)与真实情感识别(ER)之间的差距。
...
Method: 1) 构建EMER数据集：利用自发情感诱导范式收集真实情感数据，同时捕获眼动序列、注视点图等眼行为数据和面部表情视频；2) 设计EMERT模型：采用模态对抗特征解耦和多任务Transformer，将眼行为作为面部表情的强有力补充。

Result: EMERT模型在EMER数据集上显著优于其他最先进的多模态方法，证明了建模眼行为对鲁棒情感识别的重要性。提供了7个多模态基准协议进行全面评估。

Conclusion: 眼行为在情感识别中具有重要作用，能够有效弥合FER与ER之间的差距，提升情感识别的鲁棒性。EMER数据集和EMERT模型将公开提供。
...
TL;DR: 该论文提出了一种用于骨架序列时序动作定位的自监督预训练方法，通过片段判别任务和U形模块增强特征分辨率，在多个数据集上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 骨架序列的时序动作定位需要时间敏感的特征来捕捉相邻帧之间的细微差异，而现有的自监督预训练方法主要针对视频级动作识别，对动作边界检测的研究不足。

Method: 1. 提出片段判别前置任务：将骨架序列密集投影到非重叠片段，通过对比学习区分不同视频的片段；2. 在骨架动作识别模型基础上，使用U形模块融合中间特征以增强帧级定位的特征分辨率。

Result: 在BABEL数据集上，该方法持续改进了现有骨架对比学习方法在动作定位任务上的性能；在PKUMMD数据集上，通过在NTU RGB+D和BABEL上预训练，实现了最先进的迁移学习性能。

Conclusion: 提出的自监督预训练方法能有效学习时间敏感特征，显著提升骨架序列时序动作定位的性能，为这一具有挑战性的任务提供了新的解决方案。
...
Result: 实验结果显示，该框架不仅在3D定位任务上达到最先进性能，而且在3D空间推理方面也持续超越现有方法，数据规模比现有最大单图像3D检测数据集大6倍以上。

Conclusion: N3D-VLM成功将原生3D物体感知与3D感知视觉推理相结合，实现了更精确、可解释的3D空间理解，为多模态模型在3D场景中的应用提供了新思路。
...
TL;DR: 提出一个动态重建系统，能从单目RGB视频输入中生成场景的完整持久重建，包括可见和不可见部分，实现4D时空感知


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能重建当前可见的场景部分，无法保持对之前观察过的场景部分的持久重建，限制了动态场景的完整理解和回放能力

Method: 将场景分解为一组刚性3D基元，通过估计的密集2D对应关系，通过优化管道联合推断这些基元的刚性运动，并引入运动外推机制处理不可见物体

Result: 在物体扫描和多物体数据集上，该系统在定量和定性评估中都显著优于现有方法，实现了可回放的3D重建、多物体扫描和物体持久性

Conclusion: 该系统能够从单目视频中实现完整的4D时空重建，为动态场景理解提供了新的能力，包括可回放的3D重建和物体持久性
...
TL;DR: 论文提出Causal-Tune方法，通过频域分析识别和分离视觉基础模型中的因果与非因果因素，提升域泛化语义分割性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在域泛化语义分割中通常训练轻量适配器或优化中间特征，但忽视了长期预训练的视觉基础模型中存在的伪影问题，这些伪影与非因果因素相关，阻碍了有价值表征的利用，最终降低性能

Method: 提出Causal-Tune方法：1) 使用离散余弦变换提取每层特征的频谱；2) 应用高斯带通滤波器分离因果和非因果成分；3) 引入因果感知可学习令牌在频域细化因果成分；4) 通过逆DCT将细化特征转换回空间域并传递到下一层

Result: 在各种跨域任务上的实验证明了Causal-Tune的有效性，特别是在恶劣天气条件下表现优异，在雪天条件下相比基线提升了+4.8% mIoU

Conclusion: 通过因果机制分析视觉基础模型中的特征，提出了一种简单有效的频域方法识别和分离因果与非因果因素，显著提升了域泛化语义分割的鲁棒性
...
TL;DR: CRONOS是一个用于3D医学扫描时间序列预测的统一框架，支持从多个历史扫描进行多对一预测，同时处理离散和连续时间戳，首次实现3D医学数据的连续序列到图像预测。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学扫描时间预测模型存在局限性：要么依赖单一历史扫描，要么只能处理固定时间网格，要么仅针对全局标签，这限制了不规则采样下的体素级预测能力。
...
Conclusion: CRONOS首次实现了3D医学数据的连续序列到图像预测，提供了一个统一的多上下文、连续时间预测框架，作者将发布代码和评估协议以促进可重复的多数据集基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在需要视觉想象的场景中表现不足，而人类能够在统一的大脑空间中构建灵活的视觉-文本思维交互。受此启发，作者认为视觉标记可以无缝插入文本标记的推理过程中。

Method: 提出Sketch-in-Latents (SkiLa)范式，扩展MLLMs的自回归能力以原生生成连续视觉嵌入（潜在草图标记）。模型在多步推理中动态切换文本思维模式和视觉草图模式，并提出潜在视觉语义重建机制确保语义基础。

Result: 大量实验表明SkiLa在视觉中心任务上取得优越性能，同时在多样化通用多模态基准上展现出强大的泛化能力。

Conclusion: SkiLa通过统一的多模态推理范式，使MLLMs能够像人类一样在推理过程中进行视觉想象，显著提升了视觉理解和想象能力。
...
TL;DR: Yuan-TecSwin是一种基于Swin-transformer的文本条件扩散模型，通过替换CNN块为Swin-transformer块来增强非局部建模能力，在ImageNet生成基准上达到了1.37的SOTA FID分数。
...
Method: 提出Yuan-TecSwin模型：1) 用Swin-transformer块替换编码器和解码器中的CNN块；2) 通过精心选择的文本编码器、有效利用文本嵌入和精心设计的文本条件融合来改进文本-图像对齐；3) 采用自适应时间步搜索策略在不同扩散阶段优化推理性能。

Result: 1) 在ImageNet生成基准上达到1.37的SOTA FID分数；2) 通过自适应时间步搜索使推理性能提升10%；3) 在人类评估中，受访者难以区分模型生成的图像和人类绘制的图像。

Conclusion: Yuan-TecSwin通过引入Swin-transformer块有效提升了扩散模型的非局部建模能力，结合改进的文本条件融合策略，在图像生成质量和文本-图像对齐方面取得了显著进展。
...
TL;DR: Hazedefy是一个轻量级、面向应用的去雾管道，专注于实时视频和直播摄像头增强，基于暗通道先验和大气散射模型，适用于消费级硬件。


<details>
  <summary>Details</summary>
Motivation: 开发一个计算简单、实用可部署的去雾系统，能够在消费级硬件上实时处理视频和直播摄像头流，无需GPU加速。

Method: 基于暗通道先验和大气散射模型，采用伽马自适应重建、快速传输近似（带数值稳定下界）、基于分数顶部像素平均的稳定大气光估计器，以及可选的颜色平衡阶段。

Result: 在真实世界图像和视频上的实验演示显示，Hazedefy能够提高可见度和对比度，适用于移动和嵌入式应用，无需GPU加速。

Conclusion: Hazedefy是一个轻量级、实用的去雾管道，特别适合实时视频和直播摄像头增强，在消费级硬件上表现出良好的性能和部署可行性。
...
TL;DR: LLSA提出了一种对数线性复杂度的稀疏注意力机制，通过分层结构解决DiTs在长序列上的二次计算成本问题，显著加速了高分辨率图像生成


<details>
  <summary>Details</summary>
Motivation: 现有DiTs的自注意力机制具有二次计算成本，限制了其在长序列上的扩展。现有的Top-K稀疏注意力方法虽然减少了计算，但仍存在二次选择成本问题，且随着序列增长需要增加K值来维持模型质量

Method: 提出对数线性稀疏注意力（LLSA），采用分层结构：1）分层Top-K选择，利用前一层找到的索引进行渐进式稀疏选择；2）分层KV富集机制，在注意力计算中使用不同粒度的更少token来保留全局上下文；3）开发高性能GPU实现，仅使用稀疏索引进行前向和反向传播

Result: 在256×256像素token序列上，LLSA将注意力推理加速28.27倍，DiT训练加速6.09倍，同时保持生成质量。该方法支持不使用分块化和VAE编码的高分辨率像素空间图像生成

Conclusion: LLSA为高效训练长序列DiTs提供了一个有前景的方向，通过分层结构将对数线性复杂度引入稀疏注意力，解决了现有方法的计算瓶颈
...
TL;DR: 该论文提出了一种基于电源插座的室内多媒体地理定位方法，通过检测、分类插座类型并映射到国家，为打击人口贩卖等犯罪提供数字取证工具。


<details>
  <summary>Details</summary>
Motivation: 室内多媒体地理定位在打击人口贩卖、儿童剥削等严重犯罪中具有重要价值，但面临室内布局相似、装修频繁、视觉模糊、光照变化、GPS信号不可靠以及敏感领域数据集有限等挑战。

Method: 提出三阶段深度学习流水线：1) 使用YOLOv11检测电源插座；2) 使用Xception将插座分类为12种类型；3) 将检测到的插座类型映射到国家。为解决数据稀缺问题，创建了两个专用数据集：包含2,328张标注图像（增强后4,072张）的插座检测数据集，以及包含3,187张图像、覆盖12种插座类型的分类数据集。

Result: 插座检测mAP@0.5达到0.843，插座类型分类准确率达到0.912，插座类型到国家映射准确率达到0.96（置信度阈值>90%）。在Hotels-50K数据集（特别是TraffickCam子集）上评估，该子集包含真实世界条件下的酒店图像，如光照不佳和业余拍摄角度。

Conclusion: 该框架展示了向实际数字取证应用迈出的实用一步，代码、训练模型和数据均已开源，为室内多媒体地理定位提供了有效解决方案。
...
TL;DR: DeContext是一种保护输入图像免遭未经授权的上下文编辑的新方法，通过注入微小扰动来削弱跨注意力路径，有效阻断输入与输出之间的关联。


<details>
  <summary>Details</summary>
Motivation: 上下文扩散模型虽然能轻松修改图像，但也带来严重的隐私问题：个人图像可能被未经同意地用于身份冒充、虚假信息等恶意用途。现有方法主要针对个性化文本到图像生成，而现代大规模基于DiT的上下文模型的鲁棒性尚未得到充分研究。
...
Conclusion: 注意力机制是上下文扩散模型中信息传播的关键路径，通过针对性扰动这些路径可以有效地保护图像免遭未经授权的编辑，为图像隐私保护提供了简单而强大的解决方案。
...
TL;DR: SARMAE：首个面向SAR图像的噪声感知掩码自编码器，通过构建百万级SAR数据集和引入斑点噪声增强、语义锚点约束，实现了自监督SAR表示学习，在分类、检测和分割任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有SAR深度学习面临数据稀缺和斑点噪声干扰两大挑战，限制了细粒度语义表示学习。需要开发能够处理SAR特有噪声并利用大规模数据进行预训练的自监督学习方法。

Method: 1) 构建首个百万级SAR数据集SAR-1M，包含配对的SAR和光学图像；2) 设计斑点感知表示增强(SARE)，将SAR特有的斑点噪声注入掩码自编码器，实现噪声感知的鲁棒表示学习；3) 引入语义锚点表示约束(SARC)，利用配对光学先验对齐SAR特征并确保语义一致性。

Result: 在多个SAR数据集上的广泛实验表明，SARMAE在分类、检测和分割任务上均达到了最先进的性能。代码和模型将在GitHub上开源。

Conclusion: SARMAE通过构建大规模数据集、噪声感知学习和语义对齐约束，成功解决了SAR深度学习中的数据稀缺和噪声干扰问题，为SAR图像的自监督表示学习提供了有效解决方案。
...
### [51] [FrameDiffuser: G-Buffer-Conditioned Diffusion for Neural Forward Frame Rendering](https://arxiv.org/abs/2512.16670)
...
Method: 采用自回归框架，结合G-buffer数据和先前生成的帧进行时间引导；使用双条件架构（ControlNet用于结构指导，ControlLoRA用于时间一致性）；采用三阶段训练策略；针对特定环境进行专门化训练。

Result: FrameDiffuser能够在数百到数千帧中保持稳定的时间一致性生成，相比通用方法，环境特定训练实现了更优越的逼真质量，具有准确的照明、阴影和反射效果。

Conclusion: FrameDiffuser为交互应用提供了一种高效、时间一致的神经渲染解决方案，通过环境特定训练和自回归架构平衡了生成质量、时间一致性和计算效率。
...
### [52] [Few-Shot Fingerprinting Subject Re-Identification in 3D-MRI and 2D-X-Ray](https://arxiv.org/abs/2512.16685)
*Gonçalo Gaspar Alves,Shekoufeh Gorgi Zadeh,Andreas Husch,Ben Bausch*

Main category: cs.CV

TL;DR: 该研究提出使用主题指纹识别技术解决医学影像数据集中因同一受试者出现在多个数据集而导致的数据泄露问题，通过将同一受试者的所有图像映射到潜在空间中的特定区域，实现基于相似性匹配的受试者重识别。


<details>
  <summary>Details</summary>
Motivation: 在组合开源数据集时，如果同一受试者出现在多个数据集中，会导致数据泄露，从而夸大模型性能。需要一种方法来识别和避免这种数据泄露问题。

Method: 使用ResNet-50模型配合三元组边界损失进行训练，探索主题指纹识别技术。该方法将同一受试者的所有图像映射到潜在空间中的特定区域，通过相似性匹配实现受试者重识别。在3D MRI和2D X-ray数据上评估了少样本指纹识别，包括标准场景（20-way 1-shot）和挑战性场景（1000-way 1-shot）。
...
Conclusion: 主题指纹识别技术能有效解决医学影像数据集中的数据泄露问题，通过将同一受试者的图像映射到潜在空间中的特定区域，实现了高精度的受试者重识别，为组合开源数据集提供了可靠的技术保障。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF、3DGS和RadiantFoam等方法在视图合成方面取得进展，但在精确网格重建方面仍有不足，需要改进表面重建质量

Method: 联合学习显式Voronoi图和隐式符号距离场(SDF)，通过光线追踪优化场景，使用Eikonal正则化，使Voronoi单元面与零水平集对齐

Result: SDFoam在多种场景中显著改善网格重建精度（Chamfer距离），保持可比的外观质量（PSNR、SSIM），且不牺牲训练效率

Conclusion: 提出的隐式-显式混合方法SDFoam能够生成更清晰、视图一致的表面，减少漂浮物，改善拓扑结构，同时保持渲染效率和外观质量
...
<details>
  <summary>Details</summary>
Motivation: 胎儿超声生长评估依赖手动测量解剖标志点，这种方法耗时、操作者依赖性强，且在不同扫描设备和中心间存在变异性，限制了自动化方法的可重复性。需要多源标注数据集来开发AI辅助的胎儿生长评估方法。

Method: 创建了一个开放的多中心、多设备胎儿超声图像基准数据集，包含4,513张去标识化超声图像，来自1,904名受试者，涵盖三个临床中心和七种不同超声设备。提供了标准化的受试者分离训练/测试分割、评估代码和基线结果。
...
TL;DR: 该论文提出了OMG-Bench，首个用于骨架在线微手势识别的大规模公开基准，包含40个细粒度手势类别和13,948个实例，并提出了HMATr框架，统一手势检测和分类，性能优于现有方法7.6%。
...
Method: 开发了多视图自监督流水线自动生成骨架数据，结合启发式规则和专家细化进行半自动标注。提出了分层记忆增强Transformer（HMATr）端到端框架，利用分层记忆库存储帧级细节和窗口级语义以保持历史上下文，并使用从内存初始化的可学习位置感知查询隐式编码手势位置和语义。

Result: 构建了OMG-Bench基准，包含40个细粒度手势类别、13,948个实例和1,272个序列，具有细微运动、快速动态和连续执行的特点。HMATr在检测率上优于最先进方法7.6%，为在线微手势识别建立了强大基线。

Conclusion: 该工作通过创新的数据生成流水线解决了微手势数据集稀缺问题，提出的HMATr框架有效统一了手势检测和分类，为在线微手势识别领域提供了首个大规模公开基准和强大基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 遥感领域可控生成技术快速发展，训练数据合成成为扩展标注数据集、减轻人工标注负担的有效途径。然而，语义掩码控制的复杂性和采样质量的不确定性限制了合成数据在下游语义分割任务中的实用性。

Method: 提出任务导向的数据合成框架TODSynth：1）多模态扩散变换器（MM-DiT）采用统一的文本-图像-掩码三重注意力机制；2）基于任务反馈的即插即用采样策略；3）控制-校正流匹配方法（CRFM），在早期高可塑性阶段通过语义损失动态调整采样方向。

Result: 实验表明，该方法在遥感语义分割数据合成中显著优于现有可控生成方法，特别是在少样本和复杂场景下。文本-图像-掩码联合注意力机制结合图像和掩码分支的完全微调能有效提升合成数据质量。

Conclusion: TODSynth框架通过多模态注意力机制和任务反馈引导的采样策略，能够生成更稳定、任务导向的合成数据，有效缓解遥感语义分割中数据标注的挑战。
...
TL;DR: TreeNet是一种基于二叉树结构的低复杂度图像压缩模型，通过注意力特征融合机制整合多分支特征，在低比特率下相比JPEG AI实现4.83%的BD-rate提升，同时减少87.82%的模型复杂度。


<details>
  <summary>Details</summary>
Motivation: 基于学习的图像压缩技术面临计算复杂度高的挑战，这限制了其广泛应用。需要开发低复杂度的图像压缩模型来促进该技术的普及。

Method: 提出TreeNet模型，采用二叉树结构的编码器-解码器架构实现高效表示和重建，使用注意力特征融合机制有效整合多分支特征。

Result: 在三个基准数据集上的评估显示，TreeNet在低比特率下相比JPEG AI实现了4.83%的BD-rate平均提升，同时模型复杂度降低了87.82%。消融研究深入分析了不同潜在表示对重建的影响。

Conclusion: TreeNet通过创新的二叉树架构和注意力特征融合，在显著降低计算复杂度的同时实现了优于现有方法的压缩性能，为学习型图像压缩的实际应用提供了可行方案。
...
Conclusion: Make-It-Poseable通过潜在空间变换方法有效解决了传统角色摆姿方法的局限性，实现了高质量的姿势重建和灵活的3D编辑能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法通常采用分离的流水线处理不同任务，缺乏统一的框架来整合视频生成、编辑和智能推理功能。需要构建一个能够处理多种输入并支持复杂视频创作的综合系统。

Method: 采用端到端架构，将文本指令、参考图像和视频上下文等多样化输入处理为统一的多模态表示。构建了全面的数据系统作为基础，并实施了高效的大规模预训练策略和推理基础设施优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实数据集缺乏精确标注，而合成数据集虽然标注成本低，但缺乏真实的车辆运动轨迹。需要结合两者优势，创建既有精确标注又有真实车辆行为的合成数据集。

Method: 创建合成3D环境，并从SinD（无人机拍摄的鸟瞰数据集）中提取真实世界轨迹，将这些真实轨迹集成到合成环境中，生成具有真实车辆运动的合成数据集。

Result: 开发了R3ST数据集，填补了合成数据与真实轨迹之间的差距，提供了准确的多模态标注和真实的人类驾驶车辆轨迹。

Conclusion: R3ST数据集通过结合合成环境的标注优势和真实轨迹的真实性，推动了道路车辆轨迹预测研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: AR/VR应用中，基于头戴设备稀疏信号重建真实多样的全身姿态具有挑战性。现有方法要么计算成本高，要么分别建模空间和时间依赖，难以平衡精度、时间一致性和效率。
...
TL;DR: TabReX：一个无参考、基于属性的表格生成评估框架，通过图推理量化结构和事实保真度


<details>
  <summary>Details</summary>
Motivation: 现有表格生成评估方法存在局限：要么将表格扁平化为文本忽略结构，要么依赖固定参考限制泛化能力，需要更可靠的评估框架

Method: TabReX将源文本和生成表格转换为规范知识图，通过LLM引导的匹配过程对齐，计算可解释的、基于量规的分数来量化结构和事实保真度

Result: TabReX在专家排名相关性上最高，在更难的扰动下保持稳定，支持细粒度模型与提示分析，建立了可信、可解释的结构化生成评估新范式

Conclusion: TabReX提供了一个可控的敏感性与特异性权衡的评估框架，通过TabReX-Bench基准系统评估鲁棒性，为结构化生成系统评估提供了新方法
...
TL;DR: SocialStoryFrames是一个用于建模读者对故事反应的计算框架，包括感知作者意图、推理、情感反应和价值判断等，通过对话上下文和叙事理论分类法实现，并在社交媒体故事数据集上验证应用。


<details>
  <summary>Details</summary>
Motivation: 目前计算模型对读者反应的建模能力有限，无法进行细致的分析。需要开发能够捕捉读者对故事丰富解释性、情感性和评价性反应的模型，以支持对叙事的规模化研究。

Method: 提出了SocialStoryFrames形式化框架，基于叙事理论、语言语用学和心理学构建分类法，开发了SSF-Generator和SSF-Classifier两个模型，通过人类调查（382名参与者）和专家标注进行验证，并在包含6,140个社交媒体故事的SSF-Corpus数据集上应用。

Result: 模型在人类调查和专家标注中得到验证，能够有效捕捉读者反应。应用模型分析了社交媒体故事中叙事意图的频率和相互依存关系，比较了不同社区的叙事实践及其多样性。

Conclusion: SocialStoryFrames通过将细粒度、上下文敏感的建模与通用的读者反应分类法相结合，为在线社区中的叙事研究提供了新的工具，支持对讲故事实践的规模化分析。
...
TL;DR: 研究探索了个人信息（如个人陈述）对预测标注者主观判断的影响，发现人口统计信息比态度、关系、经历更重要，理论方法优于自动聚类，少量相关评论即可，多样化的标注者样本效果最佳。


<details>
  <summary>Details</summary>
Motivation: 以往研究使用个人信息来改进个体特征建模和主观任务标注预测，但信息量有限，缺乏对何种信息最有效的深入探索。本研究旨在分析不同类型个人信息对预测标注者社会规范判断的影响。

Method: 对自我披露句子进行分类，构建标注者模型来预测社会规范判断。通过多种消融实验和分析，检验信息类型对预测标注模式的影响。

Result: 1. 人口统计信息比态度、关系和经历更具影响力；2. 理论方法优于自动聚类方法；3. 与先前研究相反，仅需少量相关评论即可；4. 拥有更多样化的标注者自我披露样本能获得最佳性能。

Conclusion: 不同类型个人信息对预测标注者判断具有不同影响，人口统计信息最为关键，理论方法更有效，少量高质量信息配合多样化样本能优化预测性能。
...
TL;DR: Sage是一个无需人工标注的LLM-as-a-Judge评估套件，通过局部自一致性和全局逻辑一致性来评估LLM法官的质量，发现当前顶级LLM在约1/4困难案例中存在显著可靠性问题。
...
Result: 实验表明Sage指标稳定且与监督基准（如LLMBar和RewardBench2）高度相关。当前最先进的LLM在评分和成对设置中作为法官时表现出显著可靠性问题，即使在困难案例中也有近1/4无法保持一致性偏好。发现"情境偏好"现象，明确规则或标准可帮助模型在答案对之间保持一致性判断。

Conclusion: 微调LLM-as-a-Judge是提升性能的可行方法，基于小组的法官和深度推理可增强判断一致性。人类判断也存在显著不一致性，表明人工标注可能不是可靠的金标准。Sage为评估LLM法官的稳健性和准确性提供了可靠的无监督评估套件。
...
TL;DR: 该论文提出了一种语义驱动的强化学习方法（MRG-R1），用于医学报告生成，通过优化报告级别的临床正确性奖励而非传统的词元级监督，显著提升了生成报告的临床准确性。
...
Method: 提出语义驱动强化学习（SRL）方法，采用组相对策略优化（GRPO）来鼓励临床正确性导向的学习。优化报告级奖励：基于关键放射学发现提取的边际余弦相似度（MCCS），直接对齐临床标签一致性。同时引入轻量级推理格式约束，引导模型生成结构化的"思维报告"输出。

Result: 在IU X-Ray和MIMIC-CXR数据集上评估，MRG-R1在临床效能指标上达到最先进性能：IU X-Ray上CE-F1为51.88，MIMIC-CXR上为40.39。标签语义强化优于传统词元级监督。

Conclusion: 优化基于临床基础的报告级奖励而非词元重叠，能显著提升临床正确性。这是探索语义强化在监督医学大型视觉语言模型训练中医学正确性的先驱工作。
...
TL;DR: 该论文提出了一个用于检测印地语-英语混合社交媒体文本中虚假仇恨言论的系统，在Faux-Hate共享任务中取得了有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 社交媒体平台已成为有害内容（包括仇恨言论和虚假叙事）快速传播的中心，需要开发专门技术来检测由虚假叙事驱动的仇恨言论。

Method: 结合先进的自然语言处理技术与领域特定的预训练，采用多任务学习方法处理两个子任务：二元虚假仇恨检测以及目标和严重性预测。

Result: 系统在Faux-Hate共享任务中取得了有竞争力的结果，证明了多任务学习在处理这一复杂问题上的有效性。

Conclusion: 通过结合NLP技术和领域特定预训练，多任务学习方法能够有效检测社交媒体中的虚假仇恨言论，为应对这一社会挑战提供了技术解决方案。
...
TL;DR: 基于Qwen2.5-7B模型，通过LoRA微调和提示工程，开发了从警方通报中提取15个关键字段的信息抽取系统，在死亡检测、死亡人数和省级位置提取等任务上达到95%以上准确率。


<details>
  <summary>Details</summary>
Motivation: 从警方事件通报中结构化提取信息对于及时准确的数据处理至关重要，但由于社交媒体文本的多样性和非正式性，这一任务面临重大挑战。

Method: 开发了领域适应的抽取流水线，结合针对性提示工程和基于LoRA的参数高效微调Qwen2.5-7B模型，处理噪声和异构文本，从4,933个手动标注的微博警方通报实例中提取15个关键字段。

Result: LoRA微调显著优于基模型和指令调优模型，死亡检测准确率超过98.36%，死亡人数精确匹配率达95.31%，省级位置提取精确匹配率达95.54%。

Conclusion: 该流水线为专业领域的多任务结构化信息抽取提供了经过验证的高效解决方案，为社会科学研究中将非结构化文本转化为可靠结构化数据提供了实用框架。
...
TL;DR: 本文提出一个独立于LLM的事实核查模块和领域特定的摘要模型，用于减少医疗领域LLM输出的幻觉问题，在MIMIC-III数据集上取得了良好的性能。


<details>
  <summary>Details</summary>
Motivation: 在医疗健康领域，LLM生成的输出必须可靠准确，特别是在涉及决策制定和患者安全的关键场景中。然而，LLM存在幻觉输出风险，导致这些关键领域的输出往往不可靠。

Method: 1) 提出独立于LLM的事实核查模块，使用数值测试和自然语言处理中的离散逻辑进行细粒度逻辑检查，验证事实与电子健康记录的一致性；2) 设计领域特定的摘要模型，使用低秩适应(LoRa)在MIMIC-III数据集上进行微调；3) 将两个模块配对使用。

Result: 事实核查模块在104个摘要提取的3,786个命题上测试，达到精度0.8904、召回率0.8234、F1分数0.8556。LLM摘要模型在摘要质量上获得ROUGE-1分数0.5797和BERTScore 0.9120。

Conclusion: 提出的独立事实核查模块和领域特定摘要模型能有效减少医疗领域LLM输出的幻觉问题，提高输出的可靠性和准确性，为医疗决策和患者安全提供保障。
...
TL;DR: IBKE框架基于信息瓶颈理论，通过压缩和隔离关键信息实现可泛化的LLM知识编辑，避免完全重训练


<details>
  <summary>Details</summary>
Motivation: LLMs中的错误或过时信息会影响其准确性和安全部署，现有编辑技术难以在广泛领域泛化修正，且容易产生意外后果

Method: 基于信息瓶颈理论，提出IBKE框架，利用紧凑的潜在表示指导基于梯度的更新，实现鲁棒且广泛适用的模型编辑

Result: 在多种LLM架构和标准基准任务中验证了有效性，展示了最先进的准确性，并提高了编辑的泛化性和特异性

Conclusion: 为开放域知识编辑建立了理论原则和实践范式，提升了LLMs在实际应用中的实用性和可信度
...
TL;DR: Sigma-MoE-Tiny是一种极稀疏的混合专家语言模型，在每层使用96个专家但每个token仅激活1个专家，总参数量200亿但仅激活5亿，实现了当前开源模型中最高的稀疏度。


<details>
  <summary>Details</summary>
Motivation: 混合专家模型因其高效和强大的可扩展性成为基础模型的有前景范式，但极稀疏设置下的专家负载均衡是主要挑战，特别是在底层中传统负载均衡损失会失效。

Method: 采用细粒度专家分割（每层最多96个专家），每个token仅激活1个专家；提出渐进稀疏化调度来平衡专家利用率和训练稳定性；在多样化高质量语料上进行预训练，然后进行后训练。

Result: 训练过程非常稳定，没有出现不可恢复的损失尖峰；尽管仅激活5亿参数，但在同类或更大规模模型中达到了顶级性能；提供了对高稀疏MoE模型中负载平衡的深入讨论。

Conclusion: Sigma-MoE-Tiny展示了在极稀疏设置下实现高性能MoE模型的可行性，提出的渐进稀疏化调度有效解决了负载均衡问题，为未来MoE架构的稀疏化发展提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型翻译评估主要关注高资源语言，对低资源和濒危语言的研究存在显著空白，需要了解不同模型架构在乌拉尔语翻译中的表现差异

Method: 使用文学文本平行语料库，评估GPT模型中推理与非推理架构在芬兰语与四种低资源乌拉尔语（科米-兹良语、莫克沙语、埃尔齐亚语、乌德穆尔特语）之间的翻译表现，通过拒绝率分析模型翻译意愿

Result: 推理模型与非推理模型在翻译性能上存在显著差异，推理模型的拒绝率比非推理模型低16个百分点

Conclusion: 研究为乌拉尔语研究者和从业者提供了有价值的见解，并有助于更广泛地理解推理模型在濒危语言保护方面的能力
...
TL;DR: SpeechLLMs（语音大语言模型）在语音翻译中与传统级联系统相比表现如何？本文通过全面测试发现，级联系统仍是最可靠的，SpeechLLMs仅在特定场景下能与之匹敌。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型扩展到文本之外，将语音作为原生模态整合产生了SpeechLLMs，旨在直接翻译口语，绕过传统的基于转录的流水线。但这种整合是否比成熟的级联架构提高语音到文本翻译质量仍是一个开放性问题。

Method: 提出了"Hearing to Translate"测试套件，首次全面基准测试5个最先进的SpeechLLMs与16个强大的直接和级联系统，这些系统结合了领先的语音基础模型（SFM）与多语言LLMs。分析涵盖16个基准测试、13种语言对和9个具有挑战性的条件，包括不流畅、嘈杂和长语音。

Result: 在广泛评估中发现：级联系统总体上仍然是最可靠的；当前的SpeechLLMs仅在特定设置下能与级联系统匹配；语音基础模型（SFMs）落后于两者。这表明整合LLM（无论是在模型内部还是在流水线中）对于高质量的语音翻译至关重要。

Conclusion: 尽管SpeechLLMs作为原生语音模态整合具有潜力，但级联系统在语音翻译中仍保持优势地位。整合LLM（无论是内部整合还是流水线整合）是实现高质量语音翻译的关键因素。
...
TL;DR: 本文提出了一种高效、保护隐私的语音识别适应框架，用于解决临床环境中的技术障碍，包括数据隐私限制、计算资源有限和声学领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别在临床文档处理中具有巨大潜力，但面临数据隐私限制、计算资源有限和声学领域偏移等技术障碍，导致现有模型在实际临床音频中表现不佳（WER高达40.94%），无法实际应用。

Method: 提出一个高效、保护隐私的适应框架，采用低秩适应（LoRA）实现边缘设备上的持续学习，确保患者数据保密性，并通过多领域经验回放减少灾难性遗忘。

Result: 目标领域的WER相对改善17.1%，与朴素适应相比，灾难性遗忘减少47%，展示了在现实高影响环境中构建可靠、自我改进的ASR系统的可行途径。

Conclusion: 该研究为在资源受限的临床环境中部署可靠、自我改进的语音识别系统提供了可行方案，通过隐私保护适应框架有效解决了实际部署的技术障碍。
...
TL;DR: 本文介绍了CLEF 2025 SimpleText竞赛Task 1的提交方案，使用GPT-4.1系列模型进行科学文本的句子级和文档级简化，比较了无上下文提示工程和微调两种方法。


<details>
  <summary>Details</summary>
Motivation: 科学文本通常包含复杂术语和结构，难以被非专业读者理解。CLEF 2025 SimpleText竞赛旨在推动科学文本简化技术发展，使科学知识更易于获取。

Method: 使用OpenAI的gpt-4.1、gpt-4.1-mini和gpt-4.1-nano模型，采用两种方法：1）无上下文方法：依赖提示工程直接生成简化文本；2）微调方法：对模型进行专门训练以适应简化任务。

Result: gpt-4.1-mini模型在无上下文方法下在句子级和文档级简化都表现出稳健性能。微调模型结果参差不齐，其中gpt-4.1-nano-ft在特定情况下在文档级简化表现突出，凸显了不同粒度文本简化的复杂性。

Conclusion: 无上下文的gpt-4.1-mini模型在科学文本简化任务中表现稳健，而微调方法的效果因模型和简化粒度而异，表明需要针对不同简化需求采用不同的技术策略。
...
Method: 使用LLM作为评判员分配拒绝置信度分数，提出岭正则化变体计算更好的拒绝-顺从方向隔离的引导向量，在推理时通过激活向量控制模型行为

Result: 在Qwen3-Next-80B-A3B-Thinking上成功移除了政治敏感话题的拒绝行为，同时在JailbreakBench上保持安全性，在通用基准测试中保持接近基线的性能；方法在4B和80B模型上都能泛化，也可诱导目标拒绝行为

Conclusion: 激活向量引导可以移除政治拒绝行为同时保留对有害内容的安全对齐，为推理时可控、透明的审核提供了实用路径；拒绝信号集中在Transformer深层且分布在多个维度
...
TL;DR: JustRL提出了一种极简的单阶段强化学习方法，使用固定超参数在1.5B推理模型上达到SOTA性能，计算量减少2倍，挑战了当前RL训练中过度复杂化的趋势。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的强化学习训练方法变得越来越复杂（多阶段训练、动态超参数调度、课程学习等），作者质疑这种复杂性是否必要，希望探索更简单有效的替代方案。

Method: JustRL采用单阶段训练，使用固定超参数，不添加常见的"技巧"如显式长度惩罚和鲁棒验证器，保持训练过程的简单性和稳定性。

Result: 在两个1.5B推理模型上，JustRL在九个数学基准测试中分别达到54.9%和64.3%的平均准确率，计算量比复杂方法减少2倍，且相同超参数可在不同模型间迁移而无需调整。

Conclusion: 研究表明当前领域可能过度复杂化，许多问题在稳定、规模化的基线方法中会自然消失，JustRL为社区提供了一个简单有效的验证基线。
...
TL;DR: GinSign框架通过分层分类方法将自然语言映射到系统签名，实现自然语言到时序逻辑的翻译，避免了依赖昂贵的大语言模型，提高了接地翻译的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有NL-to-TL翻译框架要么需要准确的原子接地，要么接地翻译准确率低，这限制了在构建可信自主系统中的应用。

Method: 提出GinSign框架，引入接地模型学习将NL跨度映射到给定系统签名的抽象任务。采用分层分解方法：先预测谓词标签，再选择适当类型的常量参数，将任务从自由形式生成问题转化为结构化分类问题。

Result: 实验表明，省略接地的框架会产生语法正确但语义不等价的LTL表达式，而GinSign框架支持下游模型检查，实现了95.5%的接地逻辑等价分数，比现有技术提高了1.4倍。

Conclusion: GinSign框架通过结构化分类方法有效解决了NL-to-TL翻译中的接地问题，提高了翻译准确性，为构建可信自主系统提供了更好的规范验证能力。
...
TL;DR: 提出了一种基于推理轨迹增强的RAG框架，通过三阶段结构化推理解决检索信息冲突、过时和主观性问题，引入冲突感知信任评分管道进行评估，实验显示显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成（RAG）在处理检索到的信息冲突、过时或包含主观内容时存在局限，现有方法缺乏统一的推理监督机制，需要更系统化的解决方案。

Method: 提出推理轨迹增强的RAG框架，包含三个结构化推理阶段：文档级裁决、冲突分析和基于证据的合成，生成带引用的答案或合理的拒绝回答。引入冲突感知信任评分（CATS）管道，使用LLM作为评判者评估基础性、事实正确性、拒绝准确性和冲突行为对齐。

Result: 构建了包含539个查询的推理数据集和评估管道，为冲突感知、可解释的RAG系统奠定基础。实验结果显示相比基线有显著提升，特别是Qwen模型经过监督微调后，端到端答案正确率从0.069提升到0.883，行为一致性从0.074提升到0.722。

Conclusion: 该框架通过结构化推理和冲突感知评估，有效解决了RAG系统中的信息冲突问题，提高了答案的可靠性和可解释性，为构建更稳健的检索增强生成系统提供了新方法。
...
TL;DR: 研究比较了多模态检索增强生成中两种视觉信息处理策略：将图表转换为文本 vs OCR-free视觉检索，发现在糖生物学领域，模型能力决定最佳策略选择。
...
Method: 构建了包含120个多选题的基准数据集，按检索难度分层。实现了四种增强策略：无增强、文本RAG、多模态转换、late-interaction视觉检索（ColPali）。使用Docling解析和Qdrant索引，评估了中等规模开源模型和前沿专有模型。

Result: 对于中等规模模型（Gemma-3-27B-IT），将视觉信息转换为文本的策略优于OCR-free检索（0.722-0.740 vs 0.510准确率）。对于前沿模型（GPT-4o），多模态转换达到0.808准确率，文本RAG为0.782，ColPali为0.745，差异较小。在GPT-5家族实验中，ColPali和ColFlor将最佳结果提升约2%至0.828。

Conclusion: 管道选择取决于模型能力：将视觉信息转换为文本降低了读取负担，对中等规模模型更可靠；而OCR-free视觉检索在先进模型下变得有竞争力。ColFlor在保持性能的同时具有更小的计算开销，是强生成器下的高效默认选择。
